...
Write-Host "[1/561] Healthcheck..." -ForegroundColor Cyan
$health = Invoke-RestMethod -Uri "$base/api/health" -Method Get
if ($health.version -ne "0.5.6") { throw "Versao inesperada da API: $($health.version)" }
Write-Host "    API $($health.version) / banco $($health.database)" -ForegroundColor Green

Write-Host "[2/561] Login..." -ForegroundColor Cyan
...
Write-Host "[3/561] Listando pacientes..." -ForegroundColor Cyan
...
Write-Host "[4/561] Validando perguntas de anamnese..." -ForegroundColor Cyan
...
Write-Host "[5/561] Validando catalogo laboratorial..." -ForegroundColor Cyan
...
Write-Host "[6/561] Validando catalogo de alimentos..." -ForegroundColor Cyan
...
Write-Host "[7/561] Validando busca/paginacao..." -ForegroundColor Cyan
...
Write-Host "[8/561] Validando modulos do paciente..." -ForegroundColor Cyan
...
Write-Host "[9/561] Validando metas do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[10/561] Validando diario/resumo do dia..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[11/561] Validando portal/home do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[12/561] Validando agenda do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[13/561] Validando dashboard do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[14/561] Validando interface web..." -ForegroundColor Cyan
...
Write-Host "[15/561] Validando assets da interface..." -ForegroundColor Cyan
...
Write-Host "[16/561] Validando prontuario visual v0.2.2..." -ForegroundColor Cyan
...
Write-Host "[17/561] Validando acoes clinicas da interface..." -ForegroundColor Cyan
...
Write-Host "[18/561] Validando rotas usadas pelos formularios clinicos..." -ForegroundColor Cyan
...
Write-Host "[19/561] Validando cadastro visual de exames..." -ForegroundColor Cyan
...
Write-Host "[20/561] Validando construtor visual do plano alimentar..." -ForegroundColor Cyan
...
Write-Host "[21/561] Validando relatorios na interface..." -ForegroundColor Cyan
...
Write-Host "[22/561] Validando edicao visual do paciente..." -ForegroundColor Cyan
...
Write-Host "[23/561] Validando endpoint de relatorios do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[24/561] Validando edicao clinica visual..."
...
Write-Host "[25/561] Validando agenda operacional..."
...
Write-Host "[26/561] Validando endpoint de atualizacao de avaliacao..."
try {
...
Write-Host "[27/561] Validando tela de configuracoes..."
...
Write-Host "[28/561] Validando gerenciadores de catalogo na interface..."
...
Write-Host "[29/561] Validando resumo de configuracoes do consultorio..."
...
Write-Host "[30/561] Validando rotas dos catalogos..."
...
Write-Host "[31/561] Validando edicao de configuracoes..."
$cfg = Invoke-RestMethod -Uri "$base/api/configuracoes/resumo" -Headers $headers
...
Write-Host "[32/561] Validando assets de edicao/inativacao dos catalogos..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[35/561] Validando separacao de autorizacao profissional/paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[36/561] Validando endpoint de status de acesso do paciente..."
...
Write-Host "[37/561] Validando fluxo de convite/ativacao nos assets..."
...
Write-Host "[38/561] Validando autoatendimento do diario..."
...
Write-Host "[39/561] Validando autoatendimento das metas..."
...
Write-Host "[40/561] Validando tela dedicada do portal do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[41/561] Validando navegacao completa do portal..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[42/561] Validando endpoint proprio de plano alimentar..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[43/561] Validando historico proprio de metas e diario..."
...
Write-Host "[47/561] Validando schema/endpoint do catalogo de exercicios..."
...
Write-Host "[48/561] Validando endpoint de planos de treino do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[49/561] Validando construtor visual de treino..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[50/561] Validando videos e prescricao de exercicios..."
...
Write-Host "[51/561] Validando aba de treinos no prontuario..."
...
Write-Host "[52/561] Validando navegacao de treino do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[53/561] Validando endpoint proprio do treino do paciente..."
...
Write-Host "[54/561] Validando assets visuais do modulo de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[55/561] Validando schema de execucoes de treino..."
...
Write-Host "[56/561] Validando historico profissional de treinos..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[57/561] Validando registro visual de execucao pelo paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[58/561] Validando series, repeticoes e carga realizadas..."
...
Write-Host "[59/561] Validando esforco percebido e duracao..."
...
Write-Host "[60/561] Validando historico do paciente..."
...
Write-Host "[61/561] Validando progressao de carga no prontuario..."
...
Write-Host "[62/561] Validando estilos do acompanhamento de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[63/561] Validando motor de graficos SVG..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[64/561] Validando graficos corporais no prontuario..."
...
Write-Host "[65/561] Validando tendencias laboratoriais..."
...
Write-Host "[66/561] Validando progressao grafica de carga..."
...
Write-Host "[67/561] Validando evolucao visual no portal do paciente..."
...
Write-Host "[68/561] Validando graficos de exames no portal..."
...
Write-Host "[69/561] Validando responsividade dos graficos..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[70/561] Validando compatibilidade de schema na v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
...
Write-Host "[71/561] Validando endpoint de insights do dashboard..."
...
Write-Host "[72/561] Validando insights por paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[73/561] Validando regra de exame fora da referencia..."
...
Write-Host "[74/561] Validando regras de evolucao e retorno..."
...
Write-Host "[75/561] Validando regras de adesao..."
...
Write-Host "[76/561] Validando central de atencao visual..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[77/561] Validando insights no prontuario..."
...
Write-Host "[78/561] Validando estilos e compatibilidade do schema..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[79/561] Validando schema de pendencias clinicas..."
...
Write-Host "[80/561] Validando endpoint geral de pendencias..."
...
Write-Host "[81/561] Validando endpoint de pendencias do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[82/561] Validando acoes de ciclo de vida..."
...
Write-Host "[83/561] Validando criacao de retorno a partir da pendencia..."
...
Write-Host "[84/561] Validando transformar insight em pendencia..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[85/561] Validando tela de gerenciamento de pendencias..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[86/561] Validando acoes visuais da pendencia..."
...
Write-Host "[87/561] Validando resumo de pendencias no dashboard..."
...
Write-Host "[88/561] Validando auditoria, estilos e upgrade v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[89/561] Validando schema de notificacoes internas..."
...
Write-Host "[90/561] Validando sincronizacao de notificacoes..."
...
Write-Host "[91/561] Validando listagem e contador nao lido..."
...
Write-Host "[92/561] Validando regras de agenda profissional..."
...
Write-Host "[93/561] Validando regras de pendencias..."
...
Write-Host "[94/561] Validando lembretes do paciente..."
...
Write-Host "[95/561] Validando leitura individual e em massa..."
...
Write-Host "[96/561] Validando sino e drawer na interface..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[97/561] Validando contador e atualizacao periodica..."
...
Write-Host "[98/561] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[99/561] Validando script de popular banco..."
...
Write-Host "[100/561] Validando idempotencia do popular..."
...
Write-Host "[101/561] Validando cobertura de modulos na base demo..."
...
Write-Host "[102/561] Validando endpoint de resumo de dados..."
...
Write-Host "[103/561] Validando que popular banco e opt-in..."
...
Write-Host "[104/561] Validando versao v0.3.27 e upgrade do schema..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
...
Write-Host "[105/561] Validando endpoint da carteira..."
...
Write-Host "[106/561] Validando priorizacao da carteira..."
...
Write-Host "[107/561] Validando sinais de exames/evolucao na carteira..."
...
Write-Host "[108/561] Validando atividade recente..."
...
Write-Host "[109/561] Validando tela Carteira..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[110/561] Validando filtros e ordenacao..."
...
Write-Host "[111/561] Validando atalho da carteira no dashboard..."
...
Write-Host "[112/561] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[113/561] Validando schema de follow-up..."
...
Write-Host "[114/561] Validando endpoint de follow-up..."
...
Write-Host "[115/561] Validando canais e proximo contato..."
...
Write-Host "[116/561] Validando auditoria do contato..."
...
Write-Host "[117/561] Validando follow-up na carteira..."
$carteiraSource = Get-Content .\src\HealthPlatform.Api\Controllers\CarteiraController.cs -Encoding UTF8 -Raw
...
Write-Host "[118/561] Validando acao rapida de contato..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[119/561] Validando acao rapida de retorno..."
...
Write-Host "[120/561] Validando acao rapida de pendencia..."
...
Write-Host "[121/561] Validando historico no prontuario..."
...
Write-Host "[122/561] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[123/561] Validando endpoint da fila de follow-up..."
...
Write-Host "[124/561] Validando faixas de vencimento..."
...
Write-Host "[125/561] Validando busca e filtros da fila..."
...
Write-Host "[126/561] Validando notificacao de follow-up..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[127/561] Validando tela de follow-up..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[128/561] Validando acoes rapidas na fila..."
...
Write-Host "[129/561] Validando resumo de follow-up no dashboard..."
...
Write-Host "[130/561] Validando estilos, popular e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
$popularSource = Get-Content .\POPULAR.ps1 -Encoding UTF8 -Raw
...
Write-Host "[131/561] Validando endpoint de gestao..."
...
Write-Host "[132/561] Validando indicadores de consultas..."
...
Write-Host "[133/561] Validando indicadores de acompanhamento..."
...
Write-Host "[134/561] Validando indicadores de engajamento..."
...
Write-Host "[135/561] Validando series gerenciais..."
...
Write-Host "[136/561] Validando tela Gestao..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[137/561] Validando graficos e resumo no dashboard..."
...
Write-Host "[138/561] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[139/561] Validando PREPARAR sem atualizacao automatica do dotnet..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[140/561] Validando endpoint CSV gerencial..."
...
Write-Host "[141/561] Validando conteudo longitudinal do CSV..."
...
Write-Host "[142/561] Validando relatorio HTML imprimivel..."
...
Write-Host "[143/561] Validando botoes de exportacao na Gestao..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[144/561] Validando download autenticado..."
...
Write-Host "[145/561] Validando relatorio autenticado em nova janela..."
...
Write-Host "[146/561] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[147/561] Validando perguntas de anamnese inativas..."
...
Write-Host "[148/561] Validando totais completos de insights..."
...
Write-Host "[149/561] Validando timezone das notificacoes..."
...
Write-Host "[150/561] Validando cleanup do polling no logout..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[151/561] Validando navegacao robusta no portal..."
...
Write-Host "[152/561] Validando smoke de notificacoes sem mutacao..."
...
Write-Host "[153/561] Validando copy historica de schema..."
$schemaCheckMarker = 'Write-Host "[153/561] Validando copy historica de schema..."'
...
Write-Host "[154/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / estabilizacao + qualidade: OK."


Write-Host "[155/561] Validando endpoint de busca global..."
...
Write-Host "[156/561] Validando fontes da busca..."
...
Write-Host "[157/561] Validando isolamento por organizacao..."
...
Write-Host "[158/561] Validando botao e atalho da busca..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[159/561] Validando modal e debounce..."
...
Write-Host "[160/561] Validando acoes dos resultados..."
...
Write-Host "[161/561] Validando estilos responsivos da busca..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[162/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / busca global + central de acoes: OK."


Write-Host "[163/561] Validando endpoint da Central do Dia..."
...
Write-Host "[164/561] Validando agenda do dia..."
...
Write-Host "[165/561] Validando follow-ups do dia..."
...
Write-Host "[166/561] Validando pendencias prioritarias..."
...
Write-Host "[167/561] Validando pacientes para revisao..."
...
Write-Host "[168/561] Validando tela Hoje..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[169/561] Validando acoes rapidas e dashboard..."
...
Write-Host "[170/561] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[171/561] Validando schema de evolucoes clinicas SOAP..."
...
Write-Host "[172/561] Validando endpoint de evolucoes..."
...
Write-Host "[173/561] Validando consulta opcional e isolamento..."
...
Write-Host "[174/561] Validando auditoria da evolucao..."
...
Write-Host "[175/561] Validando evolucao na timeline..."
...
Write-Host "[176/561] Validando aba Evolucoes no prontuario..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[177/561] Validando registro visual SOAP..."
...
Write-Host "[178/561] Validando edicao visual da evolucao..."
...
Write-Host "[179/561] Validando upgrade SQL SOAP historico e PREPARAR atual..."
...
Write-Host "[180/561] Validando estilos e versao v0.3.27..."
...
Write-Host "[181/561] Validando endpoint de resumo clinico..."
...
Write-Host "[182/561] Validando consulta e SOAP no resumo..."
...
Write-Host "[183/561] Validando corpo e anamnese..."
...
Write-Host "[184/561] Validando exames alterados..."
...
Write-Host "[185/561] Validando metas, treinos e pendencias..."
...
Write-Host "[186/561] Validando resumo no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[187/561] Validando atualizacao e responsividade..."
...
Write-Host "[188/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / resumo clinico consolidado: OK."


Write-Host "[189/561] Validando texto de handoff clinico..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[190/561] Validando conteudo SOAP no handoff..."
...
Write-Host "[191/561] Validando copia para clipboard..."
...
Write-Host "[192/561] Validando impressao do resumo..."
...
Write-Host "[193/561] Validando seguranca basica do HTML imprimivel..."
...
Write-Host "[194/561] Validando botoes no resumo clinico..."
...
Write-Host "[195/561] Validando responsividade das acoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[196/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / handoff clinico + impressao: OK."


Write-Host "[197/561] Validando endpoint de equipe..."
...
Write-Host "[198/561] Validando isolamento e administracao..."
...
Write-Host "[199/561] Validando criacao de acesso..."
...
Write-Host "[200/561] Validando sincronizacao de tipo e role..."
...
Write-Host "[201/561] Validando perfil profissional..."
...
Write-Host "[202/561] Validando protecao do proprio administrador..."
...
Write-Host "[203/561] Validando auditoria da equipe..."
...
Write-Host "[204/561] Validando tela Equipe..."
...
Write-Host "[205/561] Validando visibilidade admin e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[206/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe + gestao de profissionais: OK."


Write-Host "[207/561] Validando filtros da equipe..."
$teamSource = Get-Content .\src\HealthPlatform.Api\Controllers\EquipeController.cs -Encoding UTF8 -Raw
...
Write-Host "[208/561] Validando redefinicao de senha..."
...
Write-Host "[209/561] Validando escopo e protecoes do reset..."
...
Write-Host "[210/561] Validando auditoria segura de senha..."
...
Write-Host "[211/561] Validando filtros visuais..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[212/561] Validando acao visual de senha..."
...
Write-Host "[213/561] Validando responsividade da equipe v2..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[214/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe v2 + seguranca de acesso: OK."


Write-Host "[215/561] Validando endpoint Minha Conta..."
...
Write-Host "[216/561] Validando alteracao da propria senha..."
...
Write-Host "[217/561] Validando protecoes da troca de senha..."
...
Write-Host "[218/561] Validando auditoria segura da conta..."
...
Write-Host "[219/561] Validando sincronizacao do nome profissional..."
...
Write-Host "[220/561] Validando painel Minha Conta..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[221/561] Validando responsividade da conta..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[222/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / Minha Conta + troca de senha: OK."


Write-Host "[223/561] Validando schema de progressao alimentar..."
...
Write-Host "[224/561] Validando simulador nutricional..."
...
Write-Host "[225/561] Validando duplicacao versionada..."
...
Write-Host "[226/561] Validando escala de porcoes..."
...
Write-Host "[227/561] Validando ajuste por calorias alvo..."
...
Write-Host "[228/561] Validando limites de ajuste..."
...
Write-Host "[229/561] Validando encerramento opcional do plano anterior..."
...
Write-Host "[230/561] Validando auditoria da progressao..."
...
Write-Host "[231/561] Validando interface de progressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[232/561] Validando modos percentual e calorias..."
...
Write-Host "[233/561] Validando upgrade SQL e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[234/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao do plano alimentar: OK."


Write-Host "[235/561] Validando schema de progressao de treino..."
...
Write-Host "[236/561] Validando simulador de treino..."
...
Write-Host "[237/561] Validando duplicacao versionada do treino..."
...
Write-Host "[238/561] Validando progressao de carga..."
...
Write-Host "[239/561] Validando progressao de series e descanso..."
...
Write-Host "[240/561] Validando repeticoes estruturadas..."
...
Write-Host "[241/561] Validando limites de progressao..."
...
Write-Host "[242/561] Validando encerramento e auditoria..."
...
Write-Host "[243/561] Validando interface de progressao de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[244/561] Validando projecao e modal ampliado..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[245/561] Validando upgrade SQL e PREPARAR 19..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[246/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao de treino + ciclo versionado: OK."


Write-Host "[247/561] Validando schema dos modelos alimentares..."
...
Write-Host "[248/561] Validando listagem e busca de modelos..."
...
Write-Host "[249/561] Validando salvar plano como modelo..."
...
Write-Host "[250/561] Validando criacao de plano a partir de modelo..."
...
Write-Host "[251/561] Validando alimentos do catalogo..."
...
Write-Host "[252/561] Validando isolamento e auditoria..."
...
Write-Host "[253/561] Validando interface de salvar modelo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[254/561] Validando seletor de modelos..."
...
Write-Host "[255/561] Validando criacao visual via modelo..."
...
Write-Host "[256/561] Validando responsividade dos modelos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[257/561] Validando upgrade SQL e PREPARAR 20..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[258/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de plano alimentar: OK."


Write-Host "[259/561] Validando schema dos modelos de treino..."
...
Write-Host "[260/561] Validando listagem e busca de modelos de treino..."
...
Write-Host "[261/561] Validando salvar treino como modelo..."
if (-not $modelSource.Contains("salvar-como-modelo") -or
...
Write-Host "[262/561] Validando criacao de treino a partir de modelo..."
if (-not $modelSource.Contains("criar-de-modelo") -or
...
Write-Host "[263/561] Validando catalogo de exercicios..."
...
Write-Host "[264/561] Validando prescricao completa do template..."
...
Write-Host "[265/561] Validando isolamento e auditoria dos modelos de treino..."
if (-not $modelSource.Contains("currentUser.OrganizationId") -or
    -not $modelSource.Contains('"CREATE_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("AuditLogs")) {
...
Write-Host "[266/561] Validando interface de salvar modelo de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[267/561] Validando seletor e busca de modelos..."
...
Write-Host "[268/561] Validando criacao visual via modelo..."
...
Write-Host "[269/561] Validando upgrade SQL, responsividade e PREPARAR 21..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[270/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de treino + criacao rapida: OK."


Write-Host "[271/561] Validando schema das metas nutricionais..."
$planEntity = Get-Content .\src\HealthPlatform.Domain\Entities\PlanoAlimentar.cs -Encoding UTF8 -Raw
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[272/561] Validando contratos nutricionais..."
...
Write-Host "[273/561] Validando endpoint de metas..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[274/561] Validando analise nutricional..."
...
Write-Host "[275/561] Validando metas na criacao e edicao..."
...
Write-Host "[276/561] Validando metas na progressao alimentar..."
...
Write-Host "[277/561] Validando metas nos templates alimentares..."
...
Write-Host "[278/561] Validando construtor com metas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[279/561] Validando Meta x Prescrito no prontuario..."
...
Write-Host "[280/561] Validando distribuicao por refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[281/561] Validando modal de metas..."
...
Write-Host "[282/561] Validando responsividade nutricional..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[283/561] Validando SQL e PREPARAR 22..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[284/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / metas nutricionais + distribuicao: OK."


Write-Host "[285/561] Validando schema da biblioteca de refeicoes..."
...
Write-Host "[286/561] Validando listagem e filtros da biblioteca..."
...
Write-Host "[287/561] Validando salvar refeicao como modelo..."
...
Write-Host "[288/561] Validando insercao de refeicao no plano..."
...
Write-Host "[289/561] Validando catalogo e substituicoes..."
if (-not $modelSource.Contains("alimentosValidos") -or
    -not $modelSource.Contains("alimentosInvalidos") -or
...
Write-Host "[290/561] Validando protecoes e auditoria..."
...
Write-Host "[291/561] Validando botao Salvar refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[292/561] Validando biblioteca visual..."
...
Write-Host "[293/561] Validando insercao visual rapida..."
...
Write-Host "[294/561] Validando responsividade da biblioteca..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[295/561] Validando SQL e PREPARAR 23..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[296/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de refeicoes + insercao rapida: OK."


Write-Host "[297/561] Validando schema da biblioteca de sessoes..."
...
Write-Host "[298/561] Validando listagem e filtros..."
...
Write-Host "[299/561] Validando salvar sessao como modelo..."
...
Write-Host "[300/561] Validando insercao de sessao no plano..."
...
Write-Host "[301/561] Validando catalogo e prescricao..."
if (-not $modelSource.Contains("exerciciosValidos") -or
    -not $modelSource.Contains("exerciciosInvalidos") -or
...
Write-Host "[302/561] Validando protecoes e auditoria..."
if (-not $modelSource.Contains('plano.Status == "Concluido"') -or
    -not $modelSource.Contains('"INSERT_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("currentUser.OrganizationId")) {
...
Write-Host "[303/561] Validando botao Salvar sessao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[304/561] Validando biblioteca visual..."
...
Write-Host "[305/561] Validando insercao visual rapida..."
...
Write-Host "[306/561] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[307/561] Validando SQL e PREPARAR 24..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[308/561] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de sessoes + insercao rapida: OK."


Write-Host "[309/561] Validando endpoint longitudinal de habitos..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[310/561] Validando contrato da evolucao de habitos..."
...
Write-Host "[311/561] Validando series de sono, estresse, atividade e agua..."
...
Write-Host "[312/561] Validando limite e isolamento multi-tenant..."
...
Write-Host "[313/561] Validando comparacao com registro anterior..."
...
Write-Host "[314/561] Validando graficos de habitos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[315/561] Validando resumo atual dos habitos..."
...
Write-Host "[316/561] Validando integracao na aba Anamnese..."
...
Write-Host "[317/561] Validando integracao no Resumo..."
...
Write-Host "[318/561] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[319/561] Validando compatibilidade de banco e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "[320/561] Validando versao v0.3.28..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.28 / evolucao de habitos + graficos de anamnese: OK."


Write-Host "[321/561] Validando resposta runtime com metas por refeicao..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[322/561] Validando schema de metas por refeicao..."
...
Write-Host "[323/561] Validando contratos de metas por refeicao..."
$contractsSource = Get-Content .\src\HealthPlatform.Api\Contracts\PlanosAlimentares\PlanoAlimentarContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[324/561] Validando endpoint de meta individual..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[325/561] Validando distribuicao automatica..."
...
Write-Host "[326/561] Validando isolamento e integridade da distribuicao..."
...
Write-Host "[327/561] Validando progressao com metas por refeicao..."
...
Write-Host "[328/561] Validando templates de plano..."
$templateSource = Get-Content .\src\HealthPlatform.Api\Controllers\ModelosPlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[329/561] Validando biblioteca de refeicoes..."
...
Write-Host "[330/561] Validando construtor alimentar..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[331/561] Validando edicao e comparacao visual..."
...
Write-Host "[332/561] Validando modal de distribuicao..."
...
Write-Host "[333/561] Validando SQL e PREPARAR 25..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[334/561] Validando versao v0.3.29..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.29 / metas por refeicao + distribuicao planejada: OK."


Write-Host "[335/561] Validando schema das fases nutricionais..."
...
Write-Host "[336/561] Validando listagem das fases..."
...
Write-Host "[337/561] Validando criacao de fase..."
...
Write-Host "[338/561] Validando edicao e estados..."
...
Write-Host "[339/561] Validando vinculo seguro com plano alimentar..."
...
Write-Host "[340/561] Validando reordenacao do ciclo..."
...
Write-Host "[341/561] Validando exclusao protegida..."
...
Write-Host "[342/561] Validando isolamento e auditoria..."
...
Write-Host "[343/561] Validando interface das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[344/561] Validando formulario de fase..."
...
Write-Host "[345/561] Validando reordenacao visual..."
...
Write-Host "[346/561] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[347/561] Validando SQL e PREPARAR 26..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[348/561] Validando versao v0.3.30..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.30 / fases nutricionais + planejamento de ciclo: OK."


Write-Host "[349/561] Validando schema das fases de treino..."
...
Write-Host "[350/561] Validando listagem das fases..."
...
Write-Host "[351/561] Validando criacao de fase..."
...
Write-Host "[352/561] Validando edicao e estados..."
...
Write-Host "[353/561] Validando vinculo seguro com ficha..."
if (-not $phaseSource.Contains("PlanoValido") -or
    -not $phaseSource.Contains("x.PacienteId == pacienteId") -or
    -not $phaseSource.Contains("x.Paciente.OrganizacaoId == currentUser.OrganizationId")) {
...
Write-Host "[354/561] Validando reordenacao do ciclo..."
...
Write-Host "[355/561] Validando exclusao protegida..."
if (-not $phaseSource.Contains("HttpDelete") -or
    -not $phaseSource.Contains('fase.Status == "EmAndamento"') -or
    -not $phaseSource.Contains('"DELETE"')) {
...
Write-Host "[356/561] Validando isolamento e auditoria..."
if (-not $phaseSource.Contains("currentUser.OrganizationId") -or
    -not $phaseSource.Contains("AuditLogs") -or
...
Write-Host "[357/561] Validando interface do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[358/561] Validando formulario de fase..."
...
Write-Host "[359/561] Validando reordenacao visual..."
...
Write-Host "[360/561] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[361/561] Validando SQL e PREPARAR 27..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[362/561] Validando versao v0.3.31..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.31 / ciclos de treino + periodizacao: OK."


Write-Host "[363/561] Validando schema dos check-ins..."
...
Write-Host "[364/561] Validando endpoint profissional..."
...
Write-Host "[365/561] Validando criacao e edicao..."
...
Write-Host "[366/561] Validando limites dos indicadores..."
...
Write-Host "[367/561] Validando vinculo com fases..."
...
Write-Host "[368/561] Validando auto-vinculo do paciente..."
...
Write-Host "[369/561] Validando portal do paciente..."
...
Write-Host "[370/561] Validando isolamento multi-tenant..."
...
Write-Host "[371/561] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[372/561] Validando graficos de resposta..."
...
Write-Host "[373/561] Validando formulario profissional..."
...
Write-Host "[374/561] Validando check-in no portal..."
...
Write-Host "[375/561] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[376/561] Validando SQL e PREPARAR 28..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[377/561] Validando preservacao dos ciclos..."
...
Write-Host "[378/561] Validando versao v0.3.32..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.32 / check-ins de evolucao + adesao por fase: OK."


Write-Host "[379/561] Validando endpoint de analise por fase..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[380/561] Validando metricas agregadas..."
...
Write-Host "[381/561] Validando variacao de peso por fase..."
...
Write-Host "[382/561] Validando destaques automaticos..."
...
Write-Host "[383/561] Validando isolamento multi-tenant..."
...
Write-Host "[384/561] Validando cards comparativos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[385/561] Validando destaques visuais..."
...
Write-Host "[386/561] Validando integracao com nutricao..."
...
Write-Host "[387/561] Validando integracao com treino..."
...
Write-Host "[388/561] Validando resumo consolidado..."
...
Write-Host "[389/561] Validando responsividade e compatibilidade de banco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[390/561] Validando versao v0.3.33..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.33 / analise de fases + comparativo de resposta: OK."

Write-Host "[391/561] Validando metas das fases..."
...
Write-Host "[392/561] Validando mapeamento EF..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[393/561] Validando CRUD das fases..."
...
Write-Host "[394/561] Validando limites dos criterios..."
...
Write-Host "[395/561] Validando endpoint runtime de prontidao..."
...
Write-Host "[396/561] Validando motor de criterios objetivos..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[397/561] Validando revisao profissional..."
...
Write-Host "[398/561] Validando formularios das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[399/561] Validando metas nos cards..."
...
Write-Host "[400/561] Validando painel de prontidao..."
...
Write-Host "[405/561] Validando SQL e PREPARAR 29..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[406/561] Validando versao v0.3.34..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.34 / metas de fase + criterios de transicao: OK."


Write-Host "[407/561] Validando entidade de revisao..."
...
Write-Host "[408/561] Validando mapeamento das revisoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[409/561] Validando historico runtime..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[410/561] Validando revisao nutricional..."
...
Write-Host "[411/561] Validando revisao de treino..."
...
Write-Host "[412/561] Validando decisoes e fase ativa..."
...
Write-Host "[413/561] Validando override consciente..."
...
Write-Host "[414/561] Validando transicao para proxima fase..."
...
Write-Host "[415/561] Validando transacao e auditoria..."
...
Write-Host "[416/561] Validando modal de revisao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[417/561] Validando historico visual..."
...
Write-Host "[418/561] Validando integracao com painel de transicao..."
...
Write-Host "[419/561] Validando SQL e PREPARAR 30..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[420/561] Validando versao v0.3.35..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.35 / revisao de fase + transicao assistida: OK."


Write-Host "[421/561] Validando controller de volume..."
...
Write-Host "[422/561] Validando isolamento multi-tenant..."
...
Write-Host "[423/561] Validando volume planejado por grupo..."
...
Write-Host "[424/561] Validando frequencia semanal..."
...
Write-Host "[425/561] Validando fallback de frequencia..."
...
Write-Host "[426/561] Validando execucoes reais..."
...
Write-Host "[427/561] Validando ausencia de tonelagem inventada..."
...
Write-Host "[428/561] Validando runtime da analise..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
    try {
...
Write-Host "[429/561] Validando painel de volume..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[430/561] Validando resumo analitico..."
...
Write-Host "[431/561] Validando volume por sessao..."
...
Write-Host "[432/561] Validando integracao e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[433/561] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "[434/561] Validando versao v0.3.36..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.36 / volume de treino + distribuicao muscular: OK."


Write-Host "[435/561] Validando endpoints de progressao por exercicio..."
...
Write-Host "[436/561] Validando seguranca e tenant..."
...
Write-Host "[437/561] Validando separacao por unidade..."
...
Write-Host "[438/561] Validando metricas de carga..."
...
Write-Host "[439/561] Validando recordes sucessivos..."
...
Write-Host "[440/561] Validando tendencia de carga..."
...
Write-Host "[441/561] Validando protecao contra estimativas artificiais..."
...
Write-Host "[442/561] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[443/561] Validando payload de pontos..."
...
Write-Host "[444/561] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[445/561] Validando graficos e recordes..."
...
Write-Host "[446/561] Validando portal do paciente..."
...
Write-Host "[447/561] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[448/561] Validando versao v0.3.37..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.37 / progressao por exercicio + recordes de carga: OK."


Write-Host "[449/561] Validando endpoints de sinais de progressao..."
...
Write-Host "[450/561] Validando tenant e PatientOnly..."
...
Write-Host "[451/561] Validando estados da analise..."
...
Write-Host "[452/561] Validando regra de estagnacao..."
...
Write-Host "[453/561] Validando sinal de carga/RPE..."
...
Write-Host "[454/561] Validando progressao recente..."
...
Write-Host "[455/561] Validando semantica nao diagnostica..."
...
Write-Host "[456/561] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[457/561] Validando payload analitico..."
...
Write-Host "[458/561] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[459/561] Validando portal do paciente..."
...
Write-Host "[460/561] Validando cards e graficos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[461/561] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[462/561] Validando versao v0.3.38..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.38 / estagnacao + fadiga + sinais de progressao: OK."


Write-Host "[463/561] Validando identidade MVP Preview..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.5.6") -or
...
Write-Host "[464/561] Validando aviso de demonstracao..."
...
Write-Host "[465/561] Validando roteiro da demo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[466/561] Validando checklist de avaliacao..."
...
Write-Host "[467/561] Validando modelo de feedback..."
...
Write-Host "[468/561] Validando dashboard de apresentacao..."
...
Write-Host "[469/561] Validando atalho Escape..."
...
Write-Host "[470/561] Validando feedback de conectividade..."
...
Write-Host "[471/561] Validando acabamento de foco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[472/561] Validando estados vazios..."
...
Write-Host "[473/561] Validando limpeza da navegacao de demo..."
...
Write-Host "[474/561] Validando responsividade do MVP..."
...
Write-Host "[475/561] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
...
Write-Host "[476/561] Validando versao v0.3.39..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.39 / MVP Preview + polimento de demonstracao: OK."


Write-Host "[477/561] Validando Dockerfile..."
...
Write-Host "[478/561] Validando bind dinamico de porta..."
...
Write-Host "[479/561] Validando Blueprint Render..."
...
Write-Host "[480/561] Validando secrets do Blueprint..."
...
Write-Host "[481/561] Validando conexao PostgreSQL do Render..."
...
Write-Host "[482/561] Validando bootstrap isolado do MVP..."
...
Write-Host "[483/561] Validando fluxo local preservado..."
...
Write-Host "[484/561] Validando healthcheck real..."
...
Write-Host "[485/561] Validando forwarded headers..."
...
Write-Host "[486/561] Validando POPULAR remoto..."
...
if (-not $remotePopular.Contains("[Parameter(Mandatory=`$true)][string]`$BaseUrl") -or
    -not $remotePopular.Contains("HealthPlatform v0.5.6 - POPULAR RENDER DEMO") -or
...
Write-Host "[487/561] Validando catalogos da demo remota..."
...
Write-Host "[488/561] Validando smoke test remoto..."
...
Write-Host "[489/561] Validando guia de deploy..."
...
Write-Host "[490/561] Validando ausencia de schema novo..."
...
Write-Host "[491/561] Validando arquivos de container..."
...
Write-Host "[492/561] Validando versao base do deploy..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.6 / Render Demo Deploy preservado: OK."


Write-Host "[493/561] Validando paleta RS..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[494/561] Validando monograma RS..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
...
Write-Host "[495/561] Validando linguagem editorial..."
...
Write-Host "[496/561] Validando tipografia condensada..."
...
Write-Host "[497/561] Validando login editorial..."
...
Write-Host "[498/561] Validando sidebar RS..."
...
Write-Host "[499/561] Validando experiencia iPad..."
...
Write-Host "[500/561] Validando drawer mobile..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[501/561] Validando safe areas iOS..."
...
Write-Host "[502/561] Validando modal bottom-sheet no iPhone..."
...
Write-Host "[503/561] Validando portal mobile..."
...
Write-Host "[504/561] Validando abas do prontuario no mobile..."
...
Write-Host "[505/561] Validando touch targets..."
...
Write-Host "[506/561] Validando cards analiticos no iPhone..."
...
Write-Host "[507/561] Validando deploy/demo preservados..."
$renderSource = Get-Content .\render.yaml -Encoding UTF8 -Raw
...
Write-Host "[508/561] Validando entidade de solicitacoes clinicas..."
...
Write-Host "[509/561] Validando mapeamento EF de solicitacoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[510/561] Validando endpoints profissionais de solicitacoes..."
...
Write-Host "[511/561] Validando portal do paciente para solicitacoes..."
...
Write-Host "[512/561] Validando isolamento multi-tenant..."
...
Write-Host "[513/561] Validando auditoria de solicitacoes..."
...
Write-Host "[514/561] Validando interface profissional..."
...
Write-Host "[515/561] Validando interface do paciente..."
...
Write-Host "[516/561] Validando upgrade SQL v0.5.1 preservado..."
...
Write-Host "[517/561] Validando PREPARAR 31/31..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[518/561] Validando versao v0.5.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.6 / Solicitacoes Clinicas + Connected Care: OK."

Write-Host "[519/561] Validando notificacoes de solicitacoes no backend..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[520/561] Validando prioridade por prazo para o paciente..."
...
Write-Host "[521/561] Validando notificacao de resposta para profissional..."
...
Write-Host "[522/561] Validando limpeza de notificacoes encerradas..."
...
Write-Host "[523/561] Validando navegacao do paciente pela notificacao..."
...
Write-Host "[524/561] Validando navegacao contextual do profissional..."
...
Write-Host "[525/561] Validando identidade visual de notificacao..."
...
Write-Host "[526/561] Validando versao funcional v0.5.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
if (-not $notificationSource.Contains('SolicitacaoClinica')) { throw "Feature v0.5.6 ausente." }
Write-Host "    v0.5.6 / Notificacoes Contextuais + Connected Care: OK."

Write-Host "[527/561] Validando endpoint da Central de Solicitacoes..."
$requestController = Get-Content .\src\HealthPlatform.Api\Controllers\SolicitacoesClinicasController.cs -Encoding UTF8 -Raw
...
Write-Host "[528/561] Validando resumo operacional de solicitacoes..."
...
Write-Host "[529/561] Validando filtros da Central de Solicitacoes..."
...
Write-Host "[530/561] Validando isolamento da fila profissional..."
...
Write-Host "[531/561] Validando navegacao profissional de solicitacoes..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[532/561] Validando indicadores e filtros visuais..."
...
Write-Host "[533/561] Validando acoes rapidas da Central de Solicitacoes..."
...
Write-Host "[534/561] Validando responsividade da Central de Solicitacoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[535/561] Validando versao funcional v0.5.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
if (-not $requestController.Contains('FilaProfissional')) { throw "Feature v0.5.6 ausente." }
Write-Host "    v0.5.6 / Central de Solicitacoes + Connected Care: OK."


Write-Host "[536/561] Validando solicitacoes no Hoje do paciente..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[537/561] Validando solicitacoes no progresso diario..."
...
Write-Host "[538/561] Validando card de acao necessaria..."
...
Write-Host "[539/561] Validando destaque de solicitacao vencida no portal..."
...
Write-Host "[540/561] Validando integracao da Central do Dia no backend..."
$centralSource = Get-Content .\src\HealthPlatform.Api\Controllers\CentralDiaController.cs -Encoding UTF8 -Raw
...
Write-Host "[541/561] Validando escopo profissional das solicitacoes do dia..."
...
Write-Host "[542/561] Validando card profissional de solicitacoes..."
...
Write-Host "[543/561] Validando dashboard com solicitacoes para revisao..."
...
Write-Host "[544/561] Validando responsividade das solicitacoes no Hoje..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[545/561] Validando versao funcional v0.5.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
if (-not $centralSource.Contains('SolicitacoesParaRevisao') -or -not $appJsSource.Contains('patient-today-requests')) { throw "Feature v0.5.6 ausente." }
Write-Host "    v0.5.6 / Solicitacoes no Hoje + Connected Care: OK."

Write-Host "[546/561] Validando contrato Desde a ultima consulta..."
$resumoSource = Get-Content .\src\HealthPlatform.Api\Controllers\ResumoClinicoController.cs -Encoding UTF8 -Raw
...
Write-Host "[547/561] Validando periodo baseado na ultima consulta..."
...
Write-Host "[548/561] Validando atividade do paciente no periodo..."
...
Write-Host "[549/561] Validando adesao media por check-ins..."
...
Write-Host "[550/561] Validando variacao de peso no periodo..."
...
Write-Host "[551/561] Validando solicitacoes no resumo longitudinal..."
...
Write-Host "[552/561] Validando card longitudinal no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[553/561] Validando versao funcional v0.5.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
if (-not $resumoSource.Contains('ResumoDesdeUltimaConsultaResponse') -or -not $appJsSource.Contains('data-clinical-period-summary')) { throw "Feature v0.5.6 ausente." }
Write-Host "    v0.5.6 / Desde a ultima consulta + Connected Care: OK."

Write-Host "[554/561] Validando endpoint da Jornada do paciente..."
...
Write-Host "[555/561] Validando escopo PatientOnly da Jornada..."
...
Write-Host "[556/561] Validando fontes da Jornada..."
...
Write-Host "[557/561] Validando navegacao Jornada no portal..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[558/561] Validando filtro temporal da Jornada..."
...
Write-Host "[559/561] Validando atalhos contextuais da Jornada..."
...
Write-Host "[560/561] Validando responsividade da Jornada..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[561/561] Validando versao funcional v0.5.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.6") { throw "VERSION.txt inesperado." }
if (-not $portalController.Contains('MinhaJornada') -or -not $appJsSource.Contains('loadPatientJourney')) { throw "Feature v0.5.6 ausente." }
Write-Host "    v0.5.6 / Jornada do Paciente + Connected Care: OK."
