...
Write-Host "[1/545] Healthcheck..." -ForegroundColor Cyan
$health = Invoke-RestMethod -Uri "$base/api/health" -Method Get
if ($health.version -ne "0.5.4") { throw "Versao inesperada da API: $($health.version)" }
Write-Host "    API $($health.version) / banco $($health.database)" -ForegroundColor Green

Write-Host "[2/545] Login..." -ForegroundColor Cyan
...
Write-Host "[3/545] Listando pacientes..." -ForegroundColor Cyan
...
Write-Host "[4/545] Validando perguntas de anamnese..." -ForegroundColor Cyan
...
Write-Host "[5/545] Validando catalogo laboratorial..." -ForegroundColor Cyan
...
Write-Host "[6/545] Validando catalogo de alimentos..." -ForegroundColor Cyan
...
Write-Host "[7/545] Validando busca/paginacao..." -ForegroundColor Cyan
...
Write-Host "[8/545] Validando modulos do paciente..." -ForegroundColor Cyan
...
Write-Host "[9/545] Validando metas do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[10/545] Validando diario/resumo do dia..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[11/545] Validando portal/home do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[12/545] Validando agenda do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[13/545] Validando dashboard do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[14/545] Validando interface web..." -ForegroundColor Cyan
...
Write-Host "[15/545] Validando assets da interface..." -ForegroundColor Cyan
...
Write-Host "[16/545] Validando prontuario visual v0.2.2..." -ForegroundColor Cyan
...
Write-Host "[17/545] Validando acoes clinicas da interface..." -ForegroundColor Cyan
...
Write-Host "[18/545] Validando rotas usadas pelos formularios clinicos..." -ForegroundColor Cyan
...
Write-Host "[19/545] Validando cadastro visual de exames..." -ForegroundColor Cyan
...
Write-Host "[20/545] Validando construtor visual do plano alimentar..." -ForegroundColor Cyan
...
Write-Host "[21/545] Validando relatorios na interface..." -ForegroundColor Cyan
...
Write-Host "[22/545] Validando edicao visual do paciente..." -ForegroundColor Cyan
...
Write-Host "[23/545] Validando endpoint de relatorios do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[24/545] Validando edicao clinica visual..."
...
Write-Host "[25/545] Validando agenda operacional..."
...
Write-Host "[26/545] Validando endpoint de atualizacao de avaliacao..."
try {
...
Write-Host "[27/545] Validando tela de configuracoes..."
...
Write-Host "[28/545] Validando gerenciadores de catalogo na interface..."
...
Write-Host "[29/545] Validando resumo de configuracoes do consultorio..."
...
Write-Host "[30/545] Validando rotas dos catalogos..."
...
Write-Host "[31/545] Validando edicao de configuracoes..."
$cfg = Invoke-RestMethod -Uri "$base/api/configuracoes/resumo" -Headers $headers
...
Write-Host "[32/545] Validando assets de edicao/inativacao dos catalogos..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[35/545] Validando separacao de autorizacao profissional/paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[36/545] Validando endpoint de status de acesso do paciente..."
...
Write-Host "[37/545] Validando fluxo de convite/ativacao nos assets..."
...
Write-Host "[38/545] Validando autoatendimento do diario..."
...
Write-Host "[39/545] Validando autoatendimento das metas..."
...
Write-Host "[40/545] Validando tela dedicada do portal do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[41/545] Validando navegacao completa do portal..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[42/545] Validando endpoint proprio de plano alimentar..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[43/545] Validando historico proprio de metas e diario..."
...
Write-Host "[47/545] Validando schema/endpoint do catalogo de exercicios..."
...
Write-Host "[48/545] Validando endpoint de planos de treino do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[49/545] Validando construtor visual de treino..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[50/545] Validando videos e prescricao de exercicios..."
...
Write-Host "[51/545] Validando aba de treinos no prontuario..."
...
Write-Host "[52/545] Validando navegacao de treino do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[53/545] Validando endpoint proprio do treino do paciente..."
...
Write-Host "[54/545] Validando assets visuais do modulo de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[55/545] Validando schema de execucoes de treino..."
...
Write-Host "[56/545] Validando historico profissional de treinos..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[57/545] Validando registro visual de execucao pelo paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[58/545] Validando series, repeticoes e carga realizadas..."
...
Write-Host "[59/545] Validando esforco percebido e duracao..."
...
Write-Host "[60/545] Validando historico do paciente..."
...
Write-Host "[62/545] Validando estilos do acompanhamento de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[63/545] Validando motor de graficos SVG..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[64/545] Validando graficos corporais no prontuario..."
...
Write-Host "[65/545] Validando tendencias laboratoriais..."
...
Write-Host "[66/545] Validando progressao grafica de carga..."
...
Write-Host "[69/545] Validando responsividade dos graficos..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[70/545] Validando compatibilidade de schema na v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
...
Write-Host "[71/545] Validando endpoint de insights do dashboard..."
...
Write-Host "[72/545] Validando insights por paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[73/545] Validando regra de exame fora da referencia..."
...
Write-Host "[74/545] Validando regras de evolucao e retorno..."
...
Write-Host "[75/545] Validando regras de adesao..."
...
Write-Host "[77/545] Validando insights no prontuario..."
...
Write-Host "[78/545] Validando estilos e compatibilidade do schema..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[79/545] Validando schema de pendencias clinicas..."
...
Write-Host "[80/545] Validando endpoint geral de pendencias..."
...
Write-Host "[81/545] Validando endpoint de pendencias do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[82/545] Validando acoes de ciclo de vida..."
...
Write-Host "[83/545] Validando criacao de retorno a partir da pendencia..."
...
Write-Host "[85/545] Validando tela de gerenciamento de pendencias..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[86/545] Validando acoes visuais da pendencia..."
...
Write-Host "[88/545] Validando auditoria, estilos e upgrade v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[89/545] Validando schema de notificacoes internas..."
...
Write-Host "[90/545] Validando sincronizacao de notificacoes..."
...
Write-Host "[91/545] Validando listagem e contador nao lido..."
...
Write-Host "[92/545] Validando regras de agenda profissional..."
...
Write-Host "[93/545] Validando regras de pendencias..."
...
Write-Host "[94/545] Validando lembretes do paciente..."
...
Write-Host "[95/545] Validando leitura individual e em massa..."
...
Write-Host "[96/545] Validando sino e drawer na interface..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[97/545] Validando contador e atualizacao periodica..."
...
Write-Host "[98/545] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[99/545] Validando script de popular banco..."
...
Write-Host "[100/545] Validando idempotencia do popular..."
...
Write-Host "[101/545] Validando cobertura de modulos na base demo..."
...
Write-Host "[102/545] Validando endpoint de resumo de dados..."
...
Write-Host "[103/545] Validando que popular banco e opt-in..."
...
Write-Host "[104/545] Validando versao v0.3.27 e upgrade do schema..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
...
Write-Host "[105/545] Validando endpoint da carteira..."
...
Write-Host "[106/545] Validando priorizacao da carteira..."
...
Write-Host "[107/545] Validando sinais de exames/evolucao na carteira..."
...
Write-Host "[108/545] Validando atividade recente..."
...
Write-Host "[109/545] Validando tela Carteira..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[110/545] Validando filtros e ordenacao..."
...
Write-Host "[111/545] Validando atalho da carteira no dashboard..."
...
Write-Host "[112/545] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[113/545] Validando schema de follow-up..."
...
Write-Host "[114/545] Validando endpoint de follow-up..."
...
Write-Host "[115/545] Validando canais e proximo contato..."
...
Write-Host "[116/545] Validando auditoria do contato..."
...
Write-Host "[117/545] Validando follow-up na carteira..."
$carteiraSource = Get-Content .\src\HealthPlatform.Api\Controllers\CarteiraController.cs -Encoding UTF8 -Raw
...
Write-Host "[118/545] Validando acao rapida de contato..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[119/545] Validando acao rapida de retorno..."
...
Write-Host "[121/545] Validando historico no prontuario..."
...
Write-Host "[122/545] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[123/545] Validando endpoint da fila de follow-up..."
...
Write-Host "[124/545] Validando faixas de vencimento..."
...
Write-Host "[125/545] Validando busca e filtros da fila..."
...
Write-Host "[126/545] Validando notificacao de follow-up..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[127/545] Validando tela de follow-up..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[128/545] Validando acoes rapidas na fila..."
...
Write-Host "[129/545] Validando resumo de follow-up no dashboard..."
...
Write-Host "[130/545] Validando estilos, popular e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
$popularSource = Get-Content .\POPULAR.ps1 -Encoding UTF8 -Raw
...
Write-Host "[131/545] Validando endpoint de gestao..."
...
Write-Host "[132/545] Validando indicadores de consultas..."
...
Write-Host "[133/545] Validando indicadores de acompanhamento..."
...
Write-Host "[134/545] Validando indicadores de engajamento..."
...
Write-Host "[135/545] Validando series gerenciais..."
...
Write-Host "[136/545] Validando tela Gestao..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[137/545] Validando graficos e resumo no dashboard..."
...
Write-Host "[138/545] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[139/545] Validando PREPARAR sem atualizacao automatica do dotnet..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[140/545] Validando endpoint CSV gerencial..."
...
Write-Host "[141/545] Validando conteudo longitudinal do CSV..."
...
Write-Host "[142/545] Validando relatorio HTML imprimivel..."
...
Write-Host "[143/545] Validando botoes de exportacao na Gestao..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[144/545] Validando download autenticado..."
...
Write-Host "[145/545] Validando relatorio autenticado em nova janela..."
...
Write-Host "[146/545] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[147/545] Validando perguntas de anamnese inativas..."
...
Write-Host "[148/545] Validando totais completos de insights..."
...
Write-Host "[149/545] Validando timezone das notificacoes..."
...
Write-Host "[150/545] Validando cleanup do polling no logout..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[151/545] Validando navegacao robusta no portal..."
...
Write-Host "[152/545] Validando smoke de notificacoes sem mutacao..."
...
Write-Host "[153/545] Validando copy historica de schema..."
$schemaCheckMarker = 'Write-Host "[153/545] Validando copy historica de schema..."'
...
Write-Host "[154/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / estabilizacao + qualidade: OK."


Write-Host "[155/545] Validando endpoint de busca global..."
...
Write-Host "[157/545] Validando isolamento por organizacao..."
...
Write-Host "[158/545] Validando botao e atalho da busca..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[159/545] Validando modal e debounce..."
...
Write-Host "[160/545] Validando acoes dos resultados..."
...
Write-Host "[161/545] Validando estilos responsivos da busca..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[162/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / busca global + central de acoes: OK."


Write-Host "[163/545] Validando endpoint da Central do Dia..."
...
Write-Host "[164/545] Validando agenda do dia..."
...
Write-Host "[165/545] Validando follow-ups do dia..."
...
Write-Host "[166/545] Validando pendencias prioritarias..."
...
Write-Host "[167/545] Validando pacientes para revisao..."
...
Write-Host "[168/545] Validando tela Hoje..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[169/545] Validando acoes rapidas e dashboard..."
...
Write-Host "[170/545] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[171/545] Validando schema de evolucoes clinicas SOAP..."
...
Write-Host "[172/545] Validando endpoint de evolucoes..."
...
Write-Host "[173/545] Validando consulta opcional e isolamento..."
...
Write-Host "[174/545] Validando auditoria da evolucao..."
...
Write-Host "[175/545] Validando evolucao na timeline..."
...
Write-Host "[176/545] Validando aba Evolucoes no prontuario..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[177/545] Validando registro visual SOAP..."
...
Write-Host "[178/545] Validando edicao visual da evolucao..."
...
Write-Host "[179/545] Validando upgrade SQL SOAP historico e PREPARAR atual..."
...
Write-Host "[180/545] Validando estilos e versao v0.3.27..."
...
Write-Host "[181/545] Validando endpoint de resumo clinico..."
...
Write-Host "[182/545] Validando consulta e SOAP no resumo..."
...
Write-Host "[183/545] Validando corpo e anamnese..."
...
Write-Host "[184/545] Validando exames alterados..."
...
Write-Host "[185/545] Validando metas, treinos e pendencias..."
...
Write-Host "[186/545] Validando resumo no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[187/545] Validando atualizacao e responsividade..."
...
Write-Host "[188/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / resumo clinico consolidado: OK."


Write-Host "[189/545] Validando texto de handoff clinico..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[190/545] Validando conteudo SOAP no handoff..."
...
Write-Host "[191/545] Validando copia para clipboard..."
...
Write-Host "[192/545] Validando impressao do resumo..."
...
Write-Host "[193/545] Validando seguranca basica do HTML imprimivel..."
...
Write-Host "[194/545] Validando botoes no resumo clinico..."
...
Write-Host "[195/545] Validando responsividade das acoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[196/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / handoff clinico + impressao: OK."


Write-Host "[197/545] Validando endpoint de equipe..."
...
Write-Host "[198/545] Validando isolamento e administracao..."
...
Write-Host "[199/545] Validando criacao de acesso..."
...
Write-Host "[200/545] Validando sincronizacao de tipo e role..."
...
Write-Host "[201/545] Validando perfil profissional..."
...
Write-Host "[202/545] Validando protecao do proprio administrador..."
...
Write-Host "[203/545] Validando auditoria da equipe..."
...
Write-Host "[204/545] Validando tela Equipe..."
...
Write-Host "[205/545] Validando visibilidade admin e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[206/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe + gestao de profissionais: OK."


Write-Host "[207/545] Validando filtros da equipe..."
$teamSource = Get-Content .\src\HealthPlatform.Api\Controllers\EquipeController.cs -Encoding UTF8 -Raw
...
Write-Host "[208/545] Validando redefinicao de senha..."
...
Write-Host "[209/545] Validando escopo e protecoes do reset..."
...
Write-Host "[210/545] Validando auditoria segura de senha..."
...
Write-Host "[211/545] Validando filtros visuais..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[212/545] Validando acao visual de senha..."
...
Write-Host "[213/545] Validando responsividade da equipe v2..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[214/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe v2 + seguranca de acesso: OK."


Write-Host "[215/545] Validando endpoint Minha Conta..."
...
Write-Host "[216/545] Validando alteracao da propria senha..."
...
Write-Host "[217/545] Validando protecoes da troca de senha..."
...
Write-Host "[218/545] Validando auditoria segura da conta..."
...
Write-Host "[219/545] Validando sincronizacao do nome profissional..."
...
Write-Host "[220/545] Validando painel Minha Conta..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[221/545] Validando responsividade da conta..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[222/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / Minha Conta + troca de senha: OK."


Write-Host "[223/545] Validando schema de progressao alimentar..."
...
Write-Host "[224/545] Validando simulador nutricional..."
...
Write-Host "[225/545] Validando duplicacao versionada..."
...
Write-Host "[226/545] Validando escala de porcoes..."
...
Write-Host "[227/545] Validando ajuste por calorias alvo..."
...
Write-Host "[228/545] Validando limites de ajuste..."
...
Write-Host "[229/545] Validando encerramento opcional do plano anterior..."
...
Write-Host "[230/545] Validando auditoria da progressao..."
...
Write-Host "[231/545] Validando interface de progressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[232/545] Validando modos percentual e calorias..."
...
Write-Host "[233/545] Validando upgrade SQL e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[234/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao do plano alimentar: OK."


Write-Host "[235/545] Validando schema de progressao de treino..."
...
Write-Host "[236/545] Validando simulador de treino..."
...
Write-Host "[237/545] Validando duplicacao versionada do treino..."
...
Write-Host "[238/545] Validando progressao de carga..."
...
Write-Host "[239/545] Validando progressao de series e descanso..."
...
Write-Host "[240/545] Validando repeticoes estruturadas..."
...
Write-Host "[241/545] Validando limites de progressao..."
...
Write-Host "[242/545] Validando encerramento e auditoria..."
...
Write-Host "[243/545] Validando interface de progressao de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[244/545] Validando projecao e modal ampliado..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[245/545] Validando upgrade SQL e PREPARAR 19..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[246/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao de treino + ciclo versionado: OK."


Write-Host "[247/545] Validando schema dos modelos alimentares..."
...
Write-Host "[248/545] Validando listagem e busca de modelos..."
...
Write-Host "[249/545] Validando salvar plano como modelo..."
...
Write-Host "[250/545] Validando criacao de plano a partir de modelo..."
...
Write-Host "[251/545] Validando alimentos do catalogo..."
...
Write-Host "[252/545] Validando isolamento e auditoria..."
...
Write-Host "[253/545] Validando interface de salvar modelo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[254/545] Validando seletor de modelos..."
...
Write-Host "[255/545] Validando criacao visual via modelo..."
...
Write-Host "[256/545] Validando responsividade dos modelos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[257/545] Validando upgrade SQL e PREPARAR 20..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[258/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de plano alimentar: OK."


Write-Host "[259/545] Validando schema dos modelos de treino..."
...
Write-Host "[260/545] Validando listagem e busca de modelos de treino..."
...
Write-Host "[261/545] Validando salvar treino como modelo..."
if (-not $modelSource.Contains("salvar-como-modelo") -or
...
Write-Host "[262/545] Validando criacao de treino a partir de modelo..."
if (-not $modelSource.Contains("criar-de-modelo") -or
...
Write-Host "[263/545] Validando catalogo de exercicios..."
...
Write-Host "[264/545] Validando prescricao completa do template..."
...
Write-Host "[265/545] Validando isolamento e auditoria dos modelos de treino..."
if (-not $modelSource.Contains("currentUser.OrganizationId") -or
    -not $modelSource.Contains('"CREATE_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("AuditLogs")) {
...
Write-Host "[266/545] Validando interface de salvar modelo de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[267/545] Validando seletor e busca de modelos..."
...
Write-Host "[268/545] Validando criacao visual via modelo..."
...
Write-Host "[269/545] Validando upgrade SQL, responsividade e PREPARAR 21..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[270/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de treino + criacao rapida: OK."


Write-Host "[271/545] Validando schema das metas nutricionais..."
$planEntity = Get-Content .\src\HealthPlatform.Domain\Entities\PlanoAlimentar.cs -Encoding UTF8 -Raw
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[272/545] Validando contratos nutricionais..."
...
Write-Host "[273/545] Validando endpoint de metas..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[274/545] Validando analise nutricional..."
...
Write-Host "[275/545] Validando metas na criacao e edicao..."
...
Write-Host "[276/545] Validando metas na progressao alimentar..."
...
Write-Host "[277/545] Validando metas nos templates alimentares..."
...
Write-Host "[278/545] Validando construtor com metas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[279/545] Validando Meta x Prescrito no prontuario..."
...
Write-Host "[280/545] Validando distribuicao por refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[281/545] Validando modal de metas..."
...
Write-Host "[282/545] Validando responsividade nutricional..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[283/545] Validando SQL e PREPARAR 22..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[284/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / metas nutricionais + distribuicao: OK."


Write-Host "[285/545] Validando schema da biblioteca de refeicoes..."
...
Write-Host "[286/545] Validando listagem e filtros da biblioteca..."
...
Write-Host "[287/545] Validando salvar refeicao como modelo..."
...
Write-Host "[288/545] Validando insercao de refeicao no plano..."
...
Write-Host "[289/545] Validando catalogo e substituicoes..."
if (-not $modelSource.Contains("alimentosValidos") -or
    -not $modelSource.Contains("alimentosInvalidos") -or
...
Write-Host "[290/545] Validando protecoes e auditoria..."
...
Write-Host "[291/545] Validando botao Salvar refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[292/545] Validando biblioteca visual..."
...
Write-Host "[293/545] Validando insercao visual rapida..."
...
Write-Host "[294/545] Validando responsividade da biblioteca..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[295/545] Validando SQL e PREPARAR 23..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[296/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de refeicoes + insercao rapida: OK."


Write-Host "[297/545] Validando schema da biblioteca de sessoes..."
...
Write-Host "[298/545] Validando listagem e filtros..."
...
Write-Host "[299/545] Validando salvar sessao como modelo..."
...
Write-Host "[300/545] Validando insercao de sessao no plano..."
...
Write-Host "[301/545] Validando catalogo e prescricao..."
if (-not $modelSource.Contains("exerciciosValidos") -or
    -not $modelSource.Contains("exerciciosInvalidos") -or
...
Write-Host "[302/545] Validando protecoes e auditoria..."
if (-not $modelSource.Contains('plano.Status == "Concluido"') -or
    -not $modelSource.Contains('"INSERT_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("currentUser.OrganizationId")) {
...
Write-Host "[303/545] Validando botao Salvar sessao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[304/545] Validando biblioteca visual..."
...
Write-Host "[305/545] Validando insercao visual rapida..."
...
Write-Host "[306/545] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[307/545] Validando SQL e PREPARAR 24..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[308/545] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de sessoes + insercao rapida: OK."


Write-Host "[309/545] Validando endpoint longitudinal de habitos..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[310/545] Validando contrato da evolucao de habitos..."
...
Write-Host "[311/545] Validando series de sono, estresse, atividade e agua..."
...
Write-Host "[312/545] Validando limite e isolamento multi-tenant..."
...
Write-Host "[313/545] Validando comparacao com registro anterior..."
...
Write-Host "[314/545] Validando graficos de habitos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[315/545] Validando resumo atual dos habitos..."
...
Write-Host "[316/545] Validando integracao na aba Anamnese..."
...
Write-Host "[317/545] Validando integracao no Resumo..."
...
Write-Host "[318/545] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[319/545] Validando compatibilidade de banco e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "[320/545] Validando versao v0.3.28..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.28 / evolucao de habitos + graficos de anamnese: OK."


Write-Host "[321/545] Validando resposta runtime com metas por refeicao..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[322/545] Validando schema de metas por refeicao..."
...
Write-Host "[323/545] Validando contratos de metas por refeicao..."
$contractsSource = Get-Content .\src\HealthPlatform.Api\Contracts\PlanosAlimentares\PlanoAlimentarContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[324/545] Validando endpoint de meta individual..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[325/545] Validando distribuicao automatica..."
...
Write-Host "[326/545] Validando isolamento e integridade da distribuicao..."
...
Write-Host "[327/545] Validando progressao com metas por refeicao..."
...
Write-Host "[328/545] Validando templates de plano..."
$templateSource = Get-Content .\src\HealthPlatform.Api\Controllers\ModelosPlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[329/545] Validando biblioteca de refeicoes..."
...
Write-Host "[330/545] Validando construtor alimentar..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[331/545] Validando edicao e comparacao visual..."
...
Write-Host "[332/545] Validando modal de distribuicao..."
...
Write-Host "[333/545] Validando SQL e PREPARAR 25..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[334/545] Validando versao v0.3.29..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.29 / metas por refeicao + distribuicao planejada: OK."


Write-Host "[335/545] Validando schema das fases nutricionais..."
...
Write-Host "[336/545] Validando listagem das fases..."
...
Write-Host "[337/545] Validando criacao de fase..."
...
Write-Host "[338/545] Validando edicao e estados..."
...
Write-Host "[339/545] Validando vinculo seguro com plano alimentar..."
...
Write-Host "[340/545] Validando reordenacao do ciclo..."
...
Write-Host "[341/545] Validando exclusao protegida..."
...
Write-Host "[342/545] Validando isolamento e auditoria..."
...
Write-Host "[343/545] Validando interface das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[344/545] Validando formulario de fase..."
...
Write-Host "[345/545] Validando reordenacao visual..."
...
Write-Host "[346/545] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[347/545] Validando SQL e PREPARAR 26..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[348/545] Validando versao v0.3.30..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.30 / fases nutricionais + planejamento de ciclo: OK."


Write-Host "[349/545] Validando schema das fases de treino..."
...
Write-Host "[350/545] Validando listagem das fases..."
...
Write-Host "[351/545] Validando criacao de fase..."
...
Write-Host "[352/545] Validando edicao e estados..."
...
Write-Host "[353/545] Validando vinculo seguro com ficha..."
if (-not $phaseSource.Contains("PlanoValido") -or
    -not $phaseSource.Contains("x.PacienteId == pacienteId") -or
    -not $phaseSource.Contains("x.Paciente.OrganizacaoId == currentUser.OrganizationId")) {
...
Write-Host "[354/545] Validando reordenacao do ciclo..."
...
Write-Host "[355/545] Validando exclusao protegida..."
if (-not $phaseSource.Contains("HttpDelete") -or
    -not $phaseSource.Contains('fase.Status == "EmAndamento"') -or
    -not $phaseSource.Contains('"DELETE"')) {
...
Write-Host "[356/545] Validando isolamento e auditoria..."
if (-not $phaseSource.Contains("currentUser.OrganizationId") -or
    -not $phaseSource.Contains("AuditLogs") -or
...
Write-Host "[357/545] Validando interface do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[358/545] Validando formulario de fase..."
...
Write-Host "[359/545] Validando reordenacao visual..."
...
Write-Host "[360/545] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[361/545] Validando SQL e PREPARAR 27..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[362/545] Validando versao v0.3.31..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.31 / ciclos de treino + periodizacao: OK."


Write-Host "[363/545] Validando schema dos check-ins..."
...
Write-Host "[364/545] Validando endpoint profissional..."
...
Write-Host "[365/545] Validando criacao e edicao..."
...
Write-Host "[366/545] Validando limites dos indicadores..."
...
Write-Host "[367/545] Validando vinculo com fases..."
...
Write-Host "[368/545] Validando auto-vinculo do paciente..."
...
Write-Host "[369/545] Validando portal do paciente..."
...
Write-Host "[370/545] Validando isolamento multi-tenant..."
...
Write-Host "[371/545] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[372/545] Validando graficos de resposta..."
...
Write-Host "[373/545] Validando formulario profissional..."
...
Write-Host "[374/545] Validando check-in no portal..."
...
Write-Host "[375/545] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[376/545] Validando SQL e PREPARAR 28..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[377/545] Validando preservacao dos ciclos..."
...
Write-Host "[378/545] Validando versao v0.3.32..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.32 / check-ins de evolucao + adesao por fase: OK."


Write-Host "[379/545] Validando endpoint de analise por fase..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[380/545] Validando metricas agregadas..."
...
Write-Host "[381/545] Validando variacao de peso por fase..."
...
Write-Host "[382/545] Validando destaques automaticos..."
...
Write-Host "[383/545] Validando isolamento multi-tenant..."
...
Write-Host "[384/545] Validando cards comparativos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[385/545] Validando destaques visuais..."
...
Write-Host "[386/545] Validando integracao com nutricao..."
...
Write-Host "[387/545] Validando integracao com treino..."
...
Write-Host "[388/545] Validando resumo consolidado..."
...
Write-Host "[389/545] Validando responsividade e compatibilidade de banco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[390/545] Validando versao v0.3.33..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.33 / analise de fases + comparativo de resposta: OK."

Write-Host "[391/545] Validando metas das fases..."
...
Write-Host "[392/545] Validando mapeamento EF..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[393/545] Validando CRUD das fases..."
...
Write-Host "[394/545] Validando limites dos criterios..."
...
Write-Host "[395/545] Validando endpoint runtime de prontidao..."
...
Write-Host "[396/545] Validando motor de criterios objetivos..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[397/545] Validando revisao profissional..."
...
Write-Host "[398/545] Validando formularios das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[399/545] Validando metas nos cards..."
...
Write-Host "[400/545] Validando painel de prontidao..."
...
Write-Host "[405/545] Validando SQL e PREPARAR 29..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[406/545] Validando versao v0.3.34..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.34 / metas de fase + criterios de transicao: OK."


Write-Host "[407/545] Validando entidade de revisao..."
...
Write-Host "[408/545] Validando mapeamento das revisoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[409/545] Validando historico runtime..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[410/545] Validando revisao nutricional..."
...
Write-Host "[411/545] Validando revisao de treino..."
...
Write-Host "[412/545] Validando decisoes e fase ativa..."
...
Write-Host "[413/545] Validando override consciente..."
...
Write-Host "[414/545] Validando transicao para proxima fase..."
...
Write-Host "[415/545] Validando transacao e auditoria..."
...
Write-Host "[416/545] Validando modal de revisao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[417/545] Validando historico visual..."
...
Write-Host "[418/545] Validando integracao com painel de transicao..."
...
Write-Host "[419/545] Validando SQL e PREPARAR 30..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[420/545] Validando versao v0.3.35..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.35 / revisao de fase + transicao assistida: OK."


Write-Host "[421/545] Validando controller de volume..."
...
Write-Host "[422/545] Validando isolamento multi-tenant..."
...
Write-Host "[423/545] Validando volume planejado por grupo..."
...
Write-Host "[424/545] Validando frequencia semanal..."
...
Write-Host "[425/545] Validando fallback de frequencia..."
...
Write-Host "[426/545] Validando execucoes reais..."
...
Write-Host "[427/545] Validando ausencia de tonelagem inventada..."
...
Write-Host "[428/545] Validando runtime da analise..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
    try {
...
Write-Host "[429/545] Validando painel de volume..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[430/545] Validando resumo analitico..."
...
Write-Host "[431/545] Validando volume por sessao..."
...
Write-Host "[432/545] Validando integracao e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[433/545] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "[434/545] Validando versao v0.3.36..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.36 / volume de treino + distribuicao muscular: OK."


Write-Host "[435/545] Validando endpoints de progressao por exercicio..."
...
Write-Host "[436/545] Validando seguranca e tenant..."
...
Write-Host "[437/545] Validando separacao por unidade..."
...
Write-Host "[438/545] Validando metricas de carga..."
...
Write-Host "[439/545] Validando recordes sucessivos..."
...
Write-Host "[440/545] Validando tendencia de carga..."
...
Write-Host "[441/545] Validando protecao contra estimativas artificiais..."
...
Write-Host "[442/545] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[443/545] Validando payload de pontos..."
...
Write-Host "[444/545] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[445/545] Validando graficos e recordes..."
...
Write-Host "[446/545] Validando portal do paciente..."
...
Write-Host "[447/545] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[448/545] Validando versao v0.3.37..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.37 / progressao por exercicio + recordes de carga: OK."


Write-Host "[449/545] Validando endpoints de sinais de progressao..."
...
Write-Host "[450/545] Validando tenant e PatientOnly..."
...
Write-Host "[451/545] Validando estados da analise..."
...
Write-Host "[452/545] Validando regra de estagnacao..."
...
Write-Host "[453/545] Validando sinal de carga/RPE..."
...
Write-Host "[454/545] Validando progressao recente..."
...
Write-Host "[455/545] Validando semantica nao diagnostica..."
...
Write-Host "[456/545] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[457/545] Validando payload analitico..."
...
Write-Host "[458/545] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[459/545] Validando portal do paciente..."
...
Write-Host "[460/545] Validando cards e graficos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[461/545] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[462/545] Validando versao v0.3.38..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.38 / estagnacao + fadiga + sinais de progressao: OK."


Write-Host "[463/545] Validando identidade MVP Preview..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.5.4") -or
...
Write-Host "[464/545] Validando aviso de demonstracao..."
...
Write-Host "[465/545] Validando roteiro da demo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[466/545] Validando checklist de avaliacao..."
...
Write-Host "[467/545] Validando modelo de feedback..."
...
Write-Host "[468/545] Validando dashboard de apresentacao..."
...
Write-Host "[469/545] Validando atalho Escape..."
...
Write-Host "[470/545] Validando feedback de conectividade..."
...
Write-Host "[471/545] Validando acabamento de foco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[472/545] Validando estados vazios..."
...
Write-Host "[473/545] Validando limpeza da navegacao de demo..."
...
Write-Host "[474/545] Validando responsividade do MVP..."
...
Write-Host "[475/545] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
...
Write-Host "[476/545] Validando versao v0.3.39..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.39 / MVP Preview + polimento de demonstracao: OK."


Write-Host "[477/545] Validando Dockerfile..."
...
Write-Host "[478/545] Validando bind dinamico de porta..."
...
Write-Host "[479/545] Validando Blueprint Render..."
...
Write-Host "[480/545] Validando secrets do Blueprint..."
...
Write-Host "[481/545] Validando conexao PostgreSQL do Render..."
...
Write-Host "[482/545] Validando bootstrap isolado do MVP..."
...
Write-Host "[483/545] Validando fluxo local preservado..."
...
Write-Host "[484/545] Validando healthcheck real..."
...
Write-Host "[485/545] Validando forwarded headers..."
...
Write-Host "[486/545] Validando POPULAR remoto..."
...
if (-not $remotePopular.Contains("[Parameter(Mandatory=`$true)][string]`$BaseUrl") -or
    -not $remotePopular.Contains("HealthPlatform v0.5.4 - POPULAR RENDER DEMO") -or
...
Write-Host "[487/545] Validando catalogos da demo remota..."
...
Write-Host "[488/545] Validando smoke test remoto..."
...
Write-Host "[489/545] Validando guia de deploy..."
...
Write-Host "[490/545] Validando ausencia de schema novo..."
...
Write-Host "[491/545] Validando arquivos de container..."
...
Write-Host "[492/545] Validando versao base do deploy..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.4 / Render Demo Deploy preservado: OK."


Write-Host "[493/545] Validando paleta RS..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[494/545] Validando monograma RS..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
...
Write-Host "[495/545] Validando linguagem editorial..."
...
Write-Host "[496/545] Validando tipografia condensada..."
...
Write-Host "[497/545] Validando login editorial..."
...
Write-Host "[498/545] Validando sidebar RS..."
...
Write-Host "[499/545] Validando experiencia iPad..."
...
Write-Host "[500/545] Validando drawer mobile..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[501/545] Validando safe areas iOS..."
...
Write-Host "[502/545] Validando modal bottom-sheet no iPhone..."
...
Write-Host "[503/545] Validando portal mobile..."
...
Write-Host "[504/545] Validando abas do prontuario no mobile..."
...
Write-Host "[505/545] Validando touch targets..."
...
Write-Host "[506/545] Validando cards analiticos no iPhone..."
...
Write-Host "[507/545] Validando deploy/demo preservados..."
$renderSource = Get-Content .\render.yaml -Encoding UTF8 -Raw
...
Write-Host "[508/545] Validando entidade de solicitacoes clinicas..."
...
Write-Host "[509/545] Validando mapeamento EF de solicitacoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[510/545] Validando endpoints profissionais de solicitacoes..."
...
Write-Host "[511/545] Validando portal do paciente para solicitacoes..."
...
Write-Host "[512/545] Validando isolamento multi-tenant..."
...
Write-Host "[513/545] Validando auditoria de solicitacoes..."
...
Write-Host "[514/545] Validando interface profissional..."
...
Write-Host "[515/545] Validando interface do paciente..."
...
Write-Host "[516/545] Validando upgrade SQL v0.5.1 preservado..."
...
Write-Host "[517/545] Validando PREPARAR 31/31..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[518/545] Validando versao v0.5.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.4 / Solicitacoes Clinicas + Connected Care: OK."

Write-Host "[519/545] Validando notificacoes de solicitacoes no backend..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[520/545] Validando prioridade por prazo para o paciente..."
...
Write-Host "[521/545] Validando notificacao de resposta para profissional..."
...
Write-Host "[522/545] Validando limpeza de notificacoes encerradas..."
...
Write-Host "[523/545] Validando navegacao do paciente pela notificacao..."
...
Write-Host "[524/545] Validando navegacao contextual do profissional..."
...
Write-Host "[525/545] Validando identidade visual de notificacao..."
...
Write-Host "[526/545] Validando versao funcional v0.5.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
if (-not $notificationSource.Contains('SolicitacaoClinica')) { throw "Feature v0.5.4 ausente." }
Write-Host "    v0.5.4 / Notificacoes Contextuais + Connected Care: OK."

Write-Host "[527/545] Validando endpoint da Central de Solicitacoes..."
$requestController = Get-Content .\src\HealthPlatform.Api\Controllers\SolicitacoesClinicasController.cs -Encoding UTF8 -Raw
...
Write-Host "[528/545] Validando resumo operacional de solicitacoes..."
...
Write-Host "[529/545] Validando filtros da Central de Solicitacoes..."
...
Write-Host "[530/545] Validando isolamento da fila profissional..."
...
Write-Host "[531/545] Validando navegacao profissional de solicitacoes..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[532/545] Validando indicadores e filtros visuais..."
...
Write-Host "[533/545] Validando acoes rapidas da Central de Solicitacoes..."
...
Write-Host "[534/545] Validando responsividade da Central de Solicitacoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[535/545] Validando versao funcional v0.5.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
if (-not $requestController.Contains('FilaProfissional')) { throw "Feature v0.5.4 ausente." }
Write-Host "    v0.5.4 / Central de Solicitacoes + Connected Care: OK."


Write-Host "[536/545] Validando solicitacoes no Hoje do paciente..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[537/545] Validando solicitacoes no progresso diario..."
...
Write-Host "[538/545] Validando card de acao necessaria..."
...
Write-Host "[539/545] Validando destaque de solicitacao vencida no portal..."
...
Write-Host "[540/545] Validando integracao da Central do Dia no backend..."
$centralSource = Get-Content .\src\HealthPlatform.Api\Controllers\CentralDiaController.cs -Encoding UTF8 -Raw
...
Write-Host "[541/545] Validando escopo profissional das solicitacoes do dia..."
...
Write-Host "[542/545] Validando card profissional de solicitacoes..."
...
Write-Host "[543/545] Validando dashboard com solicitacoes para revisao..."
...
Write-Host "[544/545] Validando responsividade das solicitacoes no Hoje..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[545/545] Validando versao funcional v0.5.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.4") { throw "VERSION.txt inesperado." }
if (-not $centralSource.Contains('SolicitacoesParaRevisao') -or -not $appJsSource.Contains('patient-today-requests')) { throw "Feature v0.5.4 ausente." }
Write-Host "    v0.5.4 / Solicitacoes no Hoje + Connected Care: OK."
