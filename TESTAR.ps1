...
Write-Host "[1/588] Healthcheck..." -ForegroundColor Cyan
$health = Invoke-RestMethod -Uri "$base/api/health" -Method Get
if ($health.version -ne "0.5.9") { throw "Versao inesperada da API: $($health.version)" }
Write-Host "    API $($health.version) / banco $($health.database)" -ForegroundColor Green

Write-Host "[2/588] Login..." -ForegroundColor Cyan
...
Write-Host "[3/588] Listando pacientes..." -ForegroundColor Cyan
...
Write-Host "[4/588] Validando perguntas de anamnese..." -ForegroundColor Cyan
...
Write-Host "[5/588] Validando catalogo laboratorial..." -ForegroundColor Cyan
...
Write-Host "[6/588] Validando catalogo de alimentos..." -ForegroundColor Cyan
...
Write-Host "[7/588] Validando busca/paginacao..." -ForegroundColor Cyan
...
Write-Host "[8/588] Validando modulos do paciente..." -ForegroundColor Cyan
...
Write-Host "[9/588] Validando metas do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[10/588] Validando diario/resumo do dia..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[11/588] Validando portal/home do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[12/588] Validando agenda do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[13/588] Validando dashboard do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[14/588] Validando interface web..." -ForegroundColor Cyan
...
Write-Host "[15/588] Validando assets da interface..." -ForegroundColor Cyan
...
Write-Host "[16/588] Validando prontuario visual v0.2.2..." -ForegroundColor Cyan
...
Write-Host "[17/588] Validando acoes clinicas da interface..." -ForegroundColor Cyan
...
Write-Host "[18/588] Validando rotas usadas pelos formularios clinicos..." -ForegroundColor Cyan
...
Write-Host "[19/588] Validando cadastro visual de exames..." -ForegroundColor Cyan
...
Write-Host "[20/588] Validando construtor visual do plano alimentar..." -ForegroundColor Cyan
...
Write-Host "[21/588] Validando relatorios na interface..." -ForegroundColor Cyan
...
Write-Host "[22/588] Validando edicao visual do paciente..." -ForegroundColor Cyan
...
Write-Host "[23/588] Validando endpoint de relatorios do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[24/588] Validando edicao clinica visual..."
...
Write-Host "[25/588] Validando agenda operacional..."
...
Write-Host "[26/588] Validando endpoint de atualizacao de avaliacao..."
try {
...
Write-Host "[27/588] Validando tela de configuracoes..."
...
Write-Host "[28/588] Validando gerenciadores de catalogo na interface..."
...
Write-Host "[29/588] Validando resumo de configuracoes do consultorio..."
...
Write-Host "[30/588] Validando rotas dos catalogos..."
...
Write-Host "[31/588] Validando edicao de configuracoes..."
$cfg = Invoke-RestMethod -Uri "$base/api/configuracoes/resumo" -Headers $headers
...
Write-Host "[32/588] Validando assets de edicao/inativacao dos catalogos..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[35/588] Validando separacao de autorizacao profissional/paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[36/588] Validando endpoint de status de acesso do paciente..."
...
Write-Host "[37/588] Validando fluxo de convite/ativacao nos assets..."
...
Write-Host "[38/588] Validando autoatendimento do diario..."
...
Write-Host "[39/588] Validando autoatendimento das metas..."
...
Write-Host "[40/588] Validando tela dedicada do portal do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[41/588] Validando navegacao completa do portal..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[42/588] Validando endpoint proprio de plano alimentar..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[43/588] Validando historico proprio de metas e diario..."
...
Write-Host "[47/588] Validando schema/endpoint do catalogo de exercicios..."
...
Write-Host "[48/588] Validando endpoint de planos de treino do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[49/588] Validando construtor visual de treino..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[50/588] Validando videos e prescricao de exercicios..."
...
Write-Host "[51/588] Validando aba de treinos no prontuario..."
...
Write-Host "[52/588] Validando navegacao de treino do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[53/588] Validando endpoint proprio do treino do paciente..."
...
Write-Host "[54/588] Validando assets visuais do modulo de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[55/588] Validando schema de execucoes de treino..."
...
Write-Host "[56/588] Validando historico profissional de treinos..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[57/588] Validando registro visual de execucao pelo paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[58/588] Validando series, repeticoes e carga realizadas..."
...
Write-Host "[59/588] Validando esforco percebido e duracao..."
...
Write-Host "[60/588] Validando historico do paciente..."
...
Write-Host "[61/588] Validando progressao de carga no prontuario..."
...
Write-Host "[62/588] Validando estilos do acompanhamento de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[63/588] Validando motor de graficos SVG..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[64/588] Validando graficos corporais no prontuario..."
...
Write-Host "[65/588] Validando tendencias laboratoriais..."
...
Write-Host "[66/588] Validando progressao grafica de carga..."
...
Write-Host "[67/588] Validando evolucao visual no portal do paciente..."
...
Write-Host "[68/588] Validando graficos de exames no portal..."
...
Write-Host "[69/588] Validando responsividade dos graficos..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[70/588] Validando compatibilidade de schema na v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
...
Write-Host "[71/588] Validando endpoint de insights do dashboard..."
...
Write-Host "[72/588] Validando insights por paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[73/588] Validando regra de exame fora da referencia..."
...
Write-Host "[74/588] Validando regras de evolucao e retorno..."
...
Write-Host "[75/588] Validando regras de adesao..."
...
Write-Host "[76/588] Validando central de atencao visual..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[77/588] Validando insights no prontuario..."
...
Write-Host "[78/588] Validando estilos e compatibilidade do schema..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[79/588] Validando schema de pendencias clinicas..."
...
Write-Host "[80/588] Validando endpoint geral de pendencias..."
...
Write-Host "[81/588] Validando endpoint de pendencias do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[82/588] Validando acoes de ciclo de vida..."
...
Write-Host "[83/588] Validando criacao de retorno a partir da pendencia..."
...
Write-Host "[84/588] Validando transformar insight em pendencia..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[85/588] Validando tela de gerenciamento de pendencias..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[86/588] Validando acoes visuais da pendencia..."
...
Write-Host "[87/588] Validando resumo de pendencias no dashboard..."
...
Write-Host "[88/588] Validando auditoria, estilos e upgrade v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[89/588] Validando schema de notificacoes internas..."
...
Write-Host "[90/588] Validando sincronizacao de notificacoes..."
...
Write-Host "[91/588] Validando listagem e contador nao lido..."
...
Write-Host "[92/588] Validando regras de agenda profissional..."
...
Write-Host "[93/588] Validando regras de pendencias..."
...
Write-Host "[94/588] Validando lembretes do paciente..."
...
Write-Host "[95/588] Validando leitura individual e em massa..."
...
Write-Host "[96/588] Validando sino e drawer na interface..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[97/588] Validando contador e atualizacao periodica..."
...
Write-Host "[98/588] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[99/588] Validando script de popular banco..."
...
Write-Host "[100/588] Validando idempotencia do popular..."
...
Write-Host "[101/588] Validando cobertura de modulos na base demo..."
...
Write-Host "[102/588] Validando endpoint de resumo de dados..."
...
Write-Host "[103/588] Validando que popular banco e opt-in..."
...
Write-Host "[104/588] Validando versao v0.3.27 e upgrade do schema..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if ($setupSource -notmatch "\[32/32\]") { throw "PREPARAR deveria possuir 31 etapas na v0.3.27." }
Write-Host "    v0.3.27 / PREPARAR 29 etapas / upgrade SOAP: OK."


Write-Host "[105/588] Validando endpoint da carteira..."
...
Write-Host "[106/588] Validando priorizacao da carteira..."
...
Write-Host "[107/588] Validando sinais de exames/evolucao na carteira..."
...
Write-Host "[108/588] Validando atividade recente..."
...
Write-Host "[109/588] Validando tela Carteira..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[110/588] Validando filtros e ordenacao..."
...
Write-Host "[111/588] Validando atalho da carteira no dashboard..."
...
Write-Host "[112/588] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[113/588] Validando schema de follow-up..."
...
Write-Host "[114/588] Validando endpoint de follow-up..."
...
Write-Host "[115/588] Validando canais e proximo contato..."
...
Write-Host "[116/588] Validando auditoria do contato..."
...
Write-Host "[117/588] Validando follow-up na carteira..."
$carteiraSource = Get-Content .\src\HealthPlatform.Api\Controllers\CarteiraController.cs -Encoding UTF8 -Raw
...
Write-Host "[118/588] Validando acao rapida de contato..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[119/588] Validando acao rapida de retorno..."
...
Write-Host "[120/588] Validando acao rapida de pendencia..."
...
Write-Host "[121/588] Validando historico no prontuario..."
...
Write-Host "[122/588] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[123/588] Validando endpoint da fila de follow-up..."
...
Write-Host "[124/588] Validando faixas de vencimento..."
...
Write-Host "[125/588] Validando busca e filtros da fila..."
...
Write-Host "[126/588] Validando notificacao de follow-up..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[127/588] Validando tela de follow-up..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[128/588] Validando acoes rapidas na fila..."
...
Write-Host "[129/588] Validando resumo de follow-up no dashboard..."
...
Write-Host "[130/588] Validando estilos, popular e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
$popularSource = Get-Content .\POPULAR.ps1 -Encoding UTF8 -Raw
...
Write-Host "[131/588] Validando endpoint de gestao..."
...
Write-Host "[132/588] Validando indicadores de consultas..."
...
Write-Host "[133/588] Validando indicadores de acompanhamento..."
...
Write-Host "[134/588] Validando indicadores de engajamento..."
...
Write-Host "[135/588] Validando series gerenciais..."
...
Write-Host "[136/588] Validando tela Gestao..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[137/588] Validando graficos e resumo no dashboard..."
...
Write-Host "[138/588] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[139/588] Validando PREPARAR sem atualizacao automatica do dotnet..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[140/588] Validando endpoint CSV gerencial..."
...
Write-Host "[141/588] Validando conteudo longitudinal do CSV..."
...
Write-Host "[142/588] Validando relatorio HTML imprimivel..."
...
Write-Host "[143/588] Validando botoes de exportacao na Gestao..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[144/588] Validando download autenticado..."
...
Write-Host "[145/588] Validando relatorio autenticado em nova janela..."
...
Write-Host "[146/588] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[147/588] Validando perguntas de anamnese inativas..."
...
Write-Host "[148/588] Validando totais completos de insights..."
...
Write-Host "[149/588] Validando timezone das notificacoes..."
...
Write-Host "[150/588] Validando cleanup do polling no logout..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[151/588] Validando navegacao robusta no portal..."
...
Write-Host "[152/588] Validando smoke de notificacoes sem mutacao..."
...
Write-Host "[153/588] Validando copy historica de schema..."
$schemaCheckMarker = 'Write-Host "[153/588] Validando copy historica de schema..."'
...
Write-Host "[154/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / estabilizacao + qualidade: OK."


Write-Host "[155/588] Validando endpoint de busca global..."
...
Write-Host "[156/588] Validando fontes da busca..."
...
Write-Host "[157/588] Validando isolamento por organizacao..."
...
Write-Host "[158/588] Validando botao e atalho da busca..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[159/588] Validando modal e debounce..."
...
Write-Host "[160/588] Validando acoes dos resultados..."
...
Write-Host "[161/588] Validando estilos responsivos da busca..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[162/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / busca global + central de acoes: OK."


Write-Host "[163/588] Validando endpoint da Central do Dia..."
...
Write-Host "[164/588] Validando agenda do dia..."
...
Write-Host "[165/588] Validando follow-ups do dia..."
...
Write-Host "[166/588] Validando pendencias prioritarias..."
...
Write-Host "[167/588] Validando pacientes para revisao..."
...
Write-Host "[168/588] Validando tela Hoje..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[169/588] Validando acoes rapidas e dashboard..."
...
Write-Host "[170/588] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[171/588] Validando schema de evolucoes clinicas SOAP..."
...
Write-Host "[172/588] Validando endpoint de evolucoes..."
...
Write-Host "[173/588] Validando consulta opcional e isolamento..."
...
Write-Host "[174/588] Validando auditoria da evolucao..."
...
Write-Host "[175/588] Validando evolucao na timeline..."
...
Write-Host "[176/588] Validando aba Evolucoes no prontuario..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[177/588] Validando registro visual SOAP..."
...
Write-Host "[178/588] Validando edicao visual da evolucao..."
...
Write-Host "[179/588] Validando upgrade SQL SOAP historico e PREPARAR atual..."
...
if (-not $setupSource.Contains("[32/32]") -or
...
Write-Host "    SOAP v0.3.15 preservado / PREPARAR atual 32/32: OK."

Write-Host "[180/588] Validando estilos e versao v0.3.27..."
...
Write-Host "[181/588] Validando endpoint de resumo clinico..."
...
Write-Host "[182/588] Validando consulta e SOAP no resumo..."
...
Write-Host "[183/588] Validando corpo e anamnese..."
...
Write-Host "[184/588] Validando exames alterados..."
...
Write-Host "[185/588] Validando metas, treinos e pendencias..."
...
Write-Host "[186/588] Validando resumo no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[187/588] Validando atualizacao e responsividade..."
...
Write-Host "[188/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / resumo clinico consolidado: OK."


Write-Host "[189/588] Validando texto de handoff clinico..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[190/588] Validando conteudo SOAP no handoff..."
...
Write-Host "[191/588] Validando copia para clipboard..."
...
Write-Host "[192/588] Validando impressao do resumo..."
...
Write-Host "[193/588] Validando seguranca basica do HTML imprimivel..."
...
Write-Host "[194/588] Validando botoes no resumo clinico..."
...
Write-Host "[195/588] Validando responsividade das acoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[196/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / handoff clinico + impressao: OK."


Write-Host "[197/588] Validando endpoint de equipe..."
...
Write-Host "[198/588] Validando isolamento e administracao..."
...
Write-Host "[199/588] Validando criacao de acesso..."
...
Write-Host "[200/588] Validando sincronizacao de tipo e role..."
...
Write-Host "[201/588] Validando perfil profissional..."
...
Write-Host "[202/588] Validando protecao do proprio administrador..."
...
Write-Host "[203/588] Validando auditoria da equipe..."
...
Write-Host "[204/588] Validando tela Equipe..."
...
Write-Host "[205/588] Validando visibilidade admin e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[206/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe + gestao de profissionais: OK."


Write-Host "[207/588] Validando filtros da equipe..."
$teamSource = Get-Content .\src\HealthPlatform.Api\Controllers\EquipeController.cs -Encoding UTF8 -Raw
...
Write-Host "[208/588] Validando redefinicao de senha..."
...
Write-Host "[209/588] Validando escopo e protecoes do reset..."
...
Write-Host "[210/588] Validando auditoria segura de senha..."
...
Write-Host "[211/588] Validando filtros visuais..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[212/588] Validando acao visual de senha..."
...
Write-Host "[213/588] Validando responsividade da equipe v2..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[214/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe v2 + seguranca de acesso: OK."


Write-Host "[215/588] Validando endpoint Minha Conta..."
...
Write-Host "[216/588] Validando alteracao da propria senha..."
...
Write-Host "[217/588] Validando protecoes da troca de senha..."
...
Write-Host "[218/588] Validando auditoria segura da conta..."
...
Write-Host "[219/588] Validando sincronizacao do nome profissional..."
...
Write-Host "[220/588] Validando painel Minha Conta..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[221/588] Validando responsividade da conta..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[222/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / Minha Conta + troca de senha: OK."


Write-Host "[223/588] Validando schema de progressao alimentar..."
...
Write-Host "[224/588] Validando simulador nutricional..."
...
Write-Host "[225/588] Validando duplicacao versionada..."
...
Write-Host "[226/588] Validando escala de porcoes..."
...
Write-Host "[227/588] Validando ajuste por calorias alvo..."
...
Write-Host "[228/588] Validando limites de ajuste..."
...
Write-Host "[229/588] Validando encerramento opcional do plano anterior..."
...
Write-Host "[230/588] Validando auditoria da progressao..."
...
Write-Host "[231/588] Validando interface de progressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[232/588] Validando modos percentual e calorias..."
...
Write-Host "[233/588] Validando upgrade SQL e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[234/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao do plano alimentar: OK."


Write-Host "[235/588] Validando schema de progressao de treino..."
...
Write-Host "[236/588] Validando simulador de treino..."
...
Write-Host "[237/588] Validando duplicacao versionada do treino..."
...
Write-Host "[238/588] Validando progressao de carga..."
...
Write-Host "[239/588] Validando progressao de series e descanso..."
...
Write-Host "[240/588] Validando repeticoes estruturadas..."
...
Write-Host "[241/588] Validando limites de progressao..."
...
Write-Host "[242/588] Validando encerramento e auditoria..."
...
Write-Host "[243/588] Validando interface de progressao de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[244/588] Validando projecao e modal ampliado..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[245/588] Validando upgrade SQL e PREPARAR 19..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[246/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao de treino + ciclo versionado: OK."


Write-Host "[247/588] Validando schema dos modelos alimentares..."
...
Write-Host "[248/588] Validando listagem e busca de modelos..."
...
Write-Host "[249/588] Validando salvar plano como modelo..."
...
Write-Host "[250/588] Validando criacao de plano a partir de modelo..."
...
Write-Host "[251/588] Validando alimentos do catalogo..."
...
Write-Host "[252/588] Validando isolamento e auditoria..."
...
Write-Host "[253/588] Validando interface de salvar modelo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[254/588] Validando seletor de modelos..."
...
Write-Host "[255/588] Validando criacao visual via modelo..."
...
Write-Host "[256/588] Validando responsividade dos modelos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[257/588] Validando upgrade SQL e PREPARAR 20..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[258/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de plano alimentar: OK."


Write-Host "[259/588] Validando schema dos modelos de treino..."
...
Write-Host "[260/588] Validando listagem e busca de modelos de treino..."
...
Write-Host "[261/588] Validando salvar treino como modelo..."
if (-not $modelSource.Contains("salvar-como-modelo") -or
...
Write-Host "[262/588] Validando criacao de treino a partir de modelo..."
if (-not $modelSource.Contains("criar-de-modelo") -or
...
Write-Host "[263/588] Validando catalogo de exercicios..."
...
Write-Host "[264/588] Validando prescricao completa do template..."
...
Write-Host "[265/588] Validando isolamento e auditoria dos modelos de treino..."
if (-not $modelSource.Contains("currentUser.OrganizationId") -or
    -not $modelSource.Contains('"CREATE_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("AuditLogs")) {
...
Write-Host "[266/588] Validando interface de salvar modelo de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[267/588] Validando seletor e busca de modelos..."
...
Write-Host "[268/588] Validando criacao visual via modelo..."
...
Write-Host "[269/588] Validando upgrade SQL, responsividade e PREPARAR 21..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[270/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de treino + criacao rapida: OK."


Write-Host "[271/588] Validando schema das metas nutricionais..."
$planEntity = Get-Content .\src\HealthPlatform.Domain\Entities\PlanoAlimentar.cs -Encoding UTF8 -Raw
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[272/588] Validando contratos nutricionais..."
...
Write-Host "[273/588] Validando endpoint de metas..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[274/588] Validando analise nutricional..."
...
Write-Host "[275/588] Validando metas na criacao e edicao..."
...
Write-Host "[276/588] Validando metas na progressao alimentar..."
...
Write-Host "[277/588] Validando metas nos templates alimentares..."
...
Write-Host "[278/588] Validando construtor com metas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[279/588] Validando Meta x Prescrito no prontuario..."
...
Write-Host "[280/588] Validando distribuicao por refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[281/588] Validando modal de metas..."
...
Write-Host "[282/588] Validando responsividade nutricional..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[283/588] Validando SQL e PREPARAR 22..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[284/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / metas nutricionais + distribuicao: OK."


Write-Host "[285/588] Validando schema da biblioteca de refeicoes..."
...
Write-Host "[286/588] Validando listagem e filtros da biblioteca..."
...
Write-Host "[287/588] Validando salvar refeicao como modelo..."
...
Write-Host "[288/588] Validando insercao de refeicao no plano..."
...
Write-Host "[289/588] Validando catalogo e substituicoes..."
if (-not $modelSource.Contains("alimentosValidos") -or
    -not $modelSource.Contains("alimentosInvalidos") -or
...
Write-Host "[290/588] Validando protecoes e auditoria..."
...
Write-Host "[291/588] Validando botao Salvar refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[292/588] Validando biblioteca visual..."
...
Write-Host "[293/588] Validando insercao visual rapida..."
...
Write-Host "[294/588] Validando responsividade da biblioteca..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[295/588] Validando SQL e PREPARAR 23..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[296/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de refeicoes + insercao rapida: OK."


Write-Host "[297/588] Validando schema da biblioteca de sessoes..."
...
Write-Host "[298/588] Validando listagem e filtros..."
...
Write-Host "[299/588] Validando salvar sessao como modelo..."
...
Write-Host "[300/588] Validando insercao de sessao no plano..."
...
Write-Host "[301/588] Validando catalogo e prescricao..."
if (-not $modelSource.Contains("exerciciosValidos") -or
    -not $modelSource.Contains("exerciciosInvalidos") -or
...
Write-Host "[302/588] Validando protecoes e auditoria..."
if (-not $modelSource.Contains('plano.Status == "Concluido"') -or
    -not $modelSource.Contains('"INSERT_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("currentUser.OrganizationId")) {
...
Write-Host "[303/588] Validando botao Salvar sessao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[304/588] Validando biblioteca visual..."
...
Write-Host "[305/588] Validando insercao visual rapida..."
...
Write-Host "[306/588] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[307/588] Validando SQL e PREPARAR 24..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[308/588] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de sessoes + insercao rapida: OK."


Write-Host "[309/588] Validando endpoint longitudinal de habitos..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[310/588] Validando contrato da evolucao de habitos..."
...
Write-Host "[311/588] Validando series de sono, estresse, atividade e agua..."
...
Write-Host "[312/588] Validando limite e isolamento multi-tenant..."
...
Write-Host "[313/588] Validando comparacao com registro anterior..."
...
Write-Host "[314/588] Validando graficos de habitos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[315/588] Validando resumo atual dos habitos..."
...
Write-Host "[316/588] Validando integracao na aba Anamnese..."
...
Write-Host "[317/588] Validando integracao no Resumo..."
...
Write-Host "[318/588] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[319/588] Validando compatibilidade de banco e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
...
Write-Host "    Sem schema novo / PREPARAR atual 32/32: OK."

Write-Host "[320/588] Validando versao v0.3.28..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.28 / evolucao de habitos + graficos de anamnese: OK."


Write-Host "[321/588] Validando resposta runtime com metas por refeicao..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[322/588] Validando schema de metas por refeicao..."
...
Write-Host "[323/588] Validando contratos de metas por refeicao..."
$contractsSource = Get-Content .\src\HealthPlatform.Api\Contracts\PlanosAlimentares\PlanoAlimentarContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[324/588] Validando endpoint de meta individual..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[325/588] Validando distribuicao automatica..."
...
Write-Host "[326/588] Validando isolamento e integridade da distribuicao..."
...
Write-Host "[327/588] Validando progressao com metas por refeicao..."
...
Write-Host "[328/588] Validando templates de plano..."
$templateSource = Get-Content .\src\HealthPlatform.Api\Controllers\ModelosPlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[329/588] Validando biblioteca de refeicoes..."
...
Write-Host "[330/588] Validando construtor alimentar..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[331/588] Validando edicao e comparacao visual..."
...
Write-Host "[332/588] Validando modal de distribuicao..."
...
Write-Host "[333/588] Validando SQL e PREPARAR 25..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[334/588] Validando versao v0.3.29..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.29 / metas por refeicao + distribuicao planejada: OK."


Write-Host "[335/588] Validando schema das fases nutricionais..."
...
Write-Host "[336/588] Validando listagem das fases..."
...
Write-Host "[337/588] Validando criacao de fase..."
...
Write-Host "[338/588] Validando edicao e estados..."
...
Write-Host "[339/588] Validando vinculo seguro com plano alimentar..."
...
Write-Host "[340/588] Validando reordenacao do ciclo..."
...
Write-Host "[341/588] Validando exclusao protegida..."
...
Write-Host "[342/588] Validando isolamento e auditoria..."
...
Write-Host "[343/588] Validando interface das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[344/588] Validando formulario de fase..."
...
Write-Host "[345/588] Validando reordenacao visual..."
...
Write-Host "[346/588] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[347/588] Validando SQL e PREPARAR 26..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[348/588] Validando versao v0.3.30..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.30 / fases nutricionais + planejamento de ciclo: OK."


Write-Host "[349/588] Validando schema das fases de treino..."
...
Write-Host "[350/588] Validando listagem das fases..."
...
Write-Host "[351/588] Validando criacao de fase..."
...
Write-Host "[352/588] Validando edicao e estados..."
...
Write-Host "[353/588] Validando vinculo seguro com ficha..."
if (-not $phaseSource.Contains("PlanoValido") -or
    -not $phaseSource.Contains("x.PacienteId == pacienteId") -or
    -not $phaseSource.Contains("x.Paciente.OrganizacaoId == currentUser.OrganizationId")) {
...
Write-Host "[354/588] Validando reordenacao do ciclo..."
...
Write-Host "[355/588] Validando exclusao protegida..."
if (-not $phaseSource.Contains("HttpDelete") -or
    -not $phaseSource.Contains('fase.Status == "EmAndamento"') -or
    -not $phaseSource.Contains('"DELETE"')) {
...
Write-Host "[356/588] Validando isolamento e auditoria..."
if (-not $phaseSource.Contains("currentUser.OrganizationId") -or
    -not $phaseSource.Contains("AuditLogs") -or
...
Write-Host "[357/588] Validando interface do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[358/588] Validando formulario de fase..."
...
Write-Host "[359/588] Validando reordenacao visual..."
...
Write-Host "[360/588] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[361/588] Validando SQL e PREPARAR 27..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[362/588] Validando versao v0.3.31..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.31 / ciclos de treino + periodizacao: OK."


Write-Host "[363/588] Validando schema dos check-ins..."
...
Write-Host "[364/588] Validando endpoint profissional..."
...
Write-Host "[365/588] Validando criacao e edicao..."
...
Write-Host "[366/588] Validando limites dos indicadores..."
...
Write-Host "[367/588] Validando vinculo com fases..."
...
Write-Host "[368/588] Validando auto-vinculo do paciente..."
...
Write-Host "[369/588] Validando portal do paciente..."
...
Write-Host "[370/588] Validando isolamento multi-tenant..."
...
Write-Host "[371/588] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[372/588] Validando graficos de resposta..."
...
Write-Host "[373/588] Validando formulario profissional..."
...
Write-Host "[374/588] Validando check-in no portal..."
...
Write-Host "[375/588] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[376/588] Validando SQL e PREPARAR 28..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[377/588] Validando preservacao dos ciclos..."
...
Write-Host "[378/588] Validando versao v0.3.32..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.32 / check-ins de evolucao + adesao por fase: OK."


Write-Host "[379/588] Validando endpoint de analise por fase..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[380/588] Validando metricas agregadas..."
...
Write-Host "[381/588] Validando variacao de peso por fase..."
...
Write-Host "[382/588] Validando destaques automaticos..."
...
Write-Host "[383/588] Validando isolamento multi-tenant..."
...
Write-Host "[384/588] Validando cards comparativos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[385/588] Validando destaques visuais..."
...
Write-Host "[386/588] Validando integracao com nutricao..."
...
Write-Host "[387/588] Validando integracao com treino..."
...
Write-Host "[388/588] Validando resumo consolidado..."
...
Write-Host "[389/588] Validando responsividade e compatibilidade de banco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $cssSource.Contains("phase-highlight-grid") -or
    ...
    -not $setupSource.Contains("[32/32]")) {
...
Write-Host "[390/588] Validando versao v0.3.33..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.33 / analise de fases + comparativo de resposta: OK."

Write-Host "[391/588] Validando metas das fases..."
...
Write-Host "[392/588] Validando mapeamento EF..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[393/588] Validando CRUD das fases..."
...
Write-Host "[394/588] Validando limites dos criterios..."
...
Write-Host "[395/588] Validando endpoint runtime de prontidao..."
...
Write-Host "[396/588] Validando motor de criterios objetivos..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[397/588] Validando revisao profissional..."
...
Write-Host "[398/588] Validando formularios das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[399/588] Validando metas nos cards..."
...
Write-Host "[400/588] Validando painel de prontidao..."
...
Write-Host "[405/588] Validando SQL e PREPARAR 29..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
$sqlSource = Get-Content .\scripts\sql\v0.3.34_criterios_transicao_fases.sql -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or -not $setupSource.Contains("v0.3.34_criterios_transicao_fases.sql") -or -not $sqlSource.Contains('"MetaPesoKg"') -or -not $sqlSource.Contains('"CriterioTransicao"') -or -not $setupSource.Contains("v0.3.32_checkins_acompanhamento.sql")) { throw "Upgrade dos criterios incompleto." }
Write-Host "    SQL idempotente + PREPARAR 29/29 + historico preservado."

Write-Host "[406/588] Validando versao v0.3.34..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.34 / metas de fase + criterios de transicao: OK."


Write-Host "[407/588] Validando entidade de revisao..."
...
Write-Host "[408/588] Validando mapeamento das revisoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[409/588] Validando historico runtime..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[410/588] Validando revisao nutricional..."
...
Write-Host "[411/588] Validando revisao de treino..."
...
Write-Host "[412/588] Validando decisoes e fase ativa..."
...
Write-Host "[413/588] Validando override consciente..."
...
Write-Host "[414/588] Validando transicao para proxima fase..."
...
Write-Host "[415/588] Validando transacao e auditoria..."
...
Write-Host "[416/588] Validando modal de revisao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[417/588] Validando historico visual..."
...
Write-Host "[418/588] Validando integracao com painel de transicao..."
...
Write-Host "[419/588] Validando SQL e PREPARAR 30..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "    SQL idempotente + PREPARAR 32/32 + v0.3.34 preservada."

Write-Host "[420/588] Validando versao v0.3.35..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.35 / revisao de fase + transicao assistida: OK."


Write-Host "[421/588] Validando controller de volume..."
...
Write-Host "[422/588] Validando isolamento multi-tenant..."
...
Write-Host "[423/588] Validando volume planejado por grupo..."
...
Write-Host "[424/588] Validando frequencia semanal..."
...
Write-Host "[425/588] Validando fallback de frequencia..."
...
Write-Host "[426/588] Validando execucoes reais..."
...
Write-Host "[427/588] Validando ausencia de tonelagem inventada..."
...
Write-Host "[428/588] Validando runtime da analise..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
    try {
...
Write-Host "[429/588] Validando painel de volume..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[430/588] Validando resumo analitico..."
...
Write-Host "[431/588] Validando volume por sessao..."
...
Write-Host "[432/588] Validando integracao e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[433/588] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
...
Write-Host "    Sem schema novo / PREPARAR permanece 32/32."

Write-Host "[434/588] Validando versao v0.3.36..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.36 / volume de treino + distribuicao muscular: OK."


Write-Host "[435/588] Validando endpoints de progressao por exercicio..."
...
Write-Host "[436/588] Validando seguranca e tenant..."
...
Write-Host "[437/588] Validando separacao por unidade..."
...
Write-Host "[438/588] Validando metricas de carga..."
...
Write-Host "[439/588] Validando recordes sucessivos..."
...
Write-Host "[440/588] Validando tendencia de carga..."
...
Write-Host "[441/588] Validando protecao contra estimativas artificiais..."
...
Write-Host "[442/588] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[443/588] Validando payload de pontos..."
...
Write-Host "[444/588] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[445/588] Validando graficos e recordes..."
...
Write-Host "[446/588] Validando portal do paciente..."
...
Write-Host "[447/588] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[448/588] Validando versao v0.3.37..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.37 / progressao por exercicio + recordes de carga: OK."


Write-Host "[449/588] Validando endpoints de sinais de progressao..."
...
Write-Host "[450/588] Validando tenant e PatientOnly..."
...
Write-Host "[451/588] Validando estados da analise..."
...
Write-Host "[452/588] Validando regra de estagnacao..."
...
Write-Host "[453/588] Validando sinal de carga/RPE..."
...
Write-Host "[454/588] Validando progressao recente..."
...
Write-Host "[455/588] Validando semantica nao diagnostica..."
...
Write-Host "[456/588] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[457/588] Validando payload analitico..."
...
Write-Host "[458/588] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[459/588] Validando portal do paciente..."
...
Write-Host "[460/588] Validando cards e graficos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[461/588] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[462/588] Validando versao v0.3.38..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.38 / estagnacao + fadiga + sinais de progressao: OK."


Write-Host "[463/588] Validando identidade MVP Preview..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.5.9") -or
...
Write-Host "[464/588] Validando aviso de demonstracao..."
...
Write-Host "[465/588] Validando roteiro da demo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[466/588] Validando checklist de avaliacao..."
...
Write-Host "[467/588] Validando modelo de feedback..."
...
Write-Host "[468/588] Validando dashboard de apresentacao..."
...
Write-Host "[469/588] Validando atalho Escape..."
...
Write-Host "[470/588] Validando feedback de conectividade..."
...
Write-Host "[471/588] Validando acabamento de foco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[472/588] Validando estados vazios..."
...
Write-Host "[473/588] Validando limpeza da navegacao de demo..."
...
Write-Host "[474/588] Validando responsividade do MVP..."
...
Write-Host "[475/588] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
...
Write-Host "[476/588] Validando versao v0.3.39..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.39 / MVP Preview + polimento de demonstracao: OK."


Write-Host "[477/588] Validando Dockerfile..."
...
Write-Host "[478/588] Validando bind dinamico de porta..."
...
Write-Host "[479/588] Validando Blueprint Render..."
...
Write-Host "[480/588] Validando secrets do Blueprint..."
...
Write-Host "[481/588] Validando conexao PostgreSQL do Render..."
...
Write-Host "[482/588] Validando bootstrap isolado do MVP..."
...
Write-Host "[484/588] Validando healthcheck real..."
...
Write-Host "[485/588] Validando forwarded headers..."
...
Write-Host "[486/588] Validando POPULAR remoto..."
...
if (-not $remotePopular.Contains("[Parameter(Mandatory=`$true)][string]`$BaseUrl") -or
    -not $remotePopular.Contains("HealthPlatform v0.5.9 - POPULAR RENDER DEMO") -or
...
Write-Host "[487/588] Validando catalogos da demo remota..."
...
Write-Host "[488/588] Validando smoke test remoto..."
...
Write-Host "[489/588] Validando guia de deploy..."
...
Write-Host "[490/588] Validando ausencia de schema novo..."
...
Write-Host "    PREPARAR continua 32/32 / sem SQL v0.3.40."

Write-Host "[491/588] Validando arquivos de container..."
...
Write-Host "[492/588] Validando versao base do deploy..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.9 / Render Demo Deploy preservado: OK."


Write-Host "[493/588] Validando paleta RS..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[494/588] Validando monograma RS..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
...
Write-Host "[495/588] Validando linguagem editorial..."
...
Write-Host "[496/588] Validando tipografia condensada..."
...
Write-Host "[497/588] Validando login editorial..."
...
Write-Host "[498/588] Validando sidebar RS..."
...
Write-Host "[499/588] Validando experiencia iPad..."
...
Write-Host "[500/588] Validando drawer mobile..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[501/588] Validando safe areas iOS..."
...
Write-Host "[502/588] Validando modal bottom-sheet no iPhone..."
...
Write-Host "[503/588] Validando portal mobile..."
...
Write-Host "[504/588] Validando abas do prontuario no mobile..."
...
Write-Host "[505/588] Validando touch targets..."
...
Write-Host "[506/588] Validando cards analiticos no iPhone..."
...
Write-Host "[507/588] Validando deploy/demo preservados..."
$renderSource = Get-Content .\render.yaml -Encoding UTF8 -Raw
...
Write-Host "[508/588] Validando entidade de solicitacoes clinicas..."
...
Write-Host "[509/588] Validando mapeamento EF de solicitacoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[510/588] Validando endpoints profissionais de solicitacoes..."
...
Write-Host "[511/588] Validando portal do paciente para solicitacoes..."
...
Write-Host "[512/588] Validando isolamento multi-tenant..."
...
Write-Host "[513/588] Validando auditoria de solicitacoes..."
...
Write-Host "[514/588] Validando interface profissional..."
...
Write-Host "[515/588] Validando interface do paciente..."
...
Write-Host "[516/588] Validando upgrade SQL v0.5.1 preservado..."
...
Write-Host "[517/588] Validando PREPARAR 32/32..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[31/32] Aplicando upgrade v0.5.1') -or -not $setupSource.Contains('v0.5.1_solicitacoes_clinicas.sql')) { throw "PREPARAR nao preserva o upgrade v0.5.1." }
Write-Host "    Upgrade de solicitacoes integrado ao setup."

Write-Host "[518/588] Validando versao v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.9 / Solicitacoes Clinicas + Connected Care: OK."

Write-Host "[519/588] Validando notificacoes de solicitacoes no backend..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[520/588] Validando prioridade por prazo para o paciente..."
...
Write-Host "[521/588] Validando notificacao de resposta para profissional..."
...
Write-Host "[522/588] Validando limpeza de notificacoes encerradas..."
...
Write-Host "[523/588] Validando navegacao do paciente pela notificacao..."
...
Write-Host "[524/588] Validando navegacao contextual do profissional..."
...
Write-Host "[525/588] Validando identidade visual de notificacao..."
...
Write-Host "[526/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $notificationSource.Contains('SolicitacaoClinica')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Notificacoes Contextuais + Connected Care: OK."

Write-Host "[527/588] Validando endpoint da Central de Solicitacoes..."
$requestController = Get-Content .\src\HealthPlatform.Api\Controllers\SolicitacoesClinicasController.cs -Encoding UTF8 -Raw
...
Write-Host "[528/588] Validando resumo operacional de solicitacoes..."
...
Write-Host "[529/588] Validando filtros da Central de Solicitacoes..."
...
Write-Host "[530/588] Validando isolamento da fila profissional..."
...
Write-Host "[531/588] Validando navegacao profissional de solicitacoes..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[532/588] Validando indicadores e filtros visuais..."
...
Write-Host "[533/588] Validando acoes rapidas da Central de Solicitacoes..."
...
Write-Host "[534/588] Validando responsividade da Central de Solicitacoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[535/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $requestController.Contains('FilaProfissional')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Central de Solicitacoes + Connected Care: OK."


Write-Host "[536/588] Validando solicitacoes no Hoje do paciente..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[537/588] Validando solicitacoes no progresso diario..."
...
Write-Host "[538/588] Validando card de acao necessaria..."
...
Write-Host "[539/588] Validando destaque de solicitacao vencida no portal..."
...
Write-Host "[540/588] Validando integracao da Central do Dia no backend..."
$centralSource = Get-Content .\src\HealthPlatform.Api\Controllers\CentralDiaController.cs -Encoding UTF8 -Raw
...
Write-Host "[541/588] Validando escopo profissional das solicitacoes do dia..."
...
Write-Host "[542/588] Validando card profissional de solicitacoes..."
...
Write-Host "[543/588] Validando dashboard com solicitacoes para revisao..."
...
Write-Host "[544/588] Validando responsividade das solicitacoes no Hoje..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[545/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $centralSource.Contains('SolicitacoesParaRevisao') -or -not $appJsSource.Contains('patient-today-requests')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Solicitacoes no Hoje + Connected Care: OK."

Write-Host "[546/588] Validando contrato Desde a ultima consulta..."
$resumoSource = Get-Content .\src\HealthPlatform.Api\Controllers\ResumoClinicoController.cs -Encoding UTF8 -Raw
...
Write-Host "[547/588] Validando periodo baseado na ultima consulta..."
...
Write-Host "[548/588] Validando atividade do paciente no periodo..."
...
Write-Host "[549/588] Validando adesao media por check-ins..."
...
Write-Host "[550/588] Validando variacao de peso no periodo..."
...
Write-Host "[551/588] Validando solicitacoes no resumo longitudinal..."
...
Write-Host "[552/588] Validando card longitudinal no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[553/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $resumoSource.Contains('ResumoDesdeUltimaConsultaResponse') -or -not $appJsSource.Contains('data-clinical-period-summary')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Desde a ultima consulta + Connected Care: OK."

Write-Host "[554/588] Validando endpoint da Jornada do paciente..."
...
Write-Host "[555/588] Validando escopo PatientOnly da Jornada..."
...
Write-Host "[556/588] Validando fontes da Jornada..."
...
Write-Host "[557/588] Validando navegacao Jornada no portal..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[558/588] Validando filtro temporal da Jornada..."
...
Write-Host "[559/588] Validando atalhos contextuais da Jornada..."
...
Write-Host "[560/588] Validando responsividade da Jornada..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[561/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $portalController.Contains('MinhaJornada') -or -not $appJsSource.Contains('loadPatientJourney')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Jornada do Paciente + Connected Care: OK."


Write-Host "[562/588] Validando endpoint de monitoramento profissional..."
...
Write-Host "[563/588] Validando monitoramento no portal do paciente..."
...
Write-Host "[564/588] Validando metricas guiadas..."
...
Write-Host "[565/588] Validando resumo de 7 dias..."
...
Write-Host "[566/588] Validando registro guiado de pressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[567/588] Validando registros rapidos de sinais vitais..."
...
Write-Host "[568/588] Validando resumo profissional de monitoramento..."
...
Write-Host "[569/588] Validando responsividade do monitoramento..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[570/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $monitorSource.Contains('TiposMonitorados') -or -not $appJsSource.Contains('hpMonitoringCard')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Monitoramento guiado + Connected Care: OK."

Write-Host "[571/588] Validando entidade de protocolo de acompanhamento..."
...
Write-Host "[572/588] Validando mapeamento EF dos protocolos..."
...
Write-Host "[573/588] Validando endpoints profissionais do protocolo..."
...
Write-Host "[574/588] Validando portal PatientOnly do protocolo..."
...
Write-Host "[575/588] Validando tipos e frequencias permitidos..."
...
Write-Host "[576/588] Validando gerenciamento visual do protocolo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[577/588] Validando personalizacao do Hoje do paciente..."
# v0.5.9 acrescenta offsetMinutos na chamada do protocolo para respeitar o dia local.
...
Write-Host "[578/588] Validando SQL e migration de upgrade v0.5.8 preservados..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
$sql058 = Get-Content .\scripts\sql\v0.5.8_protocolos_acompanhamento.sql -Encoding UTF8 -Raw
if (-not $setupSource.Contains('V058ProtocolosAcompanhamento') -or -not $setupSource.Contains('[32/32] Aplicando upgrade v0.5.8') -or -not $sql058.Contains('CREATE TABLE IF NOT EXISTS "ProtocolosAcompanhamento"')) { throw "Upgrade v0.5.8 de protocolos nao foi preservado." }
Write-Host "    Migration incremental + SQL idempotente v0.5.8 + PREPARAR 32/32: OK."

Write-Host "[579/588] Validando responsividade dos protocolos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[580/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $protocolSource.Contains('MeuProtocolo') -or -not $appJsSource.Contains('openProtocolManager')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Protocolos de Acompanhamento + Connected Care: OK."

Write-Host "[581/588] Validando calculo de aderencia ao protocolo..."
$protocolSource = Get-Content .\src\HealthPlatform.Api\Controllers\ProtocolosAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[582/588] Validando agenda diaria dos protocolos..."
...
Write-Host "[583/588] Validando aderencia de pressao estruturada..."
...
Write-Host "[584/588] Validando timezone da aderencia..."
...
Write-Host "[585/588] Validando status de protocolo no Hoje do paciente..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[586/588] Validando resumo profissional de aderencia..."
...
Write-Host "[587/588] Validando responsividade da aderencia..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[588/588] Validando versao funcional v0.5.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.9") { throw "VERSION.txt inesperado." }
if (-not $protocolSource.Contains('CalcularAderencia') -or -not $appJsSource.Contains('protocol-today-status')) { throw "Feature v0.5.9 ausente." }
Write-Host "    v0.5.9 / Aderencia aos Protocolos + Connected Care: OK."
