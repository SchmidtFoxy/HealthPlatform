...
Write-Host "[1/570] Healthcheck..." -ForegroundColor Cyan
$health = Invoke-RestMethod -Uri "$base/api/health" -Method Get
if ($health.version -ne "0.5.7") { throw "Versao inesperada da API: $($health.version)" }
Write-Host "    API $($health.version) / banco $($health.database)" -ForegroundColor Green

Write-Host "[2/570] Login..." -ForegroundColor Cyan
...
Write-Host "[3/570] Listando pacientes..." -ForegroundColor Cyan
...
Write-Host "[4/570] Validando perguntas de anamnese..." -ForegroundColor Cyan
...
Write-Host "[5/570] Validando catalogo laboratorial..." -ForegroundColor Cyan
...
Write-Host "[6/570] Validando catalogo de alimentos..." -ForegroundColor Cyan
...
Write-Host "[7/570] Validando busca/paginacao..." -ForegroundColor Cyan
...
Write-Host "[8/570] Validando modulos do paciente..." -ForegroundColor Cyan
...
Write-Host "[9/570] Validando metas do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[10/570] Validando diario/resumo do dia..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[11/570] Validando portal/home do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[12/570] Validando agenda do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[13/570] Validando dashboard do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[14/570] Validando interface web..." -ForegroundColor Cyan
...
Write-Host "[15/570] Validando assets da interface..." -ForegroundColor Cyan
...
Write-Host "[16/570] Validando prontuario visual v0.2.2..." -ForegroundColor Cyan
...
Write-Host "[17/570] Validando acoes clinicas da interface..." -ForegroundColor Cyan
...
Write-Host "[18/570] Validando rotas usadas pelos formularios clinicos..." -ForegroundColor Cyan
...
Write-Host "[19/570] Validando cadastro visual de exames..." -ForegroundColor Cyan
...
Write-Host "[20/570] Validando construtor visual do plano alimentar..." -ForegroundColor Cyan
...
Write-Host "[21/570] Validando relatorios na interface..." -ForegroundColor Cyan
...
Write-Host "[22/570] Validando edicao visual do paciente..." -ForegroundColor Cyan
...
Write-Host "[23/570] Validando endpoint de relatorios do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[24/570] Validando edicao clinica visual..."
...
Write-Host "[25/570] Validando agenda operacional..."
...
Write-Host "[26/570] Validando endpoint de atualizacao de avaliacao..."
try {
...
Write-Host "[27/570] Validando tela de configuracoes..."
...
Write-Host "[28/570] Validando gerenciadores de catalogo na interface..."
...
Write-Host "[29/570] Validando resumo de configuracoes do consultorio..."
...
Write-Host "[30/570] Validando rotas dos catalogos..."
...
Write-Host "[31/570] Validando edicao de configuracoes..."
$cfg = Invoke-RestMethod -Uri "$base/api/configuracoes/resumo" -Headers $headers
...
Write-Host "[32/570] Validando assets de edicao/inativacao dos catalogos..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[35/570] Validando separacao de autorizacao profissional/paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[36/570] Validando endpoint de status de acesso do paciente..."
...
Write-Host "[37/570] Validando fluxo de convite/ativacao nos assets..."
...
Write-Host "[38/570] Validando autoatendimento do diario..."
...
Write-Host "[39/570] Validando autoatendimento das metas..."
...
Write-Host "[40/570] Validando tela dedicada do portal do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[41/570] Validando navegacao completa do portal..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[42/570] Validando endpoint proprio de plano alimentar..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[43/570] Validando historico proprio de metas e diario..."
...
Write-Host "[47/570] Validando schema/endpoint do catalogo de exercicios..."
...
Write-Host "[48/570] Validando endpoint de planos de treino do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[49/570] Validando construtor visual de treino..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[50/570] Validando videos e prescricao de exercicios..."
...
Write-Host "[51/570] Validando aba de treinos no prontuario..."
...
Write-Host "[52/570] Validando navegacao de treino do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[53/570] Validando endpoint proprio do treino do paciente..."
...
Write-Host "[54/570] Validando assets visuais do modulo de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[55/570] Validando schema de execucoes de treino..."
...
Write-Host "[56/570] Validando historico profissional de treinos..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[57/570] Validando registro visual de execucao pelo paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[58/570] Validando series, repeticoes e carga realizadas..."
...
Write-Host "[59/570] Validando esforco percebido e duracao..."
...
Write-Host "[60/570] Validando historico do paciente..."
...
Write-Host "[61/570] Validando progressao de carga no prontuario..."
...
Write-Host "[62/570] Validando estilos do acompanhamento de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[63/570] Validando motor de graficos SVG..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[64/570] Validando graficos corporais no prontuario..."
...
Write-Host "[65/570] Validando tendencias laboratoriais..."
...
Write-Host "[66/570] Validando progressao grafica de carga..."
...
Write-Host "[67/570] Validando evolucao visual no portal do paciente..."
...
Write-Host "[68/570] Validando graficos de exames no portal..."
...
Write-Host "[69/570] Validando responsividade dos graficos..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[70/570] Validando compatibilidade de schema na v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
...
Write-Host "[71/570] Validando endpoint de insights do dashboard..."
...
Write-Host "[72/570] Validando insights por paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[73/570] Validando regra de exame fora da referencia..."
...
Write-Host "[74/570] Validando regras de evolucao e retorno..."
...
Write-Host "[75/570] Validando regras de adesao..."
...
Write-Host "[76/570] Validando central de atencao visual..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[77/570] Validando insights no prontuario..."
...
Write-Host "[78/570] Validando estilos e compatibilidade do schema..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[79/570] Validando schema de pendencias clinicas..."
...
Write-Host "[80/570] Validando endpoint geral de pendencias..."
...
Write-Host "[81/570] Validando endpoint de pendencias do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[82/570] Validando acoes de ciclo de vida..."
...
Write-Host "[83/570] Validando criacao de retorno a partir da pendencia..."
...
Write-Host "[84/570] Validando transformar insight em pendencia..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[85/570] Validando tela de gerenciamento de pendencias..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[86/570] Validando acoes visuais da pendencia..."
...
Write-Host "[87/570] Validando resumo de pendencias no dashboard..."
...
Write-Host "[88/570] Validando auditoria, estilos e upgrade v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[89/570] Validando schema de notificacoes internas..."
...
Write-Host "[90/570] Validando sincronizacao de notificacoes..."
...
Write-Host "[91/570] Validando listagem e contador nao lido..."
...
Write-Host "[92/570] Validando regras de agenda profissional..."
...
Write-Host "[93/570] Validando regras de pendencias..."
...
Write-Host "[94/570] Validando lembretes do paciente..."
...
Write-Host "[95/570] Validando leitura individual e em massa..."
...
Write-Host "[96/570] Validando sino e drawer na interface..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[97/570] Validando contador e atualizacao periodica..."
...
Write-Host "[98/570] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[99/570] Validando script de popular banco..."
...
Write-Host "[100/570] Validando idempotencia do popular..."
...
Write-Host "[101/570] Validando cobertura de modulos na base demo..."
...
Write-Host "[102/570] Validando endpoint de resumo de dados..."
...
Write-Host "[103/570] Validando que popular banco e opt-in..."
...
Write-Host "[104/570] Validando versao v0.3.27 e upgrade do schema..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
if ($setupSource -notmatch "\[31/31\]") { throw "PREPARAR deveria possuir 31 etapas na v0.3.27." }
Write-Host "    v0.3.27 / PREPARAR 29 etapas / upgrade SOAP: OK."


Write-Host "[105/570] Validando endpoint da carteira..."
...
Write-Host "[106/570] Validando priorizacao da carteira..."
...
Write-Host "[107/570] Validando sinais de exames/evolucao na carteira..."
...
Write-Host "[108/570] Validando atividade recente..."
...
Write-Host "[109/570] Validando tela Carteira..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[110/570] Validando filtros e ordenacao..."
...
Write-Host "[111/570] Validando atalho da carteira no dashboard..."
...
Write-Host "[112/570] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[113/570] Validando schema de follow-up..."
...
Write-Host "[114/570] Validando endpoint de follow-up..."
...
Write-Host "[115/570] Validando canais e proximo contato..."
...
Write-Host "[116/570] Validando auditoria do contato..."
...
Write-Host "[117/570] Validando follow-up na carteira..."
$carteiraSource = Get-Content .\src\HealthPlatform.Api\Controllers\CarteiraController.cs -Encoding UTF8 -Raw
...
Write-Host "[118/570] Validando acao rapida de contato..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[119/570] Validando acao rapida de retorno..."
...
Write-Host "[120/570] Validando acao rapida de pendencia..."
...
Write-Host "[121/570] Validando historico no prontuario..."
...
Write-Host "[122/570] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[123/570] Validando endpoint da fila de follow-up..."
...
Write-Host "[124/570] Validando faixas de vencimento..."
...
Write-Host "[125/570] Validando busca e filtros da fila..."
...
Write-Host "[126/570] Validando notificacao de follow-up..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[127/570] Validando tela de follow-up..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[128/570] Validando acoes rapidas na fila..."
...
Write-Host "[129/570] Validando resumo de follow-up no dashboard..."
...
Write-Host "[130/570] Validando estilos, popular e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
$popularSource = Get-Content .\POPULAR.ps1 -Encoding UTF8 -Raw
...
Write-Host "[131/570] Validando endpoint de gestao..."
...
Write-Host "[132/570] Validando indicadores de consultas..."
...
Write-Host "[133/570] Validando indicadores de acompanhamento..."
...
Write-Host "[134/570] Validando indicadores de engajamento..."
...
Write-Host "[135/570] Validando series gerenciais..."
...
Write-Host "[136/570] Validando tela Gestao..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[137/570] Validando graficos e resumo no dashboard..."
...
Write-Host "[138/570] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[139/570] Validando PREPARAR sem atualizacao automatica do dotnet..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[140/570] Validando endpoint CSV gerencial..."
...
Write-Host "[141/570] Validando conteudo longitudinal do CSV..."
...
Write-Host "[142/570] Validando relatorio HTML imprimivel..."
...
Write-Host "[143/570] Validando botoes de exportacao na Gestao..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[144/570] Validando download autenticado..."
...
Write-Host "[145/570] Validando relatorio autenticado em nova janela..."
...
Write-Host "[146/570] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[147/570] Validando perguntas de anamnese inativas..."
...
Write-Host "[148/570] Validando totais completos de insights..."
...
Write-Host "[149/570] Validando timezone das notificacoes..."
...
Write-Host "[150/570] Validando cleanup do polling no logout..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[151/570] Validando navegacao robusta no portal..."
...
Write-Host "[152/570] Validando smoke de notificacoes sem mutacao..."
...
Write-Host "[153/570] Validando copy historica de schema..."
$schemaCheckMarker = 'Write-Host "[153/570] Validando copy historica de schema..."'
...
Write-Host "[154/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / estabilizacao + qualidade: OK."


Write-Host "[155/570] Validando endpoint de busca global..."
...
Write-Host "[156/570] Validando fontes da busca..."
...
Write-Host "[157/570] Validando isolamento por organizacao..."
...
Write-Host "[158/570] Validando botao e atalho da busca..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[159/570] Validando modal e debounce..."
...
Write-Host "[160/570] Validando acoes dos resultados..."
...
Write-Host "[161/570] Validando estilos responsivos da busca..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[162/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / busca global + central de acoes: OK."


Write-Host "[163/570] Validando endpoint da Central do Dia..."
...
Write-Host "[164/570] Validando agenda do dia..."
...
Write-Host "[165/570] Validando follow-ups do dia..."
...
Write-Host "[166/570] Validando pendencias prioritarias..."
...
Write-Host "[167/570] Validando pacientes para revisao..."
...
Write-Host "[168/570] Validando tela Hoje..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[169/570] Validando acoes rapidas e dashboard..."
...
Write-Host "[170/570] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[171/570] Validando schema de evolucoes clinicas SOAP..."
...
Write-Host "[172/570] Validando endpoint de evolucoes..."
...
Write-Host "[173/570] Validando consulta opcional e isolamento..."
...
Write-Host "[174/570] Validando auditoria da evolucao..."
...
Write-Host "[175/570] Validando evolucao na timeline..."
...
Write-Host "[176/570] Validando aba Evolucoes no prontuario..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[177/570] Validando registro visual SOAP..."
...
Write-Host "[178/570] Validando edicao visual da evolucao..."
...
Write-Host "[179/570] Validando upgrade SQL SOAP historico e PREPARAR atual..."
...
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "    SOAP v0.3.15 preservado / PREPARAR atual 31/31: OK."

Write-Host "[180/570] Validando estilos e versao v0.3.27..."
...
Write-Host "[181/570] Validando endpoint de resumo clinico..."
...
Write-Host "[182/570] Validando consulta e SOAP no resumo..."
...
Write-Host "[183/570] Validando corpo e anamnese..."
...
Write-Host "[184/570] Validando exames alterados..."
...
Write-Host "[185/570] Validando metas, treinos e pendencias..."
...
Write-Host "[186/570] Validando resumo no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[187/570] Validando atualizacao e responsividade..."
...
Write-Host "[188/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / resumo clinico consolidado: OK."


Write-Host "[189/570] Validando texto de handoff clinico..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[190/570] Validando conteudo SOAP no handoff..."
...
Write-Host "[191/570] Validando copia para clipboard..."
...
Write-Host "[192/570] Validando impressao do resumo..."
...
Write-Host "[193/570] Validando seguranca basica do HTML imprimivel..."
...
Write-Host "[194/570] Validando botoes no resumo clinico..."
...
Write-Host "[195/570] Validando responsividade das acoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[196/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / handoff clinico + impressao: OK."


Write-Host "[197/570] Validando endpoint de equipe..."
...
Write-Host "[198/570] Validando isolamento e administracao..."
...
Write-Host "[199/570] Validando criacao de acesso..."
...
Write-Host "[200/570] Validando sincronizacao de tipo e role..."
...
Write-Host "[201/570] Validando perfil profissional..."
...
Write-Host "[202/570] Validando protecao do proprio administrador..."
...
Write-Host "[203/570] Validando auditoria da equipe..."
...
Write-Host "[204/570] Validando tela Equipe..."
...
Write-Host "[205/570] Validando visibilidade admin e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[206/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe + gestao de profissionais: OK."


Write-Host "[207/570] Validando filtros da equipe..."
$teamSource = Get-Content .\src\HealthPlatform.Api\Controllers\EquipeController.cs -Encoding UTF8 -Raw
...
Write-Host "[208/570] Validando redefinicao de senha..."
...
Write-Host "[209/570] Validando escopo e protecoes do reset..."
...
Write-Host "[210/570] Validando auditoria segura de senha..."
...
Write-Host "[211/570] Validando filtros visuais..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[212/570] Validando acao visual de senha..."
...
Write-Host "[213/570] Validando responsividade da equipe v2..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[214/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe v2 + seguranca de acesso: OK."


Write-Host "[215/570] Validando endpoint Minha Conta..."
...
Write-Host "[216/570] Validando alteracao da propria senha..."
...
Write-Host "[217/570] Validando protecoes da troca de senha..."
...
Write-Host "[218/570] Validando auditoria segura da conta..."
...
Write-Host "[219/570] Validando sincronizacao do nome profissional..."
...
Write-Host "[220/570] Validando painel Minha Conta..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[221/570] Validando responsividade da conta..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[222/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / Minha Conta + troca de senha: OK."


Write-Host "[223/570] Validando schema de progressao alimentar..."
...
Write-Host "[224/570] Validando simulador nutricional..."
...
Write-Host "[225/570] Validando duplicacao versionada..."
...
Write-Host "[226/570] Validando escala de porcoes..."
...
Write-Host "[227/570] Validando ajuste por calorias alvo..."
...
Write-Host "[228/570] Validando limites de ajuste..."
...
Write-Host "[229/570] Validando encerramento opcional do plano anterior..."
...
Write-Host "[230/570] Validando auditoria da progressao..."
...
Write-Host "[231/570] Validando interface de progressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[232/570] Validando modos percentual e calorias..."
...
Write-Host "[233/570] Validando upgrade SQL e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[234/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao do plano alimentar: OK."


Write-Host "[235/570] Validando schema de progressao de treino..."
...
Write-Host "[236/570] Validando simulador de treino..."
...
Write-Host "[237/570] Validando duplicacao versionada do treino..."
...
Write-Host "[238/570] Validando progressao de carga..."
...
Write-Host "[239/570] Validando progressao de series e descanso..."
...
Write-Host "[240/570] Validando repeticoes estruturadas..."
...
Write-Host "[241/570] Validando limites de progressao..."
...
Write-Host "[242/570] Validando encerramento e auditoria..."
...
Write-Host "[243/570] Validando interface de progressao de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[244/570] Validando projecao e modal ampliado..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[245/570] Validando upgrade SQL e PREPARAR 19..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[246/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao de treino + ciclo versionado: OK."


Write-Host "[247/570] Validando schema dos modelos alimentares..."
...
Write-Host "[248/570] Validando listagem e busca de modelos..."
...
Write-Host "[249/570] Validando salvar plano como modelo..."
...
Write-Host "[250/570] Validando criacao de plano a partir de modelo..."
...
Write-Host "[251/570] Validando alimentos do catalogo..."
...
Write-Host "[252/570] Validando isolamento e auditoria..."
...
Write-Host "[253/570] Validando interface de salvar modelo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[254/570] Validando seletor de modelos..."
...
Write-Host "[255/570] Validando criacao visual via modelo..."
...
Write-Host "[256/570] Validando responsividade dos modelos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[257/570] Validando upgrade SQL e PREPARAR 20..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[258/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de plano alimentar: OK."


Write-Host "[259/570] Validando schema dos modelos de treino..."
...
Write-Host "[260/570] Validando listagem e busca de modelos de treino..."
...
Write-Host "[261/570] Validando salvar treino como modelo..."
if (-not $modelSource.Contains("salvar-como-modelo") -or
...
Write-Host "[262/570] Validando criacao de treino a partir de modelo..."
if (-not $modelSource.Contains("criar-de-modelo") -or
...
Write-Host "[263/570] Validando catalogo de exercicios..."
...
Write-Host "[264/570] Validando prescricao completa do template..."
...
Write-Host "[265/570] Validando isolamento e auditoria dos modelos de treino..."
if (-not $modelSource.Contains("currentUser.OrganizationId") -or
    -not $modelSource.Contains('"CREATE_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("AuditLogs")) {
...
Write-Host "[266/570] Validando interface de salvar modelo de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[267/570] Validando seletor e busca de modelos..."
...
Write-Host "[268/570] Validando criacao visual via modelo..."
...
Write-Host "[269/570] Validando upgrade SQL, responsividade e PREPARAR 21..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[270/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de treino + criacao rapida: OK."


Write-Host "[271/570] Validando schema das metas nutricionais..."
$planEntity = Get-Content .\src\HealthPlatform.Domain\Entities\PlanoAlimentar.cs -Encoding UTF8 -Raw
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[272/570] Validando contratos nutricionais..."
...
Write-Host "[273/570] Validando endpoint de metas..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[274/570] Validando analise nutricional..."
...
Write-Host "[275/570] Validando metas na criacao e edicao..."
...
Write-Host "[276/570] Validando metas na progressao alimentar..."
...
Write-Host "[277/570] Validando metas nos templates alimentares..."
...
Write-Host "[278/570] Validando construtor com metas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[279/570] Validando Meta x Prescrito no prontuario..."
...
Write-Host "[280/570] Validando distribuicao por refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[281/570] Validando modal de metas..."
...
Write-Host "[282/570] Validando responsividade nutricional..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[283/570] Validando SQL e PREPARAR 22..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[284/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / metas nutricionais + distribuicao: OK."


Write-Host "[285/570] Validando schema da biblioteca de refeicoes..."
...
Write-Host "[286/570] Validando listagem e filtros da biblioteca..."
...
Write-Host "[287/570] Validando salvar refeicao como modelo..."
...
Write-Host "[288/570] Validando insercao de refeicao no plano..."
...
Write-Host "[289/570] Validando catalogo e substituicoes..."
if (-not $modelSource.Contains("alimentosValidos") -or
    -not $modelSource.Contains("alimentosInvalidos") -or
...
Write-Host "[290/570] Validando protecoes e auditoria..."
...
Write-Host "[291/570] Validando botao Salvar refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[292/570] Validando biblioteca visual..."
...
Write-Host "[293/570] Validando insercao visual rapida..."
...
Write-Host "[294/570] Validando responsividade da biblioteca..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[295/570] Validando SQL e PREPARAR 23..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[296/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de refeicoes + insercao rapida: OK."


Write-Host "[297/570] Validando schema da biblioteca de sessoes..."
...
Write-Host "[298/570] Validando listagem e filtros..."
...
Write-Host "[299/570] Validando salvar sessao como modelo..."
...
Write-Host "[300/570] Validando insercao de sessao no plano..."
...
Write-Host "[301/570] Validando catalogo e prescricao..."
if (-not $modelSource.Contains("exerciciosValidos") -or
    -not $modelSource.Contains("exerciciosInvalidos") -or
...
Write-Host "[302/570] Validando protecoes e auditoria..."
if (-not $modelSource.Contains('plano.Status == "Concluido"') -or
    -not $modelSource.Contains('"INSERT_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("currentUser.OrganizationId")) {
...
Write-Host "[303/570] Validando botao Salvar sessao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[304/570] Validando biblioteca visual..."
...
Write-Host "[305/570] Validando insercao visual rapida..."
...
Write-Host "[306/570] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[307/570] Validando SQL e PREPARAR 24..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[308/570] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de sessoes + insercao rapida: OK."


Write-Host "[309/570] Validando endpoint longitudinal de habitos..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[310/570] Validando contrato da evolucao de habitos..."
...
Write-Host "[311/570] Validando series de sono, estresse, atividade e agua..."
...
Write-Host "[312/570] Validando limite e isolamento multi-tenant..."
...
Write-Host "[313/570] Validando comparacao com registro anterior..."
...
Write-Host "[314/570] Validando graficos de habitos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[315/570] Validando resumo atual dos habitos..."
...
Write-Host "[316/570] Validando integracao na aba Anamnese..."
...
Write-Host "[317/570] Validando integracao no Resumo..."
...
Write-Host "[318/570] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[319/570] Validando compatibilidade de banco e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "    Sem schema novo / PREPARAR atual 31/31: OK."

Write-Host "[320/570] Validando versao v0.3.28..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.28 / evolucao de habitos + graficos de anamnese: OK."


Write-Host "[321/570] Validando resposta runtime com metas por refeicao..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[322/570] Validando schema de metas por refeicao..."
...
Write-Host "[323/570] Validando contratos de metas por refeicao..."
$contractsSource = Get-Content .\src\HealthPlatform.Api\Contracts\PlanosAlimentares\PlanoAlimentarContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[324/570] Validando endpoint de meta individual..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[325/570] Validando distribuicao automatica..."
...
Write-Host "[326/570] Validando isolamento e integridade da distribuicao..."
...
Write-Host "[327/570] Validando progressao com metas por refeicao..."
...
Write-Host "[328/570] Validando templates de plano..."
$templateSource = Get-Content .\src\HealthPlatform.Api\Controllers\ModelosPlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[329/570] Validando biblioteca de refeicoes..."
...
Write-Host "[330/570] Validando construtor alimentar..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[331/570] Validando edicao e comparacao visual..."
...
Write-Host "[332/570] Validando modal de distribuicao..."
...
Write-Host "[333/570] Validando SQL e PREPARAR 25..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[334/570] Validando versao v0.3.29..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.29 / metas por refeicao + distribuicao planejada: OK."


Write-Host "[335/570] Validando schema das fases nutricionais..."
...
Write-Host "[336/570] Validando listagem das fases..."
...
Write-Host "[337/570] Validando criacao de fase..."
...
Write-Host "[338/570] Validando edicao e estados..."
...
Write-Host "[339/570] Validando vinculo seguro com plano alimentar..."
...
Write-Host "[340/570] Validando reordenacao do ciclo..."
...
Write-Host "[341/570] Validando exclusao protegida..."
...
Write-Host "[342/570] Validando isolamento e auditoria..."
...
Write-Host "[343/570] Validando interface das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[344/570] Validando formulario de fase..."
...
Write-Host "[345/570] Validando reordenacao visual..."
...
Write-Host "[346/570] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[347/570] Validando SQL e PREPARAR 26..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[348/570] Validando versao v0.3.30..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.30 / fases nutricionais + planejamento de ciclo: OK."


Write-Host "[349/570] Validando schema das fases de treino..."
...
Write-Host "[350/570] Validando listagem das fases..."
...
Write-Host "[351/570] Validando criacao de fase..."
...
Write-Host "[352/570] Validando edicao e estados..."
...
Write-Host "[353/570] Validando vinculo seguro com ficha..."
if (-not $phaseSource.Contains("PlanoValido") -or
    -not $phaseSource.Contains("x.PacienteId == pacienteId") -or
    -not $phaseSource.Contains("x.Paciente.OrganizacaoId == currentUser.OrganizationId")) {
...
Write-Host "[354/570] Validando reordenacao do ciclo..."
...
Write-Host "[355/570] Validando exclusao protegida..."
if (-not $phaseSource.Contains("HttpDelete") -or
    -not $phaseSource.Contains('fase.Status == "EmAndamento"') -or
    -not $phaseSource.Contains('"DELETE"')) {
...
Write-Host "[356/570] Validando isolamento e auditoria..."
if (-not $phaseSource.Contains("currentUser.OrganizationId") -or
    -not $phaseSource.Contains("AuditLogs") -or
...
Write-Host "[357/570] Validando interface do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[358/570] Validando formulario de fase..."
...
Write-Host "[359/570] Validando reordenacao visual..."
...
Write-Host "[360/570] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[361/570] Validando SQL e PREPARAR 27..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[362/570] Validando versao v0.3.31..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.31 / ciclos de treino + periodizacao: OK."


Write-Host "[363/570] Validando schema dos check-ins..."
...
Write-Host "[364/570] Validando endpoint profissional..."
...
Write-Host "[365/570] Validando criacao e edicao..."
...
Write-Host "[366/570] Validando limites dos indicadores..."
...
Write-Host "[367/570] Validando vinculo com fases..."
...
Write-Host "[368/570] Validando auto-vinculo do paciente..."
...
Write-Host "[369/570] Validando portal do paciente..."
...
Write-Host "[370/570] Validando isolamento multi-tenant..."
...
Write-Host "[371/570] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[372/570] Validando graficos de resposta..."
...
Write-Host "[373/570] Validando formulario profissional..."
...
Write-Host "[374/570] Validando check-in no portal..."
...
Write-Host "[375/570] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[376/570] Validando SQL e PREPARAR 28..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[377/570] Validando preservacao dos ciclos..."
...
Write-Host "[378/570] Validando versao v0.3.32..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.32 / check-ins de evolucao + adesao por fase: OK."


Write-Host "[379/570] Validando endpoint de analise por fase..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[380/570] Validando metricas agregadas..."
...
Write-Host "[381/570] Validando variacao de peso por fase..."
...
Write-Host "[382/570] Validando destaques automaticos..."
...
Write-Host "[383/570] Validando isolamento multi-tenant..."
...
Write-Host "[384/570] Validando cards comparativos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[385/570] Validando destaques visuais..."
...
Write-Host "[386/570] Validando integracao com nutricao..."
...
Write-Host "[387/570] Validando integracao com treino..."
...
Write-Host "[388/570] Validando resumo consolidado..."
...
Write-Host "[389/570] Validando responsividade e compatibilidade de banco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $cssSource.Contains("phase-highlight-grid") -or
    ...
    -not $setupSource.Contains("[31/31]")) {
...
Write-Host "[390/570] Validando versao v0.3.33..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.33 / analise de fases + comparativo de resposta: OK."

Write-Host "[391/570] Validando metas das fases..."
...
Write-Host "[392/570] Validando mapeamento EF..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[393/570] Validando CRUD das fases..."
...
Write-Host "[394/570] Validando limites dos criterios..."
...
Write-Host "[395/570] Validando endpoint runtime de prontidao..."
...
Write-Host "[396/570] Validando motor de criterios objetivos..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[397/570] Validando revisao profissional..."
...
Write-Host "[398/570] Validando formularios das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[399/570] Validando metas nos cards..."
...
Write-Host "[400/570] Validando painel de prontidao..."
...
Write-Host "[405/570] Validando SQL e PREPARAR 29..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
$sqlSource = Get-Content .\scripts\sql\v0.3.34_criterios_transicao_fases.sql -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or -not $setupSource.Contains("v0.3.34_criterios_transicao_fases.sql") -or -not $sqlSource.Contains('"MetaPesoKg"') -or -not $sqlSource.Contains('"CriterioTransicao"') -or -not $setupSource.Contains("v0.3.32_checkins_acompanhamento.sql")) { throw "Upgrade dos criterios incompleto." }
Write-Host "    SQL idempotente + PREPARAR 29/29 + historico preservado."

Write-Host "[406/570] Validando versao v0.3.34..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.34 / metas de fase + criterios de transicao: OK."


Write-Host "[407/570] Validando entidade de revisao..."
...
Write-Host "[408/570] Validando mapeamento das revisoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[409/570] Validando historico runtime..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[410/570] Validando revisao nutricional..."
...
Write-Host "[411/570] Validando revisao de treino..."
...
Write-Host "[412/570] Validando decisoes e fase ativa..."
...
Write-Host "[413/570] Validando override consciente..."
...
Write-Host "[414/570] Validando transicao para proxima fase..."
...
Write-Host "[415/570] Validando transacao e auditoria..."
...
Write-Host "[416/570] Validando modal de revisao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[417/570] Validando historico visual..."
...
Write-Host "[418/570] Validando integracao com painel de transicao..."
...
Write-Host "[419/570] Validando SQL e PREPARAR 30..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "    SQL idempotente + PREPARAR 31/31 + v0.3.34 preservada."

Write-Host "[420/570] Validando versao v0.3.35..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.35 / revisao de fase + transicao assistida: OK."


Write-Host "[421/570] Validando controller de volume..."
...
Write-Host "[422/570] Validando isolamento multi-tenant..."
...
Write-Host "[423/570] Validando volume planejado por grupo..."
...
Write-Host "[424/570] Validando frequencia semanal..."
...
Write-Host "[425/570] Validando fallback de frequencia..."
...
Write-Host "[426/570] Validando execucoes reais..."
...
Write-Host "[427/570] Validando ausencia de tonelagem inventada..."
...
Write-Host "[428/570] Validando runtime da analise..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
    try {
...
Write-Host "[429/570] Validando painel de volume..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[430/570] Validando resumo analitico..."
...
Write-Host "[431/570] Validando volume por sessao..."
...
Write-Host "[432/570] Validando integracao e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[433/570] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "    Sem schema novo / PREPARAR permanece 31/31."

Write-Host "[434/570] Validando versao v0.3.36..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.36 / volume de treino + distribuicao muscular: OK."


Write-Host "[435/570] Validando endpoints de progressao por exercicio..."
...
Write-Host "[436/570] Validando seguranca e tenant..."
...
Write-Host "[437/570] Validando separacao por unidade..."
...
Write-Host "[438/570] Validando metricas de carga..."
...
Write-Host "[439/570] Validando recordes sucessivos..."
...
Write-Host "[440/570] Validando tendencia de carga..."
...
Write-Host "[441/570] Validando protecao contra estimativas artificiais..."
...
Write-Host "[442/570] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[443/570] Validando payload de pontos..."
...
Write-Host "[444/570] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[445/570] Validando graficos e recordes..."
...
Write-Host "[446/570] Validando portal do paciente..."
...
Write-Host "[447/570] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[448/570] Validando versao v0.3.37..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.37 / progressao por exercicio + recordes de carga: OK."


Write-Host "[449/570] Validando endpoints de sinais de progressao..."
...
Write-Host "[450/570] Validando tenant e PatientOnly..."
...
Write-Host "[451/570] Validando estados da analise..."
...
Write-Host "[452/570] Validando regra de estagnacao..."
...
Write-Host "[453/570] Validando sinal de carga/RPE..."
...
Write-Host "[454/570] Validando progressao recente..."
...
Write-Host "[455/570] Validando semantica nao diagnostica..."
...
Write-Host "[456/570] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[457/570] Validando payload analitico..."
...
Write-Host "[458/570] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[459/570] Validando portal do paciente..."
...
Write-Host "[460/570] Validando cards e graficos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[461/570] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[462/570] Validando versao v0.3.38..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.38 / estagnacao + fadiga + sinais de progressao: OK."


Write-Host "[463/570] Validando identidade MVP Preview..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.5.7") -or
...
Write-Host "[464/570] Validando aviso de demonstracao..."
...
Write-Host "[465/570] Validando roteiro da demo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[466/570] Validando checklist de avaliacao..."
...
Write-Host "[467/570] Validando modelo de feedback..."
...
Write-Host "[468/570] Validando dashboard de apresentacao..."
...
Write-Host "[469/570] Validando atalho Escape..."
...
Write-Host "[470/570] Validando feedback de conectividade..."
...
Write-Host "[471/570] Validando acabamento de foco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[472/570] Validando estados vazios..."
...
Write-Host "[473/570] Validando limpeza da navegacao de demo..."
...
Write-Host "[474/570] Validando responsividade do MVP..."
...
Write-Host "[475/570] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
...
Write-Host "[476/570] Validando versao v0.3.39..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.39 / MVP Preview + polimento de demonstracao: OK."


Write-Host "[477/570] Validando Dockerfile..."
...
Write-Host "[478/570] Validando bind dinamico de porta..."
...
Write-Host "[479/570] Validando Blueprint Render..."
...
Write-Host "[480/570] Validando secrets do Blueprint..."
...
Write-Host "[481/570] Validando conexao PostgreSQL do Render..."
...
Write-Host "[482/570] Validando bootstrap isolado do MVP..."
...
Write-Host "[484/570] Validando healthcheck real..."
...
Write-Host "[485/570] Validando forwarded headers..."
...
Write-Host "[486/570] Validando POPULAR remoto..."
...
if (-not $remotePopular.Contains("[Parameter(Mandatory=`$true)][string]`$BaseUrl") -or
    -not $remotePopular.Contains("HealthPlatform v0.5.7 - POPULAR RENDER DEMO") -or
...
Write-Host "[487/570] Validando catalogos da demo remota..."
...
Write-Host "[488/570] Validando smoke test remoto..."
...
Write-Host "[489/570] Validando guia de deploy..."
...
Write-Host "[490/570] Validando ausencia de schema novo..."
...
Write-Host "    PREPARAR continua 31/31 / sem SQL v0.3.40."

Write-Host "[491/570] Validando arquivos de container..."
...
Write-Host "[492/570] Validando versao base do deploy..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.7 / Render Demo Deploy preservado: OK."


Write-Host "[493/570] Validando paleta RS..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[494/570] Validando monograma RS..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
...
Write-Host "[495/570] Validando linguagem editorial..."
...
Write-Host "[496/570] Validando tipografia condensada..."
...
Write-Host "[497/570] Validando login editorial..."
...
Write-Host "[498/570] Validando sidebar RS..."
...
Write-Host "[499/570] Validando experiencia iPad..."
...
Write-Host "[500/570] Validando drawer mobile..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[501/570] Validando safe areas iOS..."
...
Write-Host "[502/570] Validando modal bottom-sheet no iPhone..."
...
Write-Host "[503/570] Validando portal mobile..."
...
Write-Host "[504/570] Validando abas do prontuario no mobile..."
...
Write-Host "[505/570] Validando touch targets..."
...
Write-Host "[506/570] Validando cards analiticos no iPhone..."
...
Write-Host "[507/570] Validando deploy/demo preservados..."
$renderSource = Get-Content .\render.yaml -Encoding UTF8 -Raw
...
Write-Host "[508/570] Validando entidade de solicitacoes clinicas..."
...
Write-Host "[509/570] Validando mapeamento EF de solicitacoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[510/570] Validando endpoints profissionais de solicitacoes..."
...
Write-Host "[511/570] Validando portal do paciente para solicitacoes..."
...
Write-Host "[512/570] Validando isolamento multi-tenant..."
...
Write-Host "[513/570] Validando auditoria de solicitacoes..."
...
Write-Host "[514/570] Validando interface profissional..."
...
Write-Host "[515/570] Validando interface do paciente..."
...
Write-Host "[516/570] Validando upgrade SQL v0.5.1 preservado..."
...
Write-Host "[517/570] Validando PREPARAR 31/31..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[31/31] Aplicando upgrade v0.5.1') -or -not $setupSource.Contains('v0.5.1_solicitacoes_clinicas.sql')) { throw "PREPARAR nao preserva o upgrade v0.5.1." }
Write-Host "    Upgrade de solicitacoes integrado ao setup."

Write-Host "[518/570] Validando versao v0.5.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.7 / Solicitacoes Clinicas + Connected Care: OK."

Write-Host "[519/570] Validando notificacoes de solicitacoes no backend..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[520/570] Validando prioridade por prazo para o paciente..."
...
Write-Host "[521/570] Validando notificacao de resposta para profissional..."
...
Write-Host "[522/570] Validando limpeza de notificacoes encerradas..."
...
Write-Host "[523/570] Validando navegacao do paciente pela notificacao..."
...
Write-Host "[524/570] Validando navegacao contextual do profissional..."
...
Write-Host "[525/570] Validando identidade visual de notificacao..."
...
Write-Host "[526/570] Validando versao funcional v0.5.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
if (-not $notificationSource.Contains('SolicitacaoClinica')) { throw "Feature v0.5.7 ausente." }
Write-Host "    v0.5.7 / Notificacoes Contextuais + Connected Care: OK."

Write-Host "[527/570] Validando endpoint da Central de Solicitacoes..."
$requestController = Get-Content .\src\HealthPlatform.Api\Controllers\SolicitacoesClinicasController.cs -Encoding UTF8 -Raw
...
Write-Host "[528/570] Validando resumo operacional de solicitacoes..."
...
Write-Host "[529/570] Validando filtros da Central de Solicitacoes..."
...
Write-Host "[530/570] Validando isolamento da fila profissional..."
...
Write-Host "[531/570] Validando navegacao profissional de solicitacoes..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[532/570] Validando indicadores e filtros visuais..."
...
Write-Host "[533/570] Validando acoes rapidas da Central de Solicitacoes..."
...
Write-Host "[534/570] Validando responsividade da Central de Solicitacoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[535/570] Validando versao funcional v0.5.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
if (-not $requestController.Contains('FilaProfissional')) { throw "Feature v0.5.7 ausente." }
Write-Host "    v0.5.7 / Central de Solicitacoes + Connected Care: OK."


Write-Host "[536/570] Validando solicitacoes no Hoje do paciente..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[537/570] Validando solicitacoes no progresso diario..."
...
Write-Host "[538/570] Validando card de acao necessaria..."
...
Write-Host "[539/570] Validando destaque de solicitacao vencida no portal..."
...
Write-Host "[540/570] Validando integracao da Central do Dia no backend..."
$centralSource = Get-Content .\src\HealthPlatform.Api\Controllers\CentralDiaController.cs -Encoding UTF8 -Raw
...
Write-Host "[541/570] Validando escopo profissional das solicitacoes do dia..."
...
Write-Host "[542/570] Validando card profissional de solicitacoes..."
...
Write-Host "[543/570] Validando dashboard com solicitacoes para revisao..."
...
Write-Host "[544/570] Validando responsividade das solicitacoes no Hoje..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[545/570] Validando versao funcional v0.5.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
if (-not $centralSource.Contains('SolicitacoesParaRevisao') -or -not $appJsSource.Contains('patient-today-requests')) { throw "Feature v0.5.7 ausente." }
Write-Host "    v0.5.7 / Solicitacoes no Hoje + Connected Care: OK."

Write-Host "[546/570] Validando contrato Desde a ultima consulta..."
$resumoSource = Get-Content .\src\HealthPlatform.Api\Controllers\ResumoClinicoController.cs -Encoding UTF8 -Raw
...
Write-Host "[547/570] Validando periodo baseado na ultima consulta..."
...
Write-Host "[548/570] Validando atividade do paciente no periodo..."
...
Write-Host "[549/570] Validando adesao media por check-ins..."
...
Write-Host "[550/570] Validando variacao de peso no periodo..."
...
Write-Host "[551/570] Validando solicitacoes no resumo longitudinal..."
...
Write-Host "[552/570] Validando card longitudinal no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[553/570] Validando versao funcional v0.5.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
if (-not $resumoSource.Contains('ResumoDesdeUltimaConsultaResponse') -or -not $appJsSource.Contains('data-clinical-period-summary')) { throw "Feature v0.5.7 ausente." }
Write-Host "    v0.5.7 / Desde a ultima consulta + Connected Care: OK."

Write-Host "[554/570] Validando endpoint da Jornada do paciente..."
...
Write-Host "[555/570] Validando escopo PatientOnly da Jornada..."
...
Write-Host "[556/570] Validando fontes da Jornada..."
...
Write-Host "[557/570] Validando navegacao Jornada no portal..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[558/570] Validando filtro temporal da Jornada..."
...
Write-Host "[559/570] Validando atalhos contextuais da Jornada..."
...
Write-Host "[560/570] Validando responsividade da Jornada..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[561/570] Validando versao funcional v0.5.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
if (-not $portalController.Contains('MinhaJornada') -or -not $appJsSource.Contains('loadPatientJourney')) { throw "Feature v0.5.7 ausente." }
Write-Host "    v0.5.7 / Jornada do Paciente + Connected Care: OK."


Write-Host "[562/570] Validando endpoint de monitoramento profissional..."
...
Write-Host "[563/570] Validando monitoramento no portal do paciente..."
...
Write-Host "[564/570] Validando metricas guiadas..."
...
Write-Host "[565/570] Validando resumo de 7 dias..."
...
Write-Host "[566/570] Validando registro guiado de pressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[567/570] Validando registros rapidos de sinais vitais..."
...
Write-Host "[568/570] Validando resumo profissional de monitoramento..."
...
Write-Host "[569/570] Validando responsividade do monitoramento..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[570/570] Validando versao funcional v0.5.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.7") { throw "VERSION.txt inesperado." }
if (-not $monitorSource.Contains('TiposMonitorados') -or -not $appJsSource.Contains('hpMonitoringCard')) { throw "Feature v0.5.7 ausente." }
Write-Host "    v0.5.7 / Monitoramento guiado + Connected Care: OK."
