...
if ($health.version -ne "0.6.9") { throw "Versao inesperada da API: $($health.version)" }
...
if ($version.Trim() -ne "0.6.9") { throw "VERSION.txt inesperado." }
...
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.6.9") -or
...
Write-Host "[600/672] Validando compatibilidade funcional v0.6.0..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9") { throw "VERSION.txt inesperado." }
...
Write-Host "[601/672] Validando entidade de prontidao diaria..."
...
Write-Host "[602/672] Validando mapeamento EF da prontidao..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[603/672] Validando endpoint PatientOnly de prontidao..."
...
Write-Host "[604/672] Validando travas de seguranca do score..."
...
Write-Host "[605/672] Validando Home Daily Athlete..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[606/672] Validando leitura profissional da prontidao..."
...
Write-Host "[607/672] Validando SQL e migration v0.6.1..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[608/672] Validando versao funcional v0.6.1..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao v0.6.1 inconsistente." }
Write-Host "    v0.6.1 / Daily Athlete + Prontidao Diaria: OK."



Write-Host "[609/672] Validando ledger de XP..."
...
Write-Host "[610/672] Validando mapeamento EF de XP..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[611/672] Validando XP seguro de treino..."
...
Write-Host "[612/672] Validando nivel, streak e consistencia..."
...
Write-Host "[613/672] Validando eventos de autocuidado..."
$portalMeSource = Get-Content .\src\HealthPlatform.Api\Controllers\MeuPortalPacienteController.cs -Encoding UTF8 -Raw
...
Write-Host "[614/672] Validando Home esportiva gamificada..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[615/672] Validando SQL e migration v0.6.2..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[616/672] Validando versao funcional v0.6.2..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao v0.6.2 inconsistente." }
Write-Host "    v0.6.2 / XP + Nivel + Streak + Consistencia: OK."

Write-Host "[617/672] Validando entidades de desafios e conquistas..."
...
Write-Host "[618/672] Validando mapeamento EF de missoes..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[619/672] Validando mapeamento EF de conquistas..."
...
Write-Host "[620/672] Validando missoes automaticas..."
$gameSource = Get-Content .\src\HealthPlatform.Api\Services\GamificacaoService.cs -Encoding UTF8 -Raw
...
Write-Host "[621/672] Validando conquistas esportivas..."
...
Write-Host "[622/672] Validando Home com missoes e conquistas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[623/672] Validando SQL e migration v0.6.3..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[624/672] Validando compatibilidade funcional v0.6.3..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao atual inconsistente ao validar v0.6.3." }
Write-Host "    v0.6.3 / Missoes + Desafios + Conquistas preservada: OK."


Write-Host "[625/672] Validando entidade de ciclo esportivo..."
...
Write-Host "[626/672] Validando mapeamento EF de ciclos..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[627/672] Validando gestao profissional de ciclos..."
...
Write-Host "[628/672] Validando resumo longitudinal do ciclo..."
...
Write-Host "[629/672] Validando desafio semanal adaptado ao ciclo..."
$gameSource = Get-Content .\src\HealthPlatform.Api\Services\GamificacaoService.cs -Encoding UTF8 -Raw
...
Write-Host "[630/672] Validando Home e leitura profissional do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[631/672] Validando SQL e migration v0.6.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[632/672] Validando compatibilidade funcional v0.6.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Compatibilidade v0.6.4 inconsistente na versao atual." }
Write-Host "    v0.6.4 / Ciclos Esportivos + Metas de Ciclo preservado: OK."

Write-Host "[633/672] Validando contrato da Estrategia do Dia..."
...
Write-Host "[634/672] Validando motor de adaptacao esportiva..."
...
Write-Host "[635/672] Validando trava contra sobrecarga automatica..."
...
Write-Host "[636/672] Validando integracao com ciclo e planos prescritos..."
...
Write-Host "[637/672] Validando Home Daily Athlete com Estrategia do Dia..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[638/672] Validando leitura profissional da estrategia diaria..."
...
Write-Host "[639/672] Validando ausencia de alteracao indevida de schema..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[640/672] Validando compatibilidade funcional v0.6.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao v0.6.6 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.6.9"')) { throw "Health endpoint nao anuncia v0.6.6." }
Write-Host "    v0.6.5 / Estrategia do Dia preservada: OK."



Write-Host "[641/672] Validando contrato de execucao guiada..."
$portalContracts = Get-Content .\src\HealthPlatform.Api\Contracts\Portal\PortalPacienteContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[642/672] Validando motor do Roteiro de Hoje..."
...
Write-Host "[643/672] Validando progresso baseado em registros reais..."
...
Write-Host "[644/672] Validando fechamento diario idempotente..."
$portalMeSource = Get-Content .\src\HealthPlatform.Api\Controllers\MeuPortalPacienteController.cs -Encoding UTF8 -Raw
...
Write-Host "[645/672] Validando Home com Roteiro de Hoje..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[646/672] Validando leitura profissional da execucao..."
...
Write-Host "[647/672] Validando ausencia de alteracao indevida de schema v0.6.6..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[648/672] Validando compatibilidade funcional v0.6.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao v0.6.6 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.6.9"')) { throw "Health endpoint nao anuncia v0.6.6." }
...
Write-Host "[649/672] Validando contrato de tendencias de recuperacao..."
$portalContracts = Get-Content .\src\HealthPlatform.Api\Contracts\Portal\PortalPacienteContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[650/672] Validando janela longitudinal 7 vs 7 dias..."
...
Write-Host "[651/672] Validando sinais esportivos observaveis..."
...
Write-Host "[652/672] Validando alertas conservadores de recuperacao..."
...
Write-Host "[653/672] Validando ausencia de diagnostico automatico..."
...
Write-Host "[654/672] Validando Home e prontuario com tendencia de recuperacao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[655/672] Validando ausencia de alteracao indevida de schema v0.6.7..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[37/37] Aplicando upgrade v0.6.4')) { throw "PREPARAR 37/37 foi alterado sem necessidade." }
...
Write-Host "[656/672] Validando compatibilidade funcional v0.6.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao v0.6.9 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.6.9"')) { throw "Health endpoint nao anuncia v0.6.9." }
Write-Host "    v0.6.7 / Tendencias de Recuperacao + Alertas Inteligentes preservada: OK."


Write-Host "[657/672] Validando contrato de performance esportiva..."
$portalContracts = Get-Content .\src\HealthPlatform.Api\Contracts\Portal\PortalPacienteContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[658/672] Validando motor de performance a partir das execucoes reais..."
...
Write-Host "[659/672] Validando deteccao conservadora de PR..."
...
Write-Host "[660/672] Validando contexto de prontidao no PR..."
...
Write-Host "[661/672] Validando linguagem segura de performance..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[662/672] Validando Home e prontuario com Performance & PRs..."
...
Write-Host "[663/672] Validando ausencia de alteracao indevida de schema v0.6.8..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[37/37] Aplicando upgrade v0.6.4')) { throw "PREPARAR 37/37 foi alterado sem necessidade." }
if (Test-Path .\scripts\sql\v0.6.8_performance_prs.sql) { throw "v0.6.9 nao deveria exigir migration." }
Write-Host "    Performance e calculada sobre o historico existente: OK."

Write-Host "[664/672] Validando compatibilidade funcional v0.6.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao v0.6.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.6.9"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.6.9." }
Write-Host "    v0.6.8 / Performance & PRs preservada: OK."

Write-Host "[665/672] Validando contrato de carga de treino..."
$portalContracts = Get-Content .\src\HealthPlatform.Api\Contracts\Portal\PortalPacienteContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[666/672] Validando motor sRPE sobre execucoes reais..."
...
Write-Host "[667/672] Validando linha de base de tres semanas..."
...
Write-Host "[668/672] Validando integracao com recuperacao..."
...
Write-Host "[669/672] Validando linguagem conservadora da carga..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[670/672] Validando Home e prontuario com carga semanal..."
...
Write-Host "[671/672] Validando ausencia de alteracao indevida de schema v0.6.9..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[37/37] Aplicando upgrade v0.6.4')) { throw "PREPARAR 37/37 foi alterado sem necessidade." }
if (Test-Path .\scripts\sql\v0.6.9_carga_treino.sql) { throw "v0.6.9 nao deveria exigir migration." }
Write-Host "    Carga de treino reutiliza schema existente: OK."

Write-Host "[672/672] Validando versao funcional v0.6.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.9" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.9';")) { throw "Versao v0.6.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.6.9"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.6.9." }
Write-Host "    v0.6.9 / Carga de Treino & Equilibrio: OK."
