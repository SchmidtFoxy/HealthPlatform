...
Write-Host "[1/580] Healthcheck..." -ForegroundColor Cyan
$health = Invoke-RestMethod -Uri "$base/api/health" -Method Get
if ($health.version -ne "0.5.8") { throw "Versao inesperada da API: $($health.version)" }
Write-Host "    API $($health.version) / banco $($health.database)" -ForegroundColor Green

Write-Host "[2/580] Login..." -ForegroundColor Cyan
...
Write-Host "[3/580] Listando pacientes..." -ForegroundColor Cyan
...
Write-Host "[4/580] Validando perguntas de anamnese..." -ForegroundColor Cyan
...
Write-Host "[5/580] Validando catalogo laboratorial..." -ForegroundColor Cyan
...
Write-Host "[6/580] Validando catalogo de alimentos..." -ForegroundColor Cyan
...
Write-Host "[7/580] Validando busca/paginacao..." -ForegroundColor Cyan
...
Write-Host "[8/580] Validando modulos do paciente..." -ForegroundColor Cyan
...
Write-Host "[9/580] Validando metas do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[10/580] Validando diario/resumo do dia..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[11/580] Validando portal/home do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[12/580] Validando agenda do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[13/580] Validando dashboard do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[14/580] Validando interface web..." -ForegroundColor Cyan
...
Write-Host "[15/580] Validando assets da interface..." -ForegroundColor Cyan
...
Write-Host "[16/580] Validando prontuario visual v0.2.2..." -ForegroundColor Cyan
...
Write-Host "[17/580] Validando acoes clinicas da interface..." -ForegroundColor Cyan
...
Write-Host "[18/580] Validando rotas usadas pelos formularios clinicos..." -ForegroundColor Cyan
...
Write-Host "[19/580] Validando cadastro visual de exames..." -ForegroundColor Cyan
...
Write-Host "[20/580] Validando construtor visual do plano alimentar..." -ForegroundColor Cyan
...
Write-Host "[21/580] Validando relatorios na interface..." -ForegroundColor Cyan
...
Write-Host "[22/580] Validando edicao visual do paciente..." -ForegroundColor Cyan
...
Write-Host "[23/580] Validando endpoint de relatorios do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[24/580] Validando edicao clinica visual..."
...
Write-Host "[25/580] Validando agenda operacional..."
...
Write-Host "[26/580] Validando endpoint de atualizacao de avaliacao..."
try {
...
Write-Host "[27/580] Validando tela de configuracoes..."
...
Write-Host "[28/580] Validando gerenciadores de catalogo na interface..."
...
Write-Host "[29/580] Validando resumo de configuracoes do consultorio..."
...
Write-Host "[30/580] Validando rotas dos catalogos..."
...
Write-Host "[31/580] Validando edicao de configuracoes..."
$cfg = Invoke-RestMethod -Uri "$base/api/configuracoes/resumo" -Headers $headers
...
Write-Host "[32/580] Validando assets de edicao/inativacao dos catalogos..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[35/580] Validando separacao de autorizacao profissional/paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[36/580] Validando endpoint de status de acesso do paciente..."
...
Write-Host "[37/580] Validando fluxo de convite/ativacao nos assets..."
...
Write-Host "[38/580] Validando autoatendimento do diario..."
...
Write-Host "[39/580] Validando autoatendimento das metas..."
...
Write-Host "[40/580] Validando tela dedicada do portal do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[41/580] Validando navegacao completa do portal..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[42/580] Validando endpoint proprio de plano alimentar..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[43/580] Validando historico proprio de metas e diario..."
...
Write-Host "[47/580] Validando schema/endpoint do catalogo de exercicios..."
...
Write-Host "[48/580] Validando endpoint de planos de treino do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[49/580] Validando construtor visual de treino..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[50/580] Validando videos e prescricao de exercicios..."
...
Write-Host "[51/580] Validando aba de treinos no prontuario..."
...
Write-Host "[52/580] Validando navegacao de treino do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[53/580] Validando endpoint proprio do treino do paciente..."
...
Write-Host "[54/580] Validando assets visuais do modulo de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[55/580] Validando schema de execucoes de treino..."
...
Write-Host "[56/580] Validando historico profissional de treinos..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[57/580] Validando registro visual de execucao pelo paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[58/580] Validando series, repeticoes e carga realizadas..."
...
Write-Host "[59/580] Validando esforco percebido e duracao..."
...
Write-Host "[60/580] Validando historico do paciente..."
...
Write-Host "[61/580] Validando progressao de carga no prontuario..."
...
Write-Host "[62/580] Validando estilos do acompanhamento de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[63/580] Validando motor de graficos SVG..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[64/580] Validando graficos corporais no prontuario..."
...
Write-Host "[65/580] Validando tendencias laboratoriais..."
...
Write-Host "[66/580] Validando progressao grafica de carga..."
...
Write-Host "[67/580] Validando evolucao visual no portal do paciente..."
...
Write-Host "[68/580] Validando graficos de exames no portal..."
...
Write-Host "[69/580] Validando responsividade dos graficos..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[70/580] Validando compatibilidade de schema na v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
...
Write-Host "[71/580] Validando endpoint de insights do dashboard..."
...
Write-Host "[72/580] Validando insights por paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[73/580] Validando regra de exame fora da referencia..."
...
Write-Host "[74/580] Validando regras de evolucao e retorno..."
...
Write-Host "[75/580] Validando regras de adesao..."
...
Write-Host "[76/580] Validando central de atencao visual..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[77/580] Validando insights no prontuario..."
...
Write-Host "[78/580] Validando estilos e compatibilidade do schema..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[79/580] Validando schema de pendencias clinicas..."
...
Write-Host "[80/580] Validando endpoint geral de pendencias..."
...
Write-Host "[81/580] Validando endpoint de pendencias do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[82/580] Validando acoes de ciclo de vida..."
...
Write-Host "[83/580] Validando criacao de retorno a partir da pendencia..."
...
Write-Host "[84/580] Validando transformar insight em pendencia..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[85/580] Validando tela de gerenciamento de pendencias..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[86/580] Validando acoes visuais da pendencia..."
...
Write-Host "[87/580] Validando resumo de pendencias no dashboard..."
...
Write-Host "[88/580] Validando auditoria, estilos e upgrade v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[89/580] Validando schema de notificacoes internas..."
...
Write-Host "[90/580] Validando sincronizacao de notificacoes..."
...
Write-Host "[91/580] Validando listagem e contador nao lido..."
...
Write-Host "[92/580] Validando regras de agenda profissional..."
...
Write-Host "[93/580] Validando regras de pendencias..."
...
Write-Host "[94/580] Validando lembretes do paciente..."
...
Write-Host "[95/580] Validando leitura individual e em massa..."
...
Write-Host "[96/580] Validando sino e drawer na interface..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[97/580] Validando contador e atualizacao periodica..."
...
Write-Host "[98/580] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[99/580] Validando script de popular banco..."
...
Write-Host "[100/580] Validando idempotencia do popular..."
...
Write-Host "[101/580] Validando cobertura de modulos na base demo..."
...
Write-Host "[102/580] Validando endpoint de resumo de dados..."
...
Write-Host "[103/580] Validando que popular banco e opt-in..."
...
Write-Host "[104/580] Validando versao v0.3.27 e upgrade do schema..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
...
Write-Host "[105/580] Validando endpoint da carteira..."
...
Write-Host "[106/580] Validando priorizacao da carteira..."
...
Write-Host "[107/580] Validando sinais de exames/evolucao na carteira..."
...
Write-Host "[108/580] Validando atividade recente..."
...
Write-Host "[109/580] Validando tela Carteira..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[110/580] Validando filtros e ordenacao..."
...
Write-Host "[111/580] Validando atalho da carteira no dashboard..."
...
Write-Host "[112/580] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[113/580] Validando schema de follow-up..."
...
Write-Host "[114/580] Validando endpoint de follow-up..."
...
Write-Host "[115/580] Validando canais e proximo contato..."
...
Write-Host "[116/580] Validando auditoria do contato..."
...
Write-Host "[117/580] Validando follow-up na carteira..."
$carteiraSource = Get-Content .\src\HealthPlatform.Api\Controllers\CarteiraController.cs -Encoding UTF8 -Raw
...
Write-Host "[118/580] Validando acao rapida de contato..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[119/580] Validando acao rapida de retorno..."
...
Write-Host "[120/580] Validando acao rapida de pendencia..."
...
Write-Host "[121/580] Validando historico no prontuario..."
...
Write-Host "[122/580] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[123/580] Validando endpoint da fila de follow-up..."
...
Write-Host "[124/580] Validando faixas de vencimento..."
...
Write-Host "[125/580] Validando busca e filtros da fila..."
...
Write-Host "[126/580] Validando notificacao de follow-up..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[127/580] Validando tela de follow-up..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[128/580] Validando acoes rapidas na fila..."
...
Write-Host "[129/580] Validando resumo de follow-up no dashboard..."
...
Write-Host "[130/580] Validando estilos, popular e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
$popularSource = Get-Content .\POPULAR.ps1 -Encoding UTF8 -Raw
...
Write-Host "[131/580] Validando endpoint de gestao..."
...
Write-Host "[132/580] Validando indicadores de consultas..."
...
Write-Host "[133/580] Validando indicadores de acompanhamento..."
...
Write-Host "[134/580] Validando indicadores de engajamento..."
...
Write-Host "[135/580] Validando series gerenciais..."
...
Write-Host "[136/580] Validando tela Gestao..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[137/580] Validando graficos e resumo no dashboard..."
...
Write-Host "[138/580] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[139/580] Validando PREPARAR sem atualizacao automatica do dotnet..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[140/580] Validando endpoint CSV gerencial..."
...
Write-Host "[141/580] Validando conteudo longitudinal do CSV..."
...
Write-Host "[142/580] Validando relatorio HTML imprimivel..."
...
Write-Host "[143/580] Validando botoes de exportacao na Gestao..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[144/580] Validando download autenticado..."
...
Write-Host "[145/580] Validando relatorio autenticado em nova janela..."
...
Write-Host "[146/580] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[147/580] Validando perguntas de anamnese inativas..."
...
Write-Host "[148/580] Validando totais completos de insights..."
...
Write-Host "[149/580] Validando timezone das notificacoes..."
...
Write-Host "[150/580] Validando cleanup do polling no logout..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[151/580] Validando navegacao robusta no portal..."
...
Write-Host "[152/580] Validando smoke de notificacoes sem mutacao..."
...
Write-Host "[153/580] Validando copy historica de schema..."
$schemaCheckMarker = 'Write-Host "[153/580] Validando copy historica de schema..."'
...
Write-Host "[154/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / estabilizacao + qualidade: OK."


Write-Host "[155/580] Validando endpoint de busca global..."
...
Write-Host "[156/580] Validando fontes da busca..."
...
Write-Host "[157/580] Validando isolamento por organizacao..."
...
Write-Host "[158/580] Validando botao e atalho da busca..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[159/580] Validando modal e debounce..."
...
Write-Host "[160/580] Validando acoes dos resultados..."
...
Write-Host "[161/580] Validando estilos responsivos da busca..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[162/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / busca global + central de acoes: OK."


Write-Host "[163/580] Validando endpoint da Central do Dia..."
...
Write-Host "[164/580] Validando agenda do dia..."
...
Write-Host "[165/580] Validando follow-ups do dia..."
...
Write-Host "[166/580] Validando pendencias prioritarias..."
...
Write-Host "[167/580] Validando pacientes para revisao..."
...
Write-Host "[168/580] Validando tela Hoje..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[169/580] Validando acoes rapidas e dashboard..."
...
Write-Host "[170/580] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[171/580] Validando schema de evolucoes clinicas SOAP..."
...
Write-Host "[172/580] Validando endpoint de evolucoes..."
...
Write-Host "[173/580] Validando consulta opcional e isolamento..."
...
Write-Host "[174/580] Validando auditoria da evolucao..."
...
Write-Host "[175/580] Validando evolucao na timeline..."
...
Write-Host "[176/580] Validando aba Evolucoes no prontuario..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[177/580] Validando registro visual SOAP..."
...
Write-Host "[178/580] Validando edicao visual da evolucao..."
...
Write-Host "[179/580] Validando upgrade SQL SOAP historico e PREPARAR atual..."
...
Write-Host "[180/580] Validando estilos e versao v0.3.27..."
...
Write-Host "[181/580] Validando endpoint de resumo clinico..."
...
Write-Host "[182/580] Validando consulta e SOAP no resumo..."
...
Write-Host "[183/580] Validando corpo e anamnese..."
...
Write-Host "[184/580] Validando exames alterados..."
...
Write-Host "[185/580] Validando metas, treinos e pendencias..."
...
Write-Host "[186/580] Validando resumo no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[187/580] Validando atualizacao e responsividade..."
...
Write-Host "[188/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / resumo clinico consolidado: OK."


Write-Host "[189/580] Validando texto de handoff clinico..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[190/580] Validando conteudo SOAP no handoff..."
...
Write-Host "[191/580] Validando copia para clipboard..."
...
Write-Host "[192/580] Validando impressao do resumo..."
...
Write-Host "[193/580] Validando seguranca basica do HTML imprimivel..."
...
Write-Host "[194/580] Validando botoes no resumo clinico..."
...
Write-Host "[195/580] Validando responsividade das acoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[196/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / handoff clinico + impressao: OK."


Write-Host "[197/580] Validando endpoint de equipe..."
...
Write-Host "[198/580] Validando isolamento e administracao..."
...
Write-Host "[199/580] Validando criacao de acesso..."
...
Write-Host "[200/580] Validando sincronizacao de tipo e role..."
...
Write-Host "[201/580] Validando perfil profissional..."
...
Write-Host "[202/580] Validando protecao do proprio administrador..."
...
Write-Host "[203/580] Validando auditoria da equipe..."
...
Write-Host "[204/580] Validando tela Equipe..."
...
Write-Host "[205/580] Validando visibilidade admin e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[206/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe + gestao de profissionais: OK."


Write-Host "[207/580] Validando filtros da equipe..."
$teamSource = Get-Content .\src\HealthPlatform.Api\Controllers\EquipeController.cs -Encoding UTF8 -Raw
...
Write-Host "[208/580] Validando redefinicao de senha..."
...
Write-Host "[209/580] Validando escopo e protecoes do reset..."
...
Write-Host "[210/580] Validando auditoria segura de senha..."
...
Write-Host "[211/580] Validando filtros visuais..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[212/580] Validando acao visual de senha..."
...
Write-Host "[213/580] Validando responsividade da equipe v2..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[214/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe v2 + seguranca de acesso: OK."


Write-Host "[215/580] Validando endpoint Minha Conta..."
...
Write-Host "[216/580] Validando alteracao da propria senha..."
...
Write-Host "[217/580] Validando protecoes da troca de senha..."
...
Write-Host "[218/580] Validando auditoria segura da conta..."
...
Write-Host "[219/580] Validando sincronizacao do nome profissional..."
...
Write-Host "[220/580] Validando painel Minha Conta..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[221/580] Validando responsividade da conta..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[222/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / Minha Conta + troca de senha: OK."


Write-Host "[223/580] Validando schema de progressao alimentar..."
...
Write-Host "[224/580] Validando simulador nutricional..."
...
Write-Host "[225/580] Validando duplicacao versionada..."
...
Write-Host "[226/580] Validando escala de porcoes..."
...
Write-Host "[227/580] Validando ajuste por calorias alvo..."
...
Write-Host "[228/580] Validando limites de ajuste..."
...
Write-Host "[229/580] Validando encerramento opcional do plano anterior..."
...
Write-Host "[230/580] Validando auditoria da progressao..."
...
Write-Host "[231/580] Validando interface de progressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[232/580] Validando modos percentual e calorias..."
...
Write-Host "[233/580] Validando upgrade SQL e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[234/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao do plano alimentar: OK."


Write-Host "[235/580] Validando schema de progressao de treino..."
...
Write-Host "[236/580] Validando simulador de treino..."
...
Write-Host "[237/580] Validando duplicacao versionada do treino..."
...
Write-Host "[238/580] Validando progressao de carga..."
...
Write-Host "[239/580] Validando progressao de series e descanso..."
...
Write-Host "[240/580] Validando repeticoes estruturadas..."
...
Write-Host "[241/580] Validando limites de progressao..."
...
Write-Host "[242/580] Validando encerramento e auditoria..."
...
Write-Host "[243/580] Validando interface de progressao de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[244/580] Validando projecao e modal ampliado..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[245/580] Validando upgrade SQL e PREPARAR 19..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[246/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao de treino + ciclo versionado: OK."


Write-Host "[247/580] Validando schema dos modelos alimentares..."
...
Write-Host "[248/580] Validando listagem e busca de modelos..."
...
Write-Host "[249/580] Validando salvar plano como modelo..."
...
Write-Host "[250/580] Validando criacao de plano a partir de modelo..."
...
Write-Host "[251/580] Validando alimentos do catalogo..."
...
Write-Host "[252/580] Validando isolamento e auditoria..."
...
Write-Host "[253/580] Validando interface de salvar modelo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[254/580] Validando seletor de modelos..."
...
Write-Host "[255/580] Validando criacao visual via modelo..."
...
Write-Host "[256/580] Validando responsividade dos modelos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[257/580] Validando upgrade SQL e PREPARAR 20..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[258/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de plano alimentar: OK."


Write-Host "[259/580] Validando schema dos modelos de treino..."
...
Write-Host "[260/580] Validando listagem e busca de modelos de treino..."
...
Write-Host "[261/580] Validando salvar treino como modelo..."
if (-not $modelSource.Contains("salvar-como-modelo") -or
...
Write-Host "[262/580] Validando criacao de treino a partir de modelo..."
if (-not $modelSource.Contains("criar-de-modelo") -or
...
Write-Host "[263/580] Validando catalogo de exercicios..."
...
Write-Host "[264/580] Validando prescricao completa do template..."
...
Write-Host "[265/580] Validando isolamento e auditoria dos modelos de treino..."
if (-not $modelSource.Contains("currentUser.OrganizationId") -or
    -not $modelSource.Contains('"CREATE_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("AuditLogs")) {
...
Write-Host "[266/580] Validando interface de salvar modelo de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[267/580] Validando seletor e busca de modelos..."
...
Write-Host "[268/580] Validando criacao visual via modelo..."
...
Write-Host "[269/580] Validando upgrade SQL, responsividade e PREPARAR 21..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[270/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de treino + criacao rapida: OK."


Write-Host "[271/580] Validando schema das metas nutricionais..."
$planEntity = Get-Content .\src\HealthPlatform.Domain\Entities\PlanoAlimentar.cs -Encoding UTF8 -Raw
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[272/580] Validando contratos nutricionais..."
...
Write-Host "[273/580] Validando endpoint de metas..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[274/580] Validando analise nutricional..."
...
Write-Host "[275/580] Validando metas na criacao e edicao..."
...
Write-Host "[276/580] Validando metas na progressao alimentar..."
...
Write-Host "[277/580] Validando metas nos templates alimentares..."
...
Write-Host "[278/580] Validando construtor com metas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[279/580] Validando Meta x Prescrito no prontuario..."
...
Write-Host "[280/580] Validando distribuicao por refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[281/580] Validando modal de metas..."
...
Write-Host "[282/580] Validando responsividade nutricional..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[283/580] Validando SQL e PREPARAR 22..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[284/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / metas nutricionais + distribuicao: OK."


Write-Host "[285/580] Validando schema da biblioteca de refeicoes..."
...
Write-Host "[286/580] Validando listagem e filtros da biblioteca..."
...
Write-Host "[287/580] Validando salvar refeicao como modelo..."
...
Write-Host "[288/580] Validando insercao de refeicao no plano..."
...
Write-Host "[289/580] Validando catalogo e substituicoes..."
if (-not $modelSource.Contains("alimentosValidos") -or
    -not $modelSource.Contains("alimentosInvalidos") -or
...
Write-Host "[290/580] Validando protecoes e auditoria..."
...
Write-Host "[291/580] Validando botao Salvar refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[292/580] Validando biblioteca visual..."
...
Write-Host "[293/580] Validando insercao visual rapida..."
...
Write-Host "[294/580] Validando responsividade da biblioteca..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[295/580] Validando SQL e PREPARAR 23..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[296/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de refeicoes + insercao rapida: OK."


Write-Host "[297/580] Validando schema da biblioteca de sessoes..."
...
Write-Host "[298/580] Validando listagem e filtros..."
...
Write-Host "[299/580] Validando salvar sessao como modelo..."
...
Write-Host "[300/580] Validando insercao de sessao no plano..."
...
Write-Host "[301/580] Validando catalogo e prescricao..."
if (-not $modelSource.Contains("exerciciosValidos") -or
    -not $modelSource.Contains("exerciciosInvalidos") -or
...
Write-Host "[302/580] Validando protecoes e auditoria..."
if (-not $modelSource.Contains('plano.Status == "Concluido"') -or
    -not $modelSource.Contains('"INSERT_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("currentUser.OrganizationId")) {
...
Write-Host "[303/580] Validando botao Salvar sessao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[304/580] Validando biblioteca visual..."
...
Write-Host "[305/580] Validando insercao visual rapida..."
...
Write-Host "[306/580] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[307/580] Validando SQL e PREPARAR 24..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[308/580] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de sessoes + insercao rapida: OK."


Write-Host "[309/580] Validando endpoint longitudinal de habitos..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[310/580] Validando contrato da evolucao de habitos..."
...
Write-Host "[311/580] Validando series de sono, estresse, atividade e agua..."
...
Write-Host "[312/580] Validando limite e isolamento multi-tenant..."
...
Write-Host "[313/580] Validando comparacao com registro anterior..."
...
Write-Host "[314/580] Validando graficos de habitos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[315/580] Validando resumo atual dos habitos..."
...
Write-Host "[316/580] Validando integracao na aba Anamnese..."
...
Write-Host "[317/580] Validando integracao no Resumo..."
...
Write-Host "[318/580] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[319/580] Validando compatibilidade de banco e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
...
Write-Host "[320/580] Validando versao v0.3.28..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.28 / evolucao de habitos + graficos de anamnese: OK."


Write-Host "[321/580] Validando resposta runtime com metas por refeicao..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[322/580] Validando schema de metas por refeicao..."
...
Write-Host "[323/580] Validando contratos de metas por refeicao..."
$contractsSource = Get-Content .\src\HealthPlatform.Api\Contracts\PlanosAlimentares\PlanoAlimentarContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[324/580] Validando endpoint de meta individual..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[325/580] Validando distribuicao automatica..."
...
Write-Host "[326/580] Validando isolamento e integridade da distribuicao..."
...
Write-Host "[327/580] Validando progressao com metas por refeicao..."
...
Write-Host "[328/580] Validando templates de plano..."
$templateSource = Get-Content .\src\HealthPlatform.Api\Controllers\ModelosPlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[329/580] Validando biblioteca de refeicoes..."
...
Write-Host "[330/580] Validando construtor alimentar..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[331/580] Validando edicao e comparacao visual..."
...
Write-Host "[332/580] Validando modal de distribuicao..."
...
Write-Host "[333/580] Validando SQL e PREPARAR 25..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[334/580] Validando versao v0.3.29..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.29 / metas por refeicao + distribuicao planejada: OK."


Write-Host "[335/580] Validando schema das fases nutricionais..."
...
Write-Host "[336/580] Validando listagem das fases..."
...
Write-Host "[337/580] Validando criacao de fase..."
...
Write-Host "[338/580] Validando edicao e estados..."
...
Write-Host "[339/580] Validando vinculo seguro com plano alimentar..."
...
Write-Host "[340/580] Validando reordenacao do ciclo..."
...
Write-Host "[341/580] Validando exclusao protegida..."
...
Write-Host "[342/580] Validando isolamento e auditoria..."
...
Write-Host "[343/580] Validando interface das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[344/580] Validando formulario de fase..."
...
Write-Host "[345/580] Validando reordenacao visual..."
...
Write-Host "[346/580] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[347/580] Validando SQL e PREPARAR 26..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[348/580] Validando versao v0.3.30..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.30 / fases nutricionais + planejamento de ciclo: OK."


Write-Host "[349/580] Validando schema das fases de treino..."
...
Write-Host "[350/580] Validando listagem das fases..."
...
Write-Host "[351/580] Validando criacao de fase..."
...
Write-Host "[352/580] Validando edicao e estados..."
...
Write-Host "[353/580] Validando vinculo seguro com ficha..."
if (-not $phaseSource.Contains("PlanoValido") -or
    -not $phaseSource.Contains("x.PacienteId == pacienteId") -or
    -not $phaseSource.Contains("x.Paciente.OrganizacaoId == currentUser.OrganizationId")) {
...
Write-Host "[354/580] Validando reordenacao do ciclo..."
...
Write-Host "[355/580] Validando exclusao protegida..."
if (-not $phaseSource.Contains("HttpDelete") -or
    -not $phaseSource.Contains('fase.Status == "EmAndamento"') -or
    -not $phaseSource.Contains('"DELETE"')) {
...
Write-Host "[356/580] Validando isolamento e auditoria..."
if (-not $phaseSource.Contains("currentUser.OrganizationId") -or
    -not $phaseSource.Contains("AuditLogs") -or
...
Write-Host "[357/580] Validando interface do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[358/580] Validando formulario de fase..."
...
Write-Host "[359/580] Validando reordenacao visual..."
...
Write-Host "[360/580] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[361/580] Validando SQL e PREPARAR 27..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[362/580] Validando versao v0.3.31..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.31 / ciclos de treino + periodizacao: OK."


Write-Host "[363/580] Validando schema dos check-ins..."
...
Write-Host "[364/580] Validando endpoint profissional..."
...
Write-Host "[365/580] Validando criacao e edicao..."
...
Write-Host "[366/580] Validando limites dos indicadores..."
...
Write-Host "[367/580] Validando vinculo com fases..."
...
Write-Host "[368/580] Validando auto-vinculo do paciente..."
...
Write-Host "[369/580] Validando portal do paciente..."
...
Write-Host "[370/580] Validando isolamento multi-tenant..."
...
Write-Host "[371/580] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[372/580] Validando graficos de resposta..."
...
Write-Host "[373/580] Validando formulario profissional..."
...
Write-Host "[374/580] Validando check-in no portal..."
...
Write-Host "[375/580] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[376/580] Validando SQL e PREPARAR 28..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[377/580] Validando preservacao dos ciclos..."
...
Write-Host "[378/580] Validando versao v0.3.32..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.32 / check-ins de evolucao + adesao por fase: OK."


Write-Host "[379/580] Validando endpoint de analise por fase..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[380/580] Validando metricas agregadas..."
...
Write-Host "[381/580] Validando variacao de peso por fase..."
...
Write-Host "[382/580] Validando destaques automaticos..."
...
Write-Host "[383/580] Validando isolamento multi-tenant..."
...
Write-Host "[384/580] Validando cards comparativos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[385/580] Validando destaques visuais..."
...
Write-Host "[386/580] Validando integracao com nutricao..."
...
Write-Host "[387/580] Validando integracao com treino..."
...
Write-Host "[388/580] Validando resumo consolidado..."
...
Write-Host "[389/580] Validando responsividade e compatibilidade de banco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[390/580] Validando versao v0.3.33..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.33 / analise de fases + comparativo de resposta: OK."

Write-Host "[391/580] Validando metas das fases..."
...
Write-Host "[392/580] Validando mapeamento EF..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[393/580] Validando CRUD das fases..."
...
Write-Host "[394/580] Validando limites dos criterios..."
...
Write-Host "[395/580] Validando endpoint runtime de prontidao..."
...
Write-Host "[396/580] Validando motor de criterios objetivos..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[397/580] Validando revisao profissional..."
...
Write-Host "[398/580] Validando formularios das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[399/580] Validando metas nos cards..."
...
Write-Host "[400/580] Validando painel de prontidao..."
...
Write-Host "[405/580] Validando SQL e PREPARAR 29..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[406/580] Validando versao v0.3.34..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.34 / metas de fase + criterios de transicao: OK."


Write-Host "[407/580] Validando entidade de revisao..."
...
Write-Host "[408/580] Validando mapeamento das revisoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[409/580] Validando historico runtime..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[410/580] Validando revisao nutricional..."
...
Write-Host "[411/580] Validando revisao de treino..."
...
Write-Host "[412/580] Validando decisoes e fase ativa..."
...
Write-Host "[413/580] Validando override consciente..."
...
Write-Host "[414/580] Validando transicao para proxima fase..."
...
Write-Host "[415/580] Validando transacao e auditoria..."
...
Write-Host "[416/580] Validando modal de revisao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[417/580] Validando historico visual..."
...
Write-Host "[418/580] Validando integracao com painel de transicao..."
...
Write-Host "[419/580] Validando SQL e PREPARAR 30..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[420/580] Validando versao v0.3.35..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.35 / revisao de fase + transicao assistida: OK."


Write-Host "[421/580] Validando controller de volume..."
...
Write-Host "[422/580] Validando isolamento multi-tenant..."
...
Write-Host "[423/580] Validando volume planejado por grupo..."
...
Write-Host "[424/580] Validando frequencia semanal..."
...
Write-Host "[425/580] Validando fallback de frequencia..."
...
Write-Host "[426/580] Validando execucoes reais..."
...
Write-Host "[427/580] Validando ausencia de tonelagem inventada..."
...
Write-Host "[428/580] Validando runtime da analise..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
    try {
...
Write-Host "[429/580] Validando painel de volume..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[430/580] Validando resumo analitico..."
...
Write-Host "[431/580] Validando volume por sessao..."
...
Write-Host "[432/580] Validando integracao e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[433/580] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
...
Write-Host "[434/580] Validando versao v0.3.36..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.36 / volume de treino + distribuicao muscular: OK."


Write-Host "[435/580] Validando endpoints de progressao por exercicio..."
...
Write-Host "[436/580] Validando seguranca e tenant..."
...
Write-Host "[437/580] Validando separacao por unidade..."
...
Write-Host "[438/580] Validando metricas de carga..."
...
Write-Host "[439/580] Validando recordes sucessivos..."
...
Write-Host "[440/580] Validando tendencia de carga..."
...
Write-Host "[441/580] Validando protecao contra estimativas artificiais..."
...
Write-Host "[442/580] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[443/580] Validando payload de pontos..."
...
Write-Host "[444/580] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[445/580] Validando graficos e recordes..."
...
Write-Host "[446/580] Validando portal do paciente..."
...
Write-Host "[447/580] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[448/580] Validando versao v0.3.37..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.37 / progressao por exercicio + recordes de carga: OK."


Write-Host "[449/580] Validando endpoints de sinais de progressao..."
...
Write-Host "[450/580] Validando tenant e PatientOnly..."
...
Write-Host "[451/580] Validando estados da analise..."
...
Write-Host "[452/580] Validando regra de estagnacao..."
...
Write-Host "[453/580] Validando sinal de carga/RPE..."
...
Write-Host "[454/580] Validando progressao recente..."
...
Write-Host "[455/580] Validando semantica nao diagnostica..."
...
Write-Host "[456/580] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[457/580] Validando payload analitico..."
...
Write-Host "[458/580] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[459/580] Validando portal do paciente..."
...
Write-Host "[460/580] Validando cards e graficos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[461/580] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[462/580] Validando versao v0.3.38..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.38 / estagnacao + fadiga + sinais de progressao: OK."


Write-Host "[463/580] Validando identidade MVP Preview..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.5.8") -or
...
Write-Host "[464/580] Validando aviso de demonstracao..."
...
Write-Host "[465/580] Validando roteiro da demo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[466/580] Validando checklist de avaliacao..."
...
Write-Host "[467/580] Validando modelo de feedback..."
...
Write-Host "[468/580] Validando dashboard de apresentacao..."
...
Write-Host "[469/580] Validando atalho Escape..."
...
Write-Host "[470/580] Validando feedback de conectividade..."
...
Write-Host "[471/580] Validando acabamento de foco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[472/580] Validando estados vazios..."
...
Write-Host "[473/580] Validando limpeza da navegacao de demo..."
...
Write-Host "[474/580] Validando responsividade do MVP..."
...
Write-Host "[475/580] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[32/32]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
...
Write-Host "[476/580] Validando versao v0.3.39..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.39 / MVP Preview + polimento de demonstracao: OK."


Write-Host "[477/580] Validando Dockerfile..."
...
Write-Host "[478/580] Validando bind dinamico de porta..."
...
Write-Host "[479/580] Validando Blueprint Render..."
...
Write-Host "[480/580] Validando secrets do Blueprint..."
...
Write-Host "[481/580] Validando conexao PostgreSQL do Render..."
...
Write-Host "[482/580] Validando bootstrap isolado do MVP..."
...
Write-Host "[483/580] Validando fluxo local preservado..."
...
Write-Host "[484/580] Validando healthcheck real..."
...
Write-Host "[485/580] Validando forwarded headers..."
...
Write-Host "[486/580] Validando POPULAR remoto..."
...
if (-not $remotePopular.Contains("[Parameter(Mandatory=`$true)][string]`$BaseUrl") -or
    -not $remotePopular.Contains("HealthPlatform v0.5.8 - POPULAR RENDER DEMO") -or
...
Write-Host "[487/580] Validando catalogos da demo remota..."
...
Write-Host "[488/580] Validando smoke test remoto..."
...
Write-Host "[489/580] Validando guia de deploy..."
...
Write-Host "[490/580] Validando ausencia de schema novo..."
...
Write-Host "[491/580] Validando arquivos de container..."
...
Write-Host "[492/580] Validando versao base do deploy..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.8 / Render Demo Deploy preservado: OK."


Write-Host "[493/580] Validando paleta RS..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[494/580] Validando monograma RS..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
...
Write-Host "[495/580] Validando linguagem editorial..."
...
Write-Host "[496/580] Validando tipografia condensada..."
...
Write-Host "[497/580] Validando login editorial..."
...
Write-Host "[498/580] Validando sidebar RS..."
...
Write-Host "[499/580] Validando experiencia iPad..."
...
Write-Host "[500/580] Validando drawer mobile..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[501/580] Validando safe areas iOS..."
...
Write-Host "[502/580] Validando modal bottom-sheet no iPhone..."
...
Write-Host "[503/580] Validando portal mobile..."
...
Write-Host "[504/580] Validando abas do prontuario no mobile..."
...
Write-Host "[505/580] Validando touch targets..."
...
Write-Host "[506/580] Validando cards analiticos no iPhone..."
...
Write-Host "[507/580] Validando deploy/demo preservados..."
$renderSource = Get-Content .\render.yaml -Encoding UTF8 -Raw
...
Write-Host "[508/580] Validando entidade de solicitacoes clinicas..."
...
Write-Host "[509/580] Validando mapeamento EF de solicitacoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[510/580] Validando endpoints profissionais de solicitacoes..."
...
Write-Host "[511/580] Validando portal do paciente para solicitacoes..."
...
Write-Host "[512/580] Validando isolamento multi-tenant..."
...
Write-Host "[513/580] Validando auditoria de solicitacoes..."
...
Write-Host "[514/580] Validando interface profissional..."
...
Write-Host "[515/580] Validando interface do paciente..."
...
Write-Host "[516/580] Validando upgrade SQL v0.5.1 preservado..."
...
Write-Host "[517/580] Validando PREPARAR 32/32..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[518/580] Validando versao v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.8 / Solicitacoes Clinicas + Connected Care: OK."

Write-Host "[519/580] Validando notificacoes de solicitacoes no backend..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[520/580] Validando prioridade por prazo para o paciente..."
...
Write-Host "[521/580] Validando notificacao de resposta para profissional..."
...
Write-Host "[522/580] Validando limpeza de notificacoes encerradas..."
...
Write-Host "[523/580] Validando navegacao do paciente pela notificacao..."
...
Write-Host "[524/580] Validando navegacao contextual do profissional..."
...
Write-Host "[525/580] Validando identidade visual de notificacao..."
...
Write-Host "[526/580] Validando versao funcional v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
if (-not $notificationSource.Contains('SolicitacaoClinica')) { throw "Feature v0.5.8 ausente." }
Write-Host "    v0.5.8 / Notificacoes Contextuais + Connected Care: OK."

Write-Host "[527/580] Validando endpoint da Central de Solicitacoes..."
$requestController = Get-Content .\src\HealthPlatform.Api\Controllers\SolicitacoesClinicasController.cs -Encoding UTF8 -Raw
...
Write-Host "[528/580] Validando resumo operacional de solicitacoes..."
...
Write-Host "[529/580] Validando filtros da Central de Solicitacoes..."
...
Write-Host "[530/580] Validando isolamento da fila profissional..."
...
Write-Host "[531/580] Validando navegacao profissional de solicitacoes..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[532/580] Validando indicadores e filtros visuais..."
...
Write-Host "[533/580] Validando acoes rapidas da Central de Solicitacoes..."
...
Write-Host "[534/580] Validando responsividade da Central de Solicitacoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[535/580] Validando versao funcional v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
if (-not $requestController.Contains('FilaProfissional')) { throw "Feature v0.5.8 ausente." }
Write-Host "    v0.5.8 / Central de Solicitacoes + Connected Care: OK."


Write-Host "[536/580] Validando solicitacoes no Hoje do paciente..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[537/580] Validando solicitacoes no progresso diario..."
...
Write-Host "[538/580] Validando card de acao necessaria..."
...
Write-Host "[539/580] Validando destaque de solicitacao vencida no portal..."
...
Write-Host "[540/580] Validando integracao da Central do Dia no backend..."
$centralSource = Get-Content .\src\HealthPlatform.Api\Controllers\CentralDiaController.cs -Encoding UTF8 -Raw
...
Write-Host "[541/580] Validando escopo profissional das solicitacoes do dia..."
...
Write-Host "[542/580] Validando card profissional de solicitacoes..."
...
Write-Host "[543/580] Validando dashboard com solicitacoes para revisao..."
...
Write-Host "[544/580] Validando responsividade das solicitacoes no Hoje..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[545/580] Validando versao funcional v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
if (-not $centralSource.Contains('SolicitacoesParaRevisao') -or -not $appJsSource.Contains('patient-today-requests')) { throw "Feature v0.5.8 ausente." }
Write-Host "    v0.5.8 / Solicitacoes no Hoje + Connected Care: OK."

Write-Host "[546/580] Validando contrato Desde a ultima consulta..."
$resumoSource = Get-Content .\src\HealthPlatform.Api\Controllers\ResumoClinicoController.cs -Encoding UTF8 -Raw
...
Write-Host "[547/580] Validando periodo baseado na ultima consulta..."
...
Write-Host "[548/580] Validando atividade do paciente no periodo..."
...
Write-Host "[549/580] Validando adesao media por check-ins..."
...
Write-Host "[550/580] Validando variacao de peso no periodo..."
...
Write-Host "[551/580] Validando solicitacoes no resumo longitudinal..."
...
Write-Host "[552/580] Validando card longitudinal no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[553/580] Validando versao funcional v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
if (-not $resumoSource.Contains('ResumoDesdeUltimaConsultaResponse') -or -not $appJsSource.Contains('data-clinical-period-summary')) { throw "Feature v0.5.8 ausente." }
Write-Host "    v0.5.8 / Desde a ultima consulta + Connected Care: OK."

Write-Host "[554/580] Validando endpoint da Jornada do paciente..."
...
Write-Host "[555/580] Validando escopo PatientOnly da Jornada..."
...
Write-Host "[556/580] Validando fontes da Jornada..."
...
Write-Host "[557/580] Validando navegacao Jornada no portal..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[558/580] Validando filtro temporal da Jornada..."
...
Write-Host "[559/580] Validando atalhos contextuais da Jornada..."
...
Write-Host "[560/580] Validando responsividade da Jornada..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[561/580] Validando versao funcional v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
if (-not $portalController.Contains('MinhaJornada') -or -not $appJsSource.Contains('loadPatientJourney')) { throw "Feature v0.5.8 ausente." }
Write-Host "    v0.5.8 / Jornada do Paciente + Connected Care: OK."


Write-Host "[562/580] Validando endpoint de monitoramento profissional..."
...
Write-Host "[563/580] Validando monitoramento no portal do paciente..."
...
Write-Host "[564/580] Validando metricas guiadas..."
...
Write-Host "[565/580] Validando resumo de 7 dias..."
...
Write-Host "[566/580] Validando registro guiado de pressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[567/580] Validando registros rapidos de sinais vitais..."
...
Write-Host "[568/580] Validando resumo profissional de monitoramento..."
...
Write-Host "[569/580] Validando responsividade do monitoramento..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[570/580] Validando versao funcional v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
if (-not $monitorSource.Contains('TiposMonitorados') -or -not $appJsSource.Contains('hpMonitoringCard')) { throw "Feature v0.5.8 ausente." }
Write-Host "    v0.5.8 / Monitoramento guiado + Connected Care: OK."

Write-Host "[571/580] Validando entidade de protocolo de acompanhamento..."
...
Write-Host "[572/580] Validando mapeamento EF dos protocolos..."
...
Write-Host "[573/580] Validando endpoints profissionais do protocolo..."
...
Write-Host "[574/580] Validando portal PatientOnly do protocolo..."
...
Write-Host "[575/580] Validando tipos e frequencias permitidos..."
...
Write-Host "[576/580] Validando gerenciamento visual do protocolo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[577/580] Validando personalizacao do Hoje do paciente..."
if (-not $appJsSource.Contains("api('/api/portal/me/protocolo-acompanhamento')") -or -not $appJsSource.Contains('protocolTypes') -or -not $appJsSource.Contains('allQuickTypes.filter')) { throw "Hoje nao respeita protocolo ativo." }
Write-Host "    Registros rapidos filtrados pelo protocolo: assets OK."

Write-Host "[578/580] Validando SQL e migration de upgrade v0.5.8..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
$sql058 = Get-Content .\scripts\sql\v0.5.8_protocolos_acompanhamento.sql -Encoding UTF8 -Raw
if (-not $setupSource.Contains('V058ProtocolosAcompanhamento') -or -not $setupSource.Contains('[32/32] Aplicando upgrade v0.5.8') -or -not $sql058.Contains('CREATE TABLE IF NOT EXISTS "ProtocolosAcompanhamento"')) { throw "Upgrade v0.5.8 incompleto." }
Write-Host "    Migration incremental + SQL idempotente + PREPARAR 32/32: OK."

Write-Host "[579/580] Validando responsividade dos protocolos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[580/580] Validando versao funcional v0.5.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.8") { throw "VERSION.txt inesperado." }
if (-not $protocolSource.Contains('MeuProtocolo') -or -not $appJsSource.Contains('openProtocolManager')) { throw "Feature v0.5.8 ausente." }
Write-Host "    v0.5.8 / Protocolos de Acompanhamento + Connected Care: OK."
