...
Write-Host "[1/553] Healthcheck..." -ForegroundColor Cyan
$health = Invoke-RestMethod -Uri "$base/api/health" -Method Get
if ($health.version -ne "0.5.5") { throw "Versao inesperada da API: $($health.version)" }
Write-Host "    API $($health.version) / banco $($health.database)" -ForegroundColor Green

Write-Host "[2/553] Login..." -ForegroundColor Cyan
...
Write-Host "[3/553] Listando pacientes..." -ForegroundColor Cyan
...
Write-Host "[4/553] Validando perguntas de anamnese..." -ForegroundColor Cyan
...
Write-Host "[5/553] Validando catalogo laboratorial..." -ForegroundColor Cyan
...
Write-Host "[6/553] Validando catalogo de alimentos..." -ForegroundColor Cyan
...
Write-Host "[7/553] Validando busca/paginacao..." -ForegroundColor Cyan
...
Write-Host "[8/553] Validando modulos do paciente..." -ForegroundColor Cyan
...
Write-Host "[9/553] Validando metas do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[10/553] Validando diario/resumo do dia..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[11/553] Validando portal/home do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[12/553] Validando agenda do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[13/553] Validando dashboard do profissional..." -ForegroundColor Cyan
try {
...
Write-Host "[14/553] Validando interface web..." -ForegroundColor Cyan
...
Write-Host "[15/553] Validando assets da interface..." -ForegroundColor Cyan
...
Write-Host "[16/553] Validando prontuario visual v0.2.2..." -ForegroundColor Cyan
...
Write-Host "[17/553] Validando acoes clinicas da interface..." -ForegroundColor Cyan
...
Write-Host "[18/553] Validando rotas usadas pelos formularios clinicos..." -ForegroundColor Cyan
...
Write-Host "[19/553] Validando cadastro visual de exames..." -ForegroundColor Cyan
...
Write-Host "[20/553] Validando construtor visual do plano alimentar..." -ForegroundColor Cyan
...
Write-Host "[21/553] Validando relatorios na interface..." -ForegroundColor Cyan
...
Write-Host "[22/553] Validando edicao visual do paciente..." -ForegroundColor Cyan
...
Write-Host "[23/553] Validando endpoint de relatorios do paciente..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[24/553] Validando edicao clinica visual..."
...
Write-Host "[25/553] Validando agenda operacional..."
...
Write-Host "[26/553] Validando endpoint de atualizacao de avaliacao..."
try {
...
Write-Host "[27/553] Validando tela de configuracoes..."
...
Write-Host "[28/553] Validando gerenciadores de catalogo na interface..."
...
Write-Host "[29/553] Validando resumo de configuracoes do consultorio..."
...
Write-Host "[30/553] Validando rotas dos catalogos..."
...
Write-Host "[31/553] Validando edicao de configuracoes..."
$cfg = Invoke-RestMethod -Uri "$base/api/configuracoes/resumo" -Headers $headers
...
Write-Host "[32/553] Validando assets de edicao/inativacao dos catalogos..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[35/553] Validando separacao de autorizacao profissional/paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[36/553] Validando endpoint de status de acesso do paciente..."
...
Write-Host "[37/553] Validando fluxo de convite/ativacao nos assets..."
...
Write-Host "[38/553] Validando autoatendimento do diario..."
...
Write-Host "[39/553] Validando autoatendimento das metas..."
...
Write-Host "[40/553] Validando tela dedicada do portal do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[41/553] Validando navegacao completa do portal..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[42/553] Validando endpoint proprio de plano alimentar..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[43/553] Validando historico proprio de metas e diario..."
...
Write-Host "[47/553] Validando schema/endpoint do catalogo de exercicios..."
...
Write-Host "[48/553] Validando endpoint de planos de treino do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[49/553] Validando construtor visual de treino..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[50/553] Validando videos e prescricao de exercicios..."
...
Write-Host "[51/553] Validando aba de treinos no prontuario..."
...
Write-Host "[52/553] Validando navegacao de treino do paciente..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[53/553] Validando endpoint proprio do treino do paciente..."
...
Write-Host "[54/553] Validando assets visuais do modulo de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[55/553] Validando schema de execucoes de treino..."
...
Write-Host "[56/553] Validando historico profissional de treinos..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[57/553] Validando registro visual de execucao pelo paciente..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[58/553] Validando series, repeticoes e carga realizadas..."
...
Write-Host "[59/553] Validando esforco percebido e duracao..."
...
Write-Host "[60/553] Validando historico do paciente..."
...
Write-Host "[61/553] Validando progressao de carga no prontuario..."
...
Write-Host "[62/553] Validando estilos do acompanhamento de treino..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[63/553] Validando motor de graficos SVG..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[64/553] Validando graficos corporais no prontuario..."
...
Write-Host "[65/553] Validando tendencias laboratoriais..."
...
Write-Host "[66/553] Validando progressao grafica de carga..."
...
Write-Host "[67/553] Validando evolucao visual no portal do paciente..."
...
Write-Host "[68/553] Validando graficos de exames no portal..."
...
Write-Host "[69/553] Validando responsividade dos graficos..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[70/553] Validando compatibilidade de schema na v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
...
Write-Host "[71/553] Validando endpoint de insights do dashboard..."
...
Write-Host "[72/553] Validando insights por paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[73/553] Validando regra de exame fora da referencia..."
...
Write-Host "[74/553] Validando regras de evolucao e retorno..."
...
Write-Host "[75/553] Validando regras de adesao..."
...
Write-Host "[76/553] Validando central de atencao visual..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[77/553] Validando insights no prontuario..."
...
Write-Host "[78/553] Validando estilos e compatibilidade do schema..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[79/553] Validando schema de pendencias clinicas..."
...
Write-Host "[80/553] Validando endpoint geral de pendencias..."
...
Write-Host "[81/553] Validando endpoint de pendencias do paciente..."
if ($pacientes.itens.Count -gt 0) {
    $pid = $pacientes.itens[0].id
...
Write-Host "[82/553] Validando acoes de ciclo de vida..."
...
Write-Host "[83/553] Validando criacao de retorno a partir da pendencia..."
...
Write-Host "[84/553] Validando transformar insight em pendencia..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[85/553] Validando tela de gerenciamento de pendencias..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
...
Write-Host "[86/553] Validando acoes visuais da pendencia..."
...
Write-Host "[87/553] Validando resumo de pendencias no dashboard..."
...
Write-Host "[88/553] Validando auditoria, estilos e upgrade v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[89/553] Validando schema de notificacoes internas..."
...
Write-Host "[90/553] Validando sincronizacao de notificacoes..."
...
Write-Host "[91/553] Validando listagem e contador nao lido..."
...
Write-Host "[92/553] Validando regras de agenda profissional..."
...
Write-Host "[93/553] Validando regras de pendencias..."
...
Write-Host "[94/553] Validando lembretes do paciente..."
...
Write-Host "[95/553] Validando leitura individual e em massa..."
...
Write-Host "[96/553] Validando sino e drawer na interface..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[97/553] Validando contador e atualizacao periodica..."
...
Write-Host "[98/553] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[99/553] Validando script de popular banco..."
...
Write-Host "[100/553] Validando idempotencia do popular..."
...
Write-Host "[101/553] Validando cobertura de modulos na base demo..."
...
Write-Host "[102/553] Validando endpoint de resumo de dados..."
...
Write-Host "[103/553] Validando que popular banco e opt-in..."
...
Write-Host "[104/553] Validando versao v0.3.27 e upgrade do schema..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
...
Write-Host "[105/553] Validando endpoint da carteira..."
...
Write-Host "[106/553] Validando priorizacao da carteira..."
...
Write-Host "[107/553] Validando sinais de exames/evolucao na carteira..."
...
Write-Host "[108/553] Validando atividade recente..."
...
Write-Host "[109/553] Validando tela Carteira..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[110/553] Validando filtros e ordenacao..."
...
Write-Host "[111/553] Validando atalho da carteira no dashboard..."
...
Write-Host "[112/553] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[113/553] Validando schema de follow-up..."
...
Write-Host "[114/553] Validando endpoint de follow-up..."
...
Write-Host "[115/553] Validando canais e proximo contato..."
...
Write-Host "[116/553] Validando auditoria do contato..."
...
Write-Host "[117/553] Validando follow-up na carteira..."
$carteiraSource = Get-Content .\src\HealthPlatform.Api\Controllers\CarteiraController.cs -Encoding UTF8 -Raw
...
Write-Host "[118/553] Validando acao rapida de contato..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[119/553] Validando acao rapida de retorno..."
...
Write-Host "[120/553] Validando acao rapida de pendencia..."
...
Write-Host "[121/553] Validando historico no prontuario..."
...
Write-Host "[122/553] Validando estilos, upgrade e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[123/553] Validando endpoint da fila de follow-up..."
...
Write-Host "[124/553] Validando faixas de vencimento..."
...
Write-Host "[125/553] Validando busca e filtros da fila..."
...
Write-Host "[126/553] Validando notificacao de follow-up..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[127/553] Validando tela de follow-up..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[128/553] Validando acoes rapidas na fila..."
...
Write-Host "[129/553] Validando resumo de follow-up no dashboard..."
...
Write-Host "[130/553] Validando estilos, popular e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
$popularSource = Get-Content .\POPULAR.ps1 -Encoding UTF8 -Raw
...
Write-Host "[131/553] Validando endpoint de gestao..."
...
Write-Host "[132/553] Validando indicadores de consultas..."
...
Write-Host "[133/553] Validando indicadores de acompanhamento..."
...
Write-Host "[134/553] Validando indicadores de engajamento..."
...
Write-Host "[135/553] Validando series gerenciais..."
...
Write-Host "[136/553] Validando tela Gestao..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[137/553] Validando graficos e resumo no dashboard..."
...
Write-Host "[138/553] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[139/553] Validando PREPARAR sem atualizacao automatica do dotnet..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[140/553] Validando endpoint CSV gerencial..."
...
Write-Host "[141/553] Validando conteudo longitudinal do CSV..."
...
Write-Host "[142/553] Validando relatorio HTML imprimivel..."
...
Write-Host "[143/553] Validando botoes de exportacao na Gestao..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[144/553] Validando download autenticado..."
...
Write-Host "[145/553] Validando relatorio autenticado em nova janela..."
...
Write-Host "[146/553] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[147/553] Validando perguntas de anamnese inativas..."
...
Write-Host "[148/553] Validando totais completos de insights..."
...
Write-Host "[149/553] Validando timezone das notificacoes..."
...
Write-Host "[150/553] Validando cleanup do polling no logout..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[151/553] Validando navegacao robusta no portal..."
...
Write-Host "[152/553] Validando smoke de notificacoes sem mutacao..."
...
Write-Host "[153/553] Validando copy historica de schema..."
$schemaCheckMarker = 'Write-Host "[153/553] Validando copy historica de schema..."'
...
Write-Host "[154/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / estabilizacao + qualidade: OK."


Write-Host "[155/553] Validando endpoint de busca global..."
...
Write-Host "[156/553] Validando fontes da busca..."
...
Write-Host "[157/553] Validando isolamento por organizacao..."
...
Write-Host "[158/553] Validando botao e atalho da busca..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[159/553] Validando modal e debounce..."
...
Write-Host "[160/553] Validando acoes dos resultados..."
...
Write-Host "[161/553] Validando estilos responsivos da busca..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[162/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / busca global + central de acoes: OK."


Write-Host "[163/553] Validando endpoint da Central do Dia..."
...
Write-Host "[164/553] Validando agenda do dia..."
...
Write-Host "[165/553] Validando follow-ups do dia..."
...
Write-Host "[166/553] Validando pendencias prioritarias..."
...
Write-Host "[167/553] Validando pacientes para revisao..."
...
Write-Host "[168/553] Validando tela Hoje..."
$index = Invoke-WebRequest -Uri "$base/" -UseBasicParsing
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[169/553] Validando acoes rapidas e dashboard..."
...
Write-Host "[170/553] Validando estilos e versao v0.3.27..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
...
Write-Host "[171/553] Validando schema de evolucoes clinicas SOAP..."
...
Write-Host "[172/553] Validando endpoint de evolucoes..."
...
Write-Host "[173/553] Validando consulta opcional e isolamento..."
...
Write-Host "[174/553] Validando auditoria da evolucao..."
...
Write-Host "[175/553] Validando evolucao na timeline..."
...
Write-Host "[176/553] Validando aba Evolucoes no prontuario..."
$appJs = Get-Utf8WebAsset -Uri "$base/app.js" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.js"
...
Write-Host "[177/553] Validando registro visual SOAP..."
...
Write-Host "[178/553] Validando edicao visual da evolucao..."
...
Write-Host "[179/553] Validando upgrade SQL SOAP historico e PREPARAR atual..."
...
Write-Host "[180/553] Validando estilos e versao v0.3.27..."
...
Write-Host "[181/553] Validando endpoint de resumo clinico..."
...
Write-Host "[182/553] Validando consulta e SOAP no resumo..."
...
Write-Host "[183/553] Validando corpo e anamnese..."
...
Write-Host "[184/553] Validando exames alterados..."
...
Write-Host "[185/553] Validando metas, treinos e pendencias..."
...
Write-Host "[186/553] Validando resumo no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[187/553] Validando atualizacao e responsividade..."
...
Write-Host "[188/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / resumo clinico consolidado: OK."


Write-Host "[189/553] Validando texto de handoff clinico..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[190/553] Validando conteudo SOAP no handoff..."
...
Write-Host "[191/553] Validando copia para clipboard..."
...
Write-Host "[192/553] Validando impressao do resumo..."
...
Write-Host "[193/553] Validando seguranca basica do HTML imprimivel..."
...
Write-Host "[194/553] Validando botoes no resumo clinico..."
...
Write-Host "[195/553] Validando responsividade das acoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[196/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / handoff clinico + impressao: OK."


Write-Host "[197/553] Validando endpoint de equipe..."
...
Write-Host "[198/553] Validando isolamento e administracao..."
...
Write-Host "[199/553] Validando criacao de acesso..."
...
Write-Host "[200/553] Validando sincronizacao de tipo e role..."
...
Write-Host "[201/553] Validando perfil profissional..."
...
Write-Host "[202/553] Validando protecao do proprio administrador..."
...
Write-Host "[203/553] Validando auditoria da equipe..."
...
Write-Host "[204/553] Validando tela Equipe..."
...
Write-Host "[205/553] Validando visibilidade admin e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[206/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe + gestao de profissionais: OK."


Write-Host "[207/553] Validando filtros da equipe..."
$teamSource = Get-Content .\src\HealthPlatform.Api\Controllers\EquipeController.cs -Encoding UTF8 -Raw
...
Write-Host "[208/553] Validando redefinicao de senha..."
...
Write-Host "[209/553] Validando escopo e protecoes do reset..."
...
Write-Host "[210/553] Validando auditoria segura de senha..."
...
Write-Host "[211/553] Validando filtros visuais..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[212/553] Validando acao visual de senha..."
...
Write-Host "[213/553] Validando responsividade da equipe v2..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[214/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / equipe v2 + seguranca de acesso: OK."


Write-Host "[215/553] Validando endpoint Minha Conta..."
...
Write-Host "[216/553] Validando alteracao da propria senha..."
...
Write-Host "[217/553] Validando protecoes da troca de senha..."
...
Write-Host "[218/553] Validando auditoria segura da conta..."
...
Write-Host "[219/553] Validando sincronizacao do nome profissional..."
...
Write-Host "[220/553] Validando painel Minha Conta..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[221/553] Validando responsividade da conta..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[222/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / Minha Conta + troca de senha: OK."


Write-Host "[223/553] Validando schema de progressao alimentar..."
...
Write-Host "[224/553] Validando simulador nutricional..."
...
Write-Host "[225/553] Validando duplicacao versionada..."
...
Write-Host "[226/553] Validando escala de porcoes..."
...
Write-Host "[227/553] Validando ajuste por calorias alvo..."
...
Write-Host "[228/553] Validando limites de ajuste..."
...
Write-Host "[229/553] Validando encerramento opcional do plano anterior..."
...
Write-Host "[230/553] Validando auditoria da progressao..."
...
Write-Host "[231/553] Validando interface de progressao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[232/553] Validando modos percentual e calorias..."
...
Write-Host "[233/553] Validando upgrade SQL e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[234/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao do plano alimentar: OK."


Write-Host "[235/553] Validando schema de progressao de treino..."
...
Write-Host "[236/553] Validando simulador de treino..."
...
Write-Host "[237/553] Validando duplicacao versionada do treino..."
...
Write-Host "[238/553] Validando progressao de carga..."
...
Write-Host "[239/553] Validando progressao de series e descanso..."
...
Write-Host "[240/553] Validando repeticoes estruturadas..."
...
Write-Host "[241/553] Validando limites de progressao..."
...
Write-Host "[242/553] Validando encerramento e auditoria..."
...
Write-Host "[243/553] Validando interface de progressao de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[244/553] Validando projecao e modal ampliado..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[245/553] Validando upgrade SQL e PREPARAR 19..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[246/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / progressao de treino + ciclo versionado: OK."


Write-Host "[247/553] Validando schema dos modelos alimentares..."
...
Write-Host "[248/553] Validando listagem e busca de modelos..."
...
Write-Host "[249/553] Validando salvar plano como modelo..."
...
Write-Host "[250/553] Validando criacao de plano a partir de modelo..."
...
Write-Host "[251/553] Validando alimentos do catalogo..."
...
Write-Host "[252/553] Validando isolamento e auditoria..."
...
Write-Host "[253/553] Validando interface de salvar modelo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[254/553] Validando seletor de modelos..."
...
Write-Host "[255/553] Validando criacao visual via modelo..."
...
Write-Host "[256/553] Validando responsividade dos modelos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[257/553] Validando upgrade SQL e PREPARAR 20..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[258/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de plano alimentar: OK."


Write-Host "[259/553] Validando schema dos modelos de treino..."
...
Write-Host "[260/553] Validando listagem e busca de modelos de treino..."
...
Write-Host "[261/553] Validando salvar treino como modelo..."
if (-not $modelSource.Contains("salvar-como-modelo") -or
...
Write-Host "[262/553] Validando criacao de treino a partir de modelo..."
if (-not $modelSource.Contains("criar-de-modelo") -or
...
Write-Host "[263/553] Validando catalogo de exercicios..."
...
Write-Host "[264/553] Validando prescricao completa do template..."
...
Write-Host "[265/553] Validando isolamento e auditoria dos modelos de treino..."
if (-not $modelSource.Contains("currentUser.OrganizationId") -or
    -not $modelSource.Contains('"CREATE_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("AuditLogs")) {
...
Write-Host "[266/553] Validando interface de salvar modelo de treino..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[267/553] Validando seletor e busca de modelos..."
...
Write-Host "[268/553] Validando criacao visual via modelo..."
...
Write-Host "[269/553] Validando upgrade SQL, responsividade e PREPARAR 21..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[270/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / templates de treino + criacao rapida: OK."


Write-Host "[271/553] Validando schema das metas nutricionais..."
$planEntity = Get-Content .\src\HealthPlatform.Domain\Entities\PlanoAlimentar.cs -Encoding UTF8 -Raw
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[272/553] Validando contratos nutricionais..."
...
Write-Host "[273/553] Validando endpoint de metas..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[274/553] Validando analise nutricional..."
...
Write-Host "[275/553] Validando metas na criacao e edicao..."
...
Write-Host "[276/553] Validando metas na progressao alimentar..."
...
Write-Host "[277/553] Validando metas nos templates alimentares..."
...
Write-Host "[278/553] Validando construtor com metas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[279/553] Validando Meta x Prescrito no prontuario..."
...
Write-Host "[280/553] Validando distribuicao por refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[281/553] Validando modal de metas..."
...
Write-Host "[282/553] Validando responsividade nutricional..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[283/553] Validando SQL e PREPARAR 22..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[284/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / metas nutricionais + distribuicao: OK."


Write-Host "[285/553] Validando schema da biblioteca de refeicoes..."
...
Write-Host "[286/553] Validando listagem e filtros da biblioteca..."
...
Write-Host "[287/553] Validando salvar refeicao como modelo..."
...
Write-Host "[288/553] Validando insercao de refeicao no plano..."
...
Write-Host "[289/553] Validando catalogo e substituicoes..."
if (-not $modelSource.Contains("alimentosValidos") -or
    -not $modelSource.Contains("alimentosInvalidos") -or
...
Write-Host "[290/553] Validando protecoes e auditoria..."
...
Write-Host "[291/553] Validando botao Salvar refeicao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[292/553] Validando biblioteca visual..."
...
Write-Host "[293/553] Validando insercao visual rapida..."
...
Write-Host "[294/553] Validando responsividade da biblioteca..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[295/553] Validando SQL e PREPARAR 23..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[296/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de refeicoes + insercao rapida: OK."


Write-Host "[297/553] Validando schema da biblioteca de sessoes..."
...
Write-Host "[298/553] Validando listagem e filtros..."
...
Write-Host "[299/553] Validando salvar sessao como modelo..."
...
Write-Host "[300/553] Validando insercao de sessao no plano..."
...
Write-Host "[301/553] Validando catalogo e prescricao..."
if (-not $modelSource.Contains("exerciciosValidos") -or
    -not $modelSource.Contains("exerciciosInvalidos") -or
...
Write-Host "[302/553] Validando protecoes e auditoria..."
if (-not $modelSource.Contains('plano.Status == "Concluido"') -or
    -not $modelSource.Contains('"INSERT_FROM_TEMPLATE"') -or
    -not $modelSource.Contains("currentUser.OrganizationId")) {
...
Write-Host "[303/553] Validando botao Salvar sessao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[304/553] Validando biblioteca visual..."
...
Write-Host "[305/553] Validando insercao visual rapida..."
...
Write-Host "[306/553] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[307/553] Validando SQL e PREPARAR 24..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[308/553] Validando versao v0.3.27..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.27 / biblioteca de sessoes + insercao rapida: OK."


Write-Host "[309/553] Validando endpoint longitudinal de habitos..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[310/553] Validando contrato da evolucao de habitos..."
...
Write-Host "[311/553] Validando series de sono, estresse, atividade e agua..."
...
Write-Host "[312/553] Validando limite e isolamento multi-tenant..."
...
Write-Host "[313/553] Validando comparacao com registro anterior..."
...
Write-Host "[314/553] Validando graficos de habitos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[315/553] Validando resumo atual dos habitos..."
...
Write-Host "[316/553] Validando integracao na aba Anamnese..."
...
Write-Host "[317/553] Validando integracao no Resumo..."
...
Write-Host "[318/553] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[319/553] Validando compatibilidade de banco e PREPARAR..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "[320/553] Validando versao v0.3.28..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.28 / evolucao de habitos + graficos de anamnese: OK."


Write-Host "[321/553] Validando resposta runtime com metas por refeicao..." -ForegroundColor Cyan
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[322/553] Validando schema de metas por refeicao..."
...
Write-Host "[323/553] Validando contratos de metas por refeicao..."
$contractsSource = Get-Content .\src\HealthPlatform.Api\Contracts\PlanosAlimentares\PlanoAlimentarContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[324/553] Validando endpoint de meta individual..."
$planSource = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[325/553] Validando distribuicao automatica..."
...
Write-Host "[326/553] Validando isolamento e integridade da distribuicao..."
...
Write-Host "[327/553] Validando progressao com metas por refeicao..."
...
Write-Host "[328/553] Validando templates de plano..."
$templateSource = Get-Content .\src\HealthPlatform.Api\Controllers\ModelosPlanosAlimentaresController.cs -Encoding UTF8 -Raw
...
Write-Host "[329/553] Validando biblioteca de refeicoes..."
...
Write-Host "[330/553] Validando construtor alimentar..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[331/553] Validando edicao e comparacao visual..."
...
Write-Host "[332/553] Validando modal de distribuicao..."
...
Write-Host "[333/553] Validando SQL e PREPARAR 25..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[334/553] Validando versao v0.3.29..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.29 / metas por refeicao + distribuicao planejada: OK."


Write-Host "[335/553] Validando schema das fases nutricionais..."
...
Write-Host "[336/553] Validando listagem das fases..."
...
Write-Host "[337/553] Validando criacao de fase..."
...
Write-Host "[338/553] Validando edicao e estados..."
...
Write-Host "[339/553] Validando vinculo seguro com plano alimentar..."
...
Write-Host "[340/553] Validando reordenacao do ciclo..."
...
Write-Host "[341/553] Validando exclusao protegida..."
...
Write-Host "[342/553] Validando isolamento e auditoria..."
...
Write-Host "[343/553] Validando interface das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[344/553] Validando formulario de fase..."
...
Write-Host "[345/553] Validando reordenacao visual..."
...
Write-Host "[346/553] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[347/553] Validando SQL e PREPARAR 26..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[348/553] Validando versao v0.3.30..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.30 / fases nutricionais + planejamento de ciclo: OK."


Write-Host "[349/553] Validando schema das fases de treino..."
...
Write-Host "[350/553] Validando listagem das fases..."
...
Write-Host "[351/553] Validando criacao de fase..."
...
Write-Host "[352/553] Validando edicao e estados..."
...
Write-Host "[353/553] Validando vinculo seguro com ficha..."
if (-not $phaseSource.Contains("PlanoValido") -or
    -not $phaseSource.Contains("x.PacienteId == pacienteId") -or
    -not $phaseSource.Contains("x.Paciente.OrganizacaoId == currentUser.OrganizationId")) {
...
Write-Host "[354/553] Validando reordenacao do ciclo..."
...
Write-Host "[355/553] Validando exclusao protegida..."
if (-not $phaseSource.Contains("HttpDelete") -or
    -not $phaseSource.Contains('fase.Status == "EmAndamento"') -or
    -not $phaseSource.Contains('"DELETE"')) {
...
Write-Host "[356/553] Validando isolamento e auditoria..."
if (-not $phaseSource.Contains("currentUser.OrganizationId") -or
    -not $phaseSource.Contains("AuditLogs") -or
...
Write-Host "[357/553] Validando interface do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[358/553] Validando formulario de fase..."
...
Write-Host "[359/553] Validando reordenacao visual..."
...
Write-Host "[360/553] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[361/553] Validando SQL e PREPARAR 27..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[362/553] Validando versao v0.3.31..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.31 / ciclos de treino + periodizacao: OK."


Write-Host "[363/553] Validando schema dos check-ins..."
...
Write-Host "[364/553] Validando endpoint profissional..."
...
Write-Host "[365/553] Validando criacao e edicao..."
...
Write-Host "[366/553] Validando limites dos indicadores..."
...
Write-Host "[367/553] Validando vinculo com fases..."
...
Write-Host "[368/553] Validando auto-vinculo do paciente..."
...
Write-Host "[369/553] Validando portal do paciente..."
...
Write-Host "[370/553] Validando isolamento multi-tenant..."
...
Write-Host "[371/553] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[372/553] Validando graficos de resposta..."
...
Write-Host "[373/553] Validando formulario profissional..."
...
Write-Host "[374/553] Validando check-in no portal..."
...
Write-Host "[375/553] Validando responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[376/553] Validando SQL e PREPARAR 28..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[377/553] Validando preservacao dos ciclos..."
...
Write-Host "[378/553] Validando versao v0.3.32..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.32 / check-ins de evolucao + adesao por fase: OK."


Write-Host "[379/553] Validando endpoint de analise por fase..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[380/553] Validando metricas agregadas..."
...
Write-Host "[381/553] Validando variacao de peso por fase..."
...
Write-Host "[382/553] Validando destaques automaticos..."
...
Write-Host "[383/553] Validando isolamento multi-tenant..."
...
Write-Host "[384/553] Validando cards comparativos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[385/553] Validando destaques visuais..."
...
Write-Host "[386/553] Validando integracao com nutricao..."
...
Write-Host "[387/553] Validando integracao com treino..."
...
Write-Host "[388/553] Validando resumo consolidado..."
...
Write-Host "[389/553] Validando responsividade e compatibilidade de banco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[390/553] Validando versao v0.3.33..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.33 / analise de fases + comparativo de resposta: OK."

Write-Host "[391/553] Validando metas das fases..."
...
Write-Host "[392/553] Validando mapeamento EF..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[393/553] Validando CRUD das fases..."
...
Write-Host "[394/553] Validando limites dos criterios..."
...
Write-Host "[395/553] Validando endpoint runtime de prontidao..."
...
Write-Host "[396/553] Validando motor de criterios objetivos..."
$checkinSource = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
...
Write-Host "[397/553] Validando revisao profissional..."
...
Write-Host "[398/553] Validando formularios das fases..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[399/553] Validando metas nos cards..."
...
Write-Host "[400/553] Validando painel de prontidao..."
...
Write-Host "[405/553] Validando SQL e PREPARAR 29..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[406/553] Validando versao v0.3.34..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.34 / metas de fase + criterios de transicao: OK."


Write-Host "[407/553] Validando entidade de revisao..."
...
Write-Host "[408/553] Validando mapeamento das revisoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[409/553] Validando historico runtime..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[410/553] Validando revisao nutricional..."
...
Write-Host "[411/553] Validando revisao de treino..."
...
Write-Host "[412/553] Validando decisoes e fase ativa..."
...
Write-Host "[413/553] Validando override consciente..."
...
Write-Host "[414/553] Validando transicao para proxima fase..."
...
Write-Host "[415/553] Validando transacao e auditoria..."
...
Write-Host "[416/553] Validando modal de revisao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[417/553] Validando historico visual..."
...
Write-Host "[418/553] Validando integracao com painel de transicao..."
...
Write-Host "[419/553] Validando SQL e PREPARAR 30..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[420/553] Validando versao v0.3.35..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.35 / revisao de fase + transicao assistida: OK."


Write-Host "[421/553] Validando controller de volume..."
...
Write-Host "[422/553] Validando isolamento multi-tenant..."
...
Write-Host "[423/553] Validando volume planejado por grupo..."
...
Write-Host "[424/553] Validando frequencia semanal..."
...
Write-Host "[425/553] Validando fallback de frequencia..."
...
Write-Host "[426/553] Validando execucoes reais..."
...
Write-Host "[427/553] Validando ausencia de tonelagem inventada..."
...
Write-Host "[428/553] Validando runtime da analise..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
    try {
...
Write-Host "[429/553] Validando painel de volume..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[430/553] Validando resumo analitico..."
...
Write-Host "[431/553] Validando volume por sessao..."
...
Write-Host "[432/553] Validando integracao e responsividade..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[433/553] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
...
Write-Host "[434/553] Validando versao v0.3.36..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.36 / volume de treino + distribuicao muscular: OK."


Write-Host "[435/553] Validando endpoints de progressao por exercicio..."
...
Write-Host "[436/553] Validando seguranca e tenant..."
...
Write-Host "[437/553] Validando separacao por unidade..."
...
Write-Host "[438/553] Validando metricas de carga..."
...
Write-Host "[439/553] Validando recordes sucessivos..."
...
Write-Host "[440/553] Validando tendencia de carga..."
...
Write-Host "[441/553] Validando protecao contra estimativas artificiais..."
...
Write-Host "[442/553] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[443/553] Validando payload de pontos..."
...
Write-Host "[444/553] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[445/553] Validando graficos e recordes..."
...
Write-Host "[446/553] Validando portal do paciente..."
...
Write-Host "[447/553] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[448/553] Validando versao v0.3.37..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.37 / progressao por exercicio + recordes de carga: OK."


Write-Host "[449/553] Validando endpoints de sinais de progressao..."
...
Write-Host "[450/553] Validando tenant e PatientOnly..."
...
Write-Host "[451/553] Validando estados da analise..."
...
Write-Host "[452/553] Validando regra de estagnacao..."
...
Write-Host "[453/553] Validando sinal de carga/RPE..."
...
Write-Host "[454/553] Validando progressao recente..."
...
Write-Host "[455/553] Validando semantica nao diagnostica..."
...
Write-Host "[456/553] Validando runtime profissional..."
if ($lista.total -gt 0 -and $lista.itens.Count -gt 0) {
    $pacienteSmoke = $lista.itens | Select-Object -First 1
...
Write-Host "[457/553] Validando payload analitico..."
...
Write-Host "[458/553] Validando painel profissional..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[459/553] Validando portal do paciente..."
...
Write-Host "[460/553] Validando cards e graficos..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[461/553] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
    throw "PREPARAR historico inesperado."
...
Write-Host "[462/553] Validando versao v0.3.38..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.38 / estagnacao + fadiga + sinais de progressao: OK."


Write-Host "[463/553] Validando identidade MVP Preview..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.5.5") -or
...
Write-Host "[464/553] Validando aviso de demonstracao..."
...
Write-Host "[465/553] Validando roteiro da demo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[466/553] Validando checklist de avaliacao..."
...
Write-Host "[467/553] Validando modelo de feedback..."
...
Write-Host "[468/553] Validando dashboard de apresentacao..."
...
Write-Host "[469/553] Validando atalho Escape..."
...
Write-Host "[470/553] Validando feedback de conectividade..."
...
Write-Host "[471/553] Validando acabamento de foco..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[472/553] Validando estados vazios..."
...
Write-Host "[473/553] Validando limpeza da navegacao de demo..."
...
Write-Host "[474/553] Validando responsividade do MVP..."
...
Write-Host "[475/553] Validando compatibilidade de banco..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains("[31/31]") -or
    -not $setupSource.Contains("v0.3.35_revisoes_transicoes_fases.sql")) {
...
Write-Host "[476/553] Validando versao v0.3.39..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.3.39 / MVP Preview + polimento de demonstracao: OK."


Write-Host "[477/553] Validando Dockerfile..."
...
Write-Host "[478/553] Validando bind dinamico de porta..."
...
Write-Host "[479/553] Validando Blueprint Render..."
...
Write-Host "[480/553] Validando secrets do Blueprint..."
...
Write-Host "[481/553] Validando conexao PostgreSQL do Render..."
...
Write-Host "[482/553] Validando bootstrap isolado do MVP..."
...
Write-Host "[483/553] Validando fluxo local preservado..."
...
Write-Host "[484/553] Validando healthcheck real..."
...
Write-Host "[485/553] Validando forwarded headers..."
...
Write-Host "[486/553] Validando POPULAR remoto..."
...
if (-not $remotePopular.Contains("[Parameter(Mandatory=`$true)][string]`$BaseUrl") -or
    -not $remotePopular.Contains("HealthPlatform v0.5.5 - POPULAR RENDER DEMO") -or
...
Write-Host "[487/553] Validando catalogos da demo remota..."
...
Write-Host "[488/553] Validando smoke test remoto..."
...
Write-Host "[489/553] Validando guia de deploy..."
...
Write-Host "[490/553] Validando ausencia de schema novo..."
...
Write-Host "[491/553] Validando arquivos de container..."
...
Write-Host "[492/553] Validando versao base do deploy..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.5 / Render Demo Deploy preservado: OK."


Write-Host "[493/553] Validando paleta RS..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[494/553] Validando monograma RS..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
...
Write-Host "[495/553] Validando linguagem editorial..."
...
Write-Host "[496/553] Validando tipografia condensada..."
...
Write-Host "[497/553] Validando login editorial..."
...
Write-Host "[498/553] Validando sidebar RS..."
...
Write-Host "[499/553] Validando experiencia iPad..."
...
Write-Host "[500/553] Validando drawer mobile..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[501/553] Validando safe areas iOS..."
...
Write-Host "[502/553] Validando modal bottom-sheet no iPhone..."
...
Write-Host "[503/553] Validando portal mobile..."
...
Write-Host "[504/553] Validando abas do prontuario no mobile..."
...
Write-Host "[505/553] Validando touch targets..."
...
Write-Host "[506/553] Validando cards analiticos no iPhone..."
...
Write-Host "[507/553] Validando deploy/demo preservados..."
$renderSource = Get-Content .\render.yaml -Encoding UTF8 -Raw
...
Write-Host "[508/553] Validando entidade de solicitacoes clinicas..."
...
Write-Host "[509/553] Validando mapeamento EF de solicitacoes..."
$dbSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[510/553] Validando endpoints profissionais de solicitacoes..."
...
Write-Host "[511/553] Validando portal do paciente para solicitacoes..."
...
Write-Host "[512/553] Validando isolamento multi-tenant..."
...
Write-Host "[513/553] Validando auditoria de solicitacoes..."
...
Write-Host "[514/553] Validando interface profissional..."
...
Write-Host "[515/553] Validando interface do paciente..."
...
Write-Host "[516/553] Validando upgrade SQL v0.5.1 preservado..."
...
Write-Host "[517/553] Validando PREPARAR 31/31..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[518/553] Validando versao v0.5.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
Write-Host "    v0.5.5 / Solicitacoes Clinicas + Connected Care: OK."

Write-Host "[519/553] Validando notificacoes de solicitacoes no backend..."
$notificationSource = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
...
Write-Host "[520/553] Validando prioridade por prazo para o paciente..."
...
Write-Host "[521/553] Validando notificacao de resposta para profissional..."
...
Write-Host "[522/553] Validando limpeza de notificacoes encerradas..."
...
Write-Host "[523/553] Validando navegacao do paciente pela notificacao..."
...
Write-Host "[524/553] Validando navegacao contextual do profissional..."
...
Write-Host "[525/553] Validando identidade visual de notificacao..."
...
Write-Host "[526/553] Validando versao funcional v0.5.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
if (-not $notificationSource.Contains('SolicitacaoClinica')) { throw "Feature v0.5.5 ausente." }
Write-Host "    v0.5.5 / Notificacoes Contextuais + Connected Care: OK."

Write-Host "[527/553] Validando endpoint da Central de Solicitacoes..."
$requestController = Get-Content .\src\HealthPlatform.Api\Controllers\SolicitacoesClinicasController.cs -Encoding UTF8 -Raw
...
Write-Host "[528/553] Validando resumo operacional de solicitacoes..."
...
Write-Host "[529/553] Validando filtros da Central de Solicitacoes..."
...
Write-Host "[530/553] Validando isolamento da fila profissional..."
...
Write-Host "[531/553] Validando navegacao profissional de solicitacoes..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[532/553] Validando indicadores e filtros visuais..."
...
Write-Host "[533/553] Validando acoes rapidas da Central de Solicitacoes..."
...
Write-Host "[534/553] Validando responsividade da Central de Solicitacoes..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[535/553] Validando versao funcional v0.5.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
if (-not $requestController.Contains('FilaProfissional')) { throw "Feature v0.5.5 ausente." }
Write-Host "    v0.5.5 / Central de Solicitacoes + Connected Care: OK."


Write-Host "[536/553] Validando solicitacoes no Hoje do paciente..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[537/553] Validando solicitacoes no progresso diario..."
...
Write-Host "[538/553] Validando card de acao necessaria..."
...
Write-Host "[539/553] Validando destaque de solicitacao vencida no portal..."
...
Write-Host "[540/553] Validando integracao da Central do Dia no backend..."
$centralSource = Get-Content .\src\HealthPlatform.Api\Controllers\CentralDiaController.cs -Encoding UTF8 -Raw
...
Write-Host "[541/553] Validando escopo profissional das solicitacoes do dia..."
...
Write-Host "[542/553] Validando card profissional de solicitacoes..."
...
Write-Host "[543/553] Validando dashboard com solicitacoes para revisao..."
...
Write-Host "[544/553] Validando responsividade das solicitacoes no Hoje..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[545/553] Validando versao funcional v0.5.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
if (-not $centralSource.Contains('SolicitacoesParaRevisao') -or -not $appJsSource.Contains('patient-today-requests')) { throw "Feature v0.5.5 ausente." }
Write-Host "    v0.5.5 / Solicitacoes no Hoje + Connected Care: OK."

Write-Host "[546/553] Validando contrato Desde a ultima consulta..."
$resumoSource = Get-Content .\src\HealthPlatform.Api\Controllers\ResumoClinicoController.cs -Encoding UTF8 -Raw
...
Write-Host "[547/553] Validando periodo baseado na ultima consulta..."
...
Write-Host "[548/553] Validando atividade do paciente no periodo..."
...
Write-Host "[549/553] Validando adesao media por check-ins..."
...
Write-Host "[550/553] Validando variacao de peso no periodo..."
...
Write-Host "[551/553] Validando solicitacoes no resumo longitudinal..."
...
Write-Host "[552/553] Validando card longitudinal no prontuario..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[553/553] Validando versao funcional v0.5.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.5.5") { throw "VERSION.txt inesperado." }
if (-not $resumoSource.Contains('ResumoDesdeUltimaConsultaResponse') -or -not $appJsSource.Contains('data-clinical-period-summary')) { throw "Feature v0.5.5 ausente." }
Write-Host "    v0.5.5 / Desde a ultima consulta + Connected Care: OK."
