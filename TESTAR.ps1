...
if ($health.version -ne "0.8.3") { throw "Versao inesperada da API: $($health.version)" }
...
if ($version.Trim() -ne "0.8.3") { throw "VERSION.txt inesperado." }
...
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.8.3") -or
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.6.1 inconsistente." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.6.2 inconsistente." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao atual inconsistente ao validar v0.6.3." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Compatibilidade v0.6.4 inconsistente na versao atual." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.6.6 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.8.3"')) { throw "Health endpoint nao anuncia v0.6.6." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.7.1 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.8.3"')) { throw "Health endpoint nao anuncia v0.7.1." }
...
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.7.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.3"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.1." }
...
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.7.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.3"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.7." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.7.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.3"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.9." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.8.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.3"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.0." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.8.2 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.3"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.2." }
...
Write-Host "[783/784] Validando ausencia de migration v0.8.3..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[37/37] Aplicando upgrade v0.6.4')) { throw "Schema 37/37 foi alterado sem necessidade." }
if (Test-Path .\scripts\sql\v0.8.3_relatorio_ciclo.sql) { throw "v0.8.3 nao deveria exigir migration." }
Write-Host "    Relatorio derivado dos dados existentes; schema permanece 37/37: OK."

Write-Host "[784/784] Validando versao funcional v0.8.3..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.3" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.3';")) { throw "Versao v0.8.3 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.3"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.3." }
Write-Host "    v0.8.3 / Fechamento de Ciclo & Relatorio de Evolucao: OK."
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green
