...
if ($health.version -ne "0.6.0") { throw "Versao inesperada da API: $($health.version)" }
...
if ($version.Trim() -ne "0.6.0") { throw "VERSION.txt inesperado." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.0") { throw "VERSION.txt inesperado." }
if ($setupSource -notmatch "\[33/33\]") { throw "PREPARAR atual deveria possuir 33 etapas." }
Write-Host "    v0.3.27 preservada / PREPARAR atual 33/33 / upgrade SOAP: OK."
...
if (-not $setupSource.Contains("[33/33]") -or
...
Write-Host "    SOAP v0.3.15 preservado / PREPARAR atual 33/33: OK."
...
Write-Host "    Sem schema novo / PREPARAR atual 33/33: OK."
...
if (-not $cssSource.Contains("phase-highlight-grid") -or
    ...
    -not $setupSource.Contains("[33/33]")) {
...
if (-not $setupSource.Contains("[33/33]") -or -not $setupSource.Contains("v0.3.34_criterios_transicao_fases.sql") -or -not $sqlSource.Contains('"MetaPesoKg"') -or -not $sqlSource.Contains('"CriterioTransicao"') -or -not $setupSource.Contains("v0.3.32_checkins_acompanhamento.sql")) { throw "Upgrade dos criterios incompleto." }
...
Write-Host "    SQL idempotente + PREPARAR 33/33 + v0.3.34 preservada."
...
Write-Host "    Sem schema novo / PREPARAR permanece 33/33."
...
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.6.0") -or
...
if (-not $programSource.Contains("app.Environment.IsDevelopment()") -or
    ...
    -not $setupSource.Contains("[33/33]") -or
...
Write-Host "    PREPARAR continua 33/33 / sem SQL v0.3.40."
...
Write-Host "[517/600] Validando PREPARAR 33/33..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
if (-not $setupSource.Contains('V058ProtocolosAcompanhamento') -or -not $setupSource.Contains('[32/33] Aplicando upgrade v0.5.8') -or -not $sql058.Contains('CREATE TABLE IF NOT EXISTS "ProtocolosAcompanhamento"')) { throw "Upgrade v0.5.8 de protocolos nao foi preservado." }
Write-Host "    Migration incremental + SQL idempotente v0.5.8 + PREPARAR 33/33: OK."
...
if (-not $setupSource.Contains('V060MedicamentosAdesao') -or -not $setupSource.Contains('[33/33] Aplicando upgrade v0.6.0') -or -not $sql060.Contains('CREATE TABLE IF NOT EXISTS "MedicamentosPaciente"')) { throw "Upgrade v0.6.0 de medicamentos incompleto." }
Write-Host "    Migration incremental + SQL idempotente + PREPARAR 33/33: OK."
...
Write-Host "[600/600] Validando versao funcional v0.6.0..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.0") { throw "VERSION.txt inesperado." }
...
Write-Host "    v0.6.0 / Medicamentos + Adesao ao Tratamento: OK."
