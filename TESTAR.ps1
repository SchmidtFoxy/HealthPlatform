...
if ($health.version -ne "0.6.2") { throw "Versao inesperada da API: $($health.version)" }
...
if ($version.Trim() -ne "0.6.2") { throw "VERSION.txt inesperado." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.2") { throw "VERSION.txt inesperado." }
if ($setupSource -notmatch "\[35/35\]") { throw "PREPARAR atual deveria possuir 35 etapas." }
Write-Host "    v0.3.27 preservada / PREPARAR atual 35/35 / upgrade SOAP: OK."
...
if (-not $setupSource.Contains("[35/35]") -or
...
Write-Host "    SOAP v0.3.15 preservado / PREPARAR atual 35/35: OK."
...
Write-Host "    Sem schema novo / PREPARAR atual 35/35: OK."
...
if (-not $cssSource.Contains("phase-highlight-grid") -or
    ...
    -not $setupSource.Contains("[35/35]")) {
...
if (-not $setupSource.Contains("[35/35]") -or -not $setupSource.Contains("v0.3.34_criterios_transicao_fases.sql") -or -not $sqlSource.Contains('"MetaPesoKg"') -or -not $sqlSource.Contains('"CriterioTransicao"') -or -not $setupSource.Contains("v0.3.32_checkins_acompanhamento.sql")) { throw "Upgrade dos criterios incompleto." }
...
Write-Host "    SQL idempotente + PREPARAR 35/35 + v0.3.34 preservada."
...
Write-Host "    Sem schema novo / PREPARAR permanece 35/35."
...
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.6.2") -or
...
if (-not $programSource.Contains("app.Environment.IsDevelopment()") -or
    ...
    -not $setupSource.Contains("[35/35]") -or
...
Write-Host "    PREPARAR continua 35/35 / sem SQL v0.3.40."
...
Write-Host "[517/600] Validando PREPARAR 35/35..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[31/35] Aplicando upgrade v0.5.1') -or -not $setupSource.Contains('v0.5.1_solicitacoes_clinicas.sql')) { throw "PREPARAR nao preserva o upgrade v0.5.1." }
...
if (-not $setupSource.Contains('V058ProtocolosAcompanhamento') -or -not $setupSource.Contains('[32/35] Aplicando upgrade v0.5.8') -or -not $sql058.Contains('CREATE TABLE IF NOT EXISTS "ProtocolosAcompanhamento"')) { throw "Upgrade v0.5.8 de protocolos nao foi preservado." }
Write-Host "    Migration incremental + SQL idempotente v0.5.8 + PREPARAR 35/35: OK."
...
if (-not $setupSource.Contains('V060MedicamentosAdesao') -or -not $setupSource.Contains('[33/35] Aplicando upgrade v0.6.0') -or -not $sql060.Contains('CREATE TABLE IF NOT EXISTS "MedicamentosPaciente"')) { throw "Upgrade v0.6.0 de medicamentos incompleto." }
Write-Host "    Migration incremental + SQL idempotente + PREPARAR 35/35: OK."
...
Write-Host "[600/616] Validando compatibilidade funcional v0.6.0..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.2") { throw "VERSION.txt inesperado." }
...
Write-Host "[601/616] Validando entidade de prontidao diaria..."
...
Write-Host "[602/616] Validando mapeamento EF da prontidao..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[603/616] Validando endpoint PatientOnly de prontidao..."
...
Write-Host "[604/616] Validando travas de seguranca do score..."
...
Write-Host "[605/616] Validando Home Daily Athlete..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[606/616] Validando leitura profissional da prontidao..."
...
Write-Host "[607/616] Validando SQL e migration v0.6.1..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
$sql061 = Get-Content .\scripts\sql\v0.6.1_prontidao_diaria.sql -Encoding UTF8 -Raw
if (-not $setupSource.Contains('V061ProntidaoDiaria') -or -not $setupSource.Contains('[34/35] Aplicando upgrade v0.6.1') -or -not $sql061.Contains('CREATE TABLE IF NOT EXISTS "ProntidoesDiarias"')) { throw "Upgrade v0.6.1 de prontidao incompleto." }
Write-Host "    Migration incremental + SQL idempotente + PREPARAR 35/35: OK."

Write-Host "[608/616] Validando versao funcional v0.6.1..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.2" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.2';")) { throw "Versao v0.6.1 inconsistente." }
Write-Host "    v0.6.1 / Daily Athlete + Prontidao Diaria: OK."



Write-Host "[609/616] Validando ledger de XP..."
...
Write-Host "[610/616] Validando mapeamento EF de XP..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[611/616] Validando XP seguro de treino..."
...
Write-Host "[612/616] Validando nivel, streak e consistencia..."
...
Write-Host "[613/616] Validando eventos de autocuidado..."
$portalMeSource = Get-Content .\src\HealthPlatform.Api\Controllers\MeuPortalPacienteController.cs -Encoding UTF8 -Raw
...
Write-Host "[614/616] Validando Home esportiva gamificada..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[615/616] Validando SQL e migration v0.6.2..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
$sql062 = Get-Content .\scripts\sql\v0.6.2_xp_consistencia.sql -Encoding UTF8 -Raw
if (-not $setupSource.Contains('V062XpConsistencia') -or -not $setupSource.Contains('[35/35] Aplicando upgrade v0.6.2') -or -not $sql062.Contains('CREATE TABLE IF NOT EXISTS "EventosXp"')) { throw "Upgrade v0.6.2 incompleto." }
Write-Host "    Migration incremental + SQL idempotente + PREPARAR 35/35: OK."

Write-Host "[616/616] Validando versao funcional v0.6.2..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.2" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.2';")) { throw "Versao v0.6.2 inconsistente." }
