...
if ($health.version -ne "0.8.7") { throw "Versao inesperada da API: $($health.version)" }
...
if ($version.Trim() -ne "0.8.7") { throw "VERSION.txt inesperado." }
...
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.8.7") -or
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.6.1 inconsistente." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.6.2 inconsistente." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao atual inconsistente ao validar v0.6.3." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Compatibilidade v0.6.4 inconsistente na versao atual." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.6.6 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.8.7"')) { throw "Health endpoint nao anuncia v0.6.6." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.7.1 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.8.7"')) { throw "Health endpoint nao anuncia v0.7.1." }
...
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.7.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.7"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.1." }
...
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.7.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.7"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.7." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.7.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.7"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.9." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.8.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.7"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.0." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.8.2 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.7"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.2." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.8.7" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.8.7';")) { throw "Versao v0.8.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.8.7"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.7." }
