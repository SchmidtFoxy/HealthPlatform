...
# SEGURANCA: TESTAR.ps1 e exclusivamente local. Nunca apontar este gate para producao/VPS.
if ($base -notmatch '^https?://(localhost|127\.0\.0\.1)(:\d+)?/?$') {
    throw "TESTAR.ps1 bloqueado: BaseUrl deve ser localhost/127.0.0.1. Testes de desenvolvimento nao podem atingir producao."
}
...
if ($health.version -ne "0.19.43") { throw "Versao inesperada da API: $($health.version)" }
...
if ($version.Trim() -ne "0.19.43") { throw "VERSION.txt inesperado." }
...
Write-Host "[99/600] Validando limpeza de scripts demo legados..."
if (Test-Path .\POPULAR.ps1) {
    throw "POPULAR.ps1 legado reapareceu no pacote limpo."
}
Write-Host "    POPULAR.ps1 legado ausente: OK."

Write-Host "[100/600] Validando que PREPARAR nao depende do popular legado..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if ($setupSource -match "POPULAR.ps1") {
    throw "scripts/setup.ps1 ainda referencia POPULAR.ps1 legado."
}
Write-Host "    PREPARAR independente de POPULAR.ps1: OK."

Write-Host "[101/600] Validando integridade do pacote limpo..."
$legacyDemoFiles = @(Get-ChildItem -Path . -File -Filter "POPULAR*.ps1" -ErrorAction SilentlyContinue)
if ($legacyDemoFiles.Count -gt 0) {
    throw "Scripts POPULAR legados encontrados no pacote limpo: $($legacyDemoFiles.Name -join ', ')"
}
Write-Host "    Sem scripts POPULAR legados na raiz: OK."
...
Write-Host "[103/600] Validando que PREPARAR preserva dados do usuario..."
if ($setupSource -match "POPULAR.ps1") {
    throw "PREPARAR nao deve depender do POPULAR.ps1 legado."
}
Write-Host "    PREPARAR preserva dados do usuario e nao depende de seed demo legado: OK."
...
Write-Host "[130/600] Validando estilos de follow-up e versao..."
$css = Get-Utf8WebAsset -Uri "$base/app.css" -LocalPath ".\src\HealthPlatform.Api\wwwroot\app.css"
if ($css.Content -notmatch "follow-queue-card" -or
    $css.Content -notmatch "follow-queue-toolbar") {
    throw "Estilos de follow-up incompletos."
}
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43") { throw "VERSION.txt inesperado." }
...
Write-Host "[463/600] Validando identidade publica AESYN..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("AESYN") -or
    -not $indexSource.Contains('id="loginMessage"') -or
    $indexSource.Contains('value="ChangeMe_123!"') -or
    $indexSource.Contains('>Demo</span>') -or
    $indexSource.Contains('MVP Preview')) {
    throw "Identidade/login publico AESYN incompleto."
}
Write-Host "    Login + identidade publica de producao: OK."

Write-Host "[464/600] Validando aviso de ambiente de producao..."
$indexSourceProduction = $indexSource.ToLowerInvariant()
if (-not $indexSourceProduction.Contains('class="dev-note"') -or
    -not $indexSourceProduction.Contains('ambiente aesyn') -or
    -not $indexSourceProduction.Contains('dados e acessos') -or
    $indexSourceProduction.Contains('ambiente da demo') -or
    $indexSourceProduction.Contains('senha local antiga')) {
    throw "Aviso de ambiente de producao/login incompleto."
}
Write-Host "    Copy de producao e protecao de acesso: OK."

Write-Host "[465/600] Validando Central de Ajuda AESYN..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
if (-not $appJsSource.Contains("openMvpGuide") -or
    -not $appJsSource.Contains("CENTRAL DE AJUDA AESYN") -or
    -not $appJsSource.Contains("Guia rápido da plataforma") -or
    -not $appJsSource.Contains("hpMvpChecklistItem")) {
    throw "Central de Ajuda AESYN incompleta."
}
Write-Host "    Guia operacional de producao: assets OK."

Write-Host "[466/600] Validando guia de uso da plataforma..."
if (-not $appJsSource.Contains("Acesse o prontuário") -or
    -not $appJsSource.Contains("Registre o atendimento") -or
    -not $appJsSource.Contains("Prescreva o plano") -or
    -not $appJsSource.Contains("Acompanhe a evolução") -or
    -not $appJsSource.Contains("Visualize como paciente")) {
    throw "Guia de uso da plataforma incompleto."
}
Write-Host "    Fluxos principais documentados em linguagem de producao."

Write-Host "[467/600] Validando modelo de suporte..."
if (-not $appJsSource.Contains("copyMvpFeedbackTemplate") -or
    -not $appJsSource.Contains("RESULTADO ESPERADO:") -or
    -not $appJsSource.Contains("RESULTADO ENCONTRADO:") -or
    -not $appJsSource.Contains("OBSERVAÇÕES:")) {
    throw "Modelo de suporte AESYN incompleto."
}
Write-Host "    Modelo de suporte pode ser copiado."

Write-Host "[468/600] Validando dashboard de apresentacao..."
# A partir da v0.18.1, a home principal deixa de usar o hero legado do MVP Preview
# e passa a validar o Professional Performance Dashboard da AESYN. O roteiro de
# demonstracao continua validado separadamente nos gates 465-467.
if (-not $appJsSource.Contains("aesyn-performance-hero") -or
    -not $appJsSource.Contains("PROFESSIONAL COMMAND CENTER") -or
    -not $appJsSource.Contains("performance-kpi-grid") -or
    -not $appJsSource.Contains("goAgendaHero")) {
    throw "Dashboard de apresentacao AESYN incompleto."
}
Write-Host "    Performance hero + KPIs + atalho de agenda: assets OK."
...
Write-Host "[473/600] Validando limpeza da navegacao tecnica..."
if (-not $indexSource.Contains("mvp-dev-link") -or
    -not $cssSource.Contains(".mvp-dev-link{display:none!important}")) {
    throw "Link tecnico nao foi escondido da navegacao publica."
}
Write-Host "    Swagger continua no backend, mas sai da navegacao principal."

Write-Host "[474/600] Validando responsividade da interface..."
if (-not $cssSource.Contains("@media(max-width:900px)") -or
    ...
    throw "Responsividade da interface incompleta."
}
Write-Host "    Notebook + mobile: estilos responsivos OK."
...
if (-not $dockerSource.Contains("mcr.microsoft.com/dotnet/sdk:10.0") -or
    ...
    throw "Dockerfile de producao incompleto."
}
...
if (-not $entrypointSource.Contains('PORT_VALUE="${PORT:-10000}"') -or
    ...
    throw "Entrypoint de container incompleto."
}
...
Write-Host "    Render removido; deploy atual isolado em VPS/Nginx: OK."

Write-Host "[480/600] Validando isolamento dos testes locais..."
$testarSourceAtual = Get-Content .\TESTAR.ps1 -Encoding UTF8 -Raw
if (-not $testarSourceAtual.Contains('$base = "http://localhost:5180"') -or
    -not $testarSourceAtual.Contains('TESTAR.ps1 bloqueado: BaseUrl deve ser localhost/127.0.0.1')) {
    throw "Protecao contra testes acidentais em producao ausente."
}
Write-Host "    TESTAR preso a localhost/127.0.0.1: OK."

Write-Host "[481/600] Validando resolver PostgreSQL..."
...
if (-not $resolverSource.Contains("NpgsqlConnectionStringBuilder") -or
    ...
    throw "Resolver PostgreSQL incompleto."
}
Write-Host "    Credenciais discretas -> Npgsql: OK."

Write-Host "[482/600] Validando bootstrap controlado..."
...
if (-not $programSource.Contains('GetValue<bool>("DemoBootstrap:Enabled")') -or
    ...
    throw "Bootstrap configuravel ou resolver de banco ausente."
}
Write-Host "    Bootstrap permanece opt-in por configuracao: OK."
...
Write-Host "[485/600] Validando reverse proxy VPS/Nginx..."
$httpsGuide = Get-Content .\PRODUCAO-HTTPS.md -Encoding UTF8 -Raw
if (-not $httpsGuide.Contains('proxy_set_header X-Forwarded-Proto $scheme;') -or
    -not $httpsGuide.Contains('return 301 https://$host$request_uri;')) {
    throw "Guia VPS/Nginx sem forwarded headers/redirect HTTPS."
}
Write-Host "    X-Forwarded-Proto + HTTPS via Nginx documentados: OK."

Write-Host "[486/600] Validando ausencia de seed remoto legado..."
foreach ($legacy in @('.\POPULAR-REMOTO.ps1','.\POPULAR-REMOTO-RICO.ps1')) {
    if (Test-Path $legacy) { throw "Artefato remoto legado encontrado: $legacy" }
}
Write-Host "    Seeds remotos do Render removidos: OK."

Write-Host "[487/600] Validando ausencia de smoke remoto legado..."
if (Test-Path .\TESTAR-RENDER.ps1) { throw "TESTAR-RENDER.ps1 legado encontrado." }
Write-Host "    Smoke de Render removido: OK."

Write-Host "[488/600] Validando ausencia de guia Render legado..."
if (Test-Path .\DEPLOY-RENDER-MVP.md) { throw "DEPLOY-RENDER-MVP.md legado encontrado." }
Write-Host "    Guia Render removido: OK."

Write-Host "[489/600] Validando que o gate nao referencia producao..."
if ($testarSourceAtual -match 'Invoke-(RestMethod|WebRequest)[^\r\n]*(aesyn\.com\.br|187\.6\.166\.168|render\.com)') {
    throw "TESTAR.ps1 contem chamada HTTP para ambiente remoto/producao."
}
Write-Host "    Nenhuma chamada HTTP de teste para producao/VPS: OK."
...
Write-Host "[493/600] Validando paleta AESYN..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
foreach ($token in @('--aesyn-petroleum:#0B3B36','--aesyn-emerald:#19C98B','--aesyn-offwhite:#F6F8F7','--aesyn-graphite:#111817','--aesyn-cool-gray:#A7B2AF')) {
    if (-not $cssSource.Contains($token)) { throw "Paleta AESYN incompleta: $token" }
}
Write-Host "    Petroleo + esmeralda + off-white + grafite + cinza: identidade OK."

Write-Host "[494/600] Validando marca AESYN..."
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($indexSource.Contains('<span class="brand-mark">H+</span>') -or
    $indexSource.Contains('<span class="brand-mark">RS</span>') -or
    -not $indexSource.Contains('<span class="brand-mark aesyn-mark">A</span>') -or
    -not $indexSource.Contains('<span class="brand-word">AESYN<small>PERFORMANCE</small></span>')) {
    throw "Marca AESYN nao foi aplicada de forma consistente."
}
Write-Host "    Login + profissional + paciente: AESYN OK."

Write-Host "[495/600] Validando linguagem AESYN..."
if (-not $indexSource.Contains("SAÚDE • PERFORMANCE • EVOLUÇÃO") -or
    -not $indexSource.Contains("Saúde e performance") -or
    -not $indexSource.Contains("em sincronia.")) {
    throw "Copy visual AESYN incompleta."
}
Write-Host "    Saude + performance + evolucao: copy OK."

Write-Host "[496/600] Validando sistema visual AESYN..."
if (-not $cssSource.Contains('.brand-word{display:grid') -or
    -not $cssSource.Contains('.aesyn-mark{position:relative') -or
    -not $cssSource.Contains('text-transform:uppercase')) {
    throw "Sistema visual AESYN incompleto."
}
Write-Host "    Marca + hierarquia editorial + titulos: CSS OK."

Write-Host "[497/600] Validando login AESYN..."
if (-not $cssSource.Contains(".login-visual:before") -or
    -not $cssSource.Contains(".login-visual:after") -or
    -not $cssSource.Contains("var(--aesyn-emerald)")) {
    throw "Composicao visual do login AESYN incompleta."
}
Write-Host "    Profundidade + acentos esmeralda + composicao: login OK."

Write-Host "[498/600] Validando sidebar AESYN..."
if (-not $cssSource.Contains(".nav-item.active") -or
    -not $cssSource.Contains("var(--aesyn-emerald)") -or
    -not $cssSource.Contains(".sidebar .aesyn-mark")) {
    throw "Sidebar AESYN incompleta."
}
Write-Host "    Petroleo + marcador esmeralda + marca AESYN: OK."
...
Write-Host "[507/600] Validando deploy atual VPS..."
if ((Test-Path .\render.yaml) -or (Test-Path .\TESTAR-RENDER.ps1) -or (Test-Path .\POPULAR-REMOTO.ps1) -or (Test-Path .\POPULAR-REMOTO-RICO.ps1)) {
    throw "Artefatos legados de Render foram reintroduzidos."
}
if (-not (Test-Path .\Dockerfile) -or -not (Test-Path .\docker-compose.yml) -or -not (Test-Path .\PRODUCAO-HTTPS.md)) {
    throw "Arquivos esperados para deploy VPS/container ausentes."
}
Write-Host "    Docker + Compose + Nginx/HTTPS; sem Render: OK."
...
if (-not $appJsSource.Contains("function hpResolvePatientNotificationLink(link)") -or -not $appJsSource.Contains("'solicitacoes'") -or -not $appJsSource.Contains("return allowed.has(link)?link:'inicio'")) { throw "Notificacao do paciente nao abre Solicitacoes." }
...
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
if (-not $appJsSource.Contains('function hpAthleteHome2') -or -not $appJsSource.Contains('id="athleteHome2Body"') -or -not $appJsSource.Contains("$('#athleteHome2Body')") -or -not $appJsSource.Contains('openDailyReadiness(readiness)')) { throw "Home Daily Athlete incompleta." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.6.1 inconsistente." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.6.2 inconsistente." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao atual inconsistente ao validar v0.6.3." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Compatibilidade v0.6.4 inconsistente na versao atual." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.6.6 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.19.43"')) { throw "Health endpoint nao anuncia v0.6.6." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.7.1 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.19.43"')) { throw "Health endpoint nao anuncia v0.7.1." }
...
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.7.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.1." }
...
Write-Host "[679/720] Validando ausencia de alteracao indevida de schema do Coach Diario..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[37/38] Aplicando upgrade v0.6.4')) { throw "PREPARAR 38/38 foi alterado sem necessidade." }
if (Test-Path .\scripts\sql\v0.7.1_coach_diario.sql) { throw "Coach Diario nao deveria exigir migration propria." }
Write-Host "    Coach permanece camada derivada; schema legado preservado: OK."

Write-Host "[680/720] Validando compatibilidade funcional do Coach Diario..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao corrente inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.26." }
Write-Host "    Coach Diario & Prioridades Contextuais preservados: OK."

Write-Host "[681/720] Validando politica de seeds demonstrativos..."
$anaSeedPath = '.\POPULAR-ANA-RIBEIRO.ps1'
$anaSeedDisponivel = Test-Path $anaSeedPath
if ($anaSeedDisponivel) {
    $anaSeedSource = Get-Content $anaSeedPath -Encoding UTF8 -Raw
    Write-Host "    Seed Ana Ribeiro opcional encontrado; validacoes legadas serao executadas."
} else {
    $anaSeedSource = ''
    Write-Host "    Seed Ana Ribeiro legado ausente por design; TESTAR nao depende de dados demonstrativos: OK."
}
...
Write-Host "[687/720] Validando Coach Diario preservado..."
$coachSource = Get-Content .\src\HealthPlatform.Api\Services\CoachDiarioService.cs -Encoding UTF8 -Raw
if (-not $coachSource.Contains('priorizar-recuperacao') -or -not $coachSource.Contains('não altera a prescrição automaticamente')) { throw "Coach Diario regrediu." }
Write-Host "    Sintese contextual e travas clinicas preservadas: OK."

Write-Host "[688/720] Validando seed demonstrativo opcional..."
Write-Host "    Seed Ana presente e validado sem integrar o gate obrigatorio de produto: OK."
...
Write-Host "    Erros 4xx/5xx identificam endpoint + resposta: OK."
...
Write-Host "[694/720] Validando ausencia de alteracao indevida de schema do seed..."
if (Test-Path .\scripts\sql\v0.7.7_seed_resiliente.sql) { throw "Robustez de seed nao deveria exigir migration." }
Write-Host "    Robustez de seed nao altera schema: OK."

Write-Host "[695/720] Validando versao corrente..."
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao corrente inconsistente." }
Write-Host "    API/UI/VERSION: 0.19.26 OK."

Write-Host "[696/720] Validando compatibilidade do seed demonstrativo..."
Write-Host "    Seed demonstrativo permanece opcional e isolado do produto: OK."

Write-Host "[697/720] Validando identificacao do seed Ana..."
if (-not $anaSeedSource.Contains('Seed esportivo PESADO da Ana Ribeiro (v7)')) { throw "Banner do seed Ana v7 ausente." }
Write-Host "    Seed identificado: OK."
...
Write-Host "[700/720] Validando sufixos numericos perigosos no codigo do seed..."
$linhasSeed = Get-Content $anaSeedPath -Encoding UTF8
...
Write-Host "    Literais numericos de codigo sao PowerShell nativos: OK."

Write-Host "[701/720] Validando seed pesado preservado..."
...
Write-Host "[702/720] Validando ausencia de migration numerica..."
if (Test-Path .\scripts\sql\v0.7.7_seed_numerico.sql) { throw "Correcao de seed nao deveria exigir migration." }
Write-Host "    Correcao de seed nao altera schema: OK."

Write-Host "[703/720] Validando versao corrente apos seed..."
Write-Host "    API/UI/VERSION: 0.19.26 OK."

Write-Host "[704/720] Validando compatibilidade da correcao decimal..."
Write-Host "    Correcao decimal do seed opcional preservada: OK."
...
if (-not $testSourceAtual) { $testSourceAtual = Get-Content .\TESTAR.ps1 -Encoding UTF8 -Raw }
...
Write-Host "[710/720] Validando ausencia de migration de validacao..."
if (Test-Path .\scripts\sql\v0.7.7_seed_validacao.sql) { throw "Validacao de seed nao deveria exigir migration." }
Write-Host "    Schema permanece inalterado: OK."

Write-Host "[711/720] Validando versao corrente..."
Write-Host "    API/UI/VERSION: 0.19.26 OK."

Write-Host "[712/720] Validando compatibilidade do seed..."
Write-Host "    Seed demonstrativo opcional preservado: OK."
...
Write-Host "[717/720] Validando seed pesado preservado..."
...
Write-Host "    Avaliacoes + fechamento do seed: OK."
    ...
    682..717 | ForEach-Object {
        Write-Host "[$_/720] Seed Ana Ribeiro legado aposentado; gate demonstrativo nao aplicavel: OK."
    }
}

Write-Host "[718/720] Validando ausencia de migrations exclusivas de seed..."
$seedMigrations = @(
    '.\scripts\sql\v0.7.7_seed_automatic_vars.sql',
    '.\scripts\sql\v0.7.7_seed_resiliente.sql',
    '.\scripts\sql\v0.7.7_seed_numerico.sql',
    '.\scripts\sql\v0.7.7_seed_validacao.sql'
)
foreach ($seedMigration in $seedMigrations) {
    if (Test-Path $seedMigration) { throw "Migration exclusiva de seed legado encontrada: $seedMigration" }
}
Write-Host "    Seeds demonstrativos nao interferem no schema: OK."

Write-Host "[719/720] Validando versao corrente..."
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao corrente inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.26." }
Write-Host "    API/UI/VERSION: 0.19.26 OK."

Write-Host "[720/720] Validando isolamento de dados demonstrativos..."
if (-not $anaSeedDisponivel) {
    Write-Host "    Produto/testes nao dependem do antigo POPULAR-ANA-RIBEIRO.ps1: OK."
} else {
    Write-Host "    Seed demonstrativo existe apenas como ferramenta opcional: OK."
}
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.7.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.7." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.7.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.7.9." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.8.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.0." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.8.2 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.2." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.8.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.7." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.8.8 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.8." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.8.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.8.9." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.9.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.9.0." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.9.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.9.1." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.9.3 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.9.3." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.9.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.9.5." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.9.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.9.7." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.9.8 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.9.8." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.9.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.9.9." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.10.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.10.0." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.10.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.10.1." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.10.2 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.10.2." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.10.3 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.10.3." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.10.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.10.4." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.12.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.12.0." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.12.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.12.1." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.12.2 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.12.2." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.12.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.12.4." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.12.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.12.5." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.13.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.13.0." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.13.3 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.13.3." }
...
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.13.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.13.5." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.13.6 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.13.6." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.13.6 / Mobile Interaction Experience: OK."


Write-Host "[1145/1152] Validando feedback visual v0.14.4..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[1151/1152] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_feedback_experience.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Feedback/UX mobile refinados sem migration nova: OK."

Write-Host "[1152/1152] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
foreach ($token in @('patient-today-overview','HOJE EM UM OLHAR','O que você precisa fazer hoje','athleteHome2Body')) { if (-not $appJsSource.Contains($token)) { throw "Resumo diario mobile incompleto: $token" } }
...
foreach ($token in @('CHECK-IN','TREINO','ALIMENTAÇÃO','HIDRATAÇÃO','MENSAGENS','PENDÊNCIAS')) { if (-not $appJsSource.Contains($token)) { throw "Acao essencial ausente: $token" } }
Write-Host "    Check-in, treino, alimentacao, hidratacao, mensagens e pendencias: OK."

Write-Host "[1155/1160] Validando rail visual do resumo mobile..."
foreach ($token in @('.patient-today-essential-grid{','.patient-today-essential{','grid-template-columns:repeat(2,minmax(0,1fr))')) { if (-not $cssSource.Contains($token)) { throw "Grid do resumo incompleto: $token" } }
Write-Host "    Acoes essenciais organizadas em cards responsivos: OK."

Write-Host "[1156/1160] Validando estados de prontidao no resumo..."
foreach ($token in @("${readiness?'done':'priority'}",'Fazer check-in','readiness.score','readinessLabel')) { if (-not $appJsSource.Contains($token)) { throw "Estado de prontidao incompleto: $token" } }
...
foreach ($token in @('.patient-today-essential{padding:12px;min-height:76px','readinessLabel','readinessDetail')) { if (-not $cssSource.Contains($token) -and -not $appJsSource.Contains($token)) { throw "Prontidao compacta incompleta: $token" } }
Write-Host "    Check-in compacto permanece integrado ao resumo da Home: OK."

Write-Host "[1158/1160] Validando telas estreitas v0.14.4..."
foreach ($token in @('@media(max-width:560px)','.patient-today-essential-grid{grid-template-columns:1fr','.patient-today-essential.primary-action{order:-1}')) { if (-not $cssSource.Contains($token)) { throw "Ajuste estreito v0.14.4 incompleto: $token" } }
Write-Host "    Resumo permanece legivel em celulares estreitos: OK."

Write-Host "[1159/1160] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_home_hierarchy.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Hierarquia mobile refinada sem migration nova: OK."

Write-Host "[1160/1160] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1161/1168] Validando central Hoje em um olhar v0.19.40..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
foreach ($token in @('patient-today-overview','HOJE EM UM OLHAR','athleteHome2Body','athleteHome2Now','athleteHome2Today','patientTodayNutrition','patientTodayChat','patientTodayRequests')) { if (-not $appJsSource.Contains($token)) { throw "Hoje em um olhar incompleto: $token" } }
Write-Host "    Acoes essenciais do dia concentram a primeira leitura: OK."

Write-Host "[1162/1168] Validando prioridade contextual da Home atual..."
foreach ($token in @('Fazer check-in','Após o check-in','Abrir treino','Seu plano do dia')) { if (-not $appJsSource.Contains($token)) { throw "Prioridade contextual atual ausente: $token" } }
Write-Host "    Check-in, treino e progresso mudam conforme o contexto atual: OK."

Write-Host "[1163/1168] Validando atalhos de uso diario..."
foreach ($token in @("loadPatientSection('treino')","loadPatientSection('plano')","loadPatientSection('chat')","openQuickPatientRecord({quick:'Agua'")) { if (-not $appJsSource.Contains($token)) { throw "Atalho diario incompleto: $token" } }
Write-Host "    Treino, alimentacao, mensagens e hidratacao ficam acessiveis sem navegar pelo menu: OK."

Write-Host "[1164/1168] Validando identidade visual da Home atual..."
foreach ($token in @('.patient-today-overview{display:grid','.patient-today-essential-grid{display:grid','.patient-today-essential.primary-action{background:linear-gradient')) { if (-not $cssSource.Contains($token)) { throw "Hoje em um olhar visual incompleto: $token" } }
Write-Host "    Acao principal ganha contraste e as demais mantem hierarquia consistente: OK."

Write-Host "[1165/1168] Validando ergonomia em telas estreitas..."
foreach ($token in @('@media(max-width:560px)','.patient-today-essential-grid{grid-template-columns:1fr','.patient-today-essential{padding:12px;min-height:76px}')) { if (-not $cssSource.Contains($token)) { throw "Home atual estreita incompleta: $token" } }
Write-Host "    Acoes essenciais permanecem tocaveis em celulares estreitos: OK."

Write-Host "[1166/1168] Validando movimento reduzido da Home atual..."
foreach ($token in @('prefers-reduced-motion:reduce','.patient-today-essential{transition:none}')) { if (-not $cssSource.Contains($token)) { throw "Acessibilidade de movimento incompleta: $token" } }
Write-Host "    Interacoes da Home respeitam preferencia de movimento: OK."

Write-Host "[1167/1168] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.13.11_mobile_action_hub.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Action Hub entregue sem migration nova: OK."

Write-Host "[1168/1168] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1169/1176] Validando resumo pos-treino v0.14.4..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[1175/1176] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.13.11_post_workout_mobile_loop.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Loop pos-treino entregue sem migration nova: OK."

Write-Host "[1176/1176] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1183/1184] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.13.11_recovery_pulse.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Recovery Pulse entregue sem migration nova: OK."

Write-Host "[1184/1184] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1191/1192] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_daily_athlete_timeline.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Daily Athlete Timeline entregue sem migration nova: OK."

Write-Host "[1192/1192] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1199/1200] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_weekly_athlete_rhythm.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Weekly Athlete Rhythm entregue sem migration nova: OK."

Write-Host "[1200/1200] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1207/1208] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_training_load_snapshot.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Training Load Snapshot entregue sem migration nova: OK."

Write-Host "[1208/1208] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
foreach ($token in @('patient-today-overview','athleteHome2Body','hpDailyAthleteTimeline(d,readiness)','hpMobileInsightRail(d,readiness)')) { if (-not $appJsSource.Contains($token)) { throw "Hierarquia mobile incompleta: $token" } }
Write-Host "    Acoes essenciais, check-in, timeline e insights permanecem na hierarquia principal: OK."
...
Write-Host "[1215/1216] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_insight_rail.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Insight Rail entregue sem migration nova: OK."

Write-Host "[1216/1216] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1223/1224] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_body_context_brief.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Body Context Brief entregue sem migration nova: OK."

Write-Host "[1224/1224] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1231/1232] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_consistency_compass.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Consistency Compass entregue sem migration nova: OK."

Write-Host "[1232/1232] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1239/1240] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_daily_closure.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Daily Closure entregue sem migration nova: OK."

Write-Host "[1240/1240] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
foreach ($token in @('.home-stage-checkin .mobile-insight-rail','.home-stage-checkin .daily-closure-card{display:none}', '.home-stage-closed .patient-today-overview{display:none}', '.home-stage-closed .today-actions-card{display:none}')) { if (-not $cssSource.Contains($token)) { throw "Regra adaptativa mobile ausente: $token" } }
...
Write-Host "[1247/1248] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_adaptive_mobile_home.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Adaptive Mobile Home entregue sem migration nova: OK."

Write-Host "[1248/1248] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1255/1256] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_hydration_pace.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Hydration Pace entregue sem migration nova: OK."

Write-Host "[1256/1256] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1263/1264] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_today_brief.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Today Brief entregue sem migration nova: OK."

Write-Host "[1264/1264] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1271/1272] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_quick_log_hub.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
...
Write-Host "[1279/1280] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_session_review.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Session Review entregue sem migration nova: OK."

Write-Host "[1280/1280] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao v0.14.4 inconsistente." }
...
Write-Host "[1281/1288] Validando fundacao visual mobile v0.14.4..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
...
Write-Host "[1287/1288] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_ui_foundation.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Mobile UI Foundation entregue sem migration nova: OK."

Write-Host "[1288/1288] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_UI_FOUNDATION='v0.14.3'")) { throw "Versao v0.14.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
Write-Host "[1295/1296] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_navigation_shell.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Navigation Shell entregue sem migration nova: OK."

Write-Host "[1296/1296] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_NAVIGATION_SHELL='v0.14.3'")) { throw "Versao v0.14.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
foreach ($token in @('--hp-mobile-section-gap','--hp-mobile-card-pad','--hp-mobile-muted','--hp-mobile-label')) { if (-not $cssSource.Contains($token)) { throw "Token da hierarquia mobile ausente: $token" } }
Write-Host "    Tokens de conteudo mobile centralizados: OK."

Write-Host "[1298/1304] Validando hierarquia tipografica dos cards..."
foreach ($token in @('.patient-portal-content .card-head h2','font-size:17px!important','text-transform:none!important','letter-spacing:-.018em!important')) { if (-not $cssSource.Contains($token)) { throw "Hierarquia tipografica mobile ausente: $token" } }
Write-Host "    Titulos internos ficam legiveis e consistentes: OK."

Write-Host "[1299/1304] Validando densidade e espacamento mobile..."
foreach ($token in @('gap:var(--hp-mobile-section-gap)!important','padding:var(--hp-mobile-card-pad)!important','margin-bottom:var(--hp-mobile-section-gap)!important')) { if (-not $cssSource.Contains($token)) { throw "Densidade mobile ausente: $token" } }
Write-Host "    Secoes e cards usam ritmo visual comum: OK."

Write-Host "[1300/1304] Validando campos e contraste de formulario..."
foreach ($token in @('--hp-mobile-input-bg','#91a4b0!important','border-radius:14px!important','min-height:96px')) { if (-not $cssSource.Contains($token)) { throw "Campo mobile inconsistente: $token" } }
Write-Host "    Inputs, placeholders e textareas usam contraste consistente: OK."

Write-Host "[1301/1304] Validando labels e textos auxiliares..."
foreach ($token in @('.patient-mobile-form label','var(--hp-mobile-label)','var(--hp-mobile-muted)','font-size:12px')) { if (-not $cssSource.Contains($token)) { throw "Label/texto auxiliar mobile ausente: $token" } }
Write-Host "    Labels e textos secundarios ficam distinguiveis: OK."

Write-Host "[1302/1304] Validando telas estreitas..."
foreach ($token in @('@media(max-width:390px)','--hp-mobile-section-gap:12px','--hp-mobile-card-pad:13px','font-size:16px!important')) { if (-not $cssSource.Contains($token)) { throw "Tratamento estreito da hierarquia ausente: $token" } }
Write-Host "    Conteudo preserva leitura em celulares estreitos: OK."

Write-Host "[1303/1304] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_content_hierarchy.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Content Hierarchy entregue sem migration nova: OK."

Write-Host "[1304/1304] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_CONTENT_HIERARCHY='v0.14.3'")) { throw "Versao v0.14.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
foreach ($token in @('--hp-mobile-row-gap','--hp-mobile-row-pad','--hp-mobile-row-bg','--hp-mobile-row-line')) { if (-not $cssSource.Contains($token)) { throw "Token de data view mobile ausente: $token" } }
Write-Host "    Tokens de listas e historicos mobile centralizados: OK."

Write-Host "[1306/1312] Validando tabelas como cards no celular..."
foreach ($token in @('.patient-portal-content .data-table thead','display:none!important','.patient-portal-content .data-table tbody tr','grid-template-columns:minmax(0,1fr) auto')) { if (-not $cssSource.Contains($token)) { throw "Conversao de tabela mobile incompleta: $token" } }
Write-Host "    Tabelas densas deixam de depender de colunas comprimidas: OK."

Write-Host "[1307/1312] Validando medidas e exames mobile..."
foreach ($token in @('.measurement-row:not(.header)','.patient-portal-content .lab-line','grid-template-columns:1fr 1fr!important','.lab-line .pill')) { if (-not $cssSource.Contains($token)) { throw "Data view de medidas/exames incompleta: $token" } }
Write-Host "    Medidas e exames ganham leitura compacta no celular: OK."

Write-Host "[1308/1312] Validando historicos mobile..."
foreach ($token in @('.patient-portal-content .diary-list','.patient-portal-content .followup-history-list','.patient-portal-content .checkin-history-list','.patient-portal-content .goal-history')) { if (-not $cssSource.Contains($token)) { throw "Historico mobile ausente: $token" } }
Write-Host "    Diario, follow-up, check-ins e metas seguem um mesmo padrao: OK."

Write-Host "[1309/1312] Validando historico de sessoes..."
foreach ($token in @('.patient-portal-content .patient-session-history','.patient-portal-content .workout-execution-list','border-radius:16px!important')) { if (-not $cssSource.Contains($token)) { throw "Historico de sessoes mobile incompleto: $token" } }
Write-Host "    Sessoes recentes usam superficies consistentes: OK."

Write-Host "[1310/1312] Validando telas estreitas e densidade..."
foreach ($token in @('@media(max-width:390px)','--hp-mobile-row-gap:8px','--hp-mobile-row-pad:11px','border-radius:14px!important')) { if (-not $cssSource.Contains($token)) { throw "Tratamento estreito de data views ausente: $token" } }
Write-Host "    Listas preservam leitura em celulares estreitos: OK."

Write-Host "[1311/1312] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_data_views.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Data Views entregue sem migration nova: OK."

Write-Host "[1312/1312] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_DATA_VIEWS='v0.14.3'")) { throw "Versao v0.14.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
foreach ($token in @('--hp-mobile-control-h','--hp-mobile-control-radius','--hp-mobile-control-error','--hp-mobile-action-surface')) { if (-not $cssSource.Contains($token)) { throw "Token de formulario mobile ausente: $token" } }
Write-Host "    Tokens de controles mobile centralizados: OK."

Write-Host "[1314/1320] Validando grids de formulario mobile..."
foreach ($token in @('.patient-mobile-form.form-grid','grid-template-columns:minmax(0,1fr)!important','.patient-portal-content .span-2','grid-column:auto!important')) { if (-not $cssSource.Contains($token)) { throw "Grid mobile incompleto: $token" } }
Write-Host "    Formularios deixam de comprimir duas ou tres colunas no celular: OK."

Write-Host "[1315/1320] Validando dimensoes e estados dos controles..."
foreach ($token in @('min-height:var(--hp-mobile-control-h)!important','min-height:112px!important','input:disabled','[aria-invalid="true"]','width:22px!important')) { if (-not $cssSource.Contains($token)) { throw "Estado/dimensao de controle ausente: $token" } }
Write-Host "    Inputs, selects, textareas e opcoes possuem alvos de toque e estados consistentes: OK."

Write-Host "[1316/1320] Validando acoes persistentes e safe-area..."
foreach ($token in @('.patient-sheet-actions','position:sticky!important','env(safe-area-inset-bottom)','--hp-mobile-action-surface')) { if (-not $cssSource.Contains($token)) { throw "Acao persistente mobile ausente: $token" } }
Write-Host "    Salvar/cancelar permanecem acessiveis em sheets e respeitam safe-area: OK."

Write-Host "[1317/1320] Validando teclado mobile e campos dinamicos..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
foreach ($token in @('enhancePatientMobileFormControls','inputmode','MutationObserver','enterkeyhint','scrollIntoView')) { if (-not $appJsSource.Contains($token)) { throw "Enhancement de formulario mobile ausente: $token" } }
Write-Host "    Campos dinamicos recebem teclado contextual e protecao contra sobreposicao: OK."

Write-Host "[1318/1320] Validando telas estreitas e movimento reduzido..."
foreach ($token in @('@media(max-width:390px)','--hp-mobile-control-h:50px','grid-template-columns:1fr!important','@media(prefers-reduced-motion:reduce)')) { if (-not $cssSource.Contains($token)) { throw "Tratamento estreito de formulario ausente: $token" } }
Write-Host "    Formularios preservam usabilidade em celulares estreitos: OK."

Write-Host "[1319/1320] Validando schema preservado na v0.14.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.4_mobile_forms_inputs.sql)) { throw "v0.14.4 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Forms & Inputs entregue sem migration nova: OK."

Write-Host "[1320/1320] Validando versao funcional v0.14.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_FORMS_INPUTS='v0.14.4'")) { throw "Versao v0.14.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
foreach ($token in @('patient-feedback-state','patient-feedback-icon','patient-empty-state','hp-action-pending')) { if (-not $cssSource.Contains($token)) { throw "Estado visual de feedback ausente: $token" } }
Write-Host "    Estados visuais de feedback mobile centralizados: OK."

Write-Host "[1322/1328] Validando erro recuperavel no carregamento..."
foreach ($token in @('hpPatientFeedbackState','Tentar novamente','patient-feedback-action','String(err')) { if (-not $appJsSource.Contains($token)) { throw "Feedback de erro recuperavel ausente: $token" } }
Write-Host "    Falhas de carregamento oferecem nova tentativa sem prender o spinner: OK."

Write-Host "[1323/1328] Validando feedback de submissao..."
foreach ($token in @('hpSetActionPending','aria-busy','hp-action-spinner','Salvando...')) { if (-not $appJsSource.Contains($token)) { throw "Feedback de submissao ausente: $token" } }
Write-Host "    Registro rapido bloqueia envio duplicado e comunica salvamento: OK."

Write-Host "[1324/1328] Validando loading acessivel..."
foreach ($token in @('role="status"','aria-live="polite"','patient-loading-state')) { if (-not $appJsSource.Contains($token)) { throw "Loading acessivel incompleto: $token" } }
Write-Host "    Loading anuncia progresso de maneira nao intrusiva: OK."

Write-Host "[1325/1328] Validando empty states mobile..."
foreach ($token in @('enhancePatientFeedbackStates','patient-empty-state','hpFeedbackEnhanced','role','status')) { if (-not $appJsSource.ToLower().Contains($token.ToLower())) { throw "Empty state mobile incompleto: $token" } }
Write-Host "    Estados vazios recebem semantica e superficie consistentes: OK."

Write-Host "[1326/1328] Validando movimento reduzido e telas mobile..."
foreach ($token in @('@media(max-width:720px)','@media(prefers-reduced-motion:reduce)','.hp-action-spinner{animation:none!important}','z-index:140')) { if (-not $cssSource.Contains($token)) { throw "Tratamento responsivo de feedback ausente: $token" } }
Write-Host "    Feedback respeita viewport mobile e movimento reduzido: OK."

Write-Host "[1327/1328] Validando schema preservado na v0.14.5..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.5_mobile_feedback_states.sql)) { throw "v0.14.5 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Feedback States entregue sem migration nova: OK."

Write-Host "[1328/1328] Validando compatibilidade funcional v0.14.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_FEEDBACK_STATES='v0.14.5'")) { throw "Compatibilidade v0.14.5 inconsistente na versao atual." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam a versao atual ao validar v0.14.5." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
foreach ($token in @('mobile-progress-chart','chart-mobile-summary','Mín.','Máx.')) { if (-not ($cssSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Componente de progresso mobile ausente: $token" } }
Write-Host "    Resumo visual de progresso mobile disponivel: OK."

Write-Host "[1330/1336] Validando resumo minimo/atual/maximo..."
foreach ($token in @('minValue','maxValue','chart-mobile-summary','current')) { if (-not $appJsSource.Contains($token)) { throw "Resumo de faixa do grafico incompleto: $token" } }
Write-Host "    Graficos destacam minimo, atual e maximo: OK."

Write-Host "[1331/1336] Validando tendencia e periodo explicitos..."
foreach ($token in @('const trend=','hpChartDate(series[0].date)','hpChartDate(series[series.length-1].date)','Variação')) { if (-not $appJsSource.Contains($token)) { throw "Leitura de tendencia/periodo incompleta: $token" } }
Write-Host "    Tendencia e janela temporal aparecem sem depender somente da linha: OK."

Write-Host "[1332/1336] Validando acessibilidade dos graficos..."
foreach ($token in @('chartDescription','aria-labelledby','sr-only','registros. Valor inicial')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Acessibilidade de grafico ausente: $token" } }
Write-Host "    SVG possui descricao textual com valores essenciais: OK."

Write-Host "[1333/1336] Validando toque e densidade mobile..."
foreach ($token in @('touch-action:pan-y','max-height:220px','r:5.5','chart-foot{display:none}')) { if (-not $cssSource.Contains($token)) { throw "Tratamento mobile de grafico incompleto: $token" } }
Write-Host "    Graficos preservam scroll vertical e leitura compacta: OK."

Write-Host "[1334/1336] Validando telas estreitas e movimento reduzido..."
foreach ($token in @('@media(max-width:390px)','max-height:195px','@media(prefers-reduced-motion:reduce)','transition:none!important')) { if (-not $cssSource.Contains($token)) { throw "Fallback responsivo de grafico ausente: $token" } }
Write-Host "    Charts mantem legibilidade em celulares estreitos: OK."

Write-Host "[1335/1336] Validando schema preservado na v0.14.6..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.6_mobile_charts_progress.sql)) { throw "v0.14.6 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Charts & Progress entregue sem migration nova: OK."

Write-Host "[1336/1336] Validando versao funcional v0.14.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_CHARTS_PROGRESS='v0.14.6'")) { throw "Versao v0.14.6 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.6." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
foreach ($token in @('HP_MOBILE_MODALS_SHEETS','hpInstallModalSheetExperience','hp-modal-open','hpSheetEnter')) { if (-not ($cssSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Comportamento de modal/sheet ausente: $token" } }
Write-Host "    Modais e bottom sheets mobile consolidados: OK."

Write-Host "[1338/1344] Validando semantica e foco de dialogo..."
foreach ($token in @("role','dialog",'aria-modal','hpModalFocusable','hpModalLastFocus')) { if (-not $appJsSource.Contains($token)) { throw "Semantica/foco de modal incompleto: $token" } }
Write-Host "    Dialogos recebem semantica, foco inicial e restauracao: OK."

Write-Host "[1339/1344] Validando focus trap por teclado..."
foreach ($token in @("e.key!=='Tab'",'e.shiftKey','focusables[0]','focusables[focusables.length-1]')) { if (-not $appJsSource.Contains($token)) { throw "Focus trap ausente: $token" } }
Write-Host "    Navegacao por Tab permanece dentro do overlay ativo: OK."

Write-Host "[1340/1344] Validando fechamento seguro pelo backdrop..."
foreach ($token in @('hpBackdropPressed','pointerdown','pointerup','hpCloseModalBackdrop')) { if (-not $appJsSource.Contains($token)) { throw "Fechamento por backdrop incompleto: $token" } }
Write-Host "    Toque fora fecha overlay sem capturar toque interno: OK."

Write-Host "[1341/1344] Validando safe-area e bloqueio de scroll..."
foreach ($token in @('overscroll-behavior:none','env(safe-area-inset-bottom)','max-height:min(90dvh,820px)','touch-action:manipulation')) { if (-not $cssSource.Contains($token)) { throw "Tratamento mobile de sheet ausente: $token" } }
Write-Host "    Sheet respeita viewport, safe-area e fundo bloqueado: OK."

Write-Host "[1342/1344] Validando movimento reduzido..."
foreach ($token in @('@media(prefers-reduced-motion:reduce)','animation:none!important','hpBackdropIn')) { if (-not $cssSource.Contains($token)) { throw "Fallback de movimento reduzido ausente: $token" } }
Write-Host "    Animacoes respeitam preferencia de movimento reduzido: OK."

Write-Host "[1343/1344] Validando schema preservado na v0.14.7..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.7_mobile_modals_sheets.sql)) { throw "v0.14.7 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Modals & Bottom Sheets entregue sem migration nova: OK."

Write-Host "[1344/1344] Validando compatibilidade funcional v0.14.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_MODALS_SHEETS='v0.14.7'")) { throw "Compatibilidade v0.14.7 inconsistente na versao atual." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam a versao atual ao validar v0.14.7." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
foreach ($token in @('athlete-profile-mobile','athlete-profile-hero','athlete-profile-stats','athlete-profile-cycle')) { if (-not ($cssSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Perfil mobile incompleto: $token" } }
Write-Host "    Estrutura principal do perfil esportivo mobile: OK."

Write-Host "[1346/1352] Validando acesso ao perfil pelo menu Mais..."
if (-not $indexSource.Contains('data-patient-more-view="perfil"') -or -not $indexSource.Contains('Meu perfil') -or -not $appJsSource.Contains("'perfil'")) { throw "Navegacao para perfil mobile ausente." }
Write-Host "    Perfil acessivel pela navegacao mobile secundaria: OK."

Write-Host "[1347/1352] Validando identidade e resumo esportivo..."
foreach ($token in @('MEU PERFIL ESPORTIVO','athlete-profile-avatar','Consistência','Streak','Prontidão média')) { if (-not $appJsSource.Contains($token)) { throw "Resumo esportivo do perfil incompleto: $token" } }
Write-Host "    Identidade, nivel, XP, streak e consistencia reunidos: OK."

Write-Host "[1348/1352] Validando ciclo esportivo no perfil..."
foreach ($token in @('CICLO ATUAL','athlete-profile-cycle-track','meta/semana','progressoTemporalPercentual')) { if (-not $appJsSource.Contains($token)) { throw "Contexto de ciclo no perfil incompleto: $token" } }
Write-Host "    Ciclo atual e progresso temporal visiveis no perfil: OK."

Write-Host "[1349/1352] Validando contexto corporal e identidade basica..."
foreach ($token in @('MEU CORPO','Último contexto corporal','IDENTIDADE','Perfil esportivo','data-profile-jump="evolucao"')) { if (-not $appJsSource.Contains($token)) { throw "Contexto corporal/pessoal ausente: $token" } }
Write-Host "    Ultima avaliacao corporal e dados basicos integrados: OK."

Write-Host "[1350/1352] Validando responsividade do perfil..."
foreach ($token in @('@media(max-width:720px)','.athlete-profile-body-grid{grid-template-columns:1fr 1fr}','@media(max-width:390px)','.athlete-profile-personal-grid{grid-template-columns:1fr}')) { if (-not $cssSource.Contains($token)) { throw "Responsividade do perfil incompleta: $token" } }
Write-Host "    Perfil reorganiza identidade, metricas e corpo em telas estreitas: OK."

Write-Host "[1351/1352] Validando schema preservado na v0.14.8..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.8_athlete_profile_mobile.sql)) { throw "v0.14.8 nao deveria alterar schema 38/38." }
Write-Host "    Athlete Profile Mobile entregue sem migration nova: OK."

Write-Host "[1352/1352] Validando compatibilidade funcional v0.14.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_ATHLETE_PROFILE_MOBILE='v0.14.8'")) { throw "Compatibilidade v0.14.8 inconsistente na versao atual." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam a versao atual ao validar v0.14.8." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.14.8 / Athlete Profile Mobile: OK."


Write-Host "[1353/1360] Validando Mobile Accessibility & Polish..."
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
foreach ($token in @('HP_MOBILE_ACCESSIBILITY_POLISH','hpInstallAccessibilityPolish','hpEnhanceMobileAccessibility','hpAnnounce')) { if (-not $appJsSource.Contains($token)) { throw "Acessibilidade mobile incompleta: $token" } }
Write-Host "    Camada transversal de acessibilidade instalada: OK."

Write-Host "[1354/1360] Validando skip link e regiao viva..."
foreach ($token in @('hp-skip-link','Pular para o conteúdo','hpA11yStatus','aria-live="polite"')) { if (-not ($indexSource.Contains($token) -or $cssSource.Contains($token))) { throw "Navegacao assistiva incompleta: $token" } }
Write-Host "    Skip link e anuncios assistivos presentes: OK."

Write-Host "[1355/1360] Validando estado atual da navegacao..."
foreach ($token in @('aria-current','page','Navegação do atleta','[aria-current="page"]')) { if (-not ($appJsSource.Contains($token) -or $indexSource.Contains($token) -or $cssSource.Contains($token))) { throw "Estado acessivel de navegacao ausente: $token" } }
Write-Host "    Secao ativa e identificavel por tecnologia assistiva: OK."

Write-Host "[1356/1360] Validando foco e alvos de toque..."
foreach ($token in @(':focus-visible','min-height:44px','min-width:44px','touch-action:manipulation')) { if (-not $cssSource.Contains($token)) { throw "Foco/alvo touch incompleto: $token" } }
Write-Host "    Foco visivel e alvos minimos de toque: OK."

Write-Host "[1357/1360] Validando contraste e movimento reduzido..."
foreach ($token in @('@media(prefers-contrast:more)','@media(prefers-reduced-motion:reduce)','animation-duration:.01ms','scroll-behavior:auto!important')) { if (-not $cssSource.Contains($token)) { throw "Preferencia de acessibilidade ausente: $token" } }
Write-Host "    Contraste aumentado e reduced motion respeitados: OK."

Write-Host "[1358/1360] Validando acabamento para telas estreitas..."
foreach ($token in @('overflow-wrap:anywhere','text-wrap:pretty','text-wrap:balance','tabindex="-1"')) { if (-not ($cssSource.Contains($token) -or $indexSource.Contains($token))) { throw "Polimento mobile incompleto: $token" } }
Write-Host "    Wrapping e foco programatico do conteudo principal: OK."

Write-Host "[1359/1360] Validando schema preservado na v0.14.9..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.14.9_mobile_accessibility_polish.sql)) { throw "v0.14.9 nao deveria alterar schema 38/38." }
Write-Host "    Mobile Accessibility & Polish entregue sem migration nova: OK."

Write-Host "[1360/1360] Validando versao funcional v0.14.9..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MOBILE_ACCESSIBILITY_POLISH='v0.14.9'")) { throw "Versao v0.14.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.14.9." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
foreach ($token in @('HP_ATHLETE_HOME_2','hpAthleteHome2','HOJE EM UM OLHAR','O que você precisa fazer hoje')) { if (-not $appJsSource.Contains($token)) { throw "Athlete Home 2.0 / Patient Home atual incompleta: $token" } }
Write-Host "    Camada integrada da Home 2.0 presente: OK."

Write-Host "[1362/1368] Validando quatro perguntas operacionais da Home..."
foreach ($token in @('CHECK-IN','TREINO','ALIMENTAÇÃO','HIDRATAÇÃO','MENSAGENS','PENDÊNCIAS')) { if (-not $appJsSource.Contains($token)) { throw "Prioridade operacional ausente: $token" } }
Write-Host "    Estado, proxima acao, andamento e evolucao reunidos: OK."

Write-Host "[1363/1368] Validando reuso dos dados esportivos existentes..."
foreach ($token in @('d?.estrategiaDoDia','d?.hidratacaoContextual','d?.execucaoDoDia','d?.gamificacao','d?.cicloEsportivoAtual')) { if (-not $appJsSource.Contains($token)) { throw "Dado esportivo nao reutilizado na Home 2.0: $token" } }
Write-Host "    Home 2.0 nao cria fonte paralela de dados: OK."

Write-Host "[1364/1368] Validando acoes diretas da Home 2.0..."
foreach ($token in @('athleteHome2Body','athleteHome2Now','athleteHome2Today','patientTodayNutrition','patientTodayChat','patientTodayRequests')) { if (-not $appJsSource.Contains($token)) { throw "Acao da Home atual incompleta: $token" } }
Write-Host "    Cards operacionais levam ao fluxo correspondente: OK."

Write-Host "[1365/1368] Validando responsividade da Athlete Home 2.0..."
foreach ($token in @('.athlete-home2{','.athlete-home2-grid{display:grid','grid-template-columns:repeat(4,minmax(0,1fr))','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Athlete Home 2.0 incompleta: $token" } }
Write-Host "    Hierarquia desktop/mobile da Home 2.0: OK."

Write-Host "[1366/1368] Validando reduced motion e ergonomia touch..."
foreach ($token in @('prefers-reduced-motion:reduce','min-height:104px','cursor:pointer')) { if (-not $cssSource.Contains($token)) { throw "Ergonomia da Home 2.0 incompleta: $token" } }
Write-Host "    Interacao mobile e reduced motion preservados: OK."

Write-Host "[1367/1368] Validando schema preservado na v0.15.1..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.15.1_athlete_home_2.sql)) { throw "v0.15.1 nao deveria alterar schema 38/38." }
Write-Host "    Athlete Home 2.0 entregue sem migration nova: OK."

Write-Host "[1368/1368] Validando versao funcional v0.15.1..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_ATHLETE_HOME_2='v0.15.0'")) { throw "Versao v0.15.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.15.1." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.15.1 / Athlete Home 2.0: OK."

Write-Host "[1369/1376] Validando Morning Check-in..." -ForegroundColor Cyan
foreach ($token in @('HP_MORNING_CHECKIN','hpMorningCheckin','MORNING CHECK-IN','morningCheckinAction')) { if (-not $appJsSource.Contains($token)) { throw "Morning Check-in incompleto: $token" } }
Write-Host "[1370/1376] Validando fatores do check-in matinal..." -ForegroundColor Cyan
foreach ($token in @('Sono','Energia','Dor','Disposição','Recuperação')) { if (-not $appJsSource.Contains($token)) { throw "Fator Morning Check-in ausente: $token" } }
Write-Host "[1371/1376] Validando integração com prontidão existente..." -ForegroundColor Cyan
if (-not $appJsSource.Contains("openDailyReadiness(readiness)") -or -not $appJsSource.Contains("/api/portal/me/prontidao")) { throw "Morning Check-in nao reutiliza a prontidao existente." }
Write-Host "[1372/1376] Validando experiência mobile do Morning Check-in..." -ForegroundColor Cyan
foreach ($token in @('.morning-checkin{display:grid','.morning-checkin-factors','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Morning Check-in incompleta: $token" } }
Write-Host "[1373/1376] Validando guia do formulário matinal..." -ForegroundColor Cyan
foreach ($token in @('morning-checkin-intro','morning-checkin-guide','Leva menos de 1 minuto.')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Guia Morning Check-in incompleto: $token" } }
Write-Host "[1374/1376] Validando acessibilidade do Morning Check-in..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('aria-label="Morning Check-in"') -or -not $cssSource.Contains('prefers-reduced-motion:reduce')) { throw "Acessibilidade Morning Check-in incompleta." }
...
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_MORNING_CHECKIN='v0.15.1'")) { throw "Compatibilidade Morning Check-in inconsistente na versao atual." }
...
foreach ($token in @('HP_TRAINING_DAY_FLOW','hpTrainingDayFlow','TRAINING DAY FLOW','trainingDayFlowAction')) { if (-not $appJsSource.Contains($token)) { throw "Training Day Flow incompleto: $token" } }
Write-Host "    Fluxo Contexto -> Executar -> Fechar presente: OK."

Write-Host "[1378/1384] Validando reuso de prontidao e estrategia..." -ForegroundColor Cyan
foreach ($token in @('home?.prontidaoDiaria','home?.estrategiaDoDia','recomendacaoTreino','intensidadeSugerida')) { if (-not $appJsSource.Contains($token)) { throw "Contexto esportivo ausente no Training Day Flow: $token" } }
Write-Host "    Readiness e estrategia do dia reutilizados sem fonte paralela: OK."

Write-Host "[1379/1384] Validando jornada operacional do treino..." -ForegroundColor Cyan
foreach ($token in @('Fazer check-in','Iniciar treino','Revisar sessão','openWorkoutExecutionForm','openWorkoutSessionReview')) { if (-not $appJsSource.Contains($token)) { throw "Acao operacional ausente no Training Day Flow: $token" } }
Write-Host "    CTA acompanha o estado real do dia: OK."

Write-Host "[1380/1384] Validando fechamento com RPE e duracao..." -ForegroundColor Cyan
foreach ($token in @('RPE + duração','duracaoMinutos','esforcoPercebido','training-execution-journey')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Fechamento do treino incompleto: $token" } }
Write-Host "    Execucao e fechamento permanecem no mesmo fluxo: OK."

Write-Host "[1381/1384] Validando protecao de recuperacao e prescricao..." -ForegroundColor Cyan
foreach ($token in @('Atenção à recuperação','prescrição profissional continua sendo a referência','Recuperacao','Leve')) { if (-not $appJsSource.Contains($token)) { throw "Protecao contextual do Training Day Flow ausente: $token" } }
Write-Host "    Readiness contextualiza sem reescrever prescricao: OK."

Write-Host "[1382/1384] Validando responsividade do Training Day Flow..." -ForegroundColor Cyan
foreach ($token in @('.training-day-flow{display:grid','.training-day-flow-steps{display:grid','@media(max-width:720px)','@media(max-width:390px)','training-execution-journey')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Training Day Flow incompleta: $token" } }
Write-Host "    Fluxo reorganiza etapas e CTA em telas estreitas: OK."

Write-Host "[1383/1392] Validando schema preservado no Training Day Flow..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.15.2_training_day_flow.sql)) { throw "v0.15.2 nao deveria alterar schema 38/38." }
Write-Host "    Training Day Flow entregue sem migration nova: OK."

Write-Host "[1384/1392] Validando compatibilidade do Training Day Flow..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_TRAINING_DAY_FLOW='v0.15.2'")) { throw "Compatibilidade Training Day Flow inconsistente na versao atual." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam a versao atual." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.15.2 / Training Day Flow: OK." -ForegroundColor Green


Write-Host "[1385/1392] Validando Recovery Day Flow..." -ForegroundColor Cyan
foreach ($token in @('HP_RECOVERY_DAY_FLOW','hpRecoveryDayFlow','RECOVERY DAY FLOW','recoveryDayFlowAction')) { if (-not $appJsSource.Contains($token)) { throw "Recovery Day Flow incompleto: $token" } }

Write-Host "[1386/1392] Validando contexto de recuperacao..." -ForegroundColor Cyan
foreach ($token in @("['Recuperacao','Leve']",'prontidaoDiaria','planoRecuperacao','execucaoDoDia')) { if (-not $appJsSource.Contains($token)) { throw "Contexto do Recovery Day Flow ausente: $token" } }

Write-Host "[1387/1392] Validando jornada Contexto Recuperar Reavaliar..." -ForegroundColor Cyan
foreach ($token in @('CONTEXTO','RECUPERAR','REAVALIAR','Recuperação planejada também é execução correta do plano')) { if (-not $appJsSource.Contains($token)) { throw "Jornada Recovery Day Flow incompleta: $token" } }

Write-Host "[1388/1392] Validando acao de registro da recuperacao..." -ForegroundColor Cyan
foreach ($token in @('Registrar como estou','openPatientQuickLog','recoveryDayFlowAction')) { if (-not $appJsSource.Contains($token)) { throw "Acao Recovery Day Flow ausente: $token" } }

Write-Host "[1389/1392] Validando integracao Recovery + Training Day Flow..." -ForegroundColor Cyan
foreach ($token in @('${hpRecoveryDayFlow(home)}','${hpTrainingDayFlow(home,p,h)}','prescrição profissional continua sendo a referência')) { if (-not $appJsSource.Contains($token)) { throw "Integracao Recovery/Training incompleta: $token" } }

Write-Host "[1390/1392] Validando responsividade do Recovery Day Flow..." -ForegroundColor Cyan
foreach ($token in @('.recovery-day-flow{display:grid','.recovery-day-flow-steps{display:grid','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Recovery Day Flow incompleta: $token" } }

Write-Host "[1391/1392] Validando schema preservado na v0.15.3..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.15.3_recovery_day_flow.sql)) { throw "v0.15.3 nao deveria alterar schema 38/38." }
...
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_RECOVERY_DAY_FLOW='v0.15.3'")) { throw "Compatibilidade Recovery Day Flow inconsistente na versao atual." }
...
foreach ($token in @('HP_END_OF_DAY_FLOW','hpEndOfDayFlow','END-OF-DAY FLOW','endOfDayFlowAction')) { if (-not $appJsSource.Contains($token)) { throw "End-of-Day Flow incompleto: $token" } }

Write-Host "[1394/1400] Validando consolidacao do dia..." -ForegroundColor Cyan
foreach ($token in @('execucaoDoDia','hidratacaoContextual','prontidaoDiaria','respostaSessao','CORPO','SESSÃO','HIDRATAÇÃO','ROTEIRO')) { if (-not $appJsSource.Contains($token)) { throw "Contexto ausente no End-of-Day Flow: $token" } }

Write-Host "[1395/1400] Validando reuso do fechamento existente..." -ForegroundColor Cyan
foreach ($token in @('openCloseAthleteDay','/api/portal/me/fechamento-dia','Encerrar meu dia','Revisar fechamento')) { if (-not $appJsSource.Contains($token)) { throw "Fechamento existente nao reutilizado: $token" } }

Write-Host "[1396/1400] Validando protecao contra compensacao..." -ForegroundColor Cyan
foreach ($token in @('não cria obrigação de completar pendências','não compense volume no fim do dia','não cria obrigação')) { if (-not $appJsSource.Contains($token)) { throw "Protecao do End-of-Day Flow ausente: $token" } }

Write-Host "[1397/1400] Validando integracao na Home do atleta..." -ForegroundColor Cyan
foreach ($token in @('${hpEndOfDayFlow(d)}','${hpDailyClosureCard(d.execucaoDoDia)}','endOfDayFlowAction')) { if (-not $appJsSource.Contains($token)) { throw "Integracao End-of-Day/Home incompleta: $token" } }

Write-Host "[1398/1400] Validando responsividade do End-of-Day Flow..." -ForegroundColor Cyan
foreach ($token in @('.end-of-day-flow{display:grid','.end-of-day-flow-review{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade End-of-Day Flow incompleta: $token" } }

Write-Host "[1399/1400] Validando schema preservado na v0.15.4..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.15.4_end_of_day_flow.sql)) { throw "v0.15.4 nao deveria alterar schema 38/38." }
Write-Host "    End-of-Day Flow entregue sem migration nova: OK."

Write-Host "[1400/1400] Validando versao funcional v0.15.4..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_END_OF_DAY_FLOW='v0.15.4'")) { throw "Versao v0.15.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.15.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
foreach ($token in @('HP_WEEKLY_REVIEW','hpWeeklyReview','WEEKLY REVIEW','weeklyReviewAction')) { if (-not $appJsSource.Contains($token)) { throw "Weekly Review incompleto: $token" } }

Write-Host "[1402/1408] Validando sintese de treinos e consistencia..." -ForegroundColor Cyan
foreach ($token in @('treinosConcluidosSemana','metaTreinosSemana','consistenciaScore','desafiosSemana')) { if (-not $appJsSource.Contains($token)) { throw "Sintese Weekly Review incompleta: $token" } }

Write-Host "[1403/1408] Validando contexto longitudinal semanal..." -ForegroundColor Cyan
foreach ($token in @('resumoSemanal','tendenciaSemanal','cargaIndividualizada','FOCO DA PRÓXIMA SEMANA')) { if (-not $appJsSource.Contains($token)) { throw "Contexto Weekly Review incompleto: $token" } }

Write-Host "[1404/1408] Validando protecao da proxima semana..." -ForegroundColor Cyan
foreach ($token in @('Recuperação e retorno gradual','Consolidar o que funcionou','plano profissional continua sendo a referência')) { if (-not $appJsSource.Contains($token)) { throw "Protecao Weekly Review ausente: $token" } }

Write-Host "[1405/1408] Validando integracao do Weekly Review na Home..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('${hpWeeklyReview(d)}') -or -not $appJsSource.Contains("querySelector('.weekly-summary-card')")) { throw "Integracao Weekly Review/Home incompleta." }

Write-Host "[1406/1408] Validando responsividade do Weekly Review..." -ForegroundColor Cyan
foreach ($token in @('.weekly-review{display:grid','.weekly-review-grid{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Weekly Review incompleta: $token" } }

Write-Host "[1407/1408] Validando schema preservado na v0.15.5..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.15.5_weekly_review.sql)) { throw "v0.15.5 nao deveria alterar schema 38/38." }
Write-Host "    Weekly Review entregue sem migration nova: OK."

Write-Host "[1408/1408] Validando versao funcional v0.15.5..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_WEEKLY_REVIEW='v0.15.5'")) { throw "Versao v0.15.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.15.5." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.15.5 / Weekly Review: OK." -ForegroundColor Green



Write-Host "[1409/1416] Validando Athlete Progress Story..." -ForegroundColor Cyan
foreach ($token in @('HP_ATHLETE_PROGRESS_STORY','hpAthleteProgressStory','SUA HISTÓRIA DE EVOLUÇÃO','athleteProgressStoryAction')) { if (-not $appJsSource.Contains($token)) { throw "Athlete Progress Story incompleta: $token" } }

Write-Host "[1410/1416] Validando fonte longitudinal da Progress Story..." -ForegroundColor Cyan
foreach ($token in @('evolucaoEsportiva','evo.itens','Favoravel','Atencao','Estavel')) { if (-not $appJsSource.Contains($token)) { throw "Fonte longitudinal Progress Story incompleta: $token" } }

Write-Host "[1411/1416] Validando narrativa de progresso..." -ForegroundColor Cyan
foreach ($token in @('Seu momento mostra evolução em mais de uma frente','Sua evolução continua, com sinais que merecem contexto','Sua base está sendo construída com consistência','PRÓXIMO CAPÍTULO')) { if (-not $appJsSource.Contains($token)) { throw "Narrativa Progress Story incompleta: $token" } }

Write-Host "[1412/1416] Validando protecao da Athlete Progress Story..." -ForegroundColor Cyan
foreach ($token in @('sem mudar o plano por conta própria','não substitui a interpretação profissional')) { if (-not $appJsSource.Contains($token)) { throw "Protecao Progress Story ausente: $token" } }

Write-Host "[1413/1416] Validando integracao da Progress Story na Home..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('${hpAthleteProgressStory(d)}') -or -not $appJsSource.Contains("loadPatientSection('evolucao')")) { throw "Integracao Athlete Progress Story/Home incompleta." }

Write-Host "[1414/1416] Validando responsividade da Progress Story..." -ForegroundColor Cyan
foreach ($token in @('.athlete-progress-story{display:grid','.progress-story-path{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Athlete Progress Story incompleta: $token" } }

Write-Host "[1415/1416] Validando schema preservado na v0.15.6..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.15.6_athlete_progress_story.sql)) { throw "v0.16.0 nao deveria alterar schema 38/38." }
Write-Host "    Athlete Progress Story entregue sem migration nova: OK."

Write-Host "[1416/1416] Validando compatibilidade funcional v0.15.6..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_ATHLETE_PROGRESS_STORY='v0.15.6'")) { throw "Versao v0.16.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.16.0." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.16.0 / Athlete Progress Story: OK." -ForegroundColor Green


Write-Host "[1417/1424] Validando Athlete Level System..." -ForegroundColor Cyan
foreach ($token in @('HP_ATHLETE_LEVEL_SYSTEM','hpAthleteLevelSystem','ATHLETE LEVEL SYSTEM','athlete-level-orb')) { if (-not $appJsSource.Contains($token)) { throw "Athlete Level System incompleto: $token" } }

Write-Host "[1418/1424] Validando progressao de nivel existente..." -ForegroundColor Cyan
foreach ($token in @('xpNoNivel','xpProximoNivel','consistenciaScore','streakDias','xpHoje')) { if (-not $appJsSource.Contains($token)) { throw "Progressao Athlete Level System incompleta: $token" } }

Write-Host "[1419/1424] Validando identidades de nivel..." -ForegroundColor Cyan
foreach ($token in @('Fundação','Ritmo','Consistente','Performance sustentável','Referência')) { if (-not $appJsSource.Contains($token)) { throw "Identidade de nivel ausente: $token" } }

Write-Host "[1420/1424] Validando gamificacao segura..." -ForegroundColor Cyan
foreach ($token in @('Recuperar também é progresso','Treino pesado não vale mais por ser pesado','adequação à prontidão','não rende progresso extra')) { if (-not $appJsSource.Contains($token)) { throw "Protecao Athlete Level System ausente: $token" } }

Write-Host "[1421/1424] Validando integracao do Level System na Home..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('${hpAthleteLevelSystem(d)}') -or -not $appJsSource.Contains('${hpGamification2AthleteCard(d.gamificacao2)}')) { throw "Integracao Athlete Level System/Home incompleta." }

Write-Host "[1422/1424] Validando responsividade do Athlete Level System..." -ForegroundColor Cyan
foreach ($token in @('.athlete-level-system{display:grid','.athlete-level-signals{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Athlete Level System incompleta: $token" } }

Write-Host "[1423/1424] Validando schema preservado na v0.16.0..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.16.0_athlete_level_system.sql)) { throw "v0.16.0 nao deveria alterar schema 38/38." }
Write-Host "    Athlete Level System entregue sem migration nova: OK."

Write-Host "[1424/1424] Validando versao funcional v0.16.0..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_ATHLETE_LEVEL_SYSTEM='v0.16.0'")) { throw "Versao corrente v0.16.1 inconsistente no baseline do Level System." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.16.1." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
foreach ($token in @('HP_STREAK_INTELLIGENCE','hpStreakIntelligence','STREAK INTELLIGENCE','streak-intelligence-ring')) { if (-not $appJsSource.Contains($token)) { throw "Streak Intelligence incompleta: $token" } }

Write-Host "[1426/1432] Validando sinais usados no Streak Intelligence..." -ForegroundColor Cyan
foreach ($token in @('streakDias','diasAtivos14','consistenciaScore','missoesConcluidas','totalMissoes')) { if (-not $appJsSource.Contains($token)) { throw "Sinal Streak Intelligence ausente: $token" } }

Write-Host "[1427/1432] Validando classificacoes da sequencia..." -ForegroundColor Cyan
foreach ($token in @('Sequência protegida','Sequência sustentável','Sequência em construção','Retomada inteligente')) { if (-not $appJsSource.Contains($token)) { throw "Classificacao Streak Intelligence ausente: $token" } }

Write-Host "[1428/1432] Validando protecao do streak..." -ForegroundColor Cyan
foreach ($token in @('Recuperação planejada não vira falha','Streak não é obrigação de treinar','excesso não cria mérito extra','sem exigir treino para manter streak')) { if (-not $appJsSource.Contains($token)) { throw "Protecao Streak Intelligence ausente: $token" } }

Write-Host "[1429/1432] Validando integracao do Streak Intelligence na Home..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('${hpStreakIntelligence(d)}') -or -not $appJsSource.Contains("HP_ATHLETE_LEVEL_SYSTEM='v0.16.0'")) { throw "Integracao Streak Intelligence/Home incompleta." }

Write-Host "[1430/1432] Validando responsividade do Streak Intelligence..." -ForegroundColor Cyan
foreach ($token in @('.streak-intelligence{display:grid','.streak-intelligence-grid{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Streak Intelligence incompleta: $token" } }

Write-Host "[1431/1432] Validando schema preservado na v0.16.1..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.16.1_streak_intelligence.sql)) { throw "v0.16.1 nao deveria alterar schema 38/38." }
Write-Host "    Streak Intelligence entregue sem migration nova: OK."

Write-Host "[1432/1432] Validando versao funcional v0.16.1..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_STREAK_INTELLIGENCE='v0.16.1'")) { throw "Versao v0.16.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.16.1." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
foreach ($token in @('HP_DYNAMIC_MISSIONS','hpDynamicMissions','DYNAMIC MISSIONS','PRÓXIMA MISSÃO ÚTIL')) { if (-not $appJsSource.Contains($token)) { throw "Dynamic Missions incompleta: $token" } }

Write-Host "[1434/1440] Validando fonte contextual das Dynamic Missions..." -ForegroundColor Cyan
foreach ($token in @('missoesContextuais2','focoGamificadoDoDia','prontidaoDiaria','estrategiaDoDia','scoreMission')) { if (-not $appJsSource.Contains($token)) { throw "Fonte Dynamic Missions ausente: $token" } }

Write-Host "[1435/1440] Validando adaptacao por recuperacao..." -ForegroundColor Cyan
foreach ($token in @('Missões adaptadas à recuperação','RecuperacaoProtegida','PriorizarResposta','Nenhuma missão exige treino para manter progresso')) { if (-not $appJsSource.Contains($token)) { throw "Adaptacao Dynamic Missions ausente: $token" } }

Write-Host "[1436/1440] Validando protecao da gamificacao dinamica..." -ForegroundColor Cyan
foreach ($token in @('não substituem o plano profissional','excesso não gera mérito extra','não pela busca de intensidade')) { if (-not $appJsSource.Contains($token)) { throw "Protecao Dynamic Missions ausente: $token" } }

Write-Host "[1437/1440] Validando integracao das Dynamic Missions na Home..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('${hpDynamicMissions(d)}') -or -not $appJsSource.Contains('dynamicMissionsDetails')) { throw "Integracao Dynamic Missions/Home incompleta." }

Write-Host "[1438/1440] Validando responsividade das Dynamic Missions..." -ForegroundColor Cyan
foreach ($token in @('.dynamic-missions{display:grid','.dynamic-missions-list{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Dynamic Missions incompleta: $token" } }

Write-Host "[1439/1440] Validando schema preservado na v0.16.2..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.16.2_dynamic_missions.sql)) { throw "v0.16.2 nao deveria alterar schema 38/38." }
Write-Host "    Dynamic Missions entregue sem migration nova: OK."

Write-Host "[1440/1440] Validando versao funcional v0.16.2..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_DYNAMIC_MISSIONS='v0.16.2'")) { throw "Versao v0.16.2 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.16.2." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
...
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_ACHIEVEMENT_FAMILIES='v0.16.3'")) { throw "Versao v0.16.3 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.16.3." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.16.3 / Achievement Families: OK." -ForegroundColor Green

Write-Host "[1449/1456] Validando Weekly Challenges..." -ForegroundColor Cyan
foreach ($token in @('HP_WEEKLY_CHALLENGES','hpWeeklyChallenges','WEEKLY CHALLENGES','weekly-challenges-intelligent')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Weekly Challenges incompleto: $token" } }

Write-Host "[1450/1456] Validando fonte semanal existente..." -ForegroundColor Cyan
foreach ($token in @('game.desafiosSemana','prontidaoDiaria','missoesContextuais2','planejamentoSemanal','resumoSemanal')) { if (-not $appJsSource.Contains($token)) { throw "Fonte Weekly Challenges ausente: $token" } }

Write-Host "[1451/1456] Validando protecao de recuperacao semanal..." -ForegroundColor Cyan
foreach ($token in @('Seu desafio é proteger a semana','Recuperação planejada','Não há missão de compensar carga perdida')) { if (-not $appJsSource.Contains($token)) { throw "Protecao Weekly Challenges ausente: $token" } }

Write-Host "[1452/1456] Validando gamificacao semanal segura..." -ForegroundColor Cyan
foreach ($token in @('excesso não rende progresso extra','nunca pedem compensação de treino perdido','não substituem o plano profissional')) { if (-not $appJsSource.Contains($token)) { throw "Principio seguro Weekly Challenges ausente: $token" } }

Write-Host "[1453/1456] Validando integracao Weekly Challenges/Home..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('${hpWeeklyChallenges(d)}') -or -not $appJsSource.Contains('weeklyChallengesDetails') -or -not $appJsSource.Contains("HP_ACHIEVEMENT_FAMILIES='v0.16.3'")) { throw "Integracao Weekly Challenges/Home incompleta." }

Write-Host "[1454/1456] Validando responsividade Weekly Challenges..." -ForegroundColor Cyan
foreach ($token in @('.weekly-challenges-intelligent{display:grid','.weekly-challenges-steps{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Weekly Challenges incompleta: $token" } }

Write-Host "[1455/1456] Validando schema preservado na v0.16.4..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.16.4_weekly_challenges.sql)) { throw "v0.16.4 nao deveria alterar schema 38/38." }
Write-Host "    Weekly Challenges entregue sem migration nova: OK."

Write-Host "[1456/1456] Validando versao funcional v0.16.4..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_WEEKLY_CHALLENGES='v0.16.4'")) { throw "Versao v0.16.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.16.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.16.4 / Weekly Challenges: OK." -ForegroundColor Green

Write-Host "[1457/1464] Validando Seasonal Progress..." -ForegroundColor Cyan
foreach ($token in @('HP_SEASONAL_PROGRESS','hpSeasonalProgress','SEASONAL PROGRESS','seasonal-progress')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Seasonal Progress incompleto: $token" } }

Write-Host "[1458/1464] Validando fonte longitudinal do ciclo..." -ForegroundColor Cyan
foreach ($token in @('cicloEsportivoAtual','semanaAtual','totalSemanas','progressoTemporalPercentual','treinosNoCiclo','mediaProntidao','consistenciaScore')) { if (-not $appJsSource.Contains($token)) { throw "Fonte Seasonal Progress ausente: $token" } }

Write-Host "[1459/1464] Validando narrativa sazonal..." -ForegroundColor Cyan
foreach ($token in @('Fundação','Construção','Consolidação','Fechamento do ciclo','OBJETIVO DO CICLO')) { if (-not $appJsSource.Contains($token)) { throw "Narrativa Seasonal Progress ausente: $token" } }

Write-Host "[1460/1464] Validando principio esportivo seguro do ciclo..." -ForegroundColor Cyan
foreach ($token in @('recuperação bem executada faz parte do progresso sazonal','não picos isolados de intensidade')) { if (-not $appJsSource.Contains($token)) { throw "Protecao Seasonal Progress ausente: $token" } }

Write-Host "[1461/1464] Validando integracao Seasonal Progress/Home..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('${hpSeasonalProgress(d)}') -or -not $appJsSource.Contains("HP_WEEKLY_CHALLENGES='v0.16.4'")) { throw "Integracao Seasonal Progress/Home incompleta." }

Write-Host "[1462/1464] Validando responsividade Seasonal Progress..." -ForegroundColor Cyan
foreach ($token in @('.seasonal-progress{display:grid','.seasonal-progress-grid{display:grid','@media(max-width:720px)','@media(max-width:390px)','prefers-reduced-motion:reduce')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Seasonal Progress incompleta: $token" } }

Write-Host "[1463/1464] Validando schema preservado na v0.16.5..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.16.5_seasonal_progress.sql)) { throw "v0.16.5 nao deveria alterar schema 38/38." }
Write-Host "    Seasonal Progress entregue sem migration nova: OK."

Write-Host "[1464/1464] Validando versao funcional v0.16.5..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_SEASONAL_PROGRESS='v0.16.5'")) { throw "Versao v0.16.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.16.5." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.16.5 / Seasonal Progress: OK." -ForegroundColor Green

Write-Host "[1465/1472] Validando Professional Prescription Workspace..." -ForegroundColor Cyan
foreach ($token in @('HP_PROFESSIONAL_PRESCRIPTION_WORKSPACE','loadPrescriptionWorkspace','PROFESSIONAL PRESCRIPTION WORKSPACE','prescription-workspace-grid')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Professional Prescription Workspace incompleto: $token" } }

Write-Host "[1466/1472] Validando fluxo profissional alvo..." -ForegroundColor Cyan
foreach ($token in @('Avaliação inicial','Direção + sugestão inicial','Treino + alimentação','Revisar & publicar','Avaliação → sugestão → ajuste → atribuição → acompanhamento')) { if (-not $appJsSource.Contains($token)) { throw "Fluxo profissional ausente: $token" } }

Write-Host "[1467/1472] Validando bibliotecas reutilizaveis..." -ForegroundColor Cyan
foreach ($token in @('/api/modelos-planos-treino','/api/modelos-planos-alimentares','/api/modelos-sessoes-treino','/api/modelos-refeicoes')) { if (-not $appJsSource.Contains($token)) { throw "Biblioteca profissional nao integrada: $token" } }

Write-Host "[1468/1472] Validando acesso ao workspace..." -ForegroundColor Cyan
...
foreach ($token in @('.prescription-workspace-grid{display:grid','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Professional Prescription Workspace incompleta: $token" } }

Write-Host "[1471/1472] Validando schema preservado na v0.17.0..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.0_professional_prescription_workspace.sql)) { throw "v0.17.0 nao deveria alterar schema 38/38." }

Write-Host "[1472/1472] Validando versao funcional v0.17.0..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_PROFESSIONAL_PRESCRIPTION_WORKSPACE='v0.17.0'")) { throw "Versao v0.17.0 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.3." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.0 / Professional Prescription Workspace: OK." -ForegroundColor Green


Write-Host "[1473/1480] Validando Patient Intake & Body Assessment..." -ForegroundColor Cyan
foreach ($token in @('HP_PATIENT_INTAKE_BODY_ASSESSMENT','openPatientIntake','PATIENT INTAKE • v0.17.1','Composição corporal')) { if (-not $appJsSource.Contains($token)) { throw "Patient Intake incompleto: $token" } }

Write-Host "[1474/1480] Validando cadastro rapido + continuidade opcional..." -ForegroundColor Cyan
foreach ($token in @('Paciente cadastrado. Complete a avaliação inicial agora ou depois.','Fazer depois','Editar cadastro','o preenchimento é progressivo e pode ser retomado depois')) { if (-not $appJsSource.Contains($token)) { throw "Fluxo retomavel do intake incompleto: $token" } }

Write-Host "[1475/1480] Validando campos corporais/bioimpedancia suportados..." -ForegroundColor Cyan
foreach ($token in @('pesoKg','alturaM','percentualGordura','massaMagraKg','massaGordaKg','cinturaCm','abdomenCm','quadrilCm')) { if (-not $appJsSource.Contains($token)) { throw "Campo corporal ausente no intake: $token" } }

Write-Host "[1476/1480] Validando persistencia pelo endpoint existente de avaliacoes..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('/avaliacoes') -or -not $appJsSource.Contains("method:'POST'") -or -not $appJsSource.Contains('Avaliação corporal salva. Próxima etapa: objetivo do tratamento.')) { throw "Persistencia do intake nao reutiliza Avaliacoes." }

Write-Host "[1477/1480] Validando acesso ao intake pelo prontuario..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('id="patientIntake"') -or -not $appJsSource.Contains('$(''#patientIntake'').onclick=()=>openPatientIntake(p)')) { throw "Acesso ao Patient Intake pelo prontuario ausente." }

Write-Host "[1478/1480] Validando responsividade do Patient Intake..." -ForegroundColor Cyan
foreach ($token in @('.intake-context{display:grid','.intake-progress{display:flex','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Patient Intake incompleta: $token" } }

Write-Host "[1479/1480] Validando schema preservado na v0.17.1..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.1_patient_intake_body_assessment.sql)) { throw "v0.17.1 nao deveria alterar schema 38/38." }

Write-Host "[1480/1480] Validando versao funcional v0.17.1..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_PATIENT_INTAKE_BODY_ASSESSMENT='v0.17.1'")) { throw "Versao v0.17.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.3." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.1 / Patient Intake & Body Assessment: OK." -ForegroundColor Green

Write-Host "[1481/1488] Validando Treatment Goal & Initial Recommendation..." -ForegroundColor Cyan
foreach ($token in @('HP_TREATMENT_GOAL_INITIAL_RECOMMENDATION','openTreatmentGoalRecommendation','TREATMENT GOAL • v0.17.2','buildInitialRecommendationDraft','initial-recommendation')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Treatment Goal incompleto: $token" } }

Write-Host "[1482/1488] Validando objetivos suportados..." -ForegroundColor Cyan
foreach ($token in @('Hipertrofia','Perda de peso','Recomposição corporal','Condicionamento','Qualidade de vida','Manutenção','Retorno ao exercício')) { if (-not $appJsSource.Contains($token)) { throw "Objetivo de tratamento ausente: $token" } }

Write-Host "[1483/1488] Validando sugestao assistida de hidratacao/treino/nutricao..." -ForegroundColor Cyan
foreach ($token in @('Hidratação','Treino de força','Condicionamento','Direção nutricional','hydrationLow','hydrationHigh','ml/dia')) { if (-not $appJsSource.Contains($token)) { throw "Sugestao inicial incompleta: $token" } }

Write-Host "[1484/1488] Validando adaptacao rapida da recomendacao..." -ForegroundColor Cyan
foreach ($token in @('data-water-delta','Editar contexto','Adaptar treino','Adaptar alimentação','Preferências / restrições relevantes')) { if (-not $appJsSource.Contains($token)) { throw "Tuning da recomendacao ausente: $token" } }

Write-Host "[1485/1488] Validando seguranca e decisao profissional..." -ForegroundColor Cyan
foreach ($token in @('nunca publicação automática','revisão profissional','O profissional revisa hidratação, volume, intensidade, energia, macros, refeições, contraindicações e preferências')) { if (-not $appJsSource.Contains($token)) { throw "Protecao profissional ausente: $token" } }

Write-Host "[1486/1488] Validando integracao intake -> objetivo -> prontuario..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('await openTreatmentGoalRecommendation(p)') -or -not $appJsSource.Contains('id="patientGoalRecommendation"') -or -not $appJsSource.Contains('$(''#patientGoalRecommendation'').onclick=()=>openTreatmentGoalRecommendation(p)')) { throw "Integracao Treatment Goal incompleta." }

Write-Host "[1487/1488] Validando schema preservado na v0.17.3..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.2_treatment_goal_initial_recommendation.sql)) { throw "v0.17.3 nao deveria alterar schema 38/38." }

Write-Host "[1488/1488] Validando versao funcional v0.17.3..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_TREATMENT_GOAL_INITIAL_RECOMMENDATION='v0.17.2'")) { throw "Versao v0.17.3 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.3." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.2 / Treatment Goal & Initial Recommendation: OK." -ForegroundColor Green

Write-Host "[1489/1496] Validando Recommendation Tuning Workspace..." -ForegroundColor Cyan
foreach ($token in @('HP_RECOMMENDATION_TUNING_WORKSPACE','openRecommendationTuningWorkspace','RECOMMENDATION TUNING • v0.17.3','recommendation-tuning-form','tuning-diff')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Recommendation Tuning incompleto: $token" } }

Write-Host "[1490/1496] Validando alavancas editaveis da recomendacao..." -ForegroundColor Cyan
foreach ($token in @('Hidratação (ml/dia)','Sessões de força / semana','Duração por sessão (min)','Refeições / dia','Condicionamento','Direção nutricional','Preferências e restrições')) { if (-not $appJsSource.Contains($token)) { throw "Alavanca de tuning ausente: $token" } }

Write-Host "[1491/1496] Validando atalhos conservadores do tuning..." -ForegroundColor Cyan
foreach ($token in @('Simplificar rotina','Priorizar recuperação','Restaurar sugerido','sem compensar carga')) { if (-not $appJsSource.Contains($token)) { throw "Preset de tuning ausente: $token" } }

Write-Host "[1492/1496] Validando comparacao sugerido versus ajustado..." -ForegroundColor Cyan
foreach ($token in @('SUGERIDO × AJUSTADO','Sugestão original preservada','ajuste(s) nesta proposta')) { if (-not $appJsSource.Contains($token)) { throw "Comparacao de tuning ausente: $token" } }

Write-Host "[1493/1496] Validando integracao tuning -> treino/alimentacao..." -ForegroundColor Cyan
foreach ($token in @('Levar para treino','Levar para alimentação',"state.patientTab='treinos'","state.patientTab='alimentacao'")) { if (-not $appJsSource.Contains($token)) { throw "Integracao do tuning ausente: $token" } }

Write-Host "[1494/1496] Validando decisao profissional no tuning..." -ForegroundColor Cyan
foreach ($token in @('Revisão profissional obrigatória','Os ajustes permanecem como rascunho','Profissional decide e publica','Contraindicações')) { if (-not $appJsSource.Contains($token)) { throw "Protecao profissional do tuning ausente: $token" } }

Write-Host "[1495/1496] Validando schema preservado na v0.17.3..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.3_recommendation_tuning_workspace.sql)) { throw "v0.17.3 nao deveria alterar schema 38/38." }

Write-Host "[1496/1496] Validando versao funcional v0.17.3..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_RECOMMENDATION_TUNING_WORKSPACE='v0.17.3'")) { throw "Versao v0.17.3 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.3." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.3 / Recommendation Tuning Workspace: OK." -ForegroundColor Green


Write-Host "[1497/1504] Validando Workout Builder 2.0..." -ForegroundColor Cyan
foreach ($token in @('HP_WORKOUT_BUILDER_2','workout-builder2-form','Editar treino')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Workout Builder 2.0 incompleto: $token" } }

Write-Host "[1498/1504] Validando edicao completa do plano de treino..." -ForegroundColor Cyan
foreach ($token in @("method:editing?'PUT':'POST'",'/api/treinos/${existingPlan.id}','Plano de treino atualizado.','workout-edit')) { if (-not $appJsSource.Contains($token)) { throw "Edicao de treino ausente: $token" } }

Write-Host "[1499/1504] Validando busca e catalogo de exercicios sem perda de rascunho..." -ForegroundColor Cyan
foreach ($token in @('Buscar exercício','exerciseSearch','refreshExerciseSelects','adicionado sem perder o rascunho')) { if (-not $appJsSource.Contains($token)) { throw "Busca/catalogo Workout Builder ausente: $token" } }

Write-Host "[1500/1504] Validando dias estruturados e duplicacao rapida..." -ForegroundColor Cyan
foreach ($token in @('workout-day-picker','duplicate-workout-session','duplicate-workout-item','Dias da semana')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Estrutura rapida Workout Builder ausente: $token" } }

Write-Host "[1501/1504] Validando contexto da recomendacao no builder..." -ForegroundColor Cyan
foreach ($token in @('recommendationTuningDraft','Contexto da sugestão profissional','Use como referência. A ficha continua totalmente editável pelo profissional.')) { if (-not $appJsSource.Contains($token)) { throw "Contexto profissional no builder ausente: $token" } }

Write-Host "[1502/1504] Validando responsividade do Workout Builder 2.0..." -ForegroundColor Cyan
foreach ($token in @('.workout-item-builder2{grid-template-columns','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Workout Builder 2.0 incompleta: $token" } }

Write-Host "[1503/1504] Validando schema preservado na v0.17.4..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.4_workout_builder_2.sql)) { throw "v0.17.4 nao deveria alterar schema 38/38." }

Write-Host "[1504/1504] Validando versao funcional v0.17.4..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_WORKOUT_BUILDER_2='v0.17.4'")) { throw "Versao v0.17.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.4 / Workout Builder 2.0: OK." -ForegroundColor Green

Write-Host "[1505/1512] Validando Workout Library & Assignment..." -ForegroundColor Cyan
foreach ($token in @('HP_WORKOUT_LIBRARY_ASSIGNMENT','AESYN • WORKOUT TEMPLATE LIBRARY 2.0 • v0.18.8','openWorkoutLibrary','Biblioteca de treinos')) { if (-not $appJsSource.Contains($token)) { throw "Workout Library incompleta: $token" } }

Write-Host "[1506/1512] Validando biblioteca independente e filtros..." -ForegroundColor Cyan
foreach ($token in @('incluirInativos=true','workoutLibrarySearch','workoutLibraryStatus','Ativos','Inativos')) { if (-not $appJsSource.Contains($token)) { throw "Filtros da Workout Library ausentes: $token" } }

Write-Host "[1507/1512] Validando atribuicao de modelo ao paciente..." -ForegroundColor Cyan
foreach ($token in @('openWorkoutLibraryPatientPicker','Atribuir a paciente','Atribuir ao paciente','criar-de-modelo','cópia independente')) { if (-not $appJsSource.Contains($token)) { throw "Atribuicao de treino ausente: $token" } }

Write-Host "[1508/1512] Validando gestao de modelos de treino..." -ForegroundColor Cyan
foreach ($token in @('openWorkoutLibraryEdit','Editar modelo','Desativar','Reativar','/api/modelos-planos-treino/${modelo.id}')) { if (-not $appJsSource.Contains($token)) { throw "Gestao da Workout Library ausente: $token" } }

Write-Host "[1509/1512] Validando integracao workspace/paciente -> biblioteca..." -ForegroundColor Cyan
foreach ($token in @('workspaceWorkoutLibrary','Atribuir da biblioteca','openWorkoutLibrary(patientForWorkout)')) { if (-not $appJsSource.Contains($token)) { throw "Integracao da Workout Library ausente: $token" } }

Write-Host "[1510/1512] Validando responsividade da Workout Library..." -ForegroundColor Cyan
foreach ($token in @('.workout-library-grid{display:grid','.workout-library-toolbar{display:grid','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Workout Library incompleta: $token" } }

Write-Host "[1511/1512] Validando schema preservado na v0.17.5..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.5_workout_library_assignment.sql)) { throw "v0.17.5 nao deveria alterar schema 38/38." }

Write-Host "[1512/1512] Validando versao funcional v0.17.5..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_WORKOUT_LIBRARY_ASSIGNMENT='v0.17.5'")) { throw "Versao v0.17.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.5." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.5 / Workout Library & Assignment: OK." -ForegroundColor Green

Write-Host "[1513/1520] Validando Nutrition Builder 2.0..." -ForegroundColor Cyan
foreach ($token in @('HP_NUTRITION_BUILDER_2','NUTRITION BUILDER 2.0 • v0.17.6','nutrition-builder2-form','Editar plano')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Nutrition Builder 2.0 incompleto: $token" } }

Write-Host "[1514/1520] Validando edicao completa do plano alimentar..." -ForegroundColor Cyan
foreach ($token in @("method:existingPlan?'PUT':'POST'",'/api/planos-alimentares/${existingPlan.id}','Plano alimentar atualizado.','nutrition-edit-builder2')) { if (-not $appJsSource.Contains($token)) { throw "Edicao de plano alimentar ausente: $token" } }

Write-Host "[1515/1520] Validando busca de alimentos sem perder rascunho..." -ForegroundColor Cyan
foreach ($token in @('Buscar alimento por nome ou categoria','foodSearch','hpNutritionFoodOptions','nutrition-food-search')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Busca de alimentos ausente: $token" } }

Write-Host "[1516/1520] Validando duplicacao rapida de refeicao/alimento..." -ForegroundColor Cyan
foreach ($token in @('duplicate-nutrition-meal','Duplicar refeição','duplicate-nutrition-item','Duplicar')) { if (-not $appJsSource.Contains($token)) { throw "Duplicacao no Nutrition Builder ausente: $token" } }

Write-Host "[1517/1520] Validando meta versus prescrito durante montagem..." -ForegroundColor Cyan
foreach ($token in @('nutritionBuilder2TargetDiff','Compare meta × prescrito enquanto monta.','TOTAL ESTIMADO DO PLANO','nutrition-builder2-summary')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Meta/prescrito Nutrition Builder ausente: $token" } }

Write-Host "[1518/1520] Validando contexto da Recommendation Tuning na alimentacao..." -ForegroundColor Cyan
foreach ($token in @('CONTEXTO DA SUGESTÃO PROFISSIONAL','RASCUNHO PROFISSIONAL','Novo plano com este contexto','state.recommendationTuningDraft')) { if (-not $appJsSource.Contains($token)) { throw "Contexto profissional na alimentacao ausente: $token" } }

Write-Host "[1519/1520] Validando schema preservado na v0.17.7..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.7_nutrition_builder_2.sql)) { throw "v0.17.7 nao deveria alterar schema 38/38." }

Write-Host "[1520/1520] Validando versao funcional v0.17.7..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_NUTRITION_BUILDER_2='v0.17.6'")) { throw "Versao v0.17.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.7." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.7 / Nutrition Builder 2.0: OK." -ForegroundColor Green

Write-Host "[1521/1528] Validando Diet & Meal Library..." -ForegroundColor Cyan
foreach ($token in @('HP_DIET_MEAL_LIBRARY','DIET & MEAL LIBRARY • v0.17.7','openDietMealLibrary','Biblioteca de dietas e refeições')) { if (-not $appJsSource.Contains($token)) { throw "Diet & Meal Library incompleta: $token" } }

Write-Host "[1522/1528] Validando biblioteca de planos alimentares..." -ForegroundColor Cyan
foreach ($token in @('/api/modelos-planos-alimentares?incluirInativos=true','diet-plan-assign','diet-plan-edit','diet-plan-toggle')) { if (-not $appJsSource.Contains($token)) { throw "Biblioteca de dietas incompleta: $token" } }

Write-Host "[1523/1528] Validando biblioteca de refeicoes..." -ForegroundColor Cyan
foreach ($token in @('/api/modelos-refeicoes?incluirInativos=true','meal-model-insert','meal-model-edit','meal-model-toggle')) { if (-not $appJsSource.Contains($token)) { throw "Biblioteca de refeicoes incompleta: $token" } }

Write-Host "[1524/1528] Validando atribuicao de dieta e insercao de refeicao..." -ForegroundColor Cyan
foreach ($token in @('openDietLibraryPatientPicker','openMealLibraryPlanPicker','criar-de-modelo','inserir-modelo-refeicao','cópias independentes')) { if (-not $appJsSource.Contains($token)) { throw "Atribuicao nutricional incompleta: $token" } }

Write-Host "[1525/1528] Validando integracao workspace/paciente..." -ForegroundColor Cyan
foreach ($token in @('workspaceNutritionLibrary','Dietas & refeições','dietMealLibraryButton')) { if (-not $appJsSource.Contains($token)) { throw "Integracao Diet & Meal Library ausente: $token" } }

Write-Host "[1526/1528] Validando responsividade da biblioteca nutricional..." -ForegroundColor Cyan
foreach ($token in @('.diet-library-grid{display:grid','.diet-library-toolbar{display:grid','@media(max-width:720px)','@media(max-width:390px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Diet & Meal Library incompleta: $token" } }

Write-Host "[1527/1528] Validando schema preservado na v0.17.7..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.7_diet_meal_library.sql)) { throw "v0.17.7 nao deveria alterar schema 38/38." }

Write-Host "[1528/1528] Validando versao funcional v0.17.7..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_DIET_MEAL_LIBRARY='v0.17.7'")) { throw "Versao v0.17.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.17.7." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.7 / Diet & Meal Library: OK." -ForegroundColor Green



Write-Host "[1529/1536] Validando Patient Preferences & Smart Adaptation..." -ForegroundColor Cyan
foreach ($token in @('HP_PATIENT_PREFERENCES_SMART_ADAPTATION','SMART ADAPTATION • v0.17.8','smartAdaptationPanel','renderSmartAdaptationPanel')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Smart Adaptation incompleta: $token" } }

Write-Host "[1530/1536] Validando uso de preferencias e rotina existente..." -ForegroundColor Cyan
foreach ($token in @('/api/pacientes/${p.id}/anamneses','atividadeFisicaDiasSemana','aguaLitrosDia','current.preferences')) { if (-not $appJsSource.Contains($token)) { throw "Contexto de preferencias/rotina ausente: $token" } }

Write-Host "[1531/1536] Validando regras de adaptacao assistida..." -ForegroundColor Cyan
foreach ($token in @('hpSmartAdaptationRules','Adequar treino aos dias já praticados','Aproximar hidratação em etapas','Levar restrições para o Nutrition Builder','Preservar retorno gradual')) { if (-not $appJsSource.Contains($token)) { throw "Regras de Smart Adaptation ausentes: $token" } }

Write-Host "[1532/1536] Validando aplicacao manual ao rascunho..." -ForegroundColor Cyan
foreach ($token in @('data-smart-adapt','Aplicar ao rascunho','Sugestão aplicada ao rascunho','renderDiff()')) { if (-not $appJsSource.Contains($token)) { throw "Aplicacao manual Smart Adaptation ausente: $token" } }

Write-Host "[1533/1536] Validando seguranca da adaptacao..." -ForegroundColor Cyan
foreach ($token in @('Nada automático','não substituem avaliação profissional','nunca publicam alterações')) { if (-not $appJsSource.Contains($token)) { throw "Seguranca Smart Adaptation incompleta: $token" } }

Write-Host "[1534/1536] Validando acesso pelo prontuario..." -ForegroundColor Cyan
foreach ($token in @('patientSmartAdaptation','Preferências & adaptação')) { if (-not $appJsSource.Contains($token)) { throw "Acesso Smart Adaptation pelo prontuario ausente: $token" } }

Write-Host "[1535/1536] Validando schema preservado na v0.17.8..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.8_patient_preferences_smart_adaptation.sql)) { throw "v0.17.8 nao deveria alterar schema 38/38." }

Write-Host "[1536/1536] Validando compatibilidade funcional v0.17.8..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $appJsSource.Contains("HP_PATIENT_PREFERENCES_SMART_ADAPTATION='v0.17.8'")) { throw "Compatibilidade v0.17.8 inconsistente na versao atual." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam a versao atual ao validar v0.17.8." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.17.8 / Patient Preferences & Smart Adaptation: OK." -ForegroundColor Green


Write-Host "[1537/1544] Validando Review, Publish & Versioning..."
if (-not $appJsSource.Contains("HP_REVIEW_PUBLISH_VERSIONING='v0.17.9'")) { throw "Marcador Review, Publish & Versioning ausente." }
Write-Host "[1538/1544] Validando revisao pelo prontuario..."
if (-not $appJsSource.Contains('patientPrescriptionReview') -or -not $appJsSource.Contains('openPrescriptionReview(p)')) { throw "Acesso de revisao/publicacao ausente." }
Write-Host "[1539/1544] Validando historico de versoes..."
foreach ($token in @('Histórico de versões','hpPlanVersionLabel','versao','hpPlanHistory')) { if (-not ($appJsSource.Contains($token) -or $appJsSource.Contains($token.ToLower()))) { throw "Versionamento visual incompleto: $token" } }
Write-Host "[1540/1544] Validando publicacao explicita de treino..."
if (-not $appJsSource.Contains('/api/treinos/${id}/status/Ativo') -or -not $appJsSource.Contains('/api/treinos/${x.id}/status/Inativo')) { throw "Publicacao controlada de treino incompleta." }
Write-Host "[1541/1544] Validando publicacao explicita de alimentacao..."
if (-not $appJsSource.Contains('/api/planos-alimentares/${id}/status/Ativo') -or -not $appJsSource.Contains('/api/planos-alimentares/${x.id}/status/Inativo')) { throw "Publicacao controlada de alimentacao incompleta." }
Write-Host "[1542/1544] Validando seguranca de publicacao..."
foreach ($token in @('Publicar só altera qual versão fica ativa','versões anteriores permanecem disponíveis como histórico','confirm(`Publicar esta versão')) { if (-not $appJsSource.Contains($token)) { throw "Guardrail de publicacao ausente: $token" } }
Write-Host "[1543/1544] Validando ausencia de migration na v0.17.9..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.17.9_review_publish_versioning.sql)) { throw "v0.17.9 nao deveria alterar schema 38/38." }
Write-Host "[1544/1544] Validando compatibilidade funcional da v0.17.9..."
if ($version.Trim() -ne "0.19.43" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao corrente inconsistente ao validar v0.17.9." }
Write-Host "    v0.17.9 / Review, Publish & Versioning: OK."

Write-Host "[1545/1552] Validando AESYN Performance Foundation..." -ForegroundColor Cyan
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$cssSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
foreach ($token in @('AESYN Performance','SAÚDE • PERFORMANCE • EVOLUÇÃO','Saúde e performance','em sincronia.','AESYN • ÁREA SEGURA')) { if (-not $indexSource.Contains($token)) { throw "Identidade AESYN incompleta: $token" } }

Write-Host "[1546/1552] Validando paleta AESYN..." -ForegroundColor Cyan
foreach ($token in @('#0B3B36','#19C98B','#F6F8F7','#111817','#A7B2AF')) { if (-not $cssSource.Contains($token)) { throw "Cor AESYN ausente: $token" } }

Write-Host "[1547/1552] Validando proposta integrada AESYN..." -ForegroundColor Cyan
foreach ($token in @('Consulta médica','Check-ins','Treino','Nutrição','Exames','Evolução')) { if (-not $indexSource.Contains($token)) { throw "Pilar AESYN ausente: $token" } }

Write-Host "[1548/1552] Validando navegacao profissional AESYN..." -ForegroundColor Cyan
foreach ($token in @('Performance','Acompanhamento','Treino & Nutrição','AESYN • PERFORMANCE CLÍNICA')) { if (-not ($indexSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Navegacao AESYN incompleta: $token" } }

Write-Host "[1549/1552] Validando arquitetura interna preservada..." -ForegroundColor Cyan
if (-not (Test-Path .\src\HealthPlatform.Api\HealthPlatform.Api.csproj) -or -not (Test-Path .\HealthPlatform.slnx)) { throw "Arquitetura HealthPlatform foi renomeada indevidamente." }

Write-Host "[1550/1552] Validando schema preservado na v0.18.0..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.0_aesyn_performance_foundation.sql)) { throw "v0.18.0 nao deveria alterar schema 38/38." }

Write-Host "[1551/1552] Validando responsividade AESYN..." -ForegroundColor Cyan
foreach ($token in @('@media(max-width:900px)','@media(max-width:560px)','.aesyn-hero-points','.brand-word')) { if (-not $cssSource.Contains($token)) { throw "Responsividade AESYN incompleta: $token" } }

Write-Host "[1552/1552] Validando compatibilidade AESYN Foundation v0.18.0..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Compatibilidade da AESYN Foundation inconsistente na versao atual." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam a versao atual ao validar v0.18.0." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.0 / AESYN Performance Foundation preservada: OK." -ForegroundColor Green


Write-Host "[1553/1560] Validando Professional Performance Dashboard..." -ForegroundColor Cyan
foreach ($token in @('PROFESSIONAL COMMAND CENTER','performance-kpi-grid','performance-overview-grid','performance-day-grid')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Professional Performance Dashboard incompleto: $token" } }

Write-Host "[1554/1560] Validando KPIs profissionais AESYN..." -ForegroundColor Cyan
foreach ($token in @('Pacientes ativos','Consultas hoje','Precisam de atenção','Retornos pendentes')) { if (-not $appJsSource.Contains($token)) { throw "KPI AESYN ausente: $token" } }

Write-Host "[1555/1560] Validando integracao com insights existentes..." -ForegroundColor Cyan
foreach ($token in @('/api/insights/dashboard?limite=8','pacientesComInsights','severidadeMaxima','Sinais de alta prioridade')) { if (-not $appJsSource.Contains($token)) { throw "Integracao de insights incompleta: $token" } }

Write-Host "[1556/1560] Validando continuidade operacional..." -ForegroundColor Cyan
foreach ($token in @('Agenda clínica','Próximos atendimentos','Pacientes recentes','Treino & Nutrição')) { if (-not $appJsSource.Contains($token)) { throw "Continuidade operacional incompleta: $token" } }

Write-Host "[1557/1560] Validando guardrail profissional..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('não substituem avaliação médica')) { throw "Guardrail de revisão profissional ausente." }

Write-Host "[1558/1560] Validando responsividade do Performance Dashboard..." -ForegroundColor Cyan
foreach ($token in @('@media(max-width:1100px)','@media(max-width:760px)','@media(max-width:520px)','.performance-recent-grid')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.1 incompleta: $token" } }

Write-Host "[1559/1560] Validando schema preservado na v0.18.1..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.1_professional_performance_dashboard.sql)) { throw "v0.18.1 nao deveria alterar schema 38/38." }

Write-Host "[1560/1560] Validando versao funcional v0.18.1..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.1." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.1 / Professional Performance Dashboard: OK." -ForegroundColor Green

Write-Host "[1561/1568] Validando Patient Performance Profile..." -ForegroundColor Cyan
foreach ($token in @("HP_PATIENT_PERFORMANCE_PROFILE='v0.18.2'",'PERFORMANCE PROFILE • v0.18.2','patient-performance-profile','patient-performance-snapshot')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Patient Performance Profile incompleto: $token" } }

Write-Host "[1562/1568] Validando leitura integrada de performance..." -ForegroundColor Cyan
foreach ($token in @('PRONTIDÃO HOJE','CORPO','TREINO','NUTRIÇÃO','CHECK-INS','EXAMES','HIDRATAÇÃO')) { if (-not $appJsSource.Contains($token)) { throw "Dimensao do Performance Profile ausente: $token" } }

Write-Host "[1563/1568] Validando navegacao contextual do perfil..." -ForegroundColor Cyan
foreach ($token in @('data-profile-tab="avaliacoes"','data-profile-tab="treinos"','data-profile-tab="alimentacao"','data-profile-tab="exames"','setPatientTab')) { if (-not $appJsSource.Contains($token)) { throw "Navegacao contextual do perfil incompleta: $token" } }

Write-Host "[1564/1568] Validando reorganizacao profissional do paciente..." -ForegroundColor Cyan
foreach ($token in @('Visão geral','Corpo ${avaliacoes.length}','Treino ${treinos.length}','Nutrição ${planos.length}','Check-ins ${diario.length}','Mais ações')) { if (-not $appJsSource.Contains($token)) { throw "Organizacao do Patient Profile incompleta: $token" } }

Write-Host "[1565/1568] Validando contexto integrado AESYN..." -ForegroundColor Cyan
foreach ($token in @('AESYN • VISÃO INTEGRADA','Performance, cuidado e evolução em uma única leitura.','Dados + contexto')) { if (-not $appJsSource.Contains($token)) { throw "Contexto AESYN do Patient Profile ausente: $token" } }

Write-Host "[1566/1568] Validando responsividade do Patient Performance Profile..." -ForegroundColor Cyan
foreach ($token in @('.patient-performance-profile','@media(max-width:1100px)','@media(max-width:760px)','@media(max-width:520px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.2 incompleta: $token" } }

Write-Host "[1567/1568] Validando schema preservado na v0.18.2..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.2_patient_performance_profile.sql)) { throw "v0.18.2 nao deveria alterar schema 38/38." }

Write-Host "[1568/1568] Validando versao funcional v0.18.2..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.2 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.2." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.2 / Patient Performance Profile: OK." -ForegroundColor Green



Write-Host "[1569/1576] Validando Evolution Dashboard..." -ForegroundColor Cyan
foreach ($token in @("HP_EVOLUTION_DASHBOARD='v0.18.4'",'AESYN • EVOLUTION DASHBOARD • v0.18.4','data-evolution-dashboard="v0.18.4"','evolution-kpi-grid')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Evolution Dashboard incompleto: $token" } }

Write-Host "[1570/1576] Validando integracao longitudinal..." -ForegroundColor Cyan
foreach ($token in @('Trajetória corporal','Progressão de carga','Adesão & recuperação','Últimos marcadores','Linha do tempo da evolução')) { if (-not $appJsSource.Contains($token)) { throw "Dimensao longitudinal ausente: $token" } }

Write-Host "[1571/1576] Validando KPIs de evolucao..." -ForegroundColor Cyan
foreach ($token in @('PESO','GORDURA CORPORAL','CINTURA','TREINOS • 90 DIAS','ADESÃO AO PROTOCOLO','EXAMES PARA REVISÃO')) { if (-not $appJsSource.Contains($token)) { throw "KPI de evolucao ausente: $token" } }

Write-Host "[1572/1576] Validando navegacao para Evolucao..." -ForegroundColor Cyan
foreach ($token in @("tabButton('evolution-dashboard','Evolução')","state.patientTab==='evolution-dashboard'",'hpEvolutionDashboard(d)')) { if (-not $appJsSource.Contains($token)) { throw "Acesso ao Evolution Dashboard incompleto: $token" } }

Write-Host "[1573/1576] Validando guardrail longitudinal..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('Tendências apoiam a revisão profissional e não substituem avaliação médica')) { throw "Guardrail do Evolution Dashboard ausente." }

Write-Host "[1574/1576] Validando responsividade do Evolution Dashboard..." -ForegroundColor Cyan
foreach ($token in @('.evolution-dashboard','@media(max-width:1180px)','@media(max-width:760px)','@media(max-width:520px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.4 incompleta: $token" } }

Write-Host "[1575/1576] Validando schema preservado na v0.18.4..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.4_evolution_dashboard.sql)) { throw "v0.18.4 nao deveria alterar schema 38/38." }

Write-Host "[1576/1576] Validando versao funcional v0.18.4..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.4 / Evolution Dashboard: OK." -ForegroundColor Green


Write-Host "[1577/1584] Validando AESYN Exams Core..." -ForegroundColor Cyan
foreach ($token in @("HP_EXAMS_CORE='v0.18.4'",'AESYN • EXAMS CORE • v0.18.4','data-exams-core="v0.18.4"','exams-core-kpis')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Exams Core incompleto: $token" } }

Write-Host "[1578/1584] Validando leitura laboratorial contextual..." -ForegroundColor Cyan
foreach ($token in @('Marcadores que pedem contexto','Coletas laboratoriais','Evolução laboratorial','Resultado é dado. Interpretação depende do contexto.')) { if (-not $appJsSource.Contains($token)) { throw "Dimensao do Exams Core ausente: $token" } }

Write-Host "[1579/1584] Validando KPIs laboratoriais..." -ForegroundColor Cyan
foreach ($token in @('COLETAS','MARCADORES','RESULTADOS NUMÉRICOS','PARA REVISÃO')) { if (-not $appJsSource.Contains($token)) { throw "KPI laboratorial ausente: $token" } }

Write-Host "[1580/1584] Validando tendencia e historico de exames..." -ForegroundColor Cyan
foreach ($token in @('hpExamFlatten','hpExamMarkerSeries','hpMetricChartGrid','hpLineChart')) { if (-not $appJsSource.Contains($token)) { throw "Historico/tendencia laboratorial incompleto: $token" } }

Write-Host "[1581/1584] Validando guardrail de exames..." -ForegroundColor Cyan
if (-not $appJsSource.Contains('não realiza diagnóstico automático')) { throw "Guardrail do Exams Core ausente." }

Write-Host "[1582/1584] Validando responsividade do Exams Core..." -ForegroundColor Cyan
foreach ($token in @('.exams-core','@media(max-width:1180px)','@media(max-width:760px)','@media(max-width:520px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.4 incompleta: $token" } }

Write-Host "[1583/1584] Validando schema preservado na v0.18.4..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.4_exams_core.sql)) { throw "v0.18.4 nao deveria alterar schema 38/38." }

Write-Host "[1584/1584] Validando versao funcional v0.18.4..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.4 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.4." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.4 / Exams Core: OK." -ForegroundColor Green



Write-Host "[1585/1592] Validando Exams Timeline & Trends..." -ForegroundColor Cyan
foreach ($token in @("HP_EXAMS_TIMELINE_TRENDS='v0.18.5'",'AESYN • EXAMS TIMELINE & TRENDS • v0.18.5','data-exams-timeline-trends="v0.18.5"','exams-timeline-hero')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Exams Timeline & Trends incompleto: $token" } }

Write-Host "[1586/1592] Validando comparacao entre medicoes..." -ForegroundColor Cyan
foreach ($token in @('Última medição × anterior','hpExamTrendRows','hpExamDirectionLabel','SÉRIES COMPARÁVEIS')) { if (-not $appJsSource.Contains($token)) { throw "Comparacao laboratorial incompleta: $token" } }

Write-Host "[1587/1592] Validando timeline laboratorial..." -ForegroundColor Cyan
foreach ($token in @('Linha do tempo laboratorial','hpExamTimelineRows','Coletas em ordem cronológica','exams-lab-timeline-list')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Timeline laboratorial incompleta: $token" } }

Write-Host "[1588/1592] Validando leitura neutra de tendencia..." -ForegroundColor Cyan
foreach ($token in @('Subiu','Desceu','Sem variação','Subir ou descer não significa melhorar ou piorar.')) { if (-not $appJsSource.Contains($token)) { throw "Leitura neutra de tendencia ausente: $token" } }

Write-Host "[1589/1592] Validando continuidade do Exams Core..." -ForegroundColor Cyan
foreach ($token in @("HP_EXAMS_CORE='v0.18.4'",'hpExamsCore(d)','Marcadores que pedem contexto','Evolução laboratorial')) { if (-not $appJsSource.Contains($token)) { throw "Continuidade do Exams Core ausente: $token" } }

Write-Host "[1590/1592] Validando responsividade do Exams Timeline & Trends..." -ForegroundColor Cyan
foreach ($token in @('.exams-timeline-trends','@media(max-width:1180px)','@media(max-width:760px)','@media(max-width:520px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.5 incompleta: $token" } }

Write-Host "[1591/1592] Validando schema preservado na v0.18.5..." -ForegroundColor Cyan
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.5_exams_timeline_trends.sql)) { throw "v0.18.5 nao deveria alterar schema 38/38." }

Write-Host "[1592/1592] Validando versao funcional v0.18.5..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.5." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.5 / Exams Timeline & Trends: OK." -ForegroundColor Green



Write-Host "[1593/1602] Validando Exercise Library 2.0..." -ForegroundColor Cyan
foreach ($token in @("HP_EXERCISE_LIBRARY_2='v0.18.6'",'openExerciseLibrary2','exercise-library-grid','exerciseSeedCatalog')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Exercise Library 2.0 incompleta: $token" } }
Write-Host "    Catalogo profissional + filtros + CRUD visual: assets OK."

$workoutSource = Get-Content .\src\HealthPlatform.Api\Controllers\TreinosController.cs -Encoding UTF8 -Raw
Write-Host "[1594/1602] Validando filtros profissionais de exercicios..." -ForegroundColor Cyan
foreach ($token in @('api/exercicios/filtros','grupoMuscular = null','equipamento = null','x.Descricao.ToLower().Contains(termo)')) { if (-not $workoutSource.Contains($token)) { throw "Filtros do catalogo de exercicios incompletos: $token" } }
Write-Host "    Nome + grupo + equipamento + descricao: backend OK."

Write-Host "[1595/1602] Validando catalogo-base de exercicios..." -ForegroundColor Cyan
foreach ($token in @('api/exercicios/catalogo-base','CatalogoBaseExercicios','SEED_BASE_CATALOG','Supino reto com barra','Levantamento terra romeno','Pallof press')) { if (-not $workoutSource.Contains($token)) { throw "Catalogo-base incompleto: $token" } }
Write-Host "    Base curada + deduplicacao por nome: OK."

Write-Host "[1596/1602] Validando protecao contra duplicidade de exercicios..." -ForegroundColor Cyan
if (-not $workoutSource.Contains('Ja existe um exercicio com este nome no catalogo.')) { throw "Protecao de duplicidade do catalogo incompleta." }
Write-Host "    Duplicidade nominal por organizacao bloqueada."

Write-Host "[1597/1602] Validando Standalone Workout Builder..." -ForegroundColor Cyan
foreach ($token in @("HP_STANDALONE_WORKOUT_STUDIO='v0.18.6'",'openStandaloneWorkoutBuilder','standaloneWorkoutForm','standalone-session-list','O modelo permanece independente de paciente.')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Standalone Workout Builder incompleto: $token" } }
Write-Host "    Builder independente de paciente: assets OK."

Write-Host "[1598/1602] Validando API de treino-modelo standalone..." -ForegroundColor Cyan
$modelSource = Get-Content .\src\HealthPlatform.Api\Controllers\ModelosPlanosTreinoController.cs -Encoding UTF8 -Raw
foreach ($token in @('CriarModeloStandalone','AtualizarConteudoStandalone','UpsertModeloTreinoStandaloneRequest','ValidarModeloStandalone','ToDetailedResponse','CREATE_STANDALONE','UPDATE_STANDALONE')) { if (-not $modelSource.Contains($token)) { throw "API standalone de treino-modelo incompleta: $token" } }
Write-Host "    Criacao + leitura + edicao de conteudo sem paciente: backend OK."

Write-Host "[1599/1602] Validando independencia de paciente no modelo..." -ForegroundColor Cyan
if (-not $modelSource.Contains('[HttpPost("api/modelos-planos-treino")]') -or -not $modelSource.Contains('request.Sessoes.OrderBy') -or -not $appJsSource.Contains('modelos-planos-treino/${modelSummary.id}/conteudo') -or -not $appJsSource.Contains("method:modelSummary?'PUT':'POST'")) { throw "Fluxo independente de paciente incompleto." }
Write-Host "    Modelo nasce na biblioteca e so depois pode ser atribuido."

Write-Host "[1600/1602] Validando Professional Workout Studio..." -ForegroundColor Cyan
foreach ($token in @('AESYN • PROFESSIONAL WORKOUT STUDIO • v0.18.7','Exercícios → treino-modelo → biblioteca → atribuição → adaptação','workspaceNewWorkoutTemplate','workspaceExerciseLibrary')) { if (-not $appJsSource.Contains($token)) { throw "Professional Workout Studio incompleto: $token" } }
Write-Host "    Workspace reposicionado para producao profissional de treinos."

Write-Host "[1601/1602] Validando schema preservado na v0.18.6..." -ForegroundColor Cyan
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.6_exercise_library.sql)) { throw "v0.18.6 nao deveria alterar schema 38/38." }
Write-Host "    Schema 38/38 preservado; sem migration nova."

Write-Host "[1602/1602] Validando versao funcional v0.18.6..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.6 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.6." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.6 / Exercise Library 2.0 + Standalone Workout Studio: OK." -ForegroundColor Green


Write-Host "[1603/1612] Validando Workout Builder 3.0..." -ForegroundColor Cyan
foreach ($token in @("HP_WORKOUT_BUILDER_3='v0.18.7'",'AESYN • WORKOUT BUILDER 3.0 • v0.18.7','builder3-commandbar','builder3-summary')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Workout Builder 3.0 incompleto: $token" } }
Write-Host "    Builder profissional 3.0: assets OK."

Write-Host "[1604/1612] Validando presets de prescricao..." -ForegroundColor Cyan
foreach ($token in @('data-preset="hipertrofia"','data-preset="forca"','data-preset="resistencia"','data-preset="tecnica"','builderDefaultSeries','builderDefaultReps','builderDefaultRest')) { if (-not $appJsSource.Contains($token)) { throw "Presets do Workout Builder 3.0 incompletos: $token" } }
Write-Host "    Hipertrofia + forca + resistencia + tecnica: OK."

Write-Host "[1605/1612] Validando busca contextual de exercicios no builder..." -ForegroundColor Cyan
foreach ($token in @('builder3-exercise-search','builder3-group-filter','builder3-equipment-filter','exerciseOptions')) { if (-not $appJsSource.Contains($token)) { throw "Busca contextual de exercicios incompleta: $token" } }
Write-Host "    Busca + grupo muscular + equipamento: OK."

Write-Host "[1606/1612] Validando ordenacao e duplicacao de exercicios..." -ForegroundColor Cyan
foreach ($token in @('row-up','row-down','duplicate','moveNode(row,-1)','moveNode(row,1)')) { if (-not $appJsSource.Contains($token)) { throw "Ordenacao/duplicacao de exercicios incompleta: $token" } }
Write-Host "    Reordenacao + duplicacao de itens: OK."

Write-Host "[1607/1612] Validando operacoes de sessao..." -ForegroundColor Cyan
foreach ($token in @('duplicate-session','session-up','session-down','sessionDataFromDom','cópia')) { if (-not $appJsSource.Contains($token)) { throw "Operacoes de sessao incompletas: $token" } }
Write-Host "    Duplicacao + reordenacao de sessoes: OK."

Write-Host "[1608/1612] Validando resumo dinamico da prescricao..." -ForegroundColor Cyan
foreach ($token in @('builder3SessionCount','builder3ExerciseCount','builder3SetCount','updateSummary')) { if (-not $appJsSource.Contains($token)) { throw "Resumo dinamico do builder incompleto: $token" } }
Write-Host "    Sessoes + exercicios + series prescritas: OK."

Write-Host "[1609/1612] Validando duplicacao de treino-modelo..." -ForegroundColor Cyan
foreach ($token in @('builder3DuplicateTemplate','library-duplicate','Treino-modelo duplicado.','Cópia criada na biblioteca.')) { if (-not $appJsSource.Contains($token)) { throw "Duplicacao de treino-modelo incompleta: $token" } }
Write-Host "    Copia reutilizavel independente de paciente: OK."

Write-Host "[1610/1612] Validando responsividade do Workout Builder 3.0..." -ForegroundColor Cyan
foreach ($token in @('.builder3-form','@media(max-width:980px)','@media(max-width:720px)','@media(max-width:430px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.7 incompleta: $token" } }
Write-Host "    Desktop + tablet + mobile: OK."

Write-Host "[1611/1612] Validando schema preservado na v0.18.7..." -ForegroundColor Cyan
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.7_workout_builder_3.sql)) { throw "v0.18.7 nao deveria alterar schema 38/38." }
Write-Host "    Schema 38/38 preservado; sem migration nova."

Write-Host "[1612/1612] Validando versao funcional v0.18.7..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.7 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.7." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.7 / Workout Builder 3.0: OK." -ForegroundColor Green


Write-Host "[1613/1622] Validando Workout Template Library 2.0..." -ForegroundColor Cyan
foreach ($token in @("HP_WORKOUT_TEMPLATE_LIBRARY_2='v0.18.8'",'AESYN • WORKOUT TEMPLATE LIBRARY 2.0 • v0.18.8','Catálogo profissional de treinos','Biblioteca primeiro, paciente depois')) { if (-not $appJsSource.Contains($token)) { throw "Workout Template Library 2.0 incompleta: $token" } }
Write-Host "    Biblioteca profissional independente de paciente: OK."

Write-Host "[1614/1622] Validando filtros da biblioteca 2.0..." -ForegroundColor Cyan
foreach ($token in @('workoutLibraryObjective','workoutLibrarySort','workoutLibraryFavoritesOnly','workoutLibraryObjectiveChips')) { if (-not $appJsSource.Contains($token)) { throw "Filtros da Workout Template Library incompletos: $token" } }
Write-Host "    Busca + status + objetivo + ordenacao + favoritos: OK."

Write-Host "[1615/1622] Validando favoritos locais de modelos..." -ForegroundColor Cyan
foreach ($token in @('aesynWorkoutFavoriteIds','aesynSaveWorkoutFavoriteIds','aesynToggleWorkoutFavorite','aesyn.workoutTemplateFavorites')) { if (-not $appJsSource.Contains($token)) { throw "Favoritos de modelos incompletos: $token" } }
Write-Host "    Favoritos profissionais persistentes no navegador: OK."

Write-Host "[1616/1622] Validando ordenacao do catalogo..." -ForegroundColor Cyan
foreach ($token in @("sort==='favoritos'","sort==='sessoes'","sort==='exercicios'","sort==='recentes'")) { if (-not $appJsSource.Contains($token)) { throw "Ordenacao do catalogo incompleta: $token" } }
Write-Host "    Nome + favoritos + sessoes + exercicios + recentes: OK."

Write-Host "[1617/1622] Validando Quick Template Preview..." -ForegroundColor Cyan
foreach ($token in @('openWorkoutTemplateQuickPreview','AESYN • QUICK TEMPLATE PREVIEW • v0.18.8','workout-preview-summary','workout-preview-sessions')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Quick Template Preview incompleto: $token" } }
Write-Host "    Preview estrutural do treino-modelo: OK."

Write-Host "[1618/1622] Validando leitura de exercicios no preview..." -ForegroundColor Cyan
foreach ($token in @("api('/api/exercicios')",'exerciseMap','exercicioId','Séries prescritas')) { if (-not $appJsSource.Contains($token)) { throw "Leitura de exercicios no preview incompleta: $token" } }
Write-Host "    Catalogo de exercicios integrado ao preview: OK."

Write-Host "[1619/1622] Validando fluxo modelo -> paciente preservado..." -ForegroundColor Cyan
foreach ($token in @('Atribuir a paciente','openWorkoutLibraryPatientPicker','openWorkoutTemplateCreateForm','cópia independente')) { if (-not $appJsSource.Contains($token)) { throw "Fluxo de atribuicao de modelo incompleto: $token" } }
Write-Host "    Modelo mestre -> copia independente: OK."

Write-Host "[1620/1622] Validando responsividade da biblioteca 2.0..." -ForegroundColor Cyan
foreach ($token in @('.workout-library2-toolbar','.workout-preview-exercise','@media(max-width:980px)','@media(max-width:720px)','@media(max-width:430px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.8 incompleta: $token" } }
Write-Host "    Desktop + tablet + mobile: OK."

Write-Host "[1621/1622] Validando schema preservado na v0.18.8..." -ForegroundColor Cyan
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.8_workout_template_library_2.sql)) { throw "v0.18.8 nao deveria alterar schema 38/38." }
Write-Host "    Schema 38/38 preservado; sem migration nova."

Write-Host "[1622/1622] Validando versao funcional v0.18.8..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.8 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.8." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.8 / Workout Template Library 2.0: OK." -ForegroundColor Green


Write-Host "[1623/1634] Validando Program Builder 1.0..." -ForegroundColor Cyan
foreach ($token in @("HP_PROGRAM_BUILDER_1='v0.18.9'",'AESYN • PROGRAM BUILDER 1.0 • v0.18.9','openWorkoutProgramLibrary','openWorkoutProgramBuilder')) { if (-not $appJsSource.Contains($token)) { throw "Program Builder 1.0 incompleto: $token" } }
Write-Host "    Programa profissional independente de paciente: OK."

Write-Host "[1624/1634] Validando persistencia e portabilidade de programas..." -ForegroundColor Cyan
foreach ($token in @('AESYN_PROGRAM_STORAGE','aesynWorkoutPrograms','aesynSaveWorkoutPrograms','aesynProgramDownload','programImportInput')) { if (-not $appJsSource.Contains($token)) { throw "Persistencia/portabilidade do Program Builder incompleta: $token" } }
Write-Host "    Persistencia local + export/import JSON: OK."

Write-Host "[1625/1634] Validando estrutura por fases..." -ForegroundColor Cyan
foreach ($token in @('programAddPhase','program-phase','phaseWeeks','phase-duplicate','program-phase-list')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Estrutura de fases incompleta: $token" } }
Write-Host "    Fases + duracao + duplicacao + ordenacao: OK."

Write-Host "[1626/1634] Validando agenda semanal do programa..." -ForegroundColor Cyan
foreach ($token in @('program-day-row','programDay','programModel','programDayNotes','Segunda','Domingo')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Agenda semanal do programa incompleta: $token" } }
Write-Host "    Agenda semanal com treino-modelo por dia: OK."

Write-Host "[1627/1634] Validando resumo dinamico do programa..." -ForegroundColor Cyan
foreach ($token in @('programWeeksCount','programPhasesCount','programSessionsCount','programTemplatesCount','aesynProgramWeeks','aesynProgramSessions')) { if (-not $appJsSource.Contains($token)) { throw "Resumo dinamico do programa incompleto: $token" } }
Write-Host "    Semanas + fases + slots + modelos distintos: OK."

Write-Host "[1628/1634] Validando biblioteca de programas..." -ForegroundColor Cyan
foreach ($token in @('programLibrarySearch','programLibraryObjective','programLibrarySort','program-library-grid','program-preview','program-edit','program-duplicate','program-export')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Biblioteca de programas incompleta: $token" } }
Write-Host "    Busca + filtros + duplicacao + exportacao: OK."

Write-Host "[1629/1634] Validando preview do programa..." -ForegroundColor Cyan
foreach ($token in @('openWorkoutProgramPreview','AESYN • PROGRAM PREVIEW • v0.18.9','program-preview-week','program-preview-day','Fonte: servidor AESYN')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Preview do programa incompleto: $token" } }
Write-Host "    Preview por fase e semana: OK."

Write-Host "[1630/1634] Validando integracao com treinos-modelo..." -ForegroundColor Cyan
foreach ($token in @("api('/api/modelos-planos-treino?incluirInativos=true')",'modeloId','Treinos-modelo disponíveis','Biblioteca de treinos')) { if (-not $appJsSource.Contains($token)) { throw "Integracao programa -> treinos-modelo incompleta: $token" } }
Write-Host "    Programas compostos por Workout Templates: OK."

Write-Host "[1631/1634] Validando acesso ao Program Builder pelo Workout Studio..." -ForegroundColor Cyan
foreach ($token in @('workspacePrograms','workspaceProgramBuilder','openWorkoutProgramsFromLibrary')) { if (-not $appJsSource.Contains($token)) { throw "Acesso ao Program Builder incompleto: $token" } }
Write-Host "    Workout Studio + Template Library -> Programas: OK."

Write-Host "[1632/1634] Validando responsividade do Program Builder..." -ForegroundColor Cyan
foreach ($token in @('.program-library-grid','.program-day-row','.program-preview-week','@media(max-width:900px)','@media(max-width:720px)','@media(max-width:430px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.9 incompleta: $token" } }
Write-Host "    Desktop + tablet + mobile: OK."

Write-Host "[1633/1634] Validando schema preservado na v0.18.9..." -ForegroundColor Cyan
if (-not $setupSource.Contains('[38/38] Aplicando upgrade v0.10.3') -or (Test-Path .\scripts\sql\v0.18.9_program_builder.sql)) { throw "v0.18.9 nao deveria alterar schema 38/38." }
Write-Host "    Schema 38/38 preservado; sem migration nova."

Write-Host "[1634/1634] Validando versao funcional v0.18.9..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.9 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.9." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.9 / Program Builder 1.0: OK." -ForegroundColor Green


Write-Host "[1635/1646] Validando Server-side Program Catalog..." -ForegroundColor Cyan
foreach ($token in @("HP_PROGRAM_SERVER_CATALOG='v0.18.10'",'AESYN • SERVER-SIDE PROGRAM CATALOG • v0.18.10','/api/programas-treino','Persistência server-side ativa')) { if (-not $appJsSource.Contains($token)) { throw "Server-side Program Catalog incompleto: $token" } }
Write-Host "    Catalogo profissional persistido no servidor: OK."
Write-Host "[1636/1646] Validando entidade ProgramaTreinoModelo..." -ForegroundColor Cyan
$programEntitySource = Get-Content .\src\HealthPlatform.Domain\Entities\ProgramaTreinoModelo.cs -Encoding UTF8 -Raw
foreach ($token in @('class ProgramaTreinoModelo','OrganizacaoId','ProfissionalId','ConteudoJson','Ativo')) { if (-not $programEntitySource.Contains($token)) { throw "Entidade ProgramaTreinoModelo incompleta: $token" } }
Write-Host "    Entidade server-side: OK."
Write-Host "[1637/1646] Validando mapeamento EF de programas..." -ForegroundColor Cyan
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
foreach ($token in @('DbSet<ProgramaTreinoModelo>','ProgramasTreinoModelo')) { if (-not $dbContextSource.Contains($token)) { throw "Mapeamento EF de programas incompleto: $token" } }
Write-Host "    DbSet + mapping: OK."
Write-Host "[1638/1646] Validando API de programas..." -ForegroundColor Cyan
$programControllerSource = Get-Content .\src\HealthPlatform.Api\Controllers\ProgramasTreinoController.cs -Encoding UTF8 -Raw
foreach ($token in @('[HttpGet("api/programas-treino")]','[HttpPost("api/programas-treino")]','[HttpPut("api/programas-treino/{id:guid}")]','[HttpPost("api/programas-treino/{id:guid}/duplicar")]','[HttpDelete("api/programas-treino/{id:guid}")]')) { if (-not $programControllerSource.Contains($token)) { throw "API de programas incompleta: $token" } }
Write-Host "    CRUD + duplicacao server-side: OK."
Write-Host "[1639/1646] Validando protecao multi-tenant de programas..." -ForegroundColor Cyan
foreach ($token in @('currentUser.OrganizationId','GetProfissionalAtual','Perfil profissional ativo nao encontrado','ModeloPlanoTreinoId')) { if (-not $programControllerSource.Contains($token)) { throw "Protecao multi-tenant de programas incompleta: $token" } }
Write-Host "    Organizacao + profissional + modelos validos: OK."
Write-Host "[1640/1646] Validando migracao de programas locais..." -ForegroundColor Cyan
foreach ($token in @('aesynMigrateLocalPrograms','localStorage.removeItem(AESYN_PROGRAM_STORAGE)','aesynProgramRequest')) { if (-not $appJsSource.Contains($token)) { throw "Migracao local -> servidor incompleta: $token" } }
Write-Host "    localStorage legado -> API: OK."
Write-Host "[1641/1646] Validando import/export server-aware..." -ForegroundColor Cyan
foreach ($token in @('programImportInput','Programa importado para o servidor.','aesynProgramDownload')) { if (-not $appJsSource.Contains($token)) { throw "Import/export de programas incompleto: $token" } }
Write-Host "    JSON importado via API + export local: OK."
Write-Host "[1642/1646] Validando SQL idempotente v0.18.10..." -ForegroundColor Cyan
$sql1810 = Get-Content .\scripts\sql\v0.18.10_programas_treino_modelo.sql -Encoding UTF8 -Raw
foreach ($token in @('CREATE TABLE IF NOT EXISTS "ProgramasTreinoModelo"','IX_ProgramasTreinoModelo_OrganizacaoId_Ativo_Nome','FK_ProgramasTreinoModelo_Organizacoes_OrganizacaoId')) { if (-not $sql1810.Contains($token)) { throw "SQL v0.18.10 incompleto: $token" } }
Write-Host "    Tabela + indices + FKs idempotentes: OK."
Write-Host "[1643/1646] Validando ModelSnapshot v0.18.10..." -ForegroundColor Cyan
$snapshotSource = Get-Content .\src\HealthPlatform.Infrastructure\Migrations\AppDbContextModelSnapshot.cs -Encoding UTF8 -Raw
foreach ($token in @('HealthPlatform.Domain.Entities.ProgramaTreinoModelo','ProgramasTreinoModelo','OrganizacaoId", "Ativo", "Nome','OrganizacaoId", "Objetivo')) {
    if (-not $snapshotSource.Contains($token)) { throw "ModelSnapshot v0.18.10 incompleto: $token" }
}
if ($setupSource.Contains('dotnet ef migrations add $v01810MigrationName')) { throw "v0.18.10 nao deve gerar migration EF dinamicamente." }
Write-Host "    ModelSnapshot sincronizado + sem migration EF dinamica: OK."

Write-Host "[1644/1646] Validando setup 39/39..." -ForegroundColor Cyan
Write-Host "    migration v0.18.10 usa build atual (sem --no-build): OK." -ForegroundColor DarkGray

if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10 (programas de treino server-side)')) { throw "Setup nao aplica upgrade server-side v0.18.10." }
Write-Host "    Upgrade 39/39: OK."
Write-Host "[1645/1646] Validando responsividade server-side..." -ForegroundColor Cyan
foreach ($token in @('.program-server-banner','@media(max-width:720px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.18.10 incompleta: $token" } }
Write-Host "    Banner e fluxo mobile: OK."
Write-Host "[1646/1646] Validando versao funcional v0.18.10..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.10 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.10." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.10 / Server-side Program Catalog: OK." -ForegroundColor Green


Write-Host "[1647/1658] Validando Program Assignment & Publish..." -ForegroundColor Cyan
foreach ($token in @("HP_PROGRAM_ASSIGNMENT_PUBLISH='v0.18.11'",'AESYN • PROGRAM ASSIGNMENT • v0.18.11','AESYN • REVIEW & PUBLISH • v0.18.11','openWorkoutProgramAssignmentPicker','openWorkoutProgramPublishForm')) { if (-not $appJsSource.Contains($token)) { throw "Program Assignment & Publish incompleto: $token" } }
Write-Host "    Picker + review + publish: OK."

Write-Host "[1648/1658] Validando endpoint de atribuicao de programa..." -ForegroundColor Cyan
$assignmentControllerSource = Get-Content .\src\HealthPlatform.Api\Controllers\ProgramaTreinoAssignmentController.cs -Encoding UTF8 -Raw
foreach ($token in @('[HttpPost("api/pacientes/{pacienteId:guid}/programas-treino/{programaId:guid}/atribuir")]','AtribuirProgramaRequest','ASSIGN_PROGRAM')) { if (-not $assignmentControllerSource.Contains($token)) { throw "Endpoint de atribuicao incompleto: $token" } }
Write-Host "    API de publicacao: OK."

Write-Host "[1649/1658] Validando protecao organizacional e profissional..." -ForegroundColor Cyan
foreach ($token in @('currentUser.OrganizationId','currentUser.UserId','Perfil profissional ativo nao encontrado.','Paciente nao encontrado ou inativo.')) { if (-not $assignmentControllerSource.Contains($token)) { throw "Protecao de atribuicao incompleta: $token" } }
Write-Host "    Tenant + profissional + paciente ativo: OK."

Write-Host "[1650/1658] Validando materializacao por fases..." -ForegroundColor Cyan
foreach ($token in @('new PlanoTreino','new FaseTreino','Tipo = "Programa"','DuracaoMinimaDias','EmAndamento','Planejada')) { if (-not $assignmentControllerSource.Contains($token)) { throw "Materializacao do programa incompleta: $token" } }
Write-Host "    Fase -> plano + fase longitudinal: OK."

Write-Host "[1651/1658] Validando copia de sessoes e exercicios..." -ForegroundColor Cyan
foreach ($token in @('new SessaoTreino','new ItemTreino','DiasSemana = dia.Dia','ExercicioId = itemTemplate.ExercicioId','DescansoSegundos = itemTemplate.DescansoSegundos')) { if (-not $assignmentControllerSource.Contains($token)) { throw "Copia de treino-modelo incompleta: $token" } }
Write-Host "    Sessoes + itens + dia da semana: OK."

Write-Host "[1652/1658] Validando referencias do programa antes da publicacao..." -ForegroundColor Cyan
foreach ($token in @('modelosIndisponiveis','exerciciosInvalidos','treinos-modelo inativos ou indisponiveis','exercicios inativos ou indisponiveis')) { if (-not $assignmentControllerSource.Contains($token)) { throw "Validacao de referencias incompleta: $token" } }
Write-Host "    Templates e exercicios validados: OK."

Write-Host "[1653/1658] Validando politica de planos anteriores..." -ForegroundColor Cyan
foreach ($token in @('ConcluirPlanosAtivos','ativo.Status = "Concluido"','request.DataInicio.AddDays(-1)')) { if (-not $assignmentControllerSource.Contains($token)) { throw "Politica de planos anteriores incompleta: $token" } }
Write-Host "    Conclusao opcional de plano ativo: OK."

Write-Host "[1654/1658] Validando confirmacao de publicacao..." -ForegroundColor Cyan
foreach ($token in @('AESYN • PROGRAMA PUBLICADO','Fases criadas','Planos criados','publishedOpenPatient','Programa publicado para o paciente.')) { if (-not $appJsSource.Contains($token)) { throw "Confirmacao de publicacao incompleta: $token" } }
Write-Host "    Success state + abertura do paciente: OK."

Write-Host "[1655/1658] Validando independencia entre programa mestre e copia clinica..." -ForegroundColor Cyan
foreach ($token in @('Publicação cria cópias clínicas','não modificam automaticamente as fichas já publicadas','Programa mestre')) { if (-not $appJsSource.Contains($token)) { throw "Independencia programa/copia incompleta: $token" } }
Write-Host "    Master catalog preservado: OK."

Write-Host "[1656/1658] Validando responsividade do publish..." -ForegroundColor Cyan
foreach ($token in @('.program-publish-summary','.program-publish-phases','.program-publish-warning','@media(max-width:760px)','@media(max-width:430px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Program Publish incompleta: $token" } }
Write-Host "    Desktop + mobile: OK."

Write-Host "[1657/1658] Validando ausencia de migration na v0.18.11..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.18.11_program_assignment.sql) { throw "v0.18.11 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.18.11." }
Write-Host "    Schema preservado em 39/39: OK."

Write-Host "[1658/1658] Validando versao funcional v0.18.11..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao funcional v0.18.11 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.11." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    v0.18.11 / Program Assignment & Publish: OK." -ForegroundColor Green


Write-Host "[1659/1670] Validando Professional Workout & Nutrition Flow Hardening..." -ForegroundColor Cyan
foreach ($token in @("HP_PRO_WORKOUT_NUTRITION_HARDENING='v0.18.12'",'data-pro-workout-flow="v0.18.12"','data-pro-nutrition-flow="v0.18.12"')) { if (-not $appJsSource.Contains($token)) { throw "Flow Hardening v0.18.12 incompleto: $token" } }
Write-Host "    Camada profissional v0.18.12: OK."

Write-Host "[1660/1670] Validando abertura direta do Workout Builder..." -ForegroundColor Cyan
foreach ($token in @("modal.classList.remove('hidden','nutrition-modal-open')","modal.classList.add('workout-modal-open')",'async function openWorkoutForm(p,existingPlan=null,options={})')) { if (-not $appJsSource.Contains($token)) { throw "Abertura direta do Workout Builder incompleta: $token" } }
Write-Host "    Novo treino abre modal corretamente: OK."

Write-Host "[1661/1670] Validando agregacao de Treino B/C/D..." -ForegroundColor Cyan
foreach ($token in @('workout-add-session-v1812','options?.addSession','Treino ${String.fromCharCode(65+host.children.length)}','Editar / agregar')) { if (-not $appJsSource.Contains($token)) { throw "Agregacao de sessoes incompleta: $token" } }
Write-Host "    Adicionar treino ao plano existente: OK."

Write-Host "[1662/1670] Validando acoes do plano de treino..." -ForegroundColor Cyan
foreach ($token in @('workout-edit-v1812','workout-save-template-v1812','workout-progress-v1812','Programas de treino')) { if (-not $appJsSource.Contains($token)) { throw "Acoes profissionais de treino incompletas: $token" } }
Write-Host "    Editar + modelo + progressao + programas: OK."

Write-Host "[1663/1670] Validando hardening da tela de Programas..." -ForegroundColor Cyan
foreach ($token in @('program-load-error','retryWorkoutPrograms','backWorkoutProgramsToLibrary','Não foi possível abrir os programas.')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Hardening de Programas incompleto: $token" } }
Write-Host "    Erro recuperavel + retry: OK."

Write-Host "[1664/1670] Validando fluxo profissional de Nutricao..." -ForegroundColor Cyan
foreach ($token in @('newMealPlanV1812','nutritionDietModelsV1812','nutritionMealModelsV1812','+ Montar dieta','Catálogo alimentar conectado')) { if (-not $appJsSource.Contains($token)) { throw "Fluxo profissional de nutricao incompleto: $token" } }
Write-Host "    Dieta + modelos + refeicoes: OK."

Write-Host "[1665/1670] Validando edicao e modelos nutricionais..." -ForegroundColor Cyan
foreach ($token in @('nutrition-edit-v1812','nutrition-progress-v1812','nutrition-save-template-v1812','openMealPlanForm(patient,plan)')) { if (-not $appJsSource.Contains($token)) { throw "Acoes nutricionais incompletas: $token" } }
Write-Host "    Editar dieta + progressao + modelo: OK."

Write-Host "[1666/1670] Validando responsividade do Workout Builder..." -ForegroundColor Cyan
foreach ($token in @('#clinicalActionModal.workout-modal-open .workout-item-builder2','grid-template-columns:repeat(4,minmax(0,1fr))','@media(max-width:720px)','@media(max-width:430px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Workout Builder v0.18.12 incompleta: $token" } }
Write-Host "    Campos respeitam largura desktop/tablet/mobile: OK."

Write-Host "[1667/1670] Validando layout dos fluxos profissionais..." -ForegroundColor Cyan
foreach ($token in @('.pro-workout-plan-grid','.pro-flow-actions','.nutrition-plan-grid-v1812','.nutrition-meal-summary-v1812')) { if (-not $cssSource.Contains($token)) { throw "Layout profissional v0.18.12 incompleto: $token" } }
Write-Host "    Layout de treino e nutricao: OK."

Write-Host "[1668/1670] Validando schema preservado na v0.18.12..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.18.12_professional_flow_hardening.sql) { throw "v0.18.12 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.18.12." }
Write-Host "    Schema 39/39 preservado: OK."

Write-Host "[1669/1670] Validando versao publica v0.18.14..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.18.14 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.14." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    Health + VERSION + interface de producao: OK."

Write-Host "[1670/1670] Validando versao funcional v0.18.12..." -ForegroundColor Cyan
foreach ($token in @('Novo plano','Adicionar treino','Editar / agregar','Modelos de dieta','Modelos de refeição')) { if (-not $appJsSource.Contains($token)) { throw "Entrega funcional v0.18.12 incompleta: $token" } }
Write-Host "    v0.18.12 / Professional Workout & Nutrition Flow Hardening: OK." -ForegroundColor Green


Write-Host "[1671/1682] Validando Nutrition Builder Large Catalog..." -ForegroundColor Cyan
foreach ($token in @("HP_NUTRITION_LARGE_CATALOG='v0.18.13'","HP_NUTRITION_FOOD_SEARCH_LIMIT=120","HP_NUTRITION_SEARCH_DEBOUNCE_MS=120")) { if (-not $appJsSource.Contains($token)) { throw "Nutrition Large Catalog incompleto: $token" } }
Write-Host "    Marcadores funcionais v0.18.14: OK."

Write-Host "[1672/1682] Validando busca normalizada de alimentos..." -ForegroundColor Cyan
foreach ($token in @('hpNutritionNormalizeSearch','normalize(''NFD'')','tokens.every','name.startsWith(q)','category.includes(t)')) { if (-not $appJsSource.Contains($token)) { throw "Busca nutricional normalizada incompleta: $token" } }
Write-Host "    Busca sem acentos + múltiplos termos: OK."

Write-Host "[1673/1682] Validando limite seguro do catalogo..." -ForegroundColor Cyan
foreach ($token in @('slice(0,q?120:80)','mostrando ${visible.length} de ${(alimentos||[]).length}','resultado(s)')) { if (-not $appJsSource.Contains($token)) { throw "Limite do catálogo alimentar incompleto: $token" } }
Write-Host "    Selects não renderizam o catálogo inteiro: OK."

Write-Host "[1674/1682] Validando debounce de busca..." -ForegroundColor Cyan
foreach ($token in @('let timer=null','clearTimeout(timer)','setTimeout(()=>','},120)')) { if (-not $appJsSource.Contains($token)) { throw "Debounce nutricional incompleto: $token" } }
Write-Host "    Busca incremental com debounce: OK."

Write-Host "[1675/1682] Validando totais por refeicao..." -ForegroundColor Cyan
foreach ($token in @('hpNutritionBuilderMealTotals','.nutrition-meal-live-total','Fibra ${num(mt.fib)}g')) { if (-not $appJsSource.Contains($token)) { throw "Totais por refeicao incompletos: $token" } }
Write-Host "    Kcal + P/C/G/fibra por refeição: OK."

Write-Host "[1676/1682] Validando total diario e metas..." -ForegroundColor Cyan
foreach ($token in @("['Fibra',t.fib,Number(val(f,'metaFibrasG')||0),'g']",'nutritionBuilder2MacroPreview','nutritionBuilder2TargetDiff')) { if (-not $appJsSource.Contains($token)) { throw "Resumo nutricional diário incompleto: $token" } }
Write-Host "    Total diário + comparação de fibra: OK."

Write-Host "[1677/1682] Validando estruturas rapidas de refeicoes..." -ForegroundColor Cyan
foreach ($token in @('nutritionMealPresets','data-meal-preset="3"','data-meal-preset="4"','data-meal-preset="5"','data-meal-preset="6"','Ceia')) { if (-not $appJsSource.Contains($token)) { throw "Presets de refeições incompletos: $token" } }
Write-Host "    Estruturas 3/4/5/6 refeições: OK."

Write-Host "[1678/1682] Validando preservacao antes de trocar estrutura..." -ForegroundColor Cyan
foreach ($token in @('Aplicar esta estrutura substitui as refeições do rascunho atual. Continuar?','Estrutura com ${structure.length} refeições aplicada.')) { if (-not $appJsSource.Contains($token)) { throw "Proteção de rascunho nutricional incompleta: $token" } }
Write-Host "    Confirmação antes de substituir rascunho: OK."

Write-Host "[1679/1682] Validando UX do catalogo grande..." -ForegroundColor Cyan
foreach ($token in @('.nutrition-search-hint','.nutrition-meal-footer','.nutrition-meal-presets','.nutrition-meal-live-total')) { if (-not $cssSource.Contains($token)) { throw "CSS Large Catalog incompleto: $token" } }
Write-Host "    UX desktop + mobile: OK."

Write-Host "[1680/1682] Validando schema preservado na v0.18.13..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.18.13_nutrition_large_catalog.sql) { throw "v0.18.13 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.18.13." }
Write-Host "    Schema 39/39 preservado: OK."

Write-Host "[1681/1682] Validando versao publica v0.18.14..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.18.14 inconsistente." }
...
Write-Host "[1682/1682] Validando versao funcional v0.18.13..." -ForegroundColor Cyan
foreach ($token in @("HP_NUTRITION_LARGE_CATALOG='v0.18.13'",'Estrutura rápida:','Busca inteligente por nome e categoria','TOTAL ESTIMADO DO PLANO')) { if (-not $appJsSource.Contains($token)) { throw "Entrega funcional v0.18.13 incompleta: $token" } }
Write-Host "    v0.18.13 / Nutrition Builder Large Catalog: OK." -ForegroundColor Green


Write-Host "[1683/1694] Validando Nutrition Templates 2.0..." -ForegroundColor Cyan
foreach ($token in @("HP_NUTRITION_TEMPLATES_2='v0.18.14'",'Biblioteca profissional de nutrição','hpOpenNutritionTemplates2','openDietMealLibrary=hpOpenNutritionTemplates2')) { if (-not $appJsSource.Contains($token)) { throw "Nutrition Templates 2.0 incompleto: $token" } }
Write-Host "    Biblioteca profissional v0.18.14: OK."

Write-Host "[1684/1694] Validando correcao do erro de abertura..." -ForegroundColor Cyan
foreach ($token in @('retryNutritionTemplatesV1814','nutrition-template-load-error','Não foi possível abrir os modelos de nutrição.')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Hardening de abertura dos modelos incompleto: $token" } }
if ($appJsSource.Contains("$('#newStandaloneWorkoutTemplate').onclick=()=>openStandaloneWorkoutBuilder();")) { throw "Handler de treino indevido ainda presente na biblioteca nutricional." }
Write-Host "    Abertura + retry sem handlers inexistentes: OK."

Write-Host "[1685/1694] Validando KPIs da biblioteca nutricional..." -ForegroundColor Cyan
foreach ($token in @('nutrition-template-kpis','Dietas ativas','Refeições ativas','Refeições nos planos','Itens catalogados')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "KPIs de modelos incompletos: $token" } }
Write-Host "    KPIs profissionais: OK."

Write-Host "[1686/1694] Validando busca e filtros de modelos..." -ForegroundColor Cyan
foreach ($token in @('hpNutritionTemplateNormalize','nutritionTemplateSearchV1814','nutritionTemplateStatusV1814','nutritionTemplateCategoryV1814')) { if (-not $appJsSource.Contains($token)) { throw "Busca/filtros de modelos incompletos: $token" } }
Write-Host "    Busca + status + categoria: OK."

Write-Host "[1687/1694] Validando ordenacao dos modelos..." -ForegroundColor Cyan
foreach ($token in @('hpNutritionTemplateSort','name-desc','items-desc','meals-desc')) { if (-not $appJsSource.Contains($token)) { throw "Ordenacao de modelos incompleta: $token" } }
Write-Host "    Ordenação profissional: OK."

Write-Host "[1688/1694] Validando atribuicao de modelos de dieta..." -ForegroundColor Cyan
foreach ($token in @('diet-plan-assign-v1814','openMealTemplateCreateForm(patient,m)','openDietLibraryPatientPicker(m,''plan'')')) { if (-not $appJsSource.Contains($token)) { throw "Atribuicao de dieta-modelo incompleta: $token" } }
Write-Host "    Modelo de dieta -> paciente: OK."

Write-Host "[1689/1694] Validando insercao de modelos de refeicao..." -ForegroundColor Cyan
foreach ($token in @('meal-model-insert-v1814','openMealLibraryPlanPicker(patient,m)','openDietLibraryPatientPicker(m,''meal'')')) { if (-not $appJsSource.Contains($token)) { throw "Insercao de refeicao-modelo incompleta: $token" } }
Write-Host "    Modelo de refeição -> plano: OK."

Write-Host "[1690/1694] Validando gestao de modelos..." -ForegroundColor Cyan
foreach ($token in @('diet-plan-edit-v1814','meal-model-edit-v1814','diet-plan-toggle-v1814','meal-model-toggle-v1814')) { if (-not $appJsSource.Contains($token)) { throw "Gestao de modelos incompleta: $token" } }
Write-Host "    Editar + ativar/desativar: OK."

Write-Host "[1691/1694] Validando CTA contextual de nova dieta..." -ForegroundColor Cyan
foreach ($token in @('nutritionTemplateNewDietV1814','+ Montar dieta para','openMealPlanForm(patient)')) { if (-not $appJsSource.Contains($token)) { throw "CTA contextual da dieta incompleto: $token" } }
Write-Host "    Biblioteca -> montar dieta: OK."

Write-Host "[1692/1694] Validando responsividade Nutrition Templates..." -ForegroundColor Cyan
foreach ($token in @('.nutrition-template-kpis','.nutrition-template-toolbar','.nutrition-template-guidance','@media(max-width:620px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Nutrition Templates incompleta: $token" } }
Write-Host "    Desktop + tablet + mobile: OK."

Write-Host "[1693/1694] Validando schema e versao publica v0.18.14..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.18.14_nutrition_templates_2.sql) { throw "v0.18.14 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.18.14." }
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.18.14 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.14." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    Schema 39/39 + Health + VERSION + interface de producao: OK."

Write-Host "[1694/1694] Validando versao funcional v0.18.14..." -ForegroundColor Cyan
foreach ($token in @('Biblioteca profissional de nutrição','Modelos de dieta','Modelos de refeição','Todas as categorias','↻ Atualizar')) { if (-not $appJsSource.Contains($token)) { throw "Entrega funcional v0.18.14 incompleta: $token" } }
Write-Host "    v0.18.14 / Nutrition Templates 2.0: OK." -ForegroundColor Green


Write-Host "[1695/1706] Validando Nutrition Review & Publish 2.0..." -ForegroundColor Cyan
foreach ($token in @("HP_NUTRITION_REVIEW_PUBLISH_2='v0.18.15'",'REVISÃO E PUBLICAÇÃO NUTRICIONAL','hpOpenNutritionReviewPublish2','Revisar & publicar alimentação')) { if (-not $appJsSource.Contains($token)) { throw "Nutrition Review & Publish 2.0 incompleto: $token" } }
Write-Host "    Revisão nutricional v0.18.15: OK."

Write-Host "[1696/1706] Validando entrada pela aba Nutricao..." -ForegroundColor Cyan
foreach ($token in @('nutritionReviewPublishV1815','Revisar & publicar','__hpRenderNutritionProfessionalFlow_v01815')) { if (-not $appJsSource.Contains($token)) { throw "Entrada de revisão nutricional incompleta: $token" } }
Write-Host "    Acesso direto na aba Nutrição: OK."

Write-Host "[1697/1706] Validando historico de versoes nutricionais..." -ForegroundColor Cyan
foreach ($token in @('nutrition-review-version-list-v1815','nutrition-review-open-v1815','nutrition-review-publish-v1815','Versões do plano alimentar')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Histórico nutricional incompleto: $token" } }
Write-Host "    Histórico + seleção de versões: OK."

Write-Host "[1698/1706] Validando comparativo meta versus prescrito..." -ForegroundColor Cyan
foreach ($token in @('hpNutritionDeviation','hpNutritionReviewMetric','Calorias','Proteína','Carboidrato','Gordura','Fibra')) { if (-not $appJsSource.Contains($token)) { throw "Comparativo nutricional incompleto: $token" } }
Write-Host "    Kcal + macros + fibra contra metas: OK."

Write-Host "[1699/1706] Validando revisao das refeicoes..." -ForegroundColor Cyan
foreach ($token in @('nutrition-review-meals-v1815','Horário livre','item(ns)','Orientações gerais')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Revisão de refeições incompleta: $token" } }
Write-Host "    Refeições + horários + itens + orientações: OK."

Write-Host "[1700/1706] Validando checklist de publicacao..." -ForegroundColor Cyan
foreach ($token in @('Checklist antes de publicar','Revise metas, horários, quantidades, substituições e orientações.','nutrition-review-safety-v1815')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Checklist de publicação incompleto: $token" } }
Write-Host "    Segurança antes da publicação: OK."

Write-Host "[1701/1706] Validando publicacao explicita..." -ForegroundColor Cyan
foreach ($token in @("hpPublishPlanVersion('nutrition',p.id,selected.id)","hpPublishPlanVersion('nutrition',p.id,plan.id)",'Publicar esta versão')) { if (-not $appJsSource.Contains($token)) { throw "Publicação nutricional explícita incompleta: $token" } }
Write-Host "    Publicação explícita + histórico preservado: OK."

Write-Host "[1702/1706] Validando atalhos profissionais..." -ForegroundColor Cyan
foreach ($token in @('nutritionReviewEditV1815','nutritionReviewTemplatesV1815','openMealPlanForm(p,selected)',"openDietMealLibrary(p,'plans')")) { if (-not $appJsSource.Contains($token)) { throw "Atalhos profissionais da revisão incompletos: $token" } }
Write-Host "    Editar versão + modelos de dieta: OK."

Write-Host "[1703/1706] Validando retry da revisao nutricional..." -ForegroundColor Cyan
foreach ($token in @('nutritionReviewRetryV1815','nutrition-review-load-error-v1815','Não foi possível carregar a revisão nutricional.')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Retry da revisão nutricional incompleto: $token" } }
Write-Host "    Falha recuperável + retry: OK."

Write-Host "[1704/1706] Validando responsividade da revisao..." -ForegroundColor Cyan
foreach ($token in @('.nutrition-publish-summary-v1815','.nutrition-review-metrics-v1815','.nutrition-review-version-actions','@media(max-width:430px)')) { if (-not $cssSource.Contains($token)) { throw "Responsividade Nutrition Review incompleta: $token" } }
Write-Host "    Desktop + tablet + mobile: OK."

Write-Host "[1705/1706] Validando schema e versao publica v0.18.15..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.18.15_nutrition_review_publish_2.sql) { throw "v0.18.15 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.18.15." }
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.18.15 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.18.15." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    Schema 39/39 + Health + VERSION + interface de producao: OK."

Write-Host "[1706/1706] Validando versao funcional v0.18.15..." -ForegroundColor Cyan
foreach ($token in @('Versão publicada','Versões disponíveis','Refeições da revisão','Revisar detalhes','Publicar versão')) { if (-not $appJsSource.Contains($token)) { throw "Entrega funcional v0.18.15 incompleta: $token" } }
Write-Host "    v0.18.15 / Nutrition Review & Publish 2.0: OK." -ForegroundColor Green


Write-Host "[1707/1718] Validando Workout Builder Delete + Weekly Patient Plan..." -ForegroundColor Cyan
foreach ($token in @("HP_WORKOUT_DELETE_WEEKLY_PLAN='v0.18.16'",'Remover exercício','patient-weekly-workout-v1816','Treinos da semana')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Workout/Weekly v0.18.16 incompleto: $token" } }
Write-Host "    Marcadores funcionais v0.18.16: OK."

Write-Host "[1708/1718] Validando remocao explicita de exercicio..." -ForegroundColor Cyan
foreach ($token in @('aria-label="Remover exercício"','e.preventDefault();','e.stopPropagation();','row.remove();','removido do treino.')) { if (-not $appJsSource.Contains($token)) { throw "Remocao de exercicio incompleta: $token" } }
Write-Host "    Remover exercício funciona sem submeter formulário: OK."

Write-Host "[1709/1718] Validando touch target do remover..." -ForegroundColor Cyan
foreach ($token in @('.remove-workout-item','min-height:38px','min-height:44px','workout-item-tools')) { if (-not $cssSource.Contains($token)) { throw "Touch target remover exercício incompleto: $token" } }
Write-Host "    Ação de remover legível no desktop/mobile: OK."

Write-Host "[1710/1718] Validando identificacao do treino de hoje..." -ForegroundColor Cyan
foreach ($token in @('hpWorkoutTodayAliases','hpWorkoutSessionMatchesToday','hpWorkoutTodaySession','diasSemana')) { if (-not $appJsSource.Contains($token)) { throw "Selecao de treino por dia incompleta: $token" } }
Write-Host "    Dia prescrito usado antes do fallback: OK."

Write-Host "[1711/1718] Validando visao semanal completa..." -ForegroundColor Cyan
foreach ($token in @('hpPatientWeeklyWorkoutPlan','SEU PLANO COMPLETO','Veja toda a ficha, não só o treino sugerido para hoje.','patient-weekly-workout-grid')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Visao semanal do paciente incompleta: $token" } }
Write-Host "    Todas as sessões ficam visíveis no portal: OK."

Write-Host "[1712/1718] Validando destaque da sessao de hoje..." -ForegroundColor Cyan
foreach ($token in @("String(s.id)===String(today?.id)?'today':''",'HOJE','weekly-workout-card-top')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Destaque do treino de hoje incompleto: $token" } }
Write-Host "    Hoje destacado sem esconder a semana: OK."

Write-Host "[1713/1718] Validando navegacao semanal para ficha..." -ForegroundColor Cyan
foreach ($token in @('data-week-session','patient-session-${s.id}','scrollIntoView({behavior:''smooth'',block:''start''})','weekly-focus-v1816')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Navegacao semanal -> ficha incompleta: $token" } }
Write-Host "    Card semanal -> ficha completa: OK."

Write-Host "[1714/1718] Validando inicio do treino correto..." -ForegroundColor Cyan
foreach ($token in @('const sessao=hpWorkoutTodaySession(p);','openWorkoutExecutionForm(sessao)','String(x.id)===String(b.dataset.session)')) { if (-not $appJsSource.Contains($token)) { throw "Inicio de treino correto incompleto: $token" } }
Write-Host "    CTA do dia abre a sessão correspondente: OK."

Write-Host "[1715/1718] Validando experiencia mobile da semana..." -ForegroundColor Cyan
foreach ($token in @('scroll-snap-type:x mandatory','flex:0 0 min(78vw,285px)','patient-workout-session .start-workout','@media(max-width:430px)')) { if (-not $cssSource.Contains($token)) { throw "Mobile Weekly Workout incompleto: $token" } }
Write-Host "    Carrossel semanal + CTA full width: OK."

Write-Host "[1716/1718] Validando preservacao da ficha completa..." -ForegroundColor Cyan
foreach ($token in @('patient-workout-session','patient-exercise-list','exercise-prescription','Registrar treino')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Ficha completa do paciente ausente: $token" } }
Write-Host "    Exercícios e prescrição continuam disponíveis: OK."

Write-Host "[1717/1718] Validando schema e versao publica v0.19.1..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.19.1_workout_weekly_patient.sql) { throw "v0.19.1 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.19.1." }
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.19.1 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.1." }
if (-not $indexSource.Contains('AESYN') -or $indexSource.Contains('MVP Preview')) { throw "Interface publica de producao inconsistente." }
Write-Host "    Schema 39/39 + Health + VERSION + interface de producao: OK."

Write-Host "[1718/1718] Validando versao funcional v0.19.1..." -ForegroundColor Cyan
foreach ($token in @('Remover exercício','SEU PLANO COMPLETO','Treinos da semana','Ver ficha ›','Sua programação completa continua disponível abaixo.')) { if (-not $appJsSource.Contains($token)) { throw "Entrega funcional v0.19.1 incompleta: $token" } }
...
foreach ($token in @("HP_MVP_VERSION='0.19.43'",'HP_THEME_KEY','hpInstallThemeUi','loadPatientHealth','patient-health-hub-v0190')) { if (-not ($appJsSource.Contains($token) -or $cssSource.Contains($token))) { throw "Foundation v0.19.1 incompleta: $token" } }
Write-Host "    Marcadores funcionais v0.19.1: OK."

Write-Host "[1720/1728] Validando nova identidade AESYN..." -ForegroundColor Cyan
foreach ($token in @('--aesyn-petrol-950','--aesyn-mint','#063a35','.aesyn-mark:before','.aesyn-mark:after')) { if (-not $cssSource.Contains($token)) { throw "Identidade AESYN v0.19.1 incompleta: $token" } }
Write-Host "    Verde-petroleo + monograma AESYN: OK."

Write-Host "[1721/1728] Validando escolha de tema no primeiro login..." -ForegroundColor Cyan
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
foreach ($token in @('themeChoiceModal','data-theme-choice="light"','data-theme-choice="dark"','data-theme-toggle')) { if (-not $indexSource.Contains($token)) { throw "UI de tema incompleta: $token" } }
foreach ($token in @('hp_aesyn_theme','hp_aesyn_theme_chosen','hpMaybeOpenThemeChoice','hpToggleTheme')) { if (-not $appJsSource.Contains($token)) { throw "Persistencia de tema incompleta: $token" } }
Write-Host "    Primeiro login + troca posterior de tema: OK."

Write-Host "[1722/1728] Validando tema escuro..." -ForegroundColor Cyan
foreach ($token in @('html[data-theme="dark"]','color-scheme','#071513','Dark mode hardening')) { if (-not ($cssSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Tema escuro incompleto: $token" } }
Write-Host "    Dark mode e superfícies legadas: OK."

Write-Host "[1723/1728] Validando navegacao principal do paciente..." -ForegroundColor Cyan
foreach ($token in @('data-patient-view="inicio">Hoje','data-patient-view="treino">Treino','data-patient-view="plano">Plano','data-patient-view="saude">Saúde')) { if (-not $indexSource.Contains($token)) { throw "Navegacao principal v0.19.1 incompleta: $token" } }
Write-Host "    Hoje + Treino + Plano + Saude: OK."

Write-Host "[1724/1728] Validando Hub Saude..." -ForegroundColor Cyan
foreach ($token in @('EXAMES LABORATORIAIS','MONITORAMENTO','RECUPERAÇÃO','COMO ESTOU','data-health-view')) { if (-not $appJsSource.Contains($token)) { throw "Hub Saude incompleto: $token" } }
Write-Host "    Exames + Monitoramento + Recuperacao + Como estou: OK."

Write-Host "[1725/1728] Validando responsividade da foundation..." -ForegroundColor Cyan
foreach ($token in @('.patient-health-grid-v0190','@media(max-width:720px)','.theme-choice-grid','.theme-toggle')) { if (-not $cssSource.Contains($token)) { throw "Responsividade v0.19.1 incompleta: $token" } }
Write-Host "    Desktop + mobile: OK."

Write-Host "[1726/1728] Validando ausencia de schema novo..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.19.1_identity_navigation.sql) { throw "v0.19.1 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.19.1." }
Write-Host "    Schema preservado: OK."

Write-Host "[1727/1728] Validando versao publica v0.19.1..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.19.1 inconsistente." }
...
Write-Host "[1728/1728] Validando versao funcional v0.19.1..." -ForegroundColor Cyan
foreach ($token in @('Como você prefere visualizar a AESYN?','Seu acompanhamento em um só lugar','Saúde','Tema')) { if (-not ($indexSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Entrega funcional v0.19.1 incompleta: $token" } }
Write-Host "    v0.19.1 / AESYN Identity & Patient Navigation Foundation: OK." -ForegroundColor Green

Write-Host "[1729/1735] Validando Production HTTPS & Proxy Hardening..." -ForegroundColor Cyan
$programSource = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
foreach ($token in @('ForwardedHeadersOptions','ForwardedHeaders.XForwardedFor','ForwardedHeaders.XForwardedProto','app.UseForwardedHeaders();')) { if (-not $programSource.Contains($token)) { throw "Proxy HTTPS v0.19.1 incompleto: $token" } }
Write-Host "    Forwarded headers processados antes do HTTPS: OK."

Write-Host "[1730/1735] Validando HSTS e redirecionamento HTTPS..." -ForegroundColor Cyan
foreach ($token in @('app.UseHsts();','app.UseHttpsRedirection();','!app.Environment.IsDevelopment()')) { if (-not $programSource.Contains($token)) { throw "HTTPS hardening incompleto: $token" } }
Write-Host "    HSTS + HTTPS redirect fora de Development: OK."

Write-Host "[1731/1735] Validando ordem segura do pipeline proxy/HTTPS..." -ForegroundColor Cyan
$forwardedPos = $programSource.IndexOf('app.UseForwardedHeaders();')
$hstsPos = $programSource.IndexOf('app.UseHsts();')
$httpsPos = $programSource.IndexOf('app.UseHttpsRedirection();')
if ($forwardedPos -lt 0 -or $hstsPos -lt 0 -or $httpsPos -lt 0 -or $forwardedPos -gt $hstsPos -or $forwardedPos -gt $httpsPos) { throw "Ordem do pipeline proxy/HTTPS incorreta." }
Write-Host "    ForwardedHeaders -> HSTS/HTTPS: OK."

Write-Host "[1732/1735] Validando guia operacional HTTPS..." -ForegroundColor Cyan
if (-not (Test-Path .\PRODUCAO-HTTPS.md)) { throw "PRODUCAO-HTTPS.md ausente." }
$httpsGuide = Get-Content .\PRODUCAO-HTTPS.md -Encoding UTF8 -Raw
foreach ($token in @('proxy_set_header X-Forwarded-Proto $scheme;','return 301 https://$host$request_uri;','curl -I https://aesyn.com.br/api/health')) { if (-not $httpsGuide.Contains($token)) { throw "Guia HTTPS incompleto: $token" } }
Write-Host "    Nginx + redirect + validacao de health: OK."

Write-Host "[1733/1735] Validando ausencia de schema novo v0.19.1..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.19.1_https_proxy_hardening.sql) { throw "v0.19.1 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.19.1." }
Write-Host "    Schema preservado: OK."

Write-Host "[1734/1735] Validando versao publica v0.19.1..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.19.1 inconsistente." }
...
Write-Host "[1735/1735] Validando versao funcional v0.19.1..." -ForegroundColor Cyan
foreach ($token in @('UseForwardedHeaders','UseHsts','UseHttpsRedirection','PRODUCAO-HTTPS.md')) { if ($token -eq 'PRODUCAO-HTTPS.md') { if (-not (Test-Path .\PRODUCAO-HTTPS.md)) { throw "Entrega funcional v0.19.1 incompleta: $token" } } elseif (-not $programSource.Contains($token)) { throw "Entrega funcional v0.19.1 incompleta: $token" } }
Write-Host "    v0.19.1 / Production HTTPS & Proxy Hardening: OK." -ForegroundColor Green


Write-Host "[1736/1742] Validando Production Experience Cleanup..." -ForegroundColor Cyan
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
foreach ($token in @('Ambiente AESYN.','Saúde • Performance • Longevidade','>Ajuda<','dataset.environment')) { if (-not ($indexSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Production cleanup incompleto: $token" } }
Write-Host "    Identidade pública de produção: OK."

Write-Host "[1737/1742] Validando remocao de textos de demo da home..." -ForegroundColor Cyan
foreach ($token in @('Ambiente de demonstração.','Roteiro da demo','AESYN • DEMO','Senha configurada para esta demo','MVP Preview •','>Demo</span>')) { if ($indexSource.Contains($token)) { throw "Texto de demo ainda visível na interface: $token" } }
if ($appJsSource.Contains('Neste MVP local')) { throw "Texto legado de MVP local ainda visivel na interface." }
Write-Host "    Home sem linguagem de demo/preview: OK."

Write-Host "[1738/1742] Validando central de ajuda em linguagem de producao..." -ForegroundColor Cyan
foreach ($token in @('CENTRAL DE AJUDA AESYN','Guia rápido da plataforma','AESYN Performance')) { if (-not $appJsSource.Contains($token)) { throw "Central de ajuda incompleta: $token" } }
Write-Host "    Ajuda convertida para uso real: OK."

Write-Host "[1739/1742] Validando ambiente de producao no cliente..." -ForegroundColor Cyan
if (-not $appJsSource.Contains("dataset.environment='production'")) { throw "Marcador de ambiente de producao ausente." }
if ($appJsSource.Contains("dataset.mvp='preview'")) { throw "Marcador MVP preview ainda ativo." }
Write-Host "    Ambiente de produção identificado: OK."

Write-Host "[1740/1742] Validando ausencia de schema novo v0.19.2..." -ForegroundColor Cyan
if (Test-Path .\scripts\sql\v0.19.2_production_experience_cleanup.sql) { throw "v0.19.2 nao deveria criar schema novo." }
if (-not $setupSource.Contains('[39/39] Aplicando upgrade v0.18.10')) { throw "Baseline 39/39 ausente apos v0.19.2." }
Write-Host "    Schema preservado: OK."

Write-Host "[1741/1742] Validando versao publica atual..." -ForegroundColor Cyan
$version = Get-Content .\VERSION.txt -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne '0.19.43' -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Versao publica v0.19.5 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.5." }
Write-Host "    Health + VERSION: OK."

Write-Host "[1742/1742] Validando versao funcional v0.19.2..." -ForegroundColor Cyan
foreach ($token in @('Ambiente AESYN.','CENTRAL DE AJUDA AESYN','Guia rápido da plataforma','production')) { if (-not ($indexSource.Contains($token) -or $appJsSource.Contains($token))) { throw "Entrega funcional v0.19.2 incompleta: $token" } }
Write-Host "    v0.19.2 / Production Experience Cleanup: OK." -ForegroundColor Green



# v0.19.3 - Patient Workout Access Hub
Write-Host "[1743/1749] Validando Patient Workout Access Hub..."
$appJs0193 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Raw
foreach ($token in @("HP_PATIENT_WORKOUT_ACCESS_HUB='v0.19.3'", "hpPatientWorkoutAccessHub", "data-access-session", "Escolha o treino que quer fazer hoje", "openWorkoutExecutionForm(sessao)")) {
    if (-not $appJs0193.Contains($token)) { throw "Patient Workout Access Hub incompleto: $token" }
}
Write-Host "[1744/1749] Validando atalhos A/B/C de treino..."
foreach ($token in @('hpWorkoutLetter', 'TREINO ${hpWorkoutLetter(index)}', 'patient-workout-access-grid')) {
    if (-not $appJs0193.Contains($token)) { throw "Atalhos de treino incompletos: $token" }
}
Write-Host "[1745/1749] Validando inicio livre de sessao..."
if (-not $appJs0193.Contains("if(sessao)openWorkoutExecutionForm(sessao)")) { throw "Inicio livre de treino ausente." }
Write-Host "[1746/1749] Validando estilo responsivo do hub de treinos..."
$appCss0193 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Raw
foreach ($token in @("patient-workout-access-hub", "patient-workout-access-card", "patient-workout-access-letter")) {
    if (-not $appCss0193.Contains($token)) { throw "CSS do Workout Access Hub incompleto: $token" }
}
Write-Host "[1747/1749] Validando versao publica atual (gate v0.19.3)..."
$version0193 = Get-Content (Join-Path $root "VERSION.txt") -Raw
if ($version0193.Trim() -ne "0.19.43") { throw "VERSION.txt inesperado para a versao atual." }
...
if (-not $healthSource0193.Contains('version = "0.19.43"')) { throw "Health endpoint nao anuncia a versao atual." }
Write-Host "[1749/1749] Validando funcionalidade v0.19.3..."
Write-Host "    v0.19.3 / Patient Workout Access Hub: OK."

# v0.19.4 - Advanced Workout Techniques Builder
Write-Host "[1750/1757] Validando tecnicas avancadas no Workout Builder..."
$appJs0194 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
foreach ($token in @("HP_ADVANCED_WORKOUT_TECHNIQUES='v0.19.4'", 'name="tecnica"', 'BISET / conjugado', 'DROP set', 'Progressão de carga')) {
    if (-not $appJs0194.Contains($token)) { throw "Workout Techniques v0.19.4 incompleto: $token" }
}
Write-Host "[1751/1757] Validando BISET conjugado..."
foreach ($token in @('grupoBiset', 'add-biset-pair', 'Agregar exercício conjugado', 'Par ${group} criado')) { if (-not $appJs0194.Contains($token)) { throw "BISET incompleto: $token" } }
Write-Host "[1752/1757] Validando DROP set..."
foreach ($token in @('dropReducaoPercentual', 'dropEtapas', 'Redução da carga (%)', 'Etapas adicionais')) { if (-not $appJs0194.Contains($token)) { throw "DROP incompleto: $token" } }
Write-Host "[1753/1757] Validando progressao de carga..."
foreach ($token in @('progressaoCargaPercentual', 'progressaoRegra', 'Progressão alvo (%)', 'Regra / gatilho')) { if (-not $appJs0194.Contains($token)) { throw "Progressao de carga incompleta: $token" } }
Write-Host "[1754/1757] Validando persistencia estruturada no treino-modelo..."
$modelSource0194 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/ModelosPlanosTreinoController.cs") -Encoding UTF8 -Raw
foreach ($token in @('string? Tecnica = null','string? GrupoBiset = null','decimal? DropReducaoPercentual = null','decimal? ProgressaoCargaPercentual = null','ObservacaoComTecnica')) { if (-not $modelSource0194.Contains($token)) { throw "Persistencia de tecnicas incompleta: $token" } }
Write-Host "[1755/1757] Validando estilos responsivos das tecnicas..."
$appCss0194 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @('advanced-technique-panel','advanced-technique-config','has-advanced-technique')) { if (-not $appCss0194.Contains($token)) { throw "CSS tecnicas avancadas incompleto: $token" } }
Write-Host "[1756/1757] Validando versao publica v0.19.4..."
$version0194 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health0194 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Raw
if ($version0194.Trim() -ne '0.19.43' -or -not $appJs0194.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $health0194.Contains('version = "0.19.43"')) { throw "Versao publica v0.19.5 inconsistente." }
Write-Host "[1757/1757] Validando versao funcional v0.19.4..."
Write-Host "    v0.19.4 / Advanced Workout Techniques Builder: OK." -ForegroundColor Green



# v0.19.5 - Patient Advanced Technique Guidance
Write-Host "[1758/1765] Validando leitura das tecnicas avancadas no portal do paciente..."
$appJs0195 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
foreach ($token in @("HP_PATIENT_ADVANCED_TECHNIQUE_GUIDANCE='v0.19.5'", 'hpWorkoutTechniqueMeta', 'hpWorkoutTechniqueBadge', 'hpWorkoutTechniqueGuidance')) { if (-not $appJs0195.Contains($token)) { throw "Patient Technique Guidance incompleto: $token" } }
Write-Host "[1759/1765] Validando orientacao de BISET na execucao..."
foreach ($token in @("type:'biset'", "label:'BISET'", 'Exercício conjugado', 'mesmo BISET')) { if (-not $appJs0195.Contains($token)) { throw "BISET do paciente incompleto: $token" } }
Write-Host "[1760/1765] Validando orientacao de DROP na execucao..."
foreach ($token in @("type:'drop'", "label:'DROP'", 'reduza aproximadamente', 'Carga de referência após redução')) { if (-not $appJs0195.Contains($token)) { throw "DROP do paciente incompleto: $token" } }
Write-Host "[1761/1765] Validando orientacao de progressao de carga..."
foreach ($token in @("type:'progression'", "label:'PROGRESSÃO'", 'Progressão de carga', 'Próxima carga de referência')) { if (-not $appJs0195.Contains($token)) { throw "Progressao do paciente incompleta: $token" } }
Write-Host "[1762/1765] Validando tecnicas no formulario de execucao..."
foreach ($token in @('execution-item ${hpWorkoutTechniqueClass(i)}', '${hpWorkoutTechniqueBadge(i)}', '${hpWorkoutTechniqueGuidance(i)}')) { if (-not $appJs0195.Contains($token)) { throw "Execucao com tecnica incompleta: $token" } }
Write-Host "[1763/1765] Validando estilos premium das tecnicas no paciente..."
$appCss0195 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @('workout-technique-badge','workout-technique-guidance','technique-biset','technique-drop','technique-progression')) { if (-not $appCss0195.Contains($token)) { throw "CSS Patient Technique Guidance incompleto: $token" } }
Write-Host "[1764/1765] Validando versao publica v0.19.5..."
$version0195 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health0195 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Encoding UTF8 -Raw
if ($version0195.Trim() -ne '0.19.43' -or -not $appJs0195.Contains("const HP_MVP_VERSION='0.19.43';") -or -not $health0195.Contains('version = "0.19.43"')) { throw "Versao publica v0.19.5 inconsistente." }
Write-Host "[1765/1765] Validando versao funcional v0.19.5..."
Write-Host "    v0.19.5 / Patient Advanced Technique Guidance: OK." -ForegroundColor Green



# v0.19.6 - Patient Progress Photos & Markup
Write-Host "[1766/1773] Validando fotos de evolucao corporal..." -ForegroundColor Cyan
$appJs0196 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
foreach ($token in @("HP_PATIENT_PROGRESS_PHOTOS='v0.19.6'", 'data-evolution-photos="v0.19.6"', 'hpOpenEvolutionPhotoUpload', 'hpOpenEvolutionPhotoCompare', 'hpOpenEvolutionPhotoAnnotator', 'hpSecureImageUrl', 'hpHydrateSecureImages')) { if (-not $appJs0196.Contains($token)) { throw "Fotos de evolucao v0.19.6 incompletas: $token" } }
Write-Host "[1767/1773] Validando upload seguro de imagens..." -ForegroundColor Cyan
$photoController0196 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/FotosEvolucaoController.cs") -Encoding UTF8 -Raw
foreach ($token in @('RequestSizeLimit(LimiteBytes)', 'image/jpeg', 'image/png', 'image/webp', 'OrganizacaoId == currentUser.OrganizationId', 'AssinaturaValida', 'App_Data', 'PhysicalFile', 'PacienteAutorizado')) { if (-not $photoController0196.Contains($token)) { throw "Upload de fotos incompleto: $token" } }
Write-Host "[1768/1773] Validando comparacao longitudinal..." -ForegroundColor Cyan
foreach ($token in @('Evolução lado a lado','evolutionCompareBefore','evolutionCompareAfter','Leitura longitudinal')) { if (-not $appJs0196.Contains($token)) { throw "Comparador de fotos incompleto: $token" } }
Write-Host "[1769/1773] Validando anotacao sem destruir original..." -ForegroundColor Cyan
foreach ($token in @('Marcar área de foco','evolutionAnnotatorCanvas','Salvar cópia anotada','FotoEvolucaoAnotada')) { if (-not $appJs0196.Contains($token)) { throw "Anotacao de foto incompleta: $token" } }
Write-Host "[1770/1773] Validando estilos responsivos de fotos..." -ForegroundColor Cyan
$appCss0196 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @('evolution-photo-grid','evolution-compare-stage','evolution-annotator-stage','evolution-photo-privacy')) { if (-not $appCss0196.Contains($token)) { throw "CSS de fotos incompleto: $token" } }
Write-Host "[1771/1773] Validando limpeza de versao visivel no dashboard..." -ForegroundColor Cyan
if ($appJs0196.Contains('PROFESSIONAL COMMAND CENTER • v0.18.1')) { throw "Dashboard ainda expoe versao historica." }
if ($appJs0196.Contains('Seed__AdminPassword')) { throw "Interface ainda expoe detalhe de infraestrutura de login." }
Write-Host "[1772/1773] Validando versao publica v0.19.6..." -ForegroundColor Cyan
$version0196 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health0196 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Encoding UTF8 -Raw
if ($version0196.Trim() -ne '0.19.43' -or -not $appJs0196.Contains("const HP_MVP_VERSION='0.19.43';") -or (($health0196 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2)) { throw "Versao publica v0.19.6 inconsistente." }
Write-Host "[1773/1773] Validando versao funcional v0.19.6..." -ForegroundColor Cyan
Write-Host "    v0.19.6 / Patient Progress Photos & Markup: OK." -ForegroundColor Green



# v0.19.7 - Metabolic Energy Calculator
Write-Host "[1774/1781] Validando calculadora metabolica profissional..." -ForegroundColor Cyan
$appJs0197 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
foreach ($token in @("HP_METABOLIC_ENERGY_CALCULATOR='v0.19.7'", 'data-metabolic-calculator="v0.19.7"', 'TMB e gasto energético estimado', 'hpBindMetabolicCalculator')) { if (-not $appJs0197.Contains($token)) { throw "Calculadora metabolica v0.19.8 incompleta: $token" } }
Write-Host "[1775/1781] Validando Mifflin-St Jeor..." -ForegroundColor Cyan
foreach ($token in @('Mifflin-St Jeor', '(10*w)+(6.25*h)-(5*age)', "sex==='masculino'?5:-161")) { if (-not $appJs0197.Contains($token)) { throw "Mifflin-St Jeor incompleta: $token" } }
Write-Host "[1776/1781] Validando composicao corporal opcional..." -ForegroundColor Cyan
foreach ($token in @('Katch-McArdle', '370+(21.6*lean)', 'massaMagraKg')) { if (-not $appJs0197.Contains($token)) { throw "Katch-McArdle incompleta: $token" } }
Write-Host "[1777/1781] Validando fator de atividade e GET..." -ForegroundColor Cyan
foreach ($token in @('HP_ACTIVITY_FACTORS', '1.2', '1.375', '1.55', '1.725', '1.9', 'bmr*factor')) { if (-not $appJs0197.Contains($token)) { throw "GET/fator de atividade incompleto: $token" } }
Write-Host "[1778/1781] Validando contexto automatico do paciente..." -ForegroundColor Cyan
foreach ($token in @('hpLoadMetabolicContext', '/avaliacoes`', 'dataNascimento', 'pesoKg', 'alturaM')) { if (-not $appJs0197.Contains($token)) { throw "Contexto metabolico automatico incompleto: $token" } }
Write-Host "[1779/1781] Validando aplicacao editavel da meta calorica..." -ForegroundColor Cyan
foreach ($token in @('Usar GET como meta calórica', 'form.elements.metaCalorias.value', 'Revise antes de salvar')) { if (-not $appJs0197.Contains($token)) { throw "Aplicacao de meta calorica incompleta: $token" } }
Write-Host "[1780/1781] Validando estilos responsivos da calculadora..." -ForegroundColor Cyan
$appCss0197 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @('metabolic-calculator','metabolic-input-grid','metabolic-results','metabolic-actions')) { if (-not $appCss0197.Contains($token)) { throw "CSS metabolico incompleto: $token" } }
Write-Host "[1781/1781] Validando versao funcional v0.19.7..." -ForegroundColor Cyan
$version0197 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health0197 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Encoding UTF8 -Raw
if ($version0197.Trim() -ne '0.19.43' -or -not $appJs0197.Contains("const HP_MVP_VERSION='0.19.43';") -or (($health0197 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2)) { throw "Versao publica atual inconsistente." }
Write-Host "    v0.19.7 / Metabolic Energy Calculator: OK." -ForegroundColor Green

# v0.19.8 - Weight Goal & Calorie Target Planner
Write-Host "[1782/1789] Validando planejamento de meta de peso..." -ForegroundColor Cyan
$appJs0198 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
foreach ($token in @("HP_WEIGHT_GOAL_CALORIE_TARGET='v0.19.8'", 'data-weight-goal-planner="v0.19.8"', 'Meta de peso e alvo energético', 'hpCalculateWeightGoal')) { if (-not $appJs0198.Contains($token)) { throw "Planejador de peso v0.19.8 incompleto: $token" } }
Write-Host "[1783/1789] Validando perda, manutencao e ganho..." -ForegroundColor Cyan
foreach ($token in @('<option value="perder">Perder peso</option>','<option value="manter">Manter peso</option>','<option value="ganhar">Ganhar peso</option>','weightGoalTarget','weightGoalWeeks')) { if (-not $appJs0198.Contains($token)) { throw "Objetivos de peso incompletos: $token" } }
Write-Host "[1784/1789] Validando calculo energetico por prazo..." -ForegroundColor Cyan
foreach ($token in @('HP_ENERGY_EQUIVALENT_KCAL_PER_KG=7700','weeks*7','targetCalories=get+adjustment','weeklyChange=deltaKg/weeks')) { if (-not $appJs0198.Contains($token)) { throw "Calculo de alvo energetico incompleto: $token" } }
Write-Host "[1785/1789] Validando alertas clinicos do planejamento..." -ForegroundColor Cyan
foreach ($token in @('weeklyPercent>1','targetCalories<bmr','targetCalories<1200','requer revisão profissional')) { if (-not $appJs0198.Contains($token)) { throw "Alertas de planejamento incompletos: $token" } }
Write-Host "[1786/1789] Validando aplicacao editavel do alvo..." -ForegroundColor Cyan
foreach ($token in @('Usar alvo como meta calórica','applyWeightGoalCalories','form.elements.metaCalorias.value','A projeção continua editável')) { if (-not $appJs0198.Contains($token)) { throw "Aplicacao do alvo calorico incompleta: $token" } }
Write-Host "[1787/1789] Validando estilos responsivos do objetivo..." -ForegroundColor Cyan
$appCss0198 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @('weight-goal-planner','weight-goal-grid','weight-goal-results','weight-goal-safety','weight-goal-actions')) { if (-not $appCss0198.Contains($token)) { throw "CSS de meta de peso incompleto: $token" } }
Write-Host "[1788/1789] Validando versao publica v0.19.8..." -ForegroundColor Cyan
$version0198 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health0198 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Encoding UTF8 -Raw
if ($version0198.Trim() -ne '0.19.43' -or -not $appJs0198.Contains("const HP_MVP_VERSION='0.19.43';") -or (($health0198 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2)) { throw "Versao publica atual inconsistente." }
Write-Host "[1789/1789] Validando versao funcional v0.19.8..." -ForegroundColor Cyan
Write-Host "    v0.19.8 / Weight Goal & Calorie Target Planner: OK." -ForegroundColor Green

# v0.19.9 - Macro Targets by Body Weight
Write-Host "[1790/1797] Validando metas de macronutrientes por peso..." -ForegroundColor Cyan
$appJs0199 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
foreach ($token in @("HP_MACRO_TARGETS_BY_WEIGHT='v0.19.9'", 'data-macro-target-planner="v0.19.9"', 'Metas por peso corporal', 'hpCalculateMacroTargets')) { if (-not $appJs0199.Contains($token)) { throw "Metas de macros v0.19.9 incompletas: $token" } }
Write-Host "[1791/1797] Validando faixas de referencia configuraveis..." -ForegroundColor Cyan
foreach ($token in @('HP_MACRO_REFERENCE_RANGES','2–3 g/kg','0,8–1,2 g/kg','3–7 g/kg','não substituem a decisão profissional')) { if (-not $appJs0199.Contains($token)) { throw "Faixas de macros incompletas: $token" } }
Write-Host "[1792/1797] Validando conversao g/kg para gramas diarias..." -ForegroundColor Cyan
foreach ($token in @('weight*Math.max(0,proteinPerKg||0)','weight*Math.max(0,fatPerKg||0)','weight*Math.max(0,carbPerKg||0)')) { if (-not $appJs0199.Contains($token)) { throw "Conversao de macros incompleta: $token" } }
Write-Host "[1793/1797] Validando energia 4/4/9 dos macros..." -ForegroundColor Cyan
foreach ($token in @('(protein*4)+(fat*9)+(carb*4)','Energia dos macros','4 kcal/g para proteína e carboidrato; 9 kcal/g para lipídios')) { if (-not $appJs0199.Contains($token)) { throw "Energia de macros incompleta: $token" } }
Write-Host "[1794/1797] Validando comparacao com meta calorica..." -ForegroundColor Cyan
foreach ($token in @('targetCalories>0?calories-targetCalories:null','Energia dos macros vs. meta ativa:','macro-target-balance')) { if (-not $appJs0199.Contains($token)) { throw "Comparacao calorica de macros incompleta: $token" } }
Write-Host "[1795/1797] Validando aplicacao editavel das metas..." -ForegroundColor Cyan
foreach ($token in @('Usar como metas diárias','metaProteinasG.value','metaGordurasG.value','metaCarboidratosG.value','Revise e ajuste antes de salvar o plano')) { if (-not $appJs0199.Contains($token)) { throw "Aplicacao das metas de macros incompleta: $token" } }
Write-Host "[1796/1797] Validando estilos responsivos de macros..." -ForegroundColor Cyan
$appCss0199 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @('macro-target-planner','macro-target-grid','macro-target-results','macro-target-balance','macro-target-actions')) { if (-not $appCss0199.Contains($token)) { throw "CSS de macros incompleto: $token" } }
Write-Host "[1797/1797] Validando versao funcional v0.19.9..." -ForegroundColor Cyan
$version0199 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health0199 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Encoding UTF8 -Raw
if ($version0199.Trim() -ne '0.19.43' -or -not $appJs0199.Contains("const HP_MVP_VERSION='0.19.43';") -or (($health0199 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2)) { throw "Versao publica v0.19.9 inconsistente." }
Write-Host "    v0.19.9 / Macro Targets by Body Weight: OK." -ForegroundColor Green

# v0.19.10 - Food Portions & Measure Converter
Write-Host "[1798/1805] Validando medidas flexiveis de alimentos..." -ForegroundColor Cyan
$appJs01910 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
foreach ($token in @("HP_FOOD_PORTION_MEASURES='v0.19.10'", 'data-food-measures="v0.19.10"', 'HP_FOOD_MEASURES', 'hpBindFoodMeasureRow')) { if (-not $appJs01910.Contains($token)) { throw "Medidas de alimentos v0.19.10 incompletas: $token" } }
Write-Host "[1799/1805] Validando formas de medida usuais..." -ForegroundColor Cyan
foreach ($token in @("value:'g'", "value:'ml'", "value:'unidade'", "value:'fatia'", "value:'colher de sopa'", "value:'xícara'", "value:'concha'")) { if (-not $appJs01910.Contains($token)) { throw "Forma de medida ausente: $token" } }
Write-Host "[1800/1805] Validando conversao quantidade x gramas..." -ForegroundColor Cyan
foreach ($token in @('grams.value=String(Math.round(q*factor*100)/100)', 'qty.value=String(Math.round((g/factor)*100)/100)', 'g/medida')) { if (-not $appJs01910.Contains($token)) { throw "Conversao de porcao incompleta: $token" } }
Write-Host "[1801/1805] Validando equivalencia editavel por medida..." -ForegroundColor Cyan
foreach ($token in @('gramsPerUnit', 'subGramsPerUnit', 'Quantos gramas existem em uma medida', 'informe quantos gramas existem em 1 medida')) { if (-not $appJs01910.Contains($token)) { throw "Equivalencia de medida incompleta: $token" } }
Write-Host "[1802/1805] Validando calculo nutricional pela quantidade em gramas..." -ForegroundColor Cyan
foreach ($token in @('caloriasPor100g*f', 'proteinasPor100g*f', 'carboidratosPor100g*f', 'gordurasPor100g*f')) { if (-not $appJs01910.Contains($token)) { throw "Calculo nutricional por porcao incompleto: $token" } }
Write-Host "[1803/1805] Validando substituicoes com medidas flexiveis..." -ForegroundColor Cyan
foreach ($token in @('substitution-row food-measure-row', "prefix:'sub'", 'subQty', 'subUnit', 'subGrams')) { if (-not $appJs01910.Contains($token)) { throw "Substituicao por medida incompleta: $token" } }
Write-Host "[1804/1805] Validando estilos responsivos de medidas..." -ForegroundColor Cyan
$appCss01910 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @('food-measure-row','food-measure-hint','gramsPerUnit','subGramsPerUnit')) { if (-not $appCss01910.Contains($token)) { throw "CSS de medidas incompleto: $token" } }
Write-Host "[1805/1805] Validando versao funcional v0.19.10..." -ForegroundColor Cyan
$version01910 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health01910 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Encoding UTF8 -Raw
if ($version01910.Trim() -ne '0.19.43' -or -not $appJs01910.Contains("const HP_MVP_VERSION='0.19.43';") -or (($health01910 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2)) { throw "Versao publica v0.19.10 inconsistente." }
Write-Host "    v0.19.10 / Food Portions & Measure Converter: OK." -ForegroundColor Green


# v0.19.11 - Dark UI Consistency
Write-Host "[1806/1815] Validando consistencia visual dark da carteira..." -ForegroundColor Cyan
$appJs01911 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Encoding UTF8 -Raw
$appCss01911 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Encoding UTF8 -Raw
foreach ($token in @("HP_DARK_UI_CONSISTENCY='v0.19.11'", 'portfolio-patient-card', 'portfolio-metrics', 'portfolio-priority')) { if (-not $appJs01911.Contains($token) -and -not $appCss01911.Contains($token)) { throw "Dark UI v0.19.11 incompleta: $token" } }
Write-Host "[1807/1815] Validando cards sem superficie branca no dark..." -ForegroundColor Cyan
foreach ($token in @('html[data-theme="dark"] .portfolio-patient-card','background:#0f1314!important','html[data-theme="dark"] .portfolio-metrics>div','background:#14191a!important')) { if (-not $appCss01911.Contains($token)) { throw "Cards da carteira ainda inconsistentes: $token" } }
Write-Host "[1808/1815] Validando contraste de tipografia da carteira..." -ForegroundColor Cyan
foreach ($token in @('.portfolio-identity strong{color:#edf3f1!important}', '.portfolio-reason,', '.portfolio-dates b{color:#7f8b88!important}')) { if (-not $appCss01911.Contains($token)) { throw "Contraste da carteira incompleto: $token" } }
Write-Host "[1809/1815] Validando badges de prioridade no dark..." -ForegroundColor Cyan
foreach ($token in @('portfolio-priority.portfolio-high','portfolio-priority.portfolio-medium','portfolio-priority.portfolio-low','portfolio-priority.portfolio-stable')) { if (-not $appCss01911.Contains($token)) { throw "Badge dark ausente: $token" } }
Write-Host "[1810/1815] Validando toolbar e KPIs da carteira..." -ForegroundColor Cyan
foreach ($token in @('portfolio-toolbar-card','portfolio-stats .stat-card','portfolio-card-actions .secondary')) { if (-not $appCss01911.Contains($token)) { throw "Superficie dark da carteira incompleta: $token" } }
Write-Host "[1811/1815] Validando preservacao do tema claro..." -ForegroundColor Cyan
foreach ($token in @('.portfolio-patient-card{','background:#fff','portfolio-metrics>div')) { if (-not $appCss01911.Contains($token)) { throw "Tema claro da carteira foi descaracterizado: $token" } }
Write-Host "[1812/1815] Validando versao publica v0.19.11..." -ForegroundColor Cyan
$version01911 = Get-Content (Join-Path $root "VERSION.txt") -Raw
$health01911 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Encoding UTF8 -Raw
if ($version01911.Trim() -ne '0.19.43' -or -not $appJs01911.Contains("const HP_MVP_VERSION='0.19.43';") -or (($health01911 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2)) { throw "Versao publica v0.19.11 inconsistente." }
Write-Host "[1813/1815] Validando versao funcional v0.19.11..." -ForegroundColor Cyan
Write-Host "    v0.19.11 / Dark UI Consistency: OK." -ForegroundColor Green

# ===== v0.19.12 - Meal & Portion Manager 2.0 =====
Write-Host "[1814/1815] Validando Meal & Portion Manager 2.0..." -ForegroundColor Cyan
$appJs01912 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Raw -Encoding UTF8
$appCss01912 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Raw -Encoding UTF8
foreach ($token in @("HP_MEAL_PORTION_MANAGER_2='v0.19.12'", 'data-meal-portion-manager="v0.19.12"', 'nutritionDistributeTargets', 'nutritionNormalizePortions', 'data-portion-factor', 'data-meal-scale', 'save-meal-template')) { if (-not $appJs01912.Contains($token)) { throw "Meal & Portion Manager v0.19.12 incompleto: $token" } }
foreach ($token in @('.meal-manager-toolbar','.portion-quick-tools','.meal-scale-tools')) { if (-not $appCss01912.Contains($token)) { throw "CSS Meal & Portion Manager v0.19.12 incompleto: $token" } }
...
if ($version01912 -ne "0.19.43") { throw "VERSION.txt inesperado na v0.19.12." }
$health01912 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Raw -Encoding UTF8
if (($health01912 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.12." }
if (-not $appJs01912.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Interface nao anuncia v0.19.12 internamente." }
Write-Host "    v0.19.12 / Meal & Portion Manager 2.0: OK." -ForegroundColor Green

# ===== v0.19.13 - Smart Food Equivalences =====
Write-Host "[1816/1823] Validando Smart Food Equivalences..." -ForegroundColor Cyan
$appJs01913 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Raw -Encoding UTF8
$appCss01913 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Raw -Encoding UTF8
foreach ($token in @("HP_SMART_FOOD_EQUIVALENCES='v0.19.13'", 'data-smart-food-equivalence="v0.19.13"', 'Alternativas equivalentes', 'hpFindSmartFoodEquivalences')) { if (-not $appJs01913.Contains($token)) { throw "Smart Food Equivalences incompleto: $token" } }
Write-Host "[1817/1823] Validando comparacao de macros das alternativas..." -ForegroundColor Cyan
foreach ($token in @('hpNutritionFoodMacros', 'hpNutritionEquivalenceScore', 'target.kcal', 'target.p', 'target.c', 'target.g')) { if (-not $appJs01913.Contains($token)) { throw "Comparacao nutricional incompleta: $token" } }
Write-Host "[1818/1823] Validando ajuste automatico de porcao equivalente..." -ForegroundColor Cyan
foreach ($token in @('targetKcal/Number(food.caloriasPor100g||1)', 'data-grams', "unit.value='g'", "per.value='1'")) { if (-not $appJs01913.Contains($token)) { throw "Ajuste de porcao equivalente incompleto: $token" } }
Write-Host "[1819/1823] Validando aplicacao profissional da alternativa..." -ForegroundColor Cyan
foreach ($token in @('Usar esta alternativa', 'Alternativa aplicada mantendo uma composição próxima', 'Revise a refeição antes de salvar')) { if (-not $appJs01913.Contains($token)) { throw "Aplicacao da equivalencia incompleta: $token" } }
Write-Host "[1820/1823] Validando limite e ranking de alternativas..." -ForegroundColor Cyan
foreach ($token in @('HP_SMART_EQUIVALENCE_DEFAULT_TOLERANCE=10', 'score*=0.86', '.slice(0,6)')) { if (-not $appJs01913.Contains($token)) { throw "Ranking de equivalencias incompleto: $token" } }
Write-Host "[1821/1823] Validando tema claro e escuro das equivalencias..." -ForegroundColor Cyan
...
if ($version01913 -ne "0.19.43") { throw "VERSION.txt inesperado na v0.19.13." }
$health01913 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs") -Raw -Encoding UTF8
if (($health01913 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.13." }
if (-not $appJs01913.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Interface nao anuncia v0.19.13 internamente." }
...
Write-Host "[1824/1831] Validando calendario nutricional mensal..." -ForegroundColor Cyan
$appJs01914 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js") -Raw -Encoding UTF8
$appCss01914 = Get-Content (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css") -Raw -Encoding UTF8
foreach ($token in @("HP_NUTRITION_CALENDAR='v0.19.14'", 'data-nutrition-calendar="v0.19.14"', 'Calendário nutricional', 'openNutritionCalendar')) { if (-not $appJs01914.Contains($token)) { throw "Nutrition Calendar incompleto: $token" } }
Write-Host "[1825/1831] Validando navegacao mensal e programacao por dia..." -ForegroundColor Cyan
foreach ($token in @('nutritionCalendarPrev','nutritionCalendarNext','nutritionCalendarToday','nutritionCalendarDayForm','Programar cardápio do dia')) { if (-not $appJs01914.Contains($token)) { throw "Navegacao do calendario incompleta: $token" } }
Write-Host "[1826/1831] Validando copia semanal e troca de dias..." -ForegroundColor Cyan
foreach ($token in @('calendario-nutricional/copiar-semana','calendario-nutricional/trocar-dias','Copiar semana →','Trocar dias')) { if (-not $appJs01914.Contains($token)) { throw "Operacoes do calendario incompletas: $token" } }
Write-Host "[1827/1831] Validando API persistente do calendario..." -ForegroundColor Cyan
$calendarController01914 = Get-Content (Join-Path $root "src/HealthPlatform.Api/Controllers/CalendarioNutricionalController.cs") -Raw -Encoding UTF8
foreach ($token in @('calendario-nutricional','CopiarSemana','TrocarDias','ProgramacaoNutricionalResponse','PlanoAlimentarId')) { if (-not $calendarController01914.Contains($token)) { throw "API calendario nutricional incompleta: $token" } }
Write-Host "[1828/1831] Validando dominio e persistencia do calendario..." -ForegroundColor Cyan
$calendarEntity01914 = Get-Content (Join-Path $root "src/HealthPlatform.Domain/Entities/ProgramacaoNutricionalDia.cs") -Raw -Encoding UTF8
$dbContext01914 = Get-Content (Join-Path $root "src/HealthPlatform.Infrastructure/Data/AppDbContext.cs") -Raw -Encoding UTF8
foreach ($token in @('ProgramacaoNutricionalDia','PacienteId','PlanoAlimentarId','Contexto','Observacoes')) { if (-not $calendarEntity01914.Contains($token)) { throw "Dominio do calendario incompleto: $token" } }
foreach ($token in @('ProgramacoesNutricionaisDia','ProgramacoesNutricionais','ProgramacoesCalendario')) { if (-not $dbContext01914.Contains($token)) { throw "Mapeamento do calendario incompleto: $token" } }
Write-Host "[1829/1831] Validando migration do calendario nutricional..." -ForegroundColor Cyan
$migration01914 = Get-Content (Join-Path $root "src/HealthPlatform.Infrastructure/Migrations/20260923193000_V01914CalendarioNutricional.cs") -Raw -Encoding UTF8
$migrationDesigner01914 = Get-Content (Join-Path $root "src/HealthPlatform.Infrastructure/Migrations/20260923193000_V01914CalendarioNutricional.Designer.cs") -Raw -Encoding UTF8
$modelSnapshot01914 = Get-Content (Join-Path $root "src/HealthPlatform.Infrastructure/Migrations/AppDbContextModelSnapshot.cs") -Raw -Encoding UTF8
foreach ($token in @('ProgramacoesNutricionaisDia','IX_ProgramacoesNutricionaisDia_PacienteId_Data','unique: true','ReferentialAction.Restrict')) { if (-not $migration01914.Contains($token)) { throw "Migration do calendario incompleta: $token" } }
foreach ($token in @('ProgramacaoNutricionalDia','ProgramacoesNutricionaisDia','ProgramacoesNutricionais','ProgramacoesCalendario')) { if (-not $migrationDesigner01914.Contains($token) -or -not $modelSnapshot01914.Contains($token)) { throw "Snapshot/Designer do calendario incompleto: $token" } }
Write-Host "[1830/1831] Validando visual responsivo do calendario..." -ForegroundColor Cyan
foreach ($token in @('.nutrition-calendar-shell','.nutrition-calendar-grid','.nutrition-calendar-day','.nutrition-calendar-editor','html[data-theme="dark"] .nutrition-calendar-day')) { if (-not $appCss01914.Contains($token)) { throw "CSS Nutrition Calendar incompleto: $token" } }
...
if ($version01914 -ne "0.19.43") { throw "VERSION.txt inesperado na v0.19.14." }
if (($health01914 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.14." }
if (-not $appJs01914.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Interface nao anuncia v0.19.14 internamente." }
Write-Host "    v0.19.14 / Nutrition Calendar: OK." -ForegroundColor Green


Write-Host "[1832/1833] Validando Smart Meal Swap..." -ForegroundColor Cyan
$appJsSource = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$portalController = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/MeuPortalPacienteController.cs")
$portalContracts = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Contracts/Portal/PortalPacienteContracts.cs")
$indexSource = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
foreach ($token in @("HP_SMART_MEAL_SWAP='v0.19.15'", 'meal-smart-swap', 'hpOpenSmartMealSwap', 'data-smart-meal-swap="v0.19.15"')) { if (-not $appJsSource.Contains($token)) { throw "Smart Meal Swap incompleto: $token" } }
foreach ($token in @('refeicoes/{refeicaoId:guid}/alternativas', 'PortalAlternativasRefeicaoResponse')) { if (-not $portalController.Contains($token)) { throw "API Smart Meal Swap incompleta: $token" } }
foreach ($token in @('PortalAlternativaRefeicaoResponse', 'SimilaridadePercentual')) { if (-not $portalContracts.Contains($token)) { throw "Contrato Smart Meal Swap incompleto: $token" } }
if (-not $indexSource.Contains('/app.css?v=0.19.43') -or -not $indexSource.Contains('/app.js?v=0.19.43')) { throw 'Cache busting dos assets ausente.' }
Write-Host "    Alternativas prescritas + registro de adaptação + cache busting: OK." -ForegroundColor Green
...
if ($version.Trim() -ne "0.19.43") { throw "VERSION.txt inesperado para v0.19.15." }
if (-not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "Interface nao anuncia v0.19.15." }
...
$appJs01916 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01916 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
foreach ($token in @("HP_WORKOUT_PROGRESSION_ENGINE='v0.19.16'", 'hpWorkoutProgressionDecision', 'hpWorkoutProgressionEngineCard', 'PROGRESSÃO ASSISTIDA')) { if (-not $appJs01916.Contains($token)) { throw "Workout Progression Engine incompleto: $token" } }
Write-Host "[1835/1841] Validando uso do historico real de execucao..." -ForegroundColor Cyan
foreach ($token in @('/api/portal/me/treinos/progressao-exercicios?dias=120', 'hpWorkoutProgressionSeriesMap', 'ultimaExecucao', 'tendenciaCarga')) { if (-not $appJs01916.Contains($token)) { throw "Historico de progressao incompleto: $token" } }
Write-Host "[1836/1841] Validando criterios conservadores de progressao..." -ForegroundColor Cyan
foreach ($token in @('registros>=2', 'rpe<=8', "series.tendenciaCarga!=='AbaixoDaBase'", 'Candidato à progressão', 'Manter e observar', 'Construindo base')) { if (-not $appJs01916.Contains($token)) { throw "Criterios do progression engine incompletos: $token" } }
Write-Host "[1837/1841] Validando progressao segundo regra profissional..." -ForegroundColor Cyan
foreach ($token in @('hpProgressionTargetReps', 'meta.increase', 'Regra prescrita:', 'qualquer ajuste continua subordinado à prescrição')) { if (-not $appJs01916.Contains($token)) { throw "Regra profissional de progressao incompleta: $token" } }
Write-Host "[1838/1841] Validando experiencia do paciente..." -ForegroundColor Cyan
foreach ($token in @('hpInjectPatientWorkoutProgressionEngine', 'workout-progression-summary', 'data-item-id=', 'monitorado(s)')) { if (-not $appJs01916.Contains($token)) { throw "Experiencia paciente progression engine incompleta: $token" } }
Write-Host "[1839/1841] Validando painel profissional de progressao..." -ForegroundColor Cyan
foreach ($token in @('hpInjectProfessionalWorkoutProgressionEngine', 'professional-progression-engine', '/treinos/progressao-exercicios?dias=120', 'Progressão de carga assistida')) { if (-not $appJs01916.Contains($token)) { throw "Painel profissional progression engine incompleto: $token" } }
Write-Host "[1840/1841] Validando visual responsivo do progression engine..." -ForegroundColor Cyan
foreach ($token in @('.workout-progression-engine', '.workout-progression-summary-grid', '.professional-progression-engine', '.professional-progression-list')) { if (-not $appCss01916.Contains($token)) { throw "CSS Workout Progression Engine incompleto: $token" } }
Write-Host "[1841/1841] Validando versao funcional v0.19.16..." -ForegroundColor Cyan
$version01916 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01916 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01916 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01916 -ne '0.19.43') { throw 'VERSION.txt inesperado ao validar v0.19.16.' }
if (($health01916 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health atual inconsistente ao validar v0.19.16.' }
if (-not $appJs01916.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface atual inconsistente ao validar v0.19.16.' }
if (-not $index01916.Contains('/app.css?v=0.19.43') -or -not $index01916.Contains('/app.js?v=0.19.43')) { throw 'Cache busting atual ausente ao validar v0.19.16.' }
Write-Host "    v0.19.16 / Workout Progression Engine: OK." -ForegroundColor Green

# ===== v0.19.17 - Smart Exercise Alternatives =====
Write-Host "[1842/1849] Validando Smart Exercise Alternatives..." -ForegroundColor Cyan
$appJs01917 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01917 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
$treinoController01917 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/MeuTreinoPacienteController.cs")
$execController01917 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/ExecucoesTreinoController.cs")
foreach ($token in @("HP_SMART_EXERCISE_ALTERNATIVES='v0.19.17'", 'exercise-alternative-open', 'hpLoadExerciseAlternatives', 'Ver alternativas')) { if (-not $appJs01917.Contains($token)) { throw "Smart Exercise Alternatives incompleto: $token" } }
Write-Host "[1843/1849] Validando API de alternativas do exercicio..." -ForegroundColor Cyan
foreach ($token in @('alternativas/{itemTreinoId:guid}', 'bloqueadoPorSeguranca', 'compatibilidadePercentual', 'GrupoMuscular')) { if (-not $treinoController01917.Contains($token)) { throw "API de alternativas incompleta: $token" } }
Write-Host "[1844/1849] Validando seguranca para dor/desconforto..." -ForegroundColor Cyan
foreach ($token in @('DorDesconforto', 'Dor ou desconforto merece revisao profissional', 'Interrompa o exercicio se necessario')) { if (-not $treinoController01917.Contains($token)) { throw "Protecao de seguranca das alternativas incompleta: $token" } }
Write-Host "[1845/1849] Validando registro estruturado da adaptacao..." -ForegroundColor Cyan
foreach ($token in @('ExercicioAlternativoId', 'MotivoAlternativa', 'AESYN_ALT_EXERCICIO', 'MontarObservacaoItem')) { if (-not $execController01917.Contains($token)) { throw "Registro da adaptacao de exercicio incompleto: $token" } }
Write-Host "[1846/1849] Validando restricao por grupo muscular e organizacao..." -ForegroundColor Cyan
foreach ($token in @('currentUser.OrganizationId', 'mesmo grupo muscular', 'alternativas.TryGetValue')) { if (-not $execController01917.Contains($token)) { throw "Validacao da alternativa incompleta: $token" } }
Write-Host "[1847/1849] Validando UX responsiva das alternativas..." -ForegroundColor Cyan
foreach ($token in @('.exercise-alternative-tools', '.exercise-alternative-grid', '.exercise-alternative-card', 'html[data-theme="dark"] .exercise-alternative-tools')) { if (-not $appCss01917.Contains($token)) { throw "CSS Smart Exercise Alternatives incompleto: $token" } }
Write-Host "[1848/1849] Validando versao publica v0.19.17..." -ForegroundColor Cyan
$version01917 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01917 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01917 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01917 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.17.' }
if (($health01917 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.17.' }
if (-not $appJs01917.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.17.' }
if (-not $index01917.Contains('/app.css?v=0.19.43') -or -not $index01917.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.17 ausente.' }
Write-Host "[1849/1849] Validando versao funcional v0.19.17..." -ForegroundColor Cyan
Write-Host "    v0.19.17 / Smart Exercise Alternatives: OK." -ForegroundColor Green

# ===== v0.19.18 - Patient Professional Chat =====
Write-Host "[1850/1857] Validando Patient Professional Chat..." -ForegroundColor Cyan
$appJs01918 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01918 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
$chatController01918 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/ChatAcompanhamentoController.cs")
$index01918 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
foreach ($token in @("HP_PATIENT_PROFESSIONAL_CHAT='v0.19.18'", 'loadPatientChat', 'loadProfessionalPatientChat', 'AESYN • CONNECTED CARE')) { if (-not $appJs01918.Contains($token)) { throw "Chat UI incompleto: $token" } }
Write-Host "[1851/1857] Validando endpoints paciente e profissional..." -ForegroundColor Cyan
foreach ($token in @('[HttpGet("me")]', '[HttpPost("me/mensagens")]', '[HttpGet("pacientes/{pacienteId:guid}")]', '[HttpPost("pacientes/{pacienteId:guid}/mensagens")]')) { if (-not $chatController01918.Contains($token)) { throw "Endpoint chat incompleto: $token" } }
Write-Host "[1852/1857] Validando persistencia e contexto do chat..." -ForegroundColor Cyan
foreach ($token in @('InteracoesAcompanhamento', 'Canal = $"Chat:{contexto}"', 'Resultado = autor', 'var observacoes = MontarObservacoes(mensagem, referencia)', 'Observacoes = observacoes', 'MontarObservacoes', 'SepararObservacoes', 'ContextosPermitidos')) { if (-not $chatController01918.Contains($token)) { throw "Persistencia chat incompleta: $token" } }
Write-Host "[1853/1857] Validando notificacoes de novas mensagens..." -ForegroundColor Cyan
foreach ($token in @('NotificacoesInternas.Add', 'Tipo = "Chat"', 'OrigemTipo = "ChatAcompanhamento"', 'CHAT:{item.Id}')) { if (-not $chatController01918.Contains($token)) { throw "Notificacao chat incompleta: $token" } }
Write-Host "[1854/1857] Validando orientacao de seguranca e SLA..." -ForegroundColor Cyan
foreach ($token in @('Resposta em ate 24h uteis', 'nao deve ser usado para urgencias ou emergencias', 'Acompanhamento, não emergência.')) { if (-not $chatController01918.Contains($token) -and -not $appJs01918.Contains($token)) { throw "Seguranca/SLA chat incompleto: $token" } }
Write-Host "[1855/1857] Validando navegacao do paciente e perfil profissional..." -ForegroundColor Cyan
foreach ($token in @('data-patient-view="chat"', 'data-patient-more-view="chat"')) { if (-not $index01918.Contains($token)) { throw "Navegacao chat incompleta: $token" } }
if (-not $appJs01918.Contains("tabButton('chat','Chat')")) { throw 'Aba profissional de chat ausente.' }
Write-Host "[1856/1857] Validando visual responsivo do chat..." -ForegroundColor Cyan
foreach ($token in @('.care-chat-shell','.care-chat-thread','.care-chat-message.mine','.care-chat-compose','html[data-theme="dark"] .care-chat-thread')) { if (-not $appCss01918.Contains($token)) { throw "CSS chat incompleto: $token" } }
Write-Host "[1857/1857] Validando versao funcional v0.19.18..." -ForegroundColor Cyan
$version01918 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01918 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
if ($version01918 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.18.' }
if (($health01918 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.18.' }
if (-not $appJs01918.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.18.' }
if (-not $index01918.Contains('/app.css?v=0.19.43') -or -not $index01918.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.18 ausente.' }
Write-Host "    v0.19.18 / Patient Professional Chat: OK." -ForegroundColor Green


# ===== v0.19.19 - Deviation & Adherence Events =====
Write-Host "[1858/1865] Validando Deviation & Adherence Events..." -ForegroundColor Cyan
$deviationService01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Services/EventoDesvioAdesaoService.cs")
$deviationController01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/DesviosAdesaoController.cs")
$mealController01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/MeuPortalPacienteController.cs")
$trainingExec01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/ExecucoesTreinoController.cs")
$appJs01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
foreach ($token in @('TipoRegistro = "EventoDesvioAdesao"', 'RegistrarAsync(', 'OrigemChave', 'Prioridade')) { if (-not $deviationService01919.Contains($token)) { throw "Servico de desvios incompleto: $token" } }
Write-Host "[1859/1865] Validando captura de desvios nutricionais..." -ForegroundColor Cyan
foreach ($token in @('status is "Adaptada" or "NaoRealizada"', 'REFEICAO:{refeicaoId}', 'Refeição adaptada', 'Refeição não realizada')) { if (-not $mealController01919.Contains($token)) { throw "Captura nutricional incompleta: $token" } }
Write-Host "[1860/1865] Validando captura de adaptacoes de treino..." -ForegroundColor Cyan
foreach ($token in @('ExercicioAlternativo', 'ExercicioNaoConcluido', 'TREINO:{execucao.Id}', 'EventoDesvioAdesaoService.RegistrarAsync')) { if (-not $trainingExec01919.Contains($token)) { throw "Captura de treino incompleta: $token" } }
Write-Host "[1861/1865] Validando API profissional de desvios..." -ForegroundColor Cyan
foreach ($token in @('Route("api/pacientes/{pacienteId:guid}/desvios-adesao")', 'ultimos7Dias', 'recorrencias', 'prioridadeMaior')) { if (-not $deviationController01919.Contains($token)) { throw "API de desvios incompleta: $token" } }
Write-Host "[1862/1865] Validando agregacao sem spam por recorrencia..." -ForegroundColor Cyan
foreach ($token in @('g.Count() >= 2', 'GroupBy(x => new { x.Categoria, x.Tipo })', 'Take(250)', 'Recorrente')) { if (-not $deviationController01919.Contains($token)) { throw "Agregacao de recorrencia incompleta: $token" } }
Write-Host "[1863/1865] Validando painel profissional de adesao..." -ForegroundColor Cyan
foreach ($token in @("HP_DEVIATION_ADHERENCE_EVENTS='v0.19.19'", 'hpDeviationAdherenceCard', 'Execuções diferentes do planejado', '/desvios-adesao?dias=30')) { if (-not $appJs01919.Contains($token)) { throw "Painel de desvios incompleto: $token" } }
Write-Host "[1864/1865] Validando visual dark/mobile dos eventos..." -ForegroundColor Cyan
foreach ($token in @('.deviation-adherence-card', '.deviation-adherence-stats', '.deviation-event-list', 'html[data-theme="dark"] .deviation-adherence-stats')) { if (-not $appCss01919.Contains($token)) { throw "CSS de desvios incompleto: $token" } }
Write-Host "[1865/1865] Validando versao funcional v0.19.19..." -ForegroundColor Cyan
$version01919 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01919 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01919 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.19.' }
if (($health01919 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.19.' }
if (-not $appJs01919.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.19.' }
if (-not $index01919.Contains('/app.css?v=0.19.43') -or -not $index01919.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.19 ausente.' }
Write-Host "    v0.19.19 / Deviation & Adherence Events: OK." -ForegroundColor Green


# ===== v0.19.20 - Professional Attention Center =====
Write-Host "[1866/1873] Validando Professional Attention Center..." -ForegroundColor Cyan
$central01920 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/CentralDiaController.cs")
$appJs01920 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01920 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
foreach ($token in @('CentralDiaAtencaoResponse', 'PacientesEmAtencao', 'EventosAdesao7Dias', 'IReadOnlyCollection<CentralDiaAtencaoResponse> Atencoes')) { if (-not $central01920.Contains($token)) { throw "Contrato da Central de Atencao incompleto: $token" } }
Write-Host "[1867/1873] Validando agregacao de eventos e recorrencias..." -ForegroundColor Cyan
foreach ($token in @('EventoDesvioAdesaoService.TipoRegistro', 'GroupBy(x => new { x.Categoria, x.Tipo })', 'g.Count() >= 2', 'eventos7')) { if (-not $central01920.Contains($token)) { throw "Agregacao da Central de Atencao incompleta: $token" } }
Write-Host "[1868/1873] Validando priorizacao sem spam..." -ForegroundColor Cyan
foreach ($token in @('maiorPrioridade * 15', 'recorrencias.Count', 'pendenciasAltaPaciente', 'followUpVencido', 'score < 20')) { if (-not $central01920.Contains($token)) { throw "Priorizacao profissional incompleta: $token" } }
Write-Host "[1869/1873] Validando limites de seguranca profissional..." -ForegroundColor Cyan
foreach ($token in @('A Central de Atenção só promove sinais úteis', 'não altera treino, dieta, medicamento ou conduta automaticamente')) { if (-not $central01920.Contains($token) -and -not $appJs01920.Contains($token)) { throw "Limite de seguranca da Central de Atencao incompleto: $token" } }
Write-Host "[1870/1873] Validando experiencia da Central do Dia..." -ForegroundColor Cyan
foreach ($token in @("HP_PROFESSIONAL_ATTENTION_CENTER='v0.19.20'", 'data-professional-attention="v0.19.20"', 'Quem merece atenção hoje', 'hpProfessionalAttentionPatient', 'professional-attention-open')) { if (-not $appJs01920.Contains($token)) { throw "UI da Central de Atencao incompleta: $token" } }
Write-Host "[1871/1873] Validando resumo da atencao no dashboard..." -ForegroundColor Cyan
foreach ($token in @('d.pacientesEmAtencao||0', '<span>Em atenção</span>', 'priorização inteligente')) { if (-not $appJs01920.Contains($token)) { throw "Resumo do dashboard incompleto: $token" } }
Write-Host "[1872/1873] Validando dark mode e responsividade da Central de Atencao..." -ForegroundColor Cyan
foreach ($token in @('.professional-attention-card', '.professional-attention-list', '.professional-attention-item.priority', 'html[data-theme="dark"] .professional-attention-card', '@media(max-width:560px)')) { if (-not $appCss01920.Contains($token)) { throw "CSS Professional Attention Center incompleto: $token" } }
Write-Host "[1873/1873] Validando versao funcional v0.19.20..." -ForegroundColor Cyan
$version01920 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01920 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01920 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01920 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.20.' }
if (($health01920 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.20.' }
if (-not $appJs01920.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.20.' }
if (-not $index01920.Contains('/app.css?v=0.19.43') -or -not $index01920.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.20 ausente.' }
Write-Host "    v0.19.20 / Professional Attention Center: OK." -ForegroundColor Green
...
Write-Host "[1874/1881] Validando Patient Timeline & Context..." -ForegroundColor Cyan
$appJs01921 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01921 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
$timelineController01921 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/TimelineController.cs")
$timelineContract01921 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Contracts/Timeline/TimelineItemResponse.cs")
foreach ($token in @("HP_PATIENT_TIMELINE_CONTEXT='v0.19.21'", 'data-patient-timeline-context="v0.19.21"', 'Timeline integrada do paciente', 'hpRenderPatientContextTimeline')) { if (-not $appJs01921.Contains($token)) { throw "Timeline & Context UI incompleta: $token" } }
Write-Host "[1875/1881] Validando contrato contextual da timeline..." -ForegroundColor Cyan
foreach ($token in @('string Categoria = "Clinico"', 'int Prioridade = 0', 'string? ContextoChave = null', 'string Fonte = "Prontuario"')) { if (-not $timelineContract01921.Contains($token)) { throw "Contrato da timeline incompleto: $token" } }
Write-Host "[1876/1881] Validando integracao treino, check-in e comunicacao..." -ForegroundColor Cyan
foreach ($token in @('ExecucoesTreino', 'CheckInsAcompanhamento', 'InteracoesAcompanhamento', 'planosTreino', 'Canal.StartsWith("Chat:")')) { if (-not $timelineController01921.Contains($token)) { throw "Timeline integrada incompleta: $token" } }
Write-Host "[1877/1881] Validando eventos de adesao na timeline..." -ForegroundColor Cyan
foreach ($token in @('EventoDesvioAdesaoService.TipoRegistro', 'EventoDesvioAdesaoService.Ler', 'desvio_adesao', 'x.Prioridade', 'x.OrigemChave')) { if (-not $timelineController01921.Contains($token)) { throw "Contexto de adesao na timeline incompleto: $token" } }
Write-Host "[1878/1881] Validando leitura cruzada por dia e filtros..." -ForegroundColor Cyan
foreach ($token in @('Contexto cruzado', 'data-timeline-filter', 'data-timeline-day', 'hpTimelineCrossContext', 'dimensões conectadas')) { if (-not $appJs01921.Contains($token)) { throw "Leitura contextual da timeline incompleta: $token" } }
Write-Host "[1879/1881] Validando seguranca interpretativa da timeline..." -ForegroundColor Cyan
foreach ($token in @('sem inferência clínica automática', 'não conclui causalidade, diagnóstico ou necessidade de mudança de conduta')) { if (-not $appJs01921.Contains($token)) { throw "Trava interpretativa da timeline ausente: $token" } }
Write-Host "[1880/1881] Validando visual responsivo da timeline contextual..." -ForegroundColor Cyan
foreach ($token in @('.patient-context-timeline', '.patient-context-timeline-stats', '.patient-timeline-cross', '.patient-context-timeline-item', 'html[data-theme="dark"] .patient-context-timeline-stats')) { if (-not $appCss01921.Contains($token)) { throw "CSS Patient Timeline & Context incompleto: $token" } }
Write-Host "[1881/1881] Validando compatibilidade funcional v0.19.21..." -ForegroundColor Cyan
$version01921 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01921 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01921 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01921 -ne '0.19.43') { throw 'VERSION.txt inesperado ao validar v0.19.21.' }
if (($health01921 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health atual inconsistente ao validar v0.19.21.' }
if (-not $appJs01921.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface atual inconsistente ao validar v0.19.21.' }
if (-not $index01921.Contains('/app.css?v=0.19.43') -or -not $index01921.Contains('/app.js?v=0.19.43')) { throw 'Cache busting atual ausente ao validar v0.19.21.' }
Write-Host "    v0.19.21 / Patient Timeline & Context: OK." -ForegroundColor Green


# ===== v0.19.22 - Positive Progress / AESYN XP =====
Write-Host "[1882/1888] Validando servico Positive Progress / AESYN XP..."
$positiveServicePath = Join-Path $Root "src/HealthPlatform.Api/Services/ProgressoPositivoService.cs"
if (-not (Test-Path $positiveServicePath)) { throw "ProgressoPositivoService ausente." }
$positiveService = Get-Content $positiveServicePath -Raw
foreach ($token in @("PortalProgressoPositivoResponse", "xpSemana", "diasComProgresso7", "faltamXp", "Math.Max(0, x.Pontos)")) { if (-not $positiveService.Contains($token)) { throw "Positive Progress incompleto: $token" } }
Write-Host "    XP cumulativo + resumo semanal: OK."

Write-Host "[1883/1888] Validando politica sem XP negativo..."
# Use apenas tokens ASCII aqui: Windows PowerShell 5 pode interpretar arquivos UTF-8 sem BOM como ANSI.
if (-not $positiveService.Contains("XP negativo")) { throw "Politica de XP positivo ausente." }
if (-not $positiveService.Contains("progresso") -or -not $positiveService.Contains("excesso de treino")) { throw "Protecao de progresso acumulado ausente." }
Write-Host "    Sem XP negativo / sem compensacao: OK."

Write-Host "[1884/1888] Validando contrato Positive Progress..."
$portalContracts = Get-Content (Join-Path $Root "src/HealthPlatform.Api/Contracts/Portal/PortalPacienteContracts.cs") -Raw
foreach ($token in @("PortalProgressoPositivoDestaqueResponse", "PortalProgressoPositivoResponse", "PortalProgressoPositivoResponse ProgressoPositivo", "XpSemana", "DiasComProgresso7", "XpAteProximoNivel")) { if (-not $portalContracts.Contains($token)) { throw "Contrato Positive Progress incompleto: $token" } }
Write-Host "    Contrato do portal: OK."

Write-Host "[1885/1888] Validando integracao do portal..."
$portalController = Get-Content (Join-Path $Root "src/HealthPlatform.Api/Controllers/PortalPacienteController.cs") -Raw
$meuPortalController = Get-Content (Join-Path $Root "src/HealthPlatform.Api/Controllers/MeuPortalPacienteController.cs") -Raw
foreach ($source in @($portalController,$meuPortalController)) { if (-not $source.Contains("ProgressoPositivoService.MontarAsync")) { throw "Positive Progress nao integrado a um dos portais." } }
Write-Host "    Portal profissional + paciente: OK."

Write-Host "[1886/1888] Validando UI Positive Progress..."
$appJsSource = Get-Content (Join-Path $Root "src/HealthPlatform.Api/wwwroot/app.js") -Raw
$appCssSource = Get-Content (Join-Path $Root "src/HealthPlatform.Api/wwwroot/app.css") -Raw
foreach ($token in @("HP_POSITIVE_PROGRESS_XP='v0.19.22'", "hpPositiveProgressAthleteCard", "hpPositiveProgressProfessionalCard", 'data-positive-progress="v0.19.22"')) { if (-not $appJsSource.Contains($token)) { throw "UI Positive Progress incompleta: $token" } }
foreach ($token in @(".positive-progress-card", ".positive-progress-grid", 'html[data-theme="dark"] .positive-progress-card')) { if (-not $appCssSource.Contains($token)) { throw "CSS Positive Progress incompleto: $token" } }
Write-Host "    UI responsiva + dark mode: OK."
...
if ($version.Trim() -ne "0.19.43") { throw "VERSION.txt nao anuncia v0.19.22." }
if (-not $appJsSource.Contains("const HP_MVP_VERSION='0.19.43';")) { throw "HP_MVP_VERSION nao anuncia v0.19.22." }
if (($healthSource | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.19.22." }
Write-Host "    Versao publica: OK."

Write-Host "[1888/1888] Validando versao funcional v0.19.22..."
Write-Host "    v0.19.22 / Positive Progress / AESYN XP: OK."


# ===== v0.19.23 - Passive Monitoring Foundation =====
Write-Host "[1889/1896] Validando servico Passive Monitoring Foundation..." -ForegroundColor Cyan
$passiveServicePath = Join-Path $root "src/HealthPlatform.Api/Services/MonitoramentoPassivoService.cs"
if (-not (Test-Path $passiveServicePath)) { throw "MonitoramentoPassivoService ausente." }
$passiveService = Get-Content -Raw -Encoding UTF8 $passiveServicePath
foreach ($token in @('TipoRegistro = "MonitoramentoPassivo"', 'AppleHealth', 'HealthConnect', 'Garmin', 'Passos', 'SonoMinutos', 'FrequenciaCardiacaRepouso', 'HRV', 'EnergiaAtiva', 'MinutosAtivos')) { if (-not $passiveService.Contains($token)) { throw "Passive Monitoring incompleto: $token" } }
Write-Host "    Fontes + metricas normalizadas: OK." -ForegroundColor Green

Write-Host "[1890/1896] Validando importacao e deduplicacao passiva..." -ForegroundColor Cyan
foreach ($token in @('ImportarAsync', 'ChaveDeduplicacao', 'duplicados', 'Take(500)', 'DateTime.UtcNow.AddYears(-2)', 'SaveChangesAsync')) { if (-not $passiveService.Contains($token)) { throw "Pipeline passivo incompleto: $token" } }
Write-Host "    Validacao + deduplicacao + persistencia: OK." -ForegroundColor Green

Write-Host "[1891/1896] Validando API paciente/profissional..." -ForegroundColor Cyan
$passiveControllerPath = Join-Path $root "src/HealthPlatform.Api/Controllers/MonitoramentoPassivoController.cs"
if (-not (Test-Path $passiveControllerPath)) { throw "MonitoramentoPassivoController ausente." }
$passiveController = Get-Content -Raw -Encoding UTF8 $passiveControllerPath
foreach ($token in @('[Authorize(Policy = "PatientOnly")]', 'api/portal/me/monitoramento-passivo/importar', 'api/portal/me/monitoramento-passivo', 'api/pacientes/{pacienteId:guid}/monitoramento-passivo', 'x.OrganizacaoId == currentUser.OrganizationId', 'MonitoramentoPassivoService.ResumirAsync')) { if (-not $passiveController.Contains($token)) { throw "API de monitoramento passivo incompleta: $token" } }
Write-Host "    Isolamento paciente + organizacao profissional: OK." -ForegroundColor Green

Write-Host "[1892/1896] Validando contratos Passive Monitoring..." -ForegroundColor Cyan
$passiveContractsPath = Join-Path $root "src/HealthPlatform.Api/Contracts/MonitoramentoPassivo/MonitoramentoPassivoContracts.cs"
if (-not (Test-Path $passiveContractsPath)) { throw "Contratos de monitoramento passivo ausentes." }
$passiveContracts = Get-Content -Raw -Encoding UTF8 $passiveContractsPath
foreach ($token in @('MonitoramentoPassivoSinalRequest', 'MonitoramentoPassivoImportacaoResponse', 'MonitoramentoPassivoResumoResponse', 'MonitoramentoPassivoMetricaResponse', 'MonitoramentoPassivoFonteResponse', 'VariacaoPercentual7Dias')) { if (-not $passiveContracts.Contains($token)) { throw "Contrato Passive Monitoring incompleto: $token" } }
Write-Host "    Contratos de ingestao e resumo: OK." -ForegroundColor Green

Write-Host "[1893/1896] Validando seguranca interpretativa passiva..." -ForegroundColor Cyan
foreach ($token in @('nao geram diagnostico', 'prescricao', 'mudanca automatica de conduta')) { if (-not $passiveService.Contains($token)) { throw "Trava interpretativa passiva ausente: $token" } }
Write-Host "    Sem diagnostico/prescricao automatica: OK." -ForegroundColor Green

Write-Host "[1894/1896] Validando UI e dark mode Passive Monitoring..." -ForegroundColor Cyan
$appJs01923 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01923 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
foreach ($token in @("HP_PASSIVE_MONITORING_FOUNDATION='v0.19.23'", 'data-passive-monitoring="v0.19.23"', 'hpPassiveMonitoringCard', 'hpInjectProfessionalPassiveMonitoring', '/api/portal/me/monitoramento-passivo?dias=14')) { if (-not $appJs01923.Contains($token)) { throw "UI Passive Monitoring incompleta: $token" } }
foreach ($token in @('.passive-monitoring-card', '.passive-monitoring-grid', '.passive-monitoring-sources', 'html[data-theme="dark"] .passive-monitoring-card', '@media(max-width:560px)')) { if (-not $appCss01923.Contains($token)) { throw "CSS Passive Monitoring incompleto: $token" } }
Write-Host "    Paciente + profissional + dark responsivo: OK." -ForegroundColor Green

Write-Host "[1895/1896] Validando versao publica v0.19.23..." -ForegroundColor Cyan
$version01923 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01923 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01923 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01923 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.23.' }
if (($health01923 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.23.' }
if (-not $appJs01923.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.23.' }
if (-not $index01923.Contains('/app.css?v=0.19.43') -or -not $index01923.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.23 ausente.' }
Write-Host "    Versao publica: OK." -ForegroundColor Green

Write-Host "[1896/1896] Validando versao funcional v0.19.23..." -ForegroundColor Cyan
Write-Host "    v0.19.23 / Passive Monitoring Foundation: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.24 - Metabolic Planning UX & Safety =====
Write-Host "[1897/1904] Validando Metabolic Planning UX & Safety..." -ForegroundColor Cyan
$appJs01924 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01924 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
foreach ($token in @("HP_METABOLIC_PLANNING_UX_SAFETY='v0.19.24'", 'data-metabolic-planning="v0.19.24"', 'TMB', 'GET', 'Meta calórica ativa do plano', 'Macro residual')) { if (-not $appJs01924.Contains($token)) { throw "Metabolic Planning UX incompleto: $token" } }
Write-Host "[1898/1904] Validando conciliacao assistida de macros..." -ForegroundColor Cyan
foreach ($token in @('hpReconcileMacroTargets', 'Ajustar macro residual à meta calórica', 'fixedCalories', 'remaining/kcalPerGram[residual]')) { if (-not $appJs01924.Contains($token)) { throw "Conciliacao de macros incompleta: $token" } }
Write-Host "[1899/1904] Validando delta energetico absoluto e percentual..." -ForegroundColor Cyan
foreach ($token in @('deltaPct', "toFixed(1)", 'Energia dos macros vs. meta ativa')) { if (-not $appJs01924.Contains($token)) { throw "Delta energetico incompleto: $token" } }
Write-Host "[1900/1904] Validando revisao explicita de alvo agressivo..." -ForegroundColor Cyan
foreach ($token in @('dataset.belowBmr', 'dataset.aggressive', 'exige revisão profissional', 'confirm(')) { if (-not $appJs01924.Contains($token)) { throw "Confirmacao de seguranca metabolica incompleta: $token" } }
Write-Host "[1901/1904] Validando cenarios matematicos conhecidos..." -ForegroundColor Cyan
$bmr = (10*84) + (6.25*171) - (5*27) + 5
$get = $bmr * 1.55
$adjustment = ((80-84)*7700)/(4*7)
$target = $get + $adjustment
$macroCalories = (168*4) + (84*9) + (336*4)
if ([math]::Abs($bmr-1778.75) -gt 0.01) { throw 'Cenario Mifflin conhecido falhou.' }
if ([math]::Abs($get-2757.0625) -gt 0.01) { throw 'Cenario GET conhecido falhou.' }
if ([math]::Abs($target-1657.0625) -gt 0.01) { throw 'Cenario alvo conhecido falhou.' }
if ($macroCalories -ne 2772) { throw 'Cenario macros conhecido falhou.' }
Write-Host "[1902/1904] Validando CSS responsivo do planejamento metabolico..." -ForegroundColor Cyan
foreach ($token in @('.metabolic-flow', '.metabolic-active-target', '.macro-reconcile', 'html[data-theme="dark"] .metabolic-flow', '@media(max-width:480px)')) { if (-not $appCss01924.Contains($token)) { throw "CSS metabolico v0.19.26 incompleto: $token" } }
Write-Host "[1903/1904] Validando compatibilidade publica da v0.19.24..." -ForegroundColor Cyan
$version01924 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01924 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01924 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01924 -ne '0.19.43') { throw 'VERSION.txt corrente inesperado ao validar compatibilidade da v0.19.24.' }
if (($health01924 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia a versao publica corrente.' }
if (-not $appJs01924.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia a versao publica corrente.' }
if (-not $index01924.Contains('/app.css?v=0.19.43') -or -not $index01924.Contains('/app.js?v=0.19.43')) { throw 'Cache busting da versao publica corrente ausente.' }
Write-Host "[1904/1904] Validando versao funcional v0.19.24..." -ForegroundColor Cyan
Write-Host "    v0.19.24 / Metabolic Planning UX & Safety: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green

# ===== v0.19.25 - Account & Email Foundation =====
Write-Host "[1905/1914] Validando infraestrutura transacional de email..." -ForegroundColor Cyan
$emailOptionsPath = Join-Path $root "src/HealthPlatform.Api/Services/Email/EmailOptions.cs"
$emailServicePath = Join-Path $root "src/HealthPlatform.Api/Services/Email/SmtpTransactionalEmailService.cs"
$emailInterfacePath = Join-Path $root "src/HealthPlatform.Api/Services/Email/ITransactionalEmailService.cs"
foreach ($path in @($emailOptionsPath,$emailServicePath,$emailInterfacePath)) { if (-not (Test-Path $path)) { throw "Infraestrutura de email ausente: $path" } }
$emailOptionsSource = Get-Content -Raw -Encoding UTF8 $emailOptionsPath
$emailServiceSource = Get-Content -Raw -Encoding UTF8 $emailServicePath
foreach ($token in @('Enabled','Host','Port','UseSsl','FromAddress','PublicBaseUrl')) { if (-not $emailOptionsSource.Contains($token)) { throw "EmailOptions incompleto: $token" } }
foreach ($token in @('Email:Enabled=false','SmtpClient','SendMailAsync','EmailDeliveryResult')) { if (-not $emailServiceSource.Contains($token)) { throw "Servico SMTP incompleto: $token" } }
Write-Host "    SMTP configuravel e inerte por padrao: OK." -ForegroundColor Green

Write-Host "[1906/1914] Validando configuracao segura de email..." -ForegroundColor Cyan
$appSettings01925 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/appsettings.json")
foreach ($token in @('"Email"','"Enabled": false','"FromAddress"','"PublicBaseUrl"')) { if (-not $appSettings01925.Contains($token)) { throw "Configuracao de email incompleta: $token" } }
if ($appSettings01925 -match 'smtp.*password\s*[:=]\s*[^\s\"]{6,}' -or $appSettings01925.Contains('smtp.gmail.com')) { throw 'Credencial/provedor SMTP concreto nao deve ficar embutido no appsettings do repositorio.' }
Write-Host "    Sem disparo remoto ou segredo SMTP no desenvolvimento: OK." -ForegroundColor Green

Write-Host "[1907/1914] Validando templates transacionais basicos..." -ForegroundColor Cyan
$emailTemplatesPath = Join-Path $root "src/HealthPlatform.Api/Services/Email/AccountEmailTemplates.cs"
if (-not (Test-Path $emailTemplatesPath)) { throw 'AccountEmailTemplates ausente.' }
$emailTemplatesSource = Get-Content -Raw -Encoding UTF8 $emailTemplatesPath
foreach ($token in @('Confirmation','ChangeEmail','Confirme seu email','Confirme a troca de email','WebUtility.HtmlEncode')) { if (-not $emailTemplatesSource.Contains($token)) { throw "Templates de conta incompletos: $token" } }
Write-Host "    Confirmacao + troca de email: OK." -ForegroundColor Green

Write-Host "[1908/1914] Validando API de email da conta..." -ForegroundColor Cyan
$emailControllerPath = Join-Path $root "src/HealthPlatform.Api/Controllers/ContaEmailController.cs"
$emailContractsPath = Join-Path $root "src/HealthPlatform.Api/Contracts/Conta/EmailAccountContracts.cs"
if (-not (Test-Path $emailControllerPath)) { throw 'ContaEmailController ausente.' }
if (-not (Test-Path $emailContractsPath)) { throw 'EmailAccountContracts ausente.' }
$emailController01925 = Get-Content -Raw -Encoding UTF8 $emailControllerPath
$emailContracts01925 = Get-Content -Raw -Encoding UTF8 $emailContractsPath
foreach ($token in @('[Route("api/conta/email")]','[Authorize(Policy = "AuthenticatedOnly")]','[HttpGet]','EmailAccountStatusResponse')) { if (-not $emailController01925.Contains($token)) { throw "API de email incompleta: $token" } }
foreach ($token in @('EmailAccountStatusResponse','EmailPrincipal','Confirmado','PodeSolicitarConfirmacao')) { if (-not $emailContracts01925.Contains($token)) { throw "Contrato de status de email incompleto: $token" } }
Write-Host "    Status de email principal para paciente/profissional: OK." -ForegroundColor Green

Write-Host "[1909/1914] Validando confirmacao segura de email..." -ForegroundColor Cyan
foreach ($token in @('GenerateEmailConfirmationTokenAsync','WebEncoders.Base64UrlEncode','ConfirmEmailAsync','EMAIL_CONFIRMATION_SENT','EMAIL_CONFIRMED','Token de confirmacao invalido ou expirado')) { if (-not $emailController01925.Contains($token)) { throw "Confirmacao de email incompleta: $token" } }
Write-Host "    Token Identity + encoding URL-safe + auditoria: OK." -ForegroundColor Green

Write-Host "[1910/1914] Validando troca segura de email principal..." -ForegroundColor Cyan
foreach ($token in @('GenerateChangeEmailTokenAsync','CheckPasswordAsync','FindByEmailAsync','ChangeEmailAsync','SetUserNameAsync','BeginTransactionAsync','paciente.Email = newEmail','EMAIL_CHANGED')) { if (-not $emailController01925.Contains($token)) { throw "Troca segura de email incompleta: $token" } }
Write-Host "    Senha atual + duplicidade + transacao + sincronizacao do paciente: OK." -ForegroundColor Green

Write-Host "[1911/1914] Validando rastreio sem vazamento de token..." -ForegroundColor Cyan
foreach ($token in @('EMAIL_CHANGE_SENT','EMAIL_CHANGE_DELIVERY_PENDING','MaskEmail','DadosNovosJson')) { if (-not $emailController01925.Contains($token)) { throw "Rastreio de email incompleto: $token" } }
if ($emailController01925.Contains('DadosNovosJson = JsonSerializer.Serialize(new { token')) { throw 'Token de email nao pode ser persistido na auditoria.' }
Write-Host "    Envio/falha auditados sem persistir token: OK." -ForegroundColor Green

Write-Host "[1912/1914] Validando policy de conta para qualquer usuario autenticado..." -ForegroundColor Cyan
$program01925 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Program.cs")
foreach ($token in @('AddPolicy("AuthenticatedOnly"','policy.RequireAuthenticatedUser()','AddScoped<ITransactionalEmailService, SmtpTransactionalEmailService>','Configure<EmailOptions>')) { if (-not $program01925.Contains($token)) { throw "Registro/policy v0.19.26 incompleto: $token" } }
Write-Host "    Paciente e profissional podem gerir a propria conta sem abrir o default profissional: OK." -ForegroundColor Green

Write-Host "[1913/1914] Validando versao publica v0.19.26..." -ForegroundColor Cyan
$version01925 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01925 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01925 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
$appJs01925 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
if ($version01925 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.26.' }
if (($health01925 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.26.' }
if (-not $appJs01925.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.26.' }
if (-not $index01925.Contains('/app.css?v=0.19.43') -or -not $index01925.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.26 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.26 OK." -ForegroundColor Green

Write-Host "[1914/1914] Validando versao funcional v0.19.25..." -ForegroundColor Cyan
Write-Host "    v0.19.25 / Account & Email Foundation: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green

# ===== v0.19.26 - Public Access & Password Recovery =====
Write-Host "[1915/1924] Validando contratos de recuperacao de senha..." -ForegroundColor Cyan
$passwordContractsPath = Join-Path $root "src/HealthPlatform.Api/Contracts/Auth/PasswordRecoveryContracts.cs"
if (-not (Test-Path $passwordContractsPath)) { throw 'PasswordRecoveryContracts ausente.' }
$passwordContracts01926 = Get-Content -Raw -Encoding UTF8 $passwordContractsPath
foreach ($token in @('ForgotPasswordRequest','ResetPasswordRequest','NovaSenha','ConfirmacaoNovaSenha')) { if (-not $passwordContracts01926.Contains($token)) { throw "Contratos de recuperacao incompletos: $token" } }

Write-Host "[1916/1924] Validando API publica de recuperacao..." -ForegroundColor Cyan
$passwordControllerPath = Join-Path $root "src/HealthPlatform.Api/Controllers/PasswordRecoveryController.cs"
if (-not (Test-Path $passwordControllerPath)) { throw 'PasswordRecoveryController ausente.' }
$passwordController01926 = Get-Content -Raw -Encoding UTF8 $passwordControllerPath
foreach ($token in @('[AllowAnonymous]','[Route("api/auth/password")]','[HttpPost("esqueci")]','[HttpPost("redefinir")]','GeneratePasswordResetTokenAsync','ResetPasswordAsync')) { if (-not $passwordController01926.Contains($token)) { throw "API publica de recuperacao incompleta: $token" } }

Write-Host "[1917/1924] Validando protecao contra enumeracao de contas..." -ForegroundColor Cyan
foreach ($token in @('GenericRecoveryMessage','Se existir uma conta ativa','return Ok(new { message = GenericRecoveryMessage })')) { if (-not $passwordController01926.Contains($token)) { throw "Protecao anti-enumeracao incompleta: $token" } }
if ($passwordController01926.Contains('Conta nao encontrada') -or $passwordController01926.Contains('Email nao cadastrado')) { throw 'Recuperacao publica nao deve revelar existencia da conta.' }
Write-Host "    Resposta publica generica: OK." -ForegroundColor Green

Write-Host "[1918/1924] Validando expiracao e encoding seguro do token..." -ForegroundColor Cyan
$program01926 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Program.cs")
foreach ($token in @('Configure<DataProtectionTokenProviderOptions>','TokenLifespan = TimeSpan.FromMinutes(30)','WebEncoders.Base64UrlEncode','WebEncoders.Base64UrlDecode')) { if (-not ($program01926.Contains($token) -or $passwordController01926.Contains($token))) { throw "Token de redefinicao incompleto: $token" } }
Write-Host "    Token Identity com validade de 30 minutos: OK." -ForegroundColor Green

Write-Host "[1919/1924] Validando template transacional de redefinicao..." -ForegroundColor Cyan
$emailTemplates01926 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Services/Email/AccountEmailTemplates.cs")
foreach ($token in @('PasswordReset','Redefina sua senha na AESYN','expira em 30 minutos')) { if (-not $emailTemplates01926.Contains($token)) { throw "Template de redefinicao incompleto: $token" } }

Write-Host "[1920/1924] Validando auditoria sem token ou senha..." -ForegroundColor Cyan
foreach ($token in @('PASSWORD_RESET_SENT','PASSWORD_RESET_DELIVERY_PENDING','PASSWORD_RESET_COMPLETED','MaskEmail')) { if (-not $passwordController01926.Contains($token)) { throw "Auditoria de recuperacao incompleta: $token" } }
if ($passwordController01926.Contains('DadosNovosJson = JsonSerializer.Serialize(new { token') -or $passwordController01926.Contains('DadosNovosJson = JsonSerializer.Serialize(new { senha')) { throw 'Token/senha nao podem ser persistidos na auditoria.' }

Write-Host "[1921/1924] Validando paginas publicas de acesso..." -ForegroundColor Cyan
$index01926 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
$appJs01926 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
$appCss01926 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
foreach ($token in @('forgotPasswordView','resetPasswordView','Esqueci minha senha','Criar nova senha','data-public-login')) { if (-not $index01926.Contains($token)) { throw "Pagina publica incompleta: $token" } }
foreach ($token in @("HP_PUBLIC_ACCESS_PASSWORD_RECOVERY='v0.19.26'",'/recuperar-senha','/redefinir-senha','hpResolvePublicRoute','/api/auth/password/esqueci','/api/auth/password/redefinir')) { if (-not $appJs01926.Contains($token)) { throw "Navegacao publica incompleta: $token" } }
foreach ($token in @('.login-recovery-link','.public-account-visual')) { if (-not $appCss01926.Contains($token)) { throw "CSS publico incompleto: $token" } }

Write-Host "[1922/1924] Validando isolamento de desenvolvimento e SMTP..." -ForegroundColor Cyan
$appSettings01926 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/appsettings.json")
if (-not $appSettings01926.Contains('"Enabled": false')) { throw 'SMTP deve continuar desabilitado por padrao no desenvolvimento.' }
if ($passwordController01926.Contains('aesyn.com.br/api') -or $passwordController01926.Contains('187.6.166.168')) { throw 'Recuperacao nao pode apontar diretamente para producao/VPS.' }
Write-Host "    Recuperacao usa configuracao do ambiente; sem teste em producao: OK." -ForegroundColor Green

Write-Host "[1923/1924] Validando versao publica v0.19.26..." -ForegroundColor Cyan
$version01926 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01926 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
if ($version01926 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.26.' }
if (($health01926 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.26.' }
if (-not $program01926.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.26.' }
if (-not $appJs01926.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.26.' }
if (-not $index01926.Contains('/app.css?v=0.19.43') -or -not $index01926.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.26 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.26 OK." -ForegroundColor Green

Write-Host "[1924/1924] Validando versao funcional v0.19.26..." -ForegroundColor Cyan
Write-Host "    v0.19.26 / Public Access & Password Recovery: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.27 - Session & Authorization Hardening =====
Write-Host "[1925/1937] Validando policy do chat paciente/profissional..." -ForegroundColor Cyan
$chat01927 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/ChatAcompanhamentoController.cs")
foreach ($token in @('[Authorize(Policy = "AuthenticatedOnly")]', '[Authorize(Policy = "PatientOnly")]', '[HttpGet("me")]', '[HttpPost("me/mensagens")]', '[HttpGet("pacientes/{pacienteId:guid}")]', '[HttpPost("pacientes/{pacienteId:guid}/mensagens")]')) { if (-not $chat01927.Contains($token)) { throw "Policies do chat incompletas: $token" } }
Write-Host "    Paciente e profissional usam policies explicitas: OK." -ForegroundColor Green

Write-Host "[1926/1937] Validando isolamento do profissional no chat..." -ForegroundColor Cyan
foreach ($token in @('PodeAcompanharPaciente', 'responsavel?.Id == profissionalId', 'if (!await PodeAcompanharPaciente(profissional.Id, paciente.Id, ct)) return Forbid();')) { if (-not $chat01927.Contains($token)) { throw "Isolamento do chat incompleto: $token" } }
Write-Host "    GUID de paciente nao concede acesso por si so: OK." -ForegroundColor Green

Write-Host "[1927/1937] Validando revogacao JWT por SecurityStamp..." -ForegroundColor Cyan
$jwt01927 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Services/JwtTokenService.cs")
$program01927 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Program.cs")
foreach ($token in @('security_stamp', 'usuario.SecurityStamp')) { if (-not $jwt01927.Contains($token)) { throw "JWT sem SecurityStamp: $token" } }
foreach ($token in @('OnTokenValidated', 'FindByIdAsync', 'usuario.OrganizacaoId != organizationId', 'string.Equals(usuario.SecurityStamp, stamp')) { if (-not $program01927.Contains($token)) { throw "Validacao de sessao incompleta: $token" } }
Write-Host "    Usuario ativo + organizacao + stamp validados: OK." -ForegroundColor Green

Write-Host "[1928/1937] Validando logout revogavel..." -ForegroundColor Cyan
$auth01927 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/AuthController.cs")
foreach ($token in @('[HttpPost("logout")]', 'UpdateSecurityStampAsync', '[Authorize(Policy = "AuthenticatedOnly")]')) { if (-not $auth01927.Contains($token)) { throw "Logout seguro incompleto: $token" } }
Write-Host "    Logout invalida tokens anteriores: OK." -ForegroundColor Green

Write-Host "[1929/1937] Validando renovacao de sessao..." -ForegroundColor Cyan
foreach ($token in @('[HttpPost("renovar")]', 'CriarRespostaAsync', 'Sessao expirada ou revogada')) { if (-not $auth01927.Contains($token)) { throw "Renovacao de sessao incompleta: $token" } }
Write-Host "    Renovacao autenticada: OK." -ForegroundColor Green

Write-Host "[1930/1937] Validando lockout efetivo..." -ForegroundColor Cyan
foreach ($token in @('MaxFailedAccessAttempts = 5', 'DefaultLockoutTimeSpan = TimeSpan.FromMinutes(15)')) { if (-not $program01927.Contains($token)) { throw "Politica de lockout incompleta: $token" } }
foreach ($token in @('SetLockoutEnabledAsync(usuario, true)', 'lockoutOnFailure: true', 'result.IsLockedOut')) { if (-not $auth01927.Contains($token)) { throw "Login sem lockout efetivo: $token" } }
Write-Host "    5 tentativas / 15 minutos, inclusive contas legadas: OK." -ForegroundColor Green

Write-Host "[1931/1937] Validando novas contas com lockout habilitado..." -ForegroundColor Cyan
foreach ($relative in @('src/HealthPlatform.Infrastructure/Data/DbSeeder.cs','src/HealthPlatform.Api/Controllers/AcessoPacienteController.cs','src/HealthPlatform.Api/Controllers/EquipeController.cs')) {
  $source = Get-Content -Raw -Encoding UTF8 (Join-Path $root $relative)
  if (-not $source.Contains('LockoutEnabled = true')) { throw "Lockout ausente na criacao de conta: $relative" }
}
Write-Host "    Admin, paciente e equipe: OK." -ForegroundColor Green

Write-Host "[1932/1937] Validando respostas 401/403 padronizadas..." -ForegroundColor Cyan
foreach ($token in @('OnChallenge', 'StatusCodes.Status401Unauthorized', 'OnForbidden', 'StatusCodes.Status403Forbidden', 'Voce nao tem permissao')) { if (-not $program01927.Contains($token)) { throw "Tratamento 401/403 incompleto: $token" } }
Write-Host "    Challenge e forbidden em JSON: OK." -ForegroundColor Green

Write-Host "[1933/1937] Validando ciclo de sessao no frontend..." -ForegroundColor Cyan
$appJs01927 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
foreach ($token in @("HP_SESSION_AUTHORIZATION_HARDENING='v0.19.27'", 'hpStoreSession', 'hpScheduleSessionRenewal', '/api/auth/renovar', '/api/auth/logout', 'hp_expires_at')) { if (-not $appJs01927.Contains($token)) { throw "Sessao frontend incompleta: $token" } }
Write-Host "    Expiracao persistida + renovacao agendada + logout API: OK." -ForegroundColor Green

Write-Host "[1934/1937] Validando 401 desloga e 403 preserva sessao..." -ForegroundColor Cyan
foreach ($token in @("r.status===401", "logout({notifyServer:false})", "r.status===403", 'Você não tem permissão para executar esta ação.')) { if (-not $appJs01927.Contains($token)) { throw "Tratamento frontend 401/403 incompleto: $token" } }
Write-Host "    Sem logout indevido em 403: OK." -ForegroundColor Green

Write-Host "[1935/1937] Validando isolamento de desenvolvimento/producao..." -ForegroundColor Cyan
if ($auth01927.Contains('aesyn.com.br') -or $chat01927.Contains('187.6.166.168')) { throw 'Session/Auth nao pode depender diretamente da producao/VPS.' }
Write-Host "    Session/Auth sem chamadas diretas a producao: OK." -ForegroundColor Green

Write-Host "[1936/1937] Validando versao publica v0.19.27..." -ForegroundColor Cyan
$version01927 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01927 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$index01927 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01927 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.27.' }
if (($health01927 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.27.' }
if (-not $program01927.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.27.' }
if (-not $appJs01927.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.27.' }
if (-not $index01927.Contains('/app.css?v=0.19.43') -or -not $index01927.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.27 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.27 OK." -ForegroundColor Green

Write-Host "[1937/1937] Validando versao funcional v0.19.27..." -ForegroundColor Cyan
Write-Host "    v0.19.27 / Session & Authorization Hardening: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.28 - Settings & User Profile =====
Write-Host "[1938/1949] Validando persistencia de preferencias do usuario..." -ForegroundColor Cyan
$usuario01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Infrastructure/Identity/Usuario.cs")
foreach ($token in @('FotoPerfilDataUrl','TemaPreferido','NotificarMensagens','NotificarAtualizacoesPlano','NotificarLembretes','NotificarCheckIns')) { if (-not $usuario01928.Contains($token)) { throw "Preferencias de usuario incompletas: $token" } }

Write-Host "[1939/1949] Validando migration de perfil/configuracoes..." -ForegroundColor Cyan
$migration01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Infrastructure/Migrations/20260924183000_V01928ConfiguracoesPerfilUsuario.cs")
foreach ($token in @('V01928ConfiguracoesPerfilUsuario','FotoPerfilDataUrl','TemaPreferido','NotificarMensagens','NotificarAtualizacoesPlano','NotificarLembretes','NotificarCheckIns')) { if (-not $migration01928.Contains($token)) { throw "Migration v0.19.28 incompleta: $token" } }

Write-Host "[1940/1949] Validando API de preferencias da conta..." -ForegroundColor Cyan
$config01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/ConfiguracoesController.cs")
foreach ($token in @('[HttpPut("minha-conta/preferencias")]','AtualizarPreferenciasRequest','UPDATE_PREFERENCES','Tema invalido. Use light ou dark.')) { if (-not $config01928.Contains($token)) { throw "API de preferencias incompleta: $token" } }

Write-Host "[1941/1949] Validando foto de perfil segura..." -ForegroundColor Cyan
foreach ($token in @('[HttpPut("minha-conta/foto")]','400_000','data:image/jpeg;base64,','PROFILE_PHOTO_UPDATE','PROFILE_PHOTO_REMOVE')) { if (-not $config01928.Contains($token)) { throw "Foto de perfil incompleta: $token" } }
if ($config01928.Contains('DadosNovosJson = JsonSerializer.Serialize(new { usuario.FotoPerfilDataUrl')) { throw 'Conteudo da foto nao pode ser persistido em AuditLog.' }

Write-Host "[1942/1949] Validando sessao atual da conta..." -ForegroundColor Cyan
foreach ($token in @('[HttpGet("minha-conta/sessao-atual")]','UserAgent','ExpiraEmUtc','RevogacaoLogout = "global"')) { if (-not $config01928.Contains($token)) { throw "Resumo de sessao incompleto: $token" } }

Write-Host "[1943/1949] Validando Settings & User Profile no frontend..." -ForegroundColor Cyan
$appJs01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.js")
foreach ($token in @('v0.19.28 — Settings & User Profile','hpEnhanceAccountSettings','hpCompressProfilePhoto','/api/configuracoes/minha-conta/preferencias','/api/configuracoes/minha-conta/foto','/api/configuracoes/minha-conta/sessao-atual')) { if (-not $appJs01928.Contains($token)) { throw "Settings frontend incompleto: $token" } }

Write-Host "[1944/1949] Validando compressao local da foto..." -ForegroundColor Cyan
foreach ($token in @("canvas.toDataURL('image/jpeg',0.82)",'const size=256','accept="image/jpeg,image/png,image/webp"')) { if (-not $appJs01928.Contains($token)) { throw "Compressao de foto incompleta: $token" } }

Write-Host "[1945/1949] Validando preferencias de notificacao..." -ForegroundColor Cyan
foreach ($token in @('data-pref="notificarMensagens"','data-pref="notificarAtualizacoesPlano"','data-pref="notificarLembretes"','data-pref="notificarCheckIns"')) { if (-not $appJs01928.Contains($token)) { throw "Preferencia de notificacao ausente: $token" } }

Write-Host "[1946/1949] Validando CSS responsivo de configuracoes..." -ForegroundColor Cyan
$appCss01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/app.css")
foreach ($token in @('.profile-settings-v01928','.profile-photo-preview','.profile-theme-choice','.profile-notification-grid','@media(max-width:760px)')) { if (-not $appCss01928.Contains($token)) { throw "CSS Settings v0.19.28 incompleto: $token" } }

Write-Host "[1947/1949] Validando privacidade e auditoria..." -ForegroundColor Cyan
foreach ($token in @('Senhas e conteúdo da foto não são gravados no AuditLog.','DadosNovosJson = JsonSerializer.Serialize(new { FotoAtualizada = foto is not null })')) { if (-not ($appJs01928.Contains($token) -or $config01928.Contains($token))) { throw "Protecao de privacidade incompleta: $token" } }

Write-Host "[1948/1949] Validando versao publica v0.19.28..." -ForegroundColor Cyan
$version01928 = (Get-Content (Join-Path $root "VERSION.txt") -Raw).Trim()
$health01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Controllers/HealthController.cs")
$program01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/Program.cs")
$index01928 = Get-Content -Raw -Encoding UTF8 (Join-Path $root "src/HealthPlatform.Api/wwwroot/index.html")
if ($version01928 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.28.' }
if (($health01928 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.28.' }
if (-not $program01928.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.28.' }
if (-not $appJs01928.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.28.' }
if (-not $index01928.Contains('/app.css?v=0.19.43') -or -not $index01928.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.28 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.28 OK." -ForegroundColor Green

Write-Host "[1949/1960] Validando baseline funcional v0.19.28..." -ForegroundColor Cyan
Write-Host "    v0.19.28 / Settings & User Profile: OK." -ForegroundColor Green

# ===== v0.19.29 - AESYN App Identity & Theme Completion =====
$manifest01929 = Get-Content .\src\HealthPlatform.Api\wwwroot\manifest.webmanifest -Encoding UTF8 -Raw
$index01929 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$appJs01929 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01929 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$health01929 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01929 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$version01929 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[1950/1960] Validando manifest PWA AESYN..." -ForegroundColor Cyan
foreach ($token in @('"name": "AESYN Performance"','"short_name": "AESYN"','"display": "standalone"','"theme_color": "#0B3B36"','"background_color": "#07100F"')) { if (-not $manifest01929.Contains($token)) { throw "Manifest AESYN incompleto: $token" } }
Write-Host "    Manifest AESYN: OK." -ForegroundColor Green

Write-Host "[1951/1960] Validando icones PWA..." -ForegroundColor Cyan
foreach ($icon in @('favicon-32.png','apple-touch-icon.png','icon-192.png','icon-512.png','maskable-192.png','maskable-512.png')) { if (-not (Test-Path (Join-Path '.\src\HealthPlatform.Api\wwwroot\icons' $icon))) { throw "Icone PWA ausente: $icon" } }
Write-Host "    Icones PWA: OK." -ForegroundColor Green

Write-Host "[1952/1960] Validando links de identidade no HTML..." -ForegroundColor Cyan
foreach ($token in @('/manifest.webmanifest?v=0.19.43','/icons/favicon-32.png?v=0.19.43','/icons/apple-touch-icon.png?v=0.19.43','apple-mobile-web-app-capable','AESYN Performance')) { if (-not $index01929.Contains($token)) { throw "Identidade HTML incompleta: $token" } }
Write-Host "    HTML mobile/PWA: OK." -ForegroundColor Green

Write-Host "[1953/1960] Validando launch experience standalone..." -ForegroundColor Cyan
foreach ($token in @('aesynLaunchScreen','aesyn-launch-screen','display-mode:standalone','dataset.displayMode')) { if ((-not $index01929.Contains($token)) -and (-not $appJs01929.Contains($token)) -and (-not $appCss01929.Contains($token))) { throw "Launch experience incompleta: $token" } }
Write-Host "    Launch experience: OK." -ForegroundColor Green

Write-Host "[1954/1960] Validando theme-color dinamico..." -ForegroundColor Cyan
foreach ($token in @('themeColorMeta','#07100F','#0B3B36','document.documentElement.dataset.theme')) { if ((-not $index01929.Contains($token)) -and (-not $appJs01929.Contains($token))) { throw "Theme-color incompleto: $token" } }
Write-Host "    Theme-color dinamico: OK." -ForegroundColor Green

Write-Host "[1955/1960] Validando primeiro acesso e persistencia de tema..." -ForegroundColor Cyan
foreach ($token in @('hpPrepareThemeAfterLogin','hpPersistFirstThemeChoice','HP_THEME_CHOICE_KEY','/api/configuracoes/minha-conta/preferencias')) { if (-not $appJs01929.Contains($token)) { throw "Fluxo de tema inicial incompleto: $token" } }
Write-Host "    Primeiro acesso / tema: OK." -ForegroundColor Green

Write-Host "[1956/1960] Validando dark mode do portal do paciente..." -ForegroundColor Cyan
foreach ($token in @('.patient-portal-top','.patient-portal-nav','.patient-plan-totals .metric','.workout-plan-card','.access-status-card','.exercise-prescription span')) { if (-not $appCss01929.Contains($token)) { throw "Dark mode paciente incompleto: $token" } }
Write-Host "    Dark mode paciente: OK." -ForegroundColor Green

Write-Host "[1957/1960] Validando dark mode de login, modal e formulario..." -ForegroundColor Cyan
foreach ($token in @('.login-shell','.login-panel','.modal','.login-card input','.form-grid input')) { if (-not $appCss01929.Contains($token)) { throw "Dark mode global incompleto: $token" } }
Write-Host "    Dark mode global: OK." -ForegroundColor Green

Write-Host "[1958/1960] Validando ausencia de alteracao de schema v0.19.29..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01929' }) { throw 'v0.19.29 nao deveria criar migration.' }
Write-Host "    Sem migration nova: OK." -ForegroundColor Green

Write-Host "[1959/1960] Validando versao publica v0.19.29..." -ForegroundColor Cyan
if ($version01929 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.29.' }
if (($health01929 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.29.' }
if (-not $program01929.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.29.' }
if (-not $appJs01929.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.29.' }
if (-not $index01929.Contains('/app.css?v=0.19.43') -or -not $index01929.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.29 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.29 OK." -ForegroundColor Green

Write-Host "[1960/1960] Validando versao funcional v0.19.29..." -ForegroundColor Cyan
Write-Host "    v0.19.29 / AESYN App Identity & Theme Completion: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.40 - Patient Home Cleanup =====
$appJs01930 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01930 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01930 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01930 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01930 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$version01930 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[1961/1970] Validando Hoje em um olhar simplificado..." -ForegroundColor Cyan
foreach ($token in @("HP_PATIENT_HOME_CLEANUP='v0.19.30'", 'Hoje em um olhar', 'O que você precisa fazer hoje', 'patient-today-overview')) { if (-not $appJs01930.Contains($token)) { throw "Home limpa incompleta: $token" } }
Write-Host "    Resumo diario orientado a acao: OK." -ForegroundColor Green

Write-Host "[1962/1970] Validando seis prioridades da Home..." -ForegroundColor Cyan
foreach ($token in @('CHECK-IN','TREINO','ALIMENTAÇÃO','HIDRATAÇÃO','MENSAGENS','PENDÊNCIAS')) { if (-not $appJs01930.Contains($token)) { throw "Prioridade da Home ausente: $token" } }
Write-Host "    Check-in, treino, alimentacao, hidratacao, mensagens e pendencias: OK." -ForegroundColor Green

Write-Host "[1963/1970] Validando navegacao das acoes essenciais..." -ForegroundColor Cyan
foreach ($token in @("loadPatientSection('plano')","loadPatientSection('chat')","loadPatientSection('solicitacoes')",'openDailyReadiness(readiness)')) { if (-not $appJs01930.Contains($token)) { throw "Acao essencial sem navegacao: $token" } }
Write-Host "    Acoes essenciais conectadas ao portal: OK." -ForegroundColor Green

Write-Host "[1964/1970] Validando CSS responsivo do resumo diario..." -ForegroundColor Cyan
foreach ($token in @('.patient-today-overview','.patient-today-essential-grid','.patient-today-essential.primary-action','@media(max-width:560px)')) { if (-not $appCss01930.Contains($token)) { throw "CSS Patient Home Cleanup incompleto: $token" } }
Write-Host "    Desktop + mobile: OK." -ForegroundColor Green

Write-Host "[1965/1970] Validando dark mode da nova Home..." -ForegroundColor Cyan
foreach ($token in @('html[data-theme="dark"] .patient-today-overview','html[data-theme="dark"] .patient-today-essential')) { if (-not $appCss01930.Contains($token)) { throw "Dark mode da Home incompleto: $token" } }
Write-Host "    Dark mode: OK." -ForegroundColor Green

Write-Host "[1966/1970] Validando reducao de duplicacoes na primeira leitura..." -ForegroundColor Cyan
$homeStart01930 = $appJs01930.IndexOf('async function loadMyPatientPortal()')
$homeEnd01930 = $appJs01930.IndexOf('function hpExecutionDayCard', $homeStart01930)
if ($homeStart01930 -lt 0 -or $homeEnd01930 -lt 0) { throw 'Nao foi possivel localizar o renderer da Home do paciente.' }
$homeRenderer01930 = $appJs01930.Substring($homeStart01930, $homeEnd01930 - $homeStart01930)
foreach ($legacyCall in @('${hpMorningCheckin(readiness)}','${hpTodayBrief(d,readiness)}','${hpDailyGamifiedFocusAthleteCard(d.focoGamificadoDoDia)}')) { if ($homeRenderer01930.Contains($legacyCall)) { throw "Bloco duplicado ainda exposto na primeira leitura: $legacyCall" } }
Write-Host "    Primeira leitura sem cards redundantes: OK." -ForegroundColor Green

Write-Host "[1967/1970] Validando loading, erro e retry do portal..." -ForegroundColor Cyan
foreach ($token in @('patientSectionLoading(view)',"hpPatientFeedbackState('error'",'Tentar novamente','loadPatientSection(view)')) { if (-not $appJs01930.Contains($token)) { throw "Feedback do portal incompleto: $token" } }
Write-Host "    Loading + erro + retry preservados: OK." -ForegroundColor Green

Write-Host "[1968/1970] Validando ausencia de alteracao de schema v0.19.40..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01930' }) { throw 'v0.19.40 nao deveria criar migration.' }
Write-Host "    Sem migration nova: OK." -ForegroundColor Green

Write-Host "[1969/1970] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01930 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01930 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01930.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01930.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01930.Contains('/app.css?v=0.19.43') -or -not $index01930.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[1970/1970] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / Patient Home Cleanup: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green

# ===== v0.19.31 - Athlete Data Hub =====
$appJs01931 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01931 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01931 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01931 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01931 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$version01931 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[1971/1981] Validando Athlete Data Hub..." -ForegroundColor Cyan
foreach ($token in @("HP_ATHLETE_DATA_HUB='v0.19.31'",'loadPatientAthleteData','data-athlete-data-hub="v0.19.31"','DADOS PARA ATLETAS','Seu desempenho com contexto')) { if (-not $appJs01931.Contains($token)) { throw "Athlete Data Hub incompleto: $token" } }
Write-Host "    Hub tecnico dedicado: OK." -ForegroundColor Green

Write-Host "[1972/1981] Validando metricas tecnicas do atleta..." -ForegroundColor Cyan
foreach ($token in @('Prontidão','Carga','Performance','Recuperação','Peso','Consistência')) { if (-not $appJs01931.Contains($token)) { throw "Metrica do atleta ausente: $token" } }
Write-Host "    Prontidao, carga, performance, recuperacao, peso e consistencia: OK." -ForegroundColor Green

Write-Host "[1973/1981] Validando composicao corporal e tendencia..." -ForegroundColor Cyan
foreach ($token in @('hpAthleteTrendRows','Tendência de peso','COMPOSIÇÃO CORPORAL','percentualGordura','cinturaCm')) { if (-not $appJs01931.Contains($token)) { throw "Evolucao tecnica incompleta: $token" } }
Write-Host "    Corpo + tendencia longitudinal: OK." -ForegroundColor Green

Write-Host "[1974/1981] Validando contexto de treino e ciclo..." -ForegroundColor Cyan
foreach ($token in @('Ritmo esportivo','cicloEsportivoAtual','metaTreinosSemanais','data-athlete-jump="treino"')) { if (-not $appJs01931.Contains($token)) { throw "Contexto esportivo incompleto: $token" } }
Write-Host "    Treino e ciclo: OK." -ForegroundColor Green

Write-Host "[1975/1981] Validando navegacao para Dados para Atletas..." -ForegroundColor Cyan
foreach ($token in @('data-patient-view="atleta"','data-patient-more-view="atleta"','Dados para atletas')) { if (-not $index01931.Contains($token)) { throw "Navegacao Athlete Data incompleta: $token" } }
if (-not $appJs01931.Contains("loadPatientSection('atleta')")) { throw 'Atalho da Home para Dados para Atletas ausente.' }
Write-Host "    Home + nav + Mais: OK." -ForegroundColor Green

Write-Host "[1976/1981] Validando separacao da Home diaria..." -ForegroundColor Cyan
foreach ($token in @('Dados técnicos e evolução ficam fora do caminho.','patient-athlete-data-entry','Performance, recuperação e evolução')) { if (-not $appJs01931.Contains($token)) { throw "Separacao Home/Dados incompleta: $token" } }
Write-Host "    Home simples + dados tecnicos separados: OK." -ForegroundColor Green

Write-Host "[1977/1981] Validando CSS responsivo Athlete Data Hub..." -ForegroundColor Cyan
foreach ($token in @('.athlete-data-hub','.athlete-data-metrics','.athlete-data-grid','.athlete-data-body-grid','@media(max-width:560px)')) { if (-not $appCss01931.Contains($token)) { throw "CSS Athlete Data incompleto: $token" } }
Write-Host "    Desktop + mobile: OK." -ForegroundColor Green

Write-Host "[1978/1981] Validando dark mode Athlete Data Hub..." -ForegroundColor Cyan
foreach ($token in @('html[data-theme="dark"] .patient-athlete-data-entry','html[data-theme="dark"] .athlete-data-metric','html[data-theme="dark"] .athlete-data-panel')) { if (-not $appCss01931.Contains($token)) { throw "Dark mode Athlete Data incompleto: $token" } }
Write-Host "    Dark mode: OK." -ForegroundColor Green

Write-Host "[1979/1981] Validando ausencia de alteracao de schema v0.19.40..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01931' }) { throw 'v0.19.40 nao deveria criar migration.' }
Write-Host "    Sem migration nova: OK." -ForegroundColor Green

Write-Host "[1980/1981] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01931 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01931 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01931.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01931.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01931.Contains('/app.css?v=0.19.43') -or -not $index01931.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[1981/1981] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.31 / Athlete Data Hub preservado sobre a base 0.19.40: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green
...
$appJs01932 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01932 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$planController01932 = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
$foodController01932 = Get-Content .\src\HealthPlatform.Api\Controllers\AlimentosController.cs -Encoding UTF8 -Raw
$index01932 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01932 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01932 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$version01932 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[1982/1994] Validando acesso profissional de nutricao..." -ForegroundColor Cyan
foreach ($token in @("HP_PRO_NUTRITION_ACCESS='v0.19.32'",'data-pro-nutrition-flow="v0.19.32"','Treino e nutrição • Nutrição','+ Criar dieta')) { if (-not $appJs01932.Contains($token)) { throw "Professional Nutrition Access incompleto: $token" } }
Write-Host "    Workspace nutricional profissional: OK." -ForegroundColor Green

Write-Host "[1983/1994] Validando CRUD visual de dietas..." -ForegroundColor Cyan
foreach ($token in @('openMealPlanFormV01932','Editar dieta','Dieta atualizada.','method:editing?','/api/planos-alimentares/${plan.id}')) { if (-not $appJs01932.Contains($token)) { throw "CRUD de dieta incompleto: $token" } }
Write-Host "    Criacao + edicao: OK." -ForegroundColor Green

Write-Host "[1984/1994] Validando refeicoes, alimentos e equivalencias..." -ForegroundColor Cyan
foreach ($token in @('Refeições e equivalências','meal-item-builder','substitution-row','Catálogo de alimentos','hpOpenFoodCatalogV01932')) { if (-not $appJs01932.Contains($token)) { throw "Builder nutricional incompleto: $token" } }
Write-Host "    Refeicoes + alimentos + substituicoes: OK." -ForegroundColor Green

Write-Host "[1985/1994] Validando CRUD do catalogo alimentar..." -ForegroundColor Cyan
foreach ($token in @('[HttpPost]','[HttpPut("{id:guid}")]','[HttpDelete("{id:guid}")]','[HttpPost("{id:guid}/reativar")]')) { if (-not $foodController01932.Contains($token)) { throw "API de alimentos incompleta: $token" } }
foreach ($token in @('food-edit-v01932','food-toggle-v01932','Novo alimento','Salvar alimento')) { if (-not $appJs01932.Contains($token)) { throw "UI de alimentos incompleta: $token" } }
Write-Host "    Catalogo alimentar gerenciavel: OK." -ForegroundColor Green

Write-Host "[1986/1994] Validando metas nutricionais no builder..." -ForegroundColor Cyan
foreach ($token in @('metaCalorias','metaProteinasG','metaCarboidratosG','metaGordurasG','metaFibrasG','Compare meta × prescrito antes de publicar.')) { if (-not $appJs01932.Contains($token)) { throw "Metas nutricionais incompletas: $token" } }
Write-Host "    Metas kcal/P/C/G/fibra: OK." -ForegroundColor Green

Write-Host "[1987/1994] Validando duplicacao e progressao..." -ForegroundColor Cyan
if (-not $planController01932.Contains('[HttpPost("api/planos-alimentares/{id:guid}/duplicar")]')) { throw 'Endpoint de duplicacao nutricional ausente.' }
foreach ($token in @('Duplicar / progressão','openNutritionProgression(patient,plan)')) { if (-not $appJs01932.Contains($token)) { throw "Duplicacao/progressao nao exposta: $token" } }
Write-Host "    Duplicacao + progressao: OK." -ForegroundColor Green

Write-Host "[1988/1994] Validando arquivamento sem apagar historico..." -ForegroundColor Cyan
if (-not $planController01932.Contains('[HttpPost("api/planos-alimentares/{id:guid}/status/{status}")]')) { throw 'Endpoint de status do plano ausente.' }
foreach ($token in @('nutrition-status-v01932','data-next-status="${next}"','Dieta arquivada sem apagar o histórico.')) { if (-not $appJs01932.Contains($token)) { throw "Arquivamento nutricional incompleto: $token" } }
Write-Host "    Arquivamento preservando historico: OK." -ForegroundColor Green

Write-Host "[1989/1994] Validando revisao e publicacao..." -ForegroundColor Cyan
foreach ($token in @('Revisar & publicar','nutritionReviewPublishV01932','openPrescriptionReview(patient)','Dieta publicada.')) { if (-not $appJs01932.Contains($token)) { throw "Review/publish nutricional incompleto: $token" } }
Write-Host "    Revisao + publicacao: OK." -ForegroundColor Green

Write-Host "[1990/1994] Validando modelos reutilizaveis e atribuicao..." -ForegroundColor Cyan
foreach ($token in @('Modelos de dieta','Dietas & refeições','openDietMealLibrary(patient,''plans'')','openSaveMealPlanTemplate(plan)','criar-de-modelo')) { if (-not $appJs01932.Contains($token) -and -not $planController01932.Contains($token)) { throw "Templates nutricionais incompletos: $token" } }
Write-Host "    Templates + atribuicao ao paciente: OK." -ForegroundColor Green

Write-Host "[1991/1994] Validando CSS responsivo do workspace nutricional..." -ForegroundColor Cyan
foreach ($token in @('[data-pro-nutrition-flow="v0.19.32"] .nutrition-plan-actions','[data-pro-nutrition-flow="v0.19.32"] .nutrition-catalog-ready','@media(max-width:720px)')) { if (-not $appCss01932.Contains($token)) { throw "CSS nutricional profissional incompleto: $token" } }
Write-Host "    Desktop + mobile: OK." -ForegroundColor Green

Write-Host "[1992/1994] Validando ausencia de alteracao de schema v0.19.40..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01932' }) { throw 'v0.19.40 nao deveria criar migration.' }
Write-Host "    Sem migration nova: OK." -ForegroundColor Green

Write-Host "[1993/1994] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01932 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01932 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01932.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01932.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01932.Contains('/app.css?v=0.19.43') -or -not $index01932.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[1994/1994] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.32 / Professional Nutrition Access Completion preservado sobre a base 0.19.40: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green

# ===== v0.19.40 - Workout CRUD & Multi-Plan Completion =====
$appJs01933 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01933 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$workoutController01933 = Get-Content .\src\HealthPlatform.Api\Controllers\TreinosController.cs -Encoding UTF8 -Raw
$patientWorkoutController01933 = Get-Content .\src\HealthPlatform.Api\Controllers\MeuTreinoPacienteController.cs -Encoding UTF8 -Raw
$index01933 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01933 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01933 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$version01933 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[1995/2006] Validando CRUD profissional de treino v0.19.40..." -ForegroundColor Cyan
foreach ($token in @("HP_WORKOUT_CRUD_MULTI_PLAN='v0.19.33'",'data-pro-workout-flow="v0.19.33"','Editar','Duplicar','Arquivar','Reativar')) { if (-not $appJs01933.Contains($token)) { throw "Workout CRUD profissional incompleto: $token" } }
Write-Host "    Criar + editar + duplicar + arquivar + reativar: OK." -ForegroundColor Green

Write-Host "[1996/2006] Validando endpoints de arquivamento e reativacao..." -ForegroundColor Cyan
foreach ($token in @('[HttpDelete("api/treinos/{id:guid}")]','[HttpPost("api/treinos/{id:guid}/reativar")]','Auditar("ARCHIVE"','Auditar("REACTIVATE"')) { if (-not $workoutController01933.Contains($token)) { throw "API de ciclo de vida do treino incompleta: $token" } }
Write-Host "    Arquivamento reversivel preservando historico: OK." -ForegroundColor Green

Write-Host "[1997/2006] Validando desvinculacao sem exclusao de historico..." -ForegroundColor Cyan
foreach ($token in @('[HttpPost("api/treinos/{id:guid}/desvincular")]','UNLINK_ARCHIVE','HistoricoPreservado = true','Desvincular da rotina')) { if (-not $workoutController01933.Contains($token) -and -not $appJs01933.Contains($token)) { throw "Desvinculacao de treino incompleta: $token" } }
Write-Host "    Retirada da rotina sem apagar dados: OK." -ForegroundColor Green

Write-Host "[1998/2006] Validando multiplos planos por paciente..." -ForegroundColor Cyan
foreach ($token in @('Múltiplos planos são permitidos','sem plano, ter um ou vários planos simultâneos','workout-plan-grid-v01933')) { if (-not $appJs01933.Contains($token)) { throw "UI multi-plan profissional incompleta: $token" } }
if (-not $workoutController01933.Contains('.Where(x => x.PacienteId == pacienteId)')) { throw 'Listagem de todos os planos do paciente ausente.' }
Write-Host "    Zero/um/varios planos: OK." -ForegroundColor Green

Write-Host "[1999/2006] Validando portal do paciente multi-plan..." -ForegroundColor Cyan
foreach ($token in @('[HttpGet("todos")]','permiteInicioLivre = true','planos = planos.Select','api(''/api/portal/me/treino/todos'')','data-patient-multiplan="v0.19.33"')) { if (-not $patientWorkoutController01933.Contains($token) -and -not $appJs01933.Contains($token)) { throw "Portal multi-plan incompleto: $token" } }
Write-Host "    Todos os planos ativos expostos ao paciente: OK." -ForegroundColor Green

Write-Host "[2000/2006] Validando inicio livre de sessoes publicadas..." -ForegroundColor Cyan
foreach ($token in @('workout-start-any-v01933','Escolha o treino que quer iniciar','openWorkoutExecutionForm(sessao)')) { if (-not $appJs01933.Contains($token)) { throw "Inicio livre de treino incompleto: $token" } }
Write-Host "    Treino A/B/C/cardio/recuperacao pode ser iniciado livremente: OK." -ForegroundColor Green

Write-Host "[2001/2006] Validando duplicacao com linhagem/versionamento..." -ForegroundColor Cyan
foreach ($token in @('[HttpPost("api/treinos/{id:guid}/duplicar")]','PlanoOrigemId = raizId','Versao = maiorVersao + 1','hpDuplicateWorkoutV01933')) { if (-not $workoutController01933.Contains($token) -and -not $appJs01933.Contains($token)) { throw "Duplicacao/versionamento de treino incompleto: $token" } }
Write-Host "    Duplicacao preserva origem e versao: OK." -ForegroundColor Green

Write-Host "[2002/2006] Validando auditoria do ciclo de vida dos treinos..." -ForegroundColor Cyan
foreach ($token in @('Auditar("CREATE"','Auditar("UPDATE"','Auditar("ARCHIVE"','Auditar("REACTIVATE"','Auditar("UNLINK_ARCHIVE"','Auditar("DUPLICATE_PROGRESS"')) { if (-not $workoutController01933.Contains($token)) { throw "Auditoria de treino incompleta: $token" } }
Write-Host "    Auditoria de CRUD/versionamento: OK." -ForegroundColor Green

Write-Host "[2003/2006] Validando CSS responsivo multi-plan..." -ForegroundColor Cyan
foreach ($token in @('.patient-multiplan-grid-v01933','.workout-crud-actions-v01933','@media(max-width:720px)')) { if (-not $appCss01933.Contains($token)) { throw "CSS multi-plan incompleto: $token" } }
Write-Host "    Desktop + mobile: OK." -ForegroundColor Green

Write-Host "[2004/2006] Validando ausencia de alteracao de schema v0.19.40..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01933' }) { throw 'v0.19.40 nao deveria criar migration.' }
Write-Host "    Sem migration nova: OK." -ForegroundColor Green

Write-Host "[2005/2006] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01933 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01933 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01933.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01933.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01933.Contains('/app.css?v=0.19.43') -or -not $index01933.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2006/2006] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.33 / Workout CRUD & Multi-Plan Completion: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.40 - Patient Files Mobile 2.0 =====
$appJs01934 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01934 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$filesController01934 = Get-Content .\src\HealthPlatform.Api\Controllers\ArquivosPacienteController.cs -Encoding UTF8 -Raw
$index01934 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01934 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01934 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$version01934 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2007/2020] Validando biblioteca de arquivos do paciente..." -ForegroundColor Cyan
foreach ($token in @("HP_PATIENT_FILES_MOBILE='v0.19.34'",'data-patient-files="v0.19.34"','Biblioteca clínica de arquivos','Meus arquivos')) { if (-not $appJs01934.Contains($token)) { throw "Patient Files incompleto: $token" } }
Write-Host "    Biblioteca profissional + paciente: OK." -ForegroundColor Green

Write-Host "[2008/2020] Validando upload mobile de arquivos..." -ForegroundColor Cyan
foreach ($token in @('accept="application/pdf,image/jpeg,image/png,image/webp"','capture="environment"','até 15 MB','hpUploadPatientFileV01934')) { if (-not $appJs01934.Contains($token)) { throw "Upload mobile incompleto: $token" } }
Write-Host "    Camera/galeria/arquivos + PDF: OK." -ForegroundColor Green

Write-Host "[2009/2020] Validando API de upload protegida..." -ForegroundColor Cyan
foreach ($token in @('[RequestSizeLimit(LimiteBytes)]','AssinaturaValida','application/pdf','image/jpeg','image/png','image/webp','15 * 1024 * 1024')) { if (-not $filesController01934.Contains($token)) { throw "Seguranca de upload incompleta: $token" } }
Write-Host "    Limite + MIME + assinatura magica: OK." -ForegroundColor Green

Write-Host "[2010/2020] Validando metadados e pesquisa..." -ForegroundColor Cyan
foreach ($token in @('Categoria','Descricao','Tags','DataUtc','Filtrar(','Nome.Contains','Tags.Any')) { if (-not $filesController01934.Contains($token)) { throw "Metadados/pesquisa incompletos: $token" } }
foreach ($token in @('data-file-search','data-file-category','Exame','Laudo','Receita','Documento')) { if (-not $appJs01934.Contains($token)) { throw "UI de pesquisa/categoria incompleta: $token" } }
Write-Host "    Categoria + descricao + tags + busca: OK." -ForegroundColor Green

Write-Host "[2011/2020] Validando acesso paciente e profissional..." -ForegroundColor Cyan
foreach ($token in @('[Authorize(Policy = "PatientOnly")]','PodeAcessarComoProfissional','MeuPaciente','currentUser.OrganizationId')) { if (-not $filesController01934.Contains($token)) { throw "Autorizacao Patient Files incompleta: $token" } }
Write-Host "    Isolamento por conta/organizacao: OK." -ForegroundColor Green

Write-Host "[2012/2020] Validando download autenticado..." -ForegroundColor Cyan
foreach ($token in @('PhysicalFile','enableRangeProcessing: true','hpOpenSecureFileV01934','Authorization=`Bearer ${state.token}`')) { if (-not $filesController01934.Contains($token) -and -not $appJs01934.Contains($token)) { throw "Download protegido incompleto: $token" } }
Write-Host "    Download nao exposto via StaticFiles: OK." -ForegroundColor Green

Write-Host "[2013/2020] Validando remocao auditavel..." -ForegroundColor Cyan
foreach ($token in @('Removido = true','RemovidoEmUtc','PATIENT_FILE_{acao}','data-file-remove')) { if (-not $filesController01934.Contains($token) -and -not $appJs01934.Contains($token)) { throw "Remocao/auditoria incompleta: $token" } }
Write-Host "    Remocao logica + auditoria: OK." -ForegroundColor Green

Write-Host "[2014/2020] Validando auditoria de upload/download..." -ForegroundColor Cyan
foreach ($token in @('Auditar("UPLOAD"','Auditar("DOWNLOAD"','AuditLogs.Add','IpAddress')) { if (-not $filesController01934.Contains($token)) { throw "Auditoria de arquivos incompleta: $token" } }
Write-Host "    Upload/download auditados: OK." -ForegroundColor Green

Write-Host "[2015/2020] Validando indexacao no chat..." -ForegroundColor Cyan
foreach ($token in @('data-file-chat','Arquivo compartilhado:','/api/chat/me/mensagens','/api/chat/pacientes/${patientId}/mensagens',"contexto:'Exames'","referenciaTipo:'Arquivo'",'referenciaId:id','referenciaTitulo:name')) { if (-not $appJs01934.Contains($token)) { throw "Integracao arquivo/chat incompleta: $token" } }
Write-Host "    Referencia de arquivo pode ser enviada no chat: OK." -ForegroundColor Green

Write-Host "[2016/2020] Validando navegacao mobile do paciente..." -ForegroundColor Cyan
foreach ($token in @('data-patient-view="arquivos"','data-patient-more-view="arquivos"','Exames, laudos, fotos e PDFs')) { if (-not $index01934.Contains($token)) { throw "Navegacao Patient Files incompleta: $token" } }
if (-not $appJs01934.Contains('arquivos:loadPatientFilesV01934')) { throw 'Loader de arquivos do paciente ausente.' }
Write-Host "    Navegacao portal + Mais: OK." -ForegroundColor Green

Write-Host "[2017/2020] Validando aba profissional de arquivos..." -ForegroundColor Cyan
foreach ($token in @("tabButton('arquivos','Arquivos')","state.patientTab==='arquivos'",'loadProfessionalPatientFilesV01934')) { if (-not $appJs01934.Contains($token)) { throw "Aba profissional de arquivos incompleta: $token" } }
Write-Host "    Arquivos no perfil profissional: OK." -ForegroundColor Green

Write-Host "[2018/2020] Validando CSS mobile/dark Patient Files..." -ForegroundColor Cyan
foreach ($token in @('.patient-files-shell-v01934','.patient-file-card-v01934','html[data-theme="dark"] .patient-file-card-v01934','@media(max-width:720px)')) { if (-not $appCss01934.Contains($token)) { throw "CSS Patient Files incompleto: $token" } }
Write-Host "    Mobile + dark mode: OK." -ForegroundColor Green

Write-Host "[2019/2020] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01934 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01934 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01934.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01934.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01934.Contains('/app.css?v=0.19.43') -or -not $index01934.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2020/2020] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / Patient Files Mobile 2.0: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.40 - Chat Reliability & Context Foundation =====
$appJs01935 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01935 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$chatController01935 = Get-Content .\src\HealthPlatform.Api\Controllers\ChatAcompanhamentoController.cs -Encoding UTF8 -Raw
$index01935 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01935 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01935 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$version01935 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2021/2035] Validando autorizacao definitiva do chat..." -ForegroundColor Cyan
foreach ($token in @('[Authorize(Policy = "PatientOnly")]','PodeAcompanharPaciente','currentUser.OrganizationId','ResolverProfissionalDoPaciente')) { if (-not $chatController01935.Contains($token)) { throw "Autorizacao do chat incompleta: $token" } }
Write-Host "    Paciente/profissional/organizacao isolados: OK." -ForegroundColor Green

Write-Host "[2022/2035] Validando leitura automatica da conversa..." -ForegroundColor Cyan
foreach ($token in @('MarcarConversaComoLida','LidaEmUtc = agora','OrigemTipo == "ChatAcompanhamento"')) { if (-not $chatController01935.Contains($token)) { throw "Leitura do chat incompleta: $token" } }
Write-Host "    Abertura da conversa marca notificacoes correspondentes: OK." -ForegroundColor Green

Write-Host "[2023/2035] Validando contador de nao lidas..." -ForegroundColor Cyan
foreach ($token in @('[HttpGet("nao-lidas")]','possuiNaoLidas','hpChatUnreadV01935','chat-unread-badge-v01935')) { if (-not $chatController01935.Contains($token) -and -not $appJs01935.Contains($token) -and -not $appCss01935.Contains($token)) { throw "Nao lidas/badge incompleto: $token" } }
Write-Host "    API + badge de mensagens nao lidas: OK." -ForegroundColor Green

Write-Host "[2024/2035] Validando status enviada/entregue/lida..." -ForegroundColor Cyan
foreach ($token in @('"Enviada"','"Entregue"','"Lida"','care-chat-status-v01935')) { if (-not $chatController01935.Contains($token) -and -not $appJs01935.Contains($token)) { throw "Status de mensagem incompleto: $token" } }
Write-Host "    Status derivado da notificacao do destinatario: OK." -ForegroundColor Green

Write-Host "[2025/2035] Validando persistencia de contexto estruturado..." -ForegroundColor Cyan
foreach ($token in @('ContextMarkerPrefix','ChatReferencia','ReferenciaTipo','ReferenciaId','ReferenciaTitulo','SepararObservacoes')) { if (-not $chatController01935.Contains($token)) { throw "Contexto estruturado incompleto: $token" } }
Write-Host "    Referencia estruturada sem migration nova: OK." -ForegroundColor Green

Write-Host "[2026/2035] Validando tipos de contexto futuros..." -ForegroundColor Cyan
foreach ($token in @('"Arquivo", "Treino", "Exercicio", "Refeicao", "Exame"')) { if (-not $chatController01935.Contains($token)) { throw "Tipos de contexto incompletos: $token" } }
Write-Host "    Arquivo/treino/exercicio/refeicao/exame preparados: OK." -ForegroundColor Green

Write-Host "[2027/2035] Validando renderizacao de referencia no chat..." -ForegroundColor Cyan
foreach ($token in @('hpChatReferenceV01935','data-chat-reference','Contexto ${esc(ref.tipo)}')) { if (-not $appJs01935.Contains($token)) { throw "Referencia visual incompleta: $token" } }
Write-Host "    Contexto aparece como card dentro da mensagem: OK." -ForegroundColor Green

Write-Host "[2028/2035] Validando arquivos estruturados no chat..." -ForegroundColor Cyan
foreach ($token in @("referenciaTipo:'Arquivo'",'referenciaId:id','referenciaTitulo:name','Arquivo compartilhado:')) { if (-not $appJs01935.Contains($token)) { throw "Arquivo contextual no chat incompleto: $token" } }
Write-Host "    Biblioteca envia referencia estruturada: OK." -ForegroundColor Green

Write-Host "[2029/2035] Validando retry e preservacao da mensagem..." -ForegroundColor Cyan
foreach ($token in @('care-chat-error-v01935','data-chat-retry','const original=text.value','text.value=original')) { if (-not $appJs01935.Contains($token)) { throw "Retry do chat incompleto: $token" } }
Write-Host "    Falha preserva texto e oferece tentar novamente: OK." -ForegroundColor Green

Write-Host "[2030/2035] Validando acesso rapido a arquivos pelo chat..." -ForegroundColor Cyan
foreach ($token in @('data-chat-open-files','Abrir meus arquivos','Abrir arquivos do paciente')) { if (-not $appJs01935.Contains($token)) { throw "Atalho de anexos incompleto: $token" } }
Write-Host "    Chat e biblioteca conectados: OK." -ForegroundColor Green

Write-Host "[2031/2035] Validando CSS mobile/dark do chat..." -ForegroundColor Cyan
foreach ($token in @('.care-chat-reference-v01935','.chat-unread-badge-v01935','.care-chat-error-v01935','html[data-theme="dark"] .care-chat-context-help-v01935','@media(max-width:720px)')) { if (-not $appCss01935.Contains($token)) { throw "CSS Chat Reliability incompleto: $token" } }
Write-Host "    Mobile + dark + badge: OK." -ForegroundColor Green

Write-Host "[2032/2035] Validando ausencia de alteracao de schema v0.19.40..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01935' }) { throw 'v0.19.40 nao deveria criar migration.' }
Write-Host "    Status/contexto reutilizam estruturas persistentes existentes: OK." -ForegroundColor Green

Write-Host "[2033/2035] Validando isolamento de desenvolvimento/produção..." -ForegroundColor Cyan
if ($chatController01935.Contains('aesyn.com.br') -or $appJs01935.Contains('187.6.166.168')) { throw 'Chat v0.19.40 nao deve apontar para producao.' }
Write-Host "    Nenhum endpoint de producao hardcoded: OK." -ForegroundColor Green

Write-Host "[2034/2035] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01935 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01935 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01935.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01935.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01935.Contains('/app.css?v=0.19.43') -or -not $index01935.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2035/2035] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / Chat Reliability & Context Foundation: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.40 - Administrator Professional Impersonation =====
$impController01936 = Get-Content .\src\HealthPlatform.Api\Controllers\ImpersonacaoController.cs -Encoding UTF8 -Raw
$jwtService01936 = Get-Content .\src\HealthPlatform.Api\Services\JwtTokenService.cs -Encoding UTF8 -Raw
$jwtInterface01936 = Get-Content .\src\HealthPlatform.Api\Services\IJwtTokenService.cs -Encoding UTF8 -Raw
$program01936 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$appJs01936 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01936 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01936 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01936 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$version01936 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2036/2050] Validando endpoints de impersonacao..." -ForegroundColor Cyan
foreach ($token in @('[Route("api/impersonacao")]','[HttpPost("{usuarioId:guid}/iniciar")]','[HttpPost("finalizar")]','ImpersonacaoResponse')) { if (-not $impController01936.Contains($token)) { throw "Impersonacao API incompleta: $token" } }
Write-Host "    Inicio/fim de simulacao: OK." -ForegroundColor Green

Write-Host "[2037/2050] Validando alvo profissional permitido..." -ForegroundColor Cyan
foreach ($token in @('TipoUsuario.Medico','TipoUsuario.Nutricionista','TipoUsuario.Personal','TiposSimulaveis.Contains')) { if (-not $impController01936.Contains($token)) { throw "Perfis simulaveis incompletos: $token" } }
Write-Host "    Apenas perfis profissionais ativos: OK." -ForegroundColor Green

Write-Host "[2038/2050] Validando origem administrativa..." -ForegroundColor Cyan
foreach ($token in @('[Authorize(Roles = "Admin")]','TipoUsuario == TipoUsuario.Admin','currentUser.OrganizationId')) { if (-not $impController01936.Contains($token)) { throw "Origem administrativa incompleta: $token" } }
Write-Host "    Inicio exclusivo de Admin na mesma organizacao: OK." -ForegroundColor Green

Write-Host "[2039/2050] Validando claims de simulacao..." -ForegroundColor Cyan
foreach ($token in @('["impersonation"] = "true"','["impersonator_id"]','["impersonator_name"]','["impersonator_security_stamp"]')) { if (-not $impController01936.Contains($token)) { throw "Claims de impersonacao incompletas: $token" } }
Write-Host "    Origem administrativa viaja no token: OK." -ForegroundColor Green

Write-Host "[2040/2050] Validando JWT extensivel e origem revogavel..." -ForegroundColor Cyan
foreach ($token in @('additionalClaims','new Claim(pair.Key, pair.Value)')) { if (-not $jwtService01936.Contains($token) -and -not $jwtInterface01936.Contains($token)) { throw "JWT de impersonacao incompleto: $token" } }
foreach ($token in @('impersonator_security_stamp','Administrador de origem nao esta mais autorizado')) { if (-not $program01936.Contains($token)) { throw "Validacao de origem incompleta: $token" } }
Write-Host "    Token invalida se Admin perder autorizacao: OK." -ForegroundColor Green

Write-Host "[2041/2050] Validando modo somente leitura..." -ForegroundColor Cyan
foreach ($token in @('a impersonacao administrativa e deliberadamente somente leitura','HttpMethods.IsGet','Modo de simulacao e somente leitura','/api/impersonacao/finalizar')) { if (-not $program01936.Contains($token)) { throw "Protecao read-only incompleta: $token" } }
Write-Host "    Mutacoes reais bloqueadas durante simulacao: OK." -ForegroundColor Green

Write-Host "[2042/2050] Validando retorno seguro ao administrador..." -ForegroundColor Cyan
foreach ($token in @('FindFirstValue("impersonator_id")','jwt.Create(admin, roles.ToArray())','Nao existe simulacao ativa nesta sessao')) { if (-not $impController01936.Contains($token)) { throw "Finalizacao de simulacao incompleta: $token" } }
Write-Host "    Saida reemite sessao administrativa: OK." -ForegroundColor Green

Write-Host "[2043/2050] Validando auditoria inicio/fim..." -ForegroundColor Cyan
foreach ($token in @('IMPERSONATION_START','IMPERSONATION_END','ImpersonacaoProfissional','UsuarioId = atorId','Modo = "READ_ONLY"')) { if (-not $impController01936.Contains($token)) { throw "Auditoria de impersonacao incompleta: $token" } }
Write-Host "    Admin de origem auditado explicitamente: OK." -ForegroundColor Green

Write-Host "[2044/2050] Validando banner persistente..." -ForegroundColor Cyan
foreach ($token in @('id="impersonationBanner"','Modo de simulação','Sair da simulação','hpSyncImpersonationUi')) { if (-not $index01936.Contains($token) -and -not $appJs01936.Contains($token)) { throw "Banner de simulacao incompleto: $token" } }
Write-Host "    Simulacao nunca e silenciosa: OK." -ForegroundColor Green

Write-Host "[2045/2050] Validando acao Simular na equipe..." -ForegroundColor Cyan
foreach ($token in @('team-impersonate','hpStartImpersonation','/api/impersonacao/${m.usuarioId}/iniciar','tipoProfissional(m.tipoUsuario)&&m.ativo')) { if (-not $appJs01936.Contains($token)) { throw "Acao Simular incompleta: $token" } }
Write-Host "    Admin inicia pela gestao de equipe: OK." -ForegroundColor Green

Write-Host "[2046/2050] Validando ciclo da sessao simulada..." -ForegroundColor Cyan
foreach ($token in @('impersonating:!!d.impersonating','impersonatorId:d.impersonatorId','if(state.user?.impersonating)return','hpEndImpersonation')) { if (-not $appJs01936.Contains($token)) { throw "Sessao simulada frontend incompleta: $token" } }
Write-Host "    Renovacao automatica nao remove contexto de simulacao: OK." -ForegroundColor Green

Write-Host "[2047/2050] Validando CSS mobile/dark do banner..." -ForegroundColor Cyan
foreach ($token in @('.impersonation-banner','[data-theme="dark"] .impersonation-banner','@media(max-width:720px)')) { if (-not $appCss01936.Contains($token)) { throw "CSS de impersonacao incompleto: $token" } }
Write-Host "    Banner responsivo e dark: OK." -ForegroundColor Green

Write-Host "[2048/2050] Validando ausencia de schema/producao..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01936' }) { throw 'v0.19.40 nao deveria criar migration.' }
if ($impController01936.Contains('aesyn.com.br') -or $appJs01936.Contains('187.6.166.168')) { throw 'Impersonacao nao deve apontar para producao.' }
Write-Host "    Sem migration e sem endpoint de producao: OK." -ForegroundColor Green

Write-Host "[2049/2050] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01936 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01936 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01936.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01936.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01936.Contains('/app.css?v=0.19.43') -or -not $index01936.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2050/2050] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / Administrator Professional Impersonation: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.40 - Patient First Access & Tutorials =====
$config01937 = Get-Content .\src\HealthPlatform.Api\Controllers\ConfiguracoesController.cs -Encoding UTF8 -Raw
$usuario01937 = Get-Content .\src\HealthPlatform.Infrastructure\Identity\Usuario.cs -Encoding UTF8 -Raw
$migration01937 = Get-Content .\src\HealthPlatform.Infrastructure\Migrations\20260924204500_V01937OnboardingPaciente.cs -Encoding UTF8 -Raw
$snapshot01937 = Get-Content .\src\HealthPlatform.Infrastructure\Migrations\AppDbContextModelSnapshot.cs -Encoding UTF8 -Raw
$appJs01937 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01937 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01937 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$health01937 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01937 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$roadmap01937 = Get-Content .\ROADMAP.md -Encoding UTF8 -Raw
$version01937 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2051/2063] Validando persistencia do onboarding..." -ForegroundColor Cyan
foreach ($token in @('OnboardingPacienteConcluidoEmUtc','V01937OnboardingPaciente','timestamp with time zone')) { if (-not $usuario01937.Contains($token) -and -not $migration01937.Contains($token) -and -not $snapshot01937.Contains($token)) { throw "Persistencia onboarding incompleta: $token" } }
Write-Host "    Estado de primeiro acesso persistente: OK." -ForegroundColor Green

Write-Host "[2052/2063] Validando API de onboarding do paciente..." -ForegroundColor Cyan
foreach ($token in @('[HttpGet("minha-conta/onboarding-paciente")]','[HttpPost("minha-conta/onboarding-paciente/concluir")]','TipoUsuario.Paciente','podeRefazer = true')) { if (-not $config01937.Contains($token)) { throw "API onboarding incompleta: $token" } }
Write-Host "    Consulta + conclusao: OK." -ForegroundColor Green

Write-Host "[2053/2063] Validando auditoria do onboarding..." -ForegroundColor Cyan
foreach ($token in @('PATIENT_ONBOARDING_COMPLETE','AuditLogs.Add','Versao = "v0.19.37"')) { if (-not $config01937.Contains($token)) { throw "Auditoria onboarding incompleta: $token" } }
Write-Host "    Conclusao auditada: OK." -ForegroundColor Green

Write-Host "[2054/2063] Validando trilha de primeiro acesso..." -ForegroundColor Cyan
foreach ($token in @('hpPatientTutorialStepsV01937','BEM-VINDO AO AESYN','PASSO 1 • CHECK-IN','PASSO 2 • PRIMEIRO TREINO','PASSO 3 • ALIMENTAÇÃO','PASSO 4 • CHAT','PRONTO PARA COMEÇAR')) { if (-not $appJs01937.Contains($token)) { throw "Trilha primeiro acesso incompleta: $token" } }
Write-Host "    Home/check-in/treino/alimentacao/chat: OK." -ForegroundColor Green

Write-Host "[2055/2063] Validando inicio automatico somente quando necessario..." -ForegroundColor Cyan
foreach ($token in @('hpMaybeStartPatientOnboardingV01937','if(!d?.concluido)','hpPatientTutorialShownV01937','__loadMyPatientPortal_v01937')) { if (-not $appJs01937.Contains($token)) { throw "Inicio automatico incompleto: $token" } }
Write-Host "    Primeiro acesso sem loop por navegacao: OK." -ForegroundColor Green

Write-Host "[2056/2063] Validando auxilio pratico de check-in e treino..." -ForegroundColor Cyan
foreach ($token in @("action:'Fazer meu check-in'","run:()=>openDailyReadiness(null)","action:'Ver meus treinos'","view:'treino'",'Treino A, B, C')) { if (-not $appJs01937.Contains($token)) { throw "Tutorial pratico incompleto: $token" } }
Write-Host "    Acoes contextuais do tutorial: OK." -ForegroundColor Green

Write-Host "[2057/2063] Validando alimentacao e chat no tutorial..." -ForegroundColor Cyan
foreach ($token in @("action:'Abrir meu plano'","view:'plano'","action:'Abrir chat'","view:'chat'",'Arquivos podem ser enviados pela biblioteca')) { if (-not $appJs01937.Contains($token)) { throw "Tutorial plano/chat incompleto: $token" } }
Write-Host "    Plano e comunicacao explicados: OK." -ForegroundColor Green

Write-Host "[2058/2063] Validando possibilidade de refazer tutorial..." -ForegroundColor Cyan
foreach ($token in @('id="patientTutorialTrigger"','Refazer tutorial','data-tutorial-replay','replay:true')) { if (-not $index01937.Contains($token) -and -not $appJs01937.Contains($token)) { throw "Replay do tutorial incompleto: $token" } }
Write-Host "    Ajuda ? + Meu perfil: OK." -ForegroundColor Green

Write-Host "[2059/2063] Validando que fechar nao marca conclusao..." -ForegroundColor Cyan
foreach ($token in @('Agora não','hpClosePatientTutorialV01937','hpFinishPatientTutorialV01937')) { if (-not $appJs01937.Contains($token)) { throw "Controle de conclusao incompleto: $token" } }
Write-Host "    Conclusao somente no passo final: OK." -ForegroundColor Green

Write-Host "[2060/2063] Validando CSS mobile/dark/acessivel..." -ForegroundColor Cyan
foreach ($token in @('.patient-tutorial-overlay-v01937','.patient-tutorial-card-v01937','.patient-tutorial-trigger','html[data-theme="dark"] .patient-tutorial-card-v01937','@media(max-width:720px)')) { if (-not $appCss01937.Contains($token)) { throw "CSS tutorial incompleto: $token" } }
foreach ($token in @('role="dialog"','aria-modal="true"','aria-labelledby="patientTutorialTitle"')) { if (-not $appJs01937.Contains($token)) { throw "Acessibilidade tutorial incompleta: $token" } }
Write-Host "    Mobile + dark + dialog semantico: OK." -ForegroundColor Green

Write-Host "[2061/2063] Validando roadmap e isolamento de producao..." -ForegroundColor Cyan
if (-not $roadmap01937.Contains('**Status:** implementado na v0.19.37')) { throw 'Roadmap nao registra conclusao v0.19.40.' }
if ($config01937.Contains('aesyn.com.br') -or $appJs01937.Contains('187.6.166.168')) { throw 'Onboarding nao deve depender de producao.' }
Write-Host "    Roadmap preservado + desenvolvimento local: OK." -ForegroundColor Green

Write-Host "[2062/2063] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01937 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01937 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01937.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01937.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01937.Contains('/app.css?v=0.19.43') -or -not $index01937.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2063/2063] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / Patient First Access & Tutorials: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green

# ===== v0.19.40 - PWA Install & Mobile Ergonomics =====
$appJs01938 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01938 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01938 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$manifest01938 = Get-Content .\src\HealthPlatform.Api\wwwroot\manifest.webmanifest -Encoding UTF8 -Raw
$sw01938 = Get-Content .\src\HealthPlatform.Api\wwwroot\sw.js -Encoding UTF8 -Raw
$health01938 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01938 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$roadmap01938 = Get-Content .\ROADMAP.md -Encoding UTF8 -Raw
$version01938 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2064/2077] Validando manifest e modo standalone..." -ForegroundColor Cyan
foreach ($token in @('"display": "standalone"','"name": "AESYN Performance"','"purpose": "maskable"')) { if (-not $manifest01938.Contains($token)) { throw "Manifest PWA incompleto: $token" } }
Write-Host "    Manifest instalavel: OK." -ForegroundColor Green

Write-Host "[2065/2077] Validando captura do prompt de instalacao..." -ForegroundColor Cyan
foreach ($token in @('beforeinstallprompt','hpDeferredInstallPromptV01938','prompt.userChoice','appinstalled')) { if (-not $appJs01938.Contains($token)) { throw "Instalacao Chrome/Android incompleta: $token" } }
Write-Host "    beforeinstallprompt + install lifecycle: OK." -ForegroundColor Green

Write-Host "[2066/2077] Validando instrucao dedicada para iPhone/Safari..." -ForegroundColor Cyan
foreach ($token in @('hpIsIosV01938','Adicionar à Tela de Início','Compartilhar','Safari')) { if (-not $appJs01938.Contains($token)) { throw "Fluxo iOS incompleto: $token" } }
Write-Host "    Instalacao manual iOS orientada: OK." -ForegroundColor Green

Write-Host "[2067/2077] Validando pontos de entrada da instalacao..." -ForegroundColor Cyan
foreach ($token in @('pwaInstallHeaderButton','patientPwaInstallButton','pwaInstallNudgeV01938','pwaInstallSheetV01938')) { if (-not $index01938.Contains($token)) { throw "CTA de instalacao incompleto: $token" } }
Write-Host "    Publico + paciente + nudge: OK." -ForegroundColor Green

Write-Host "[2068/2077] Validando Service Worker seguro..." -ForegroundColor Cyan
foreach ($token in @('aesyn-static-v0.19.43','request.method !==','url.pathname.startsWith','caches.open','self.skipWaiting')) { if (-not $sw01938.Contains($token)) { throw "Service Worker incompleto: $token" } }
if (-not $sw01938.Contains("'/api/'")) { throw 'Service Worker nao exclui /api/.' }
if ($sw01938.Contains('/api/auth') -or $sw01938.Contains('/api/pacientes')) { throw 'Service Worker nao deve cachear endpoints clinicos/autenticados.' }
foreach ($token in @('navigator.serviceWorker.register','window.isSecureContext','localhost','127.0.0.1')) { if (-not $appJs01938.Contains($token)) { throw "Registro Service Worker incompleto: $token" } }
Write-Host "    Cache somente shell/assets; API excluida: OK." -ForegroundColor Green

Write-Host "[2069/2077] Validando viewport e safe areas..." -ForegroundColor Cyan
foreach ($token in @('viewport-fit=cover','interactive-widget=resizes-content')) { if (-not $index01938.Contains($token)) { throw "Viewport mobile incompleto: $token" } }
foreach ($token in @('env(safe-area-inset-bottom)','--hp-visual-viewport-height')) { if (-not $appCss01938.Contains($token)) { throw "Safe area/viewport CSS incompleto: $token" } }
Write-Host "    Notch/home indicator/viewport: OK." -ForegroundColor Green

Write-Host "[2070/2077] Validando teclado movel..." -ForegroundColor Cyan
foreach ($token in @('window.visualViewport','hp-keyboard-open-v01938','scrollIntoView')) { if (-not $appJs01938.Contains($token)) { throw "Tratamento teclado incompleto: $token" } }
foreach ($token in @('body.hp-keyboard-open-v01938','.patient-portal-nav','scroll-margin-bottom:160px')) { if (-not $appCss01938.Contains($token)) { throw "CSS teclado incompleto: $token" } }
Write-Host "    Campos permanecem visiveis com teclado: OK." -ForegroundColor Green

Write-Host "[2071/2077] Validando touch targets e telas pequenas..." -ForegroundColor Cyan
foreach ($token in @('--hp-touch-target:44px','@media(pointer:coarse)','@media(max-width:360px)')) { if (-not $appCss01938.Contains($token)) { throw "Ergonomia touch incompleta: $token" } }
Write-Host "    Touch target 44px + small screens: OK." -ForegroundColor Green

Write-Host "[2072/2077] Validando modais/action sheets na viewport visual..." -ForegroundColor Cyan
foreach ($token in @('max-height:min(92dvh,var(--hp-visual-viewport-height))','.pwa-install-sheet-v01938','max-height:min(88dvh,var(--hp-visual-viewport-height))')) { if (-not $appCss01938.Contains($token)) { throw "Modais mobile incompletos: $token" } }
Write-Host "    Overlays respeitam area visivel: OK." -ForegroundColor Green

Write-Host "[2073/2077] Validando botao Voltar Android/portal..." -ForegroundColor Cyan
foreach ($token in @('hpPatientViewV01938','history.pushState({hpPatientView:next}','event.state?.hpPatientView','closePatientMoreSheet','closePatientQuickLog','closeClinicalAction')) { if (-not $appJs01938.Contains($token)) { throw "Historico mobile incompleto: $token" } }
Write-Host "    Back fecha overlays e navega no portal: OK." -ForegroundColor Green

Write-Host "[2074/2077] Validando dark/responsividade da instalacao..." -ForegroundColor Cyan
foreach ($token in @('html[data-theme="dark"] .pwa-install-nudge-v01938','@media(max-width:720px)','.pwa-install-actions-v01938')) { if (-not $appCss01938.Contains($token)) { throw "UI instalacao responsiva incompleta: $token" } }
Write-Host "    Install UI dark + mobile: OK." -ForegroundColor Green

Write-Host "[2075/2077] Validando roadmap/schema/producao..." -ForegroundColor Cyan
if (-not $roadmap01938.Contains('**Status:** implementado na v0.19.40')) { throw 'Roadmap nao registra conclusao v0.19.40.' }
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01938' }) { throw 'v0.19.40 nao deveria criar migration.' }
if ($appJs01938.Contains('187.6.166.168') -or $sw01938.Contains('aesyn.com.br')) { throw 'PWA development nao deve depender de producao.' }
Write-Host "    Roadmap preservado + sem migration + sem producao hardcoded: OK." -ForegroundColor Green

Write-Host "[2076/2077] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01938 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01938 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01938.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01938.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01938.Contains('/app.css?v=0.19.43') -or -not $index01938.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2077/2077] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / PWA Install & Mobile Ergonomics: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.40 - Push Notifications End-to-End =====
$pushController01939 = Get-Content .\src\HealthPlatform.Api\Controllers\PushNotificationsController.cs -Encoding UTF8 -Raw
$pushService01939 = Get-Content .\src\HealthPlatform.Api\Services\Push\WebPushNotificationService.cs -Encoding UTF8 -Raw
$pushOptions01939 = Get-Content .\src\HealthPlatform.Api\Services\Push\PushOptions.cs -Encoding UTF8 -Raw
$program01939 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$appSettings01939 = Get-Content .\src\HealthPlatform.Api\appsettings.Development.json -Encoding UTF8 -Raw
$chat01939 = Get-Content .\src\HealthPlatform.Api\Controllers\ChatAcompanhamentoController.cs -Encoding UTF8 -Raw
$checkin01939 = Get-Content .\src\HealthPlatform.Api\Controllers\CheckInsAcompanhamentoController.cs -Encoding UTF8 -Raw
$files01939 = Get-Content .\src\HealthPlatform.Api\Controllers\ArquivosPacienteController.cs -Encoding UTF8 -Raw
$appJs01939 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01939 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$sw01939 = Get-Content .\src\HealthPlatform.Api\wwwroot\sw.js -Encoding UTF8 -Raw
$roadmap01939 = Get-Content .\ROADMAP.md -Encoding UTF8 -Raw
$health01939 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$index01939 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$version01939 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2078/2092] Validando configuracao VAPID sem segredo hardcoded..." -ForegroundColor Cyan
foreach ($token in @('PushOptions.SectionName','IPushNotificationService','WebPushNotificationService')) { if (-not $program01939.Contains($token)) { throw "DI push incompleta: $token" } }
foreach ($token in @('public bool Enabled','PublicKey','PrivateKey','Subject')) { if (-not $pushOptions01939.Contains($token)) { throw "PushOptions incompleto: $token" } }
if (-not $appSettings01939.Contains('"Enabled": false')) { throw 'Development deve manter push desligado.' }
Write-Host "    VAPID via configuracao; Development off: OK." -ForegroundColor Green

Write-Host "[2079/2092] Validando armazenamento multi-dispositivo sem schema novo..." -ForegroundColor Cyan
foreach ($token in @('AESYN.WebPush','IdentityUserToken<Guid>','NomeToken(request.Endpoint)','SHA256.HashData')) { if (-not $pushController01939.Contains($token) -and -not $pushService01939.Contains($token)) { throw "Persistencia push incompleta: $token" } }
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01939' }) { throw 'v0.19.40 nao deveria criar migration.' }
Write-Host "    UsuariosTokens suporta varios dispositivos: OK." -ForegroundColor Green

Write-Host "[2080/2092] Validando API de inscricao push..." -ForegroundColor Cyan
foreach ($token in @('[Route("api/push")]','[HttpGet("status")]','[HttpPost("subscriptions")]','[HttpDelete("subscriptions")]','[HttpPost("test")]')) { if (-not $pushController01939.Contains($token)) { throw "API push incompleta: $token" } }
Write-Host "    Status/subscribe/unsubscribe/teste: OK." -ForegroundColor Green

Write-Host "[2081/2092] Validando protecao de dados da inscricao..." -ForegroundColor Cyan
foreach ($token in @('PUSH_SUBSCRIBE','PUSH_UNSUBSCRIBE','UserAgent = Limitar','Dispositivo = name')) { if (-not $pushController01939.Contains($token)) { throw "Auditoria push incompleta: $token" } }
if ($pushController01939.Contains('DadosNovosJson = payload')) { throw 'Chaves da subscription nao devem ir ao AuditLog.' }
Write-Host "    Auditoria sem p256dh/auth/endpoint bruto: OK." -ForegroundColor Green

Write-Host "[2082/2092] Validando preferencias por categoria..." -ForegroundColor Cyan
foreach ($token in @('"mensagem" => usuario.NotificarMensagens','"plano" => usuario.NotificarAtualizacoesPlano','"lembrete" => usuario.NotificarLembretes','"checkin" => usuario.NotificarCheckIns')) { if (-not $pushService01939.Contains($token)) { throw "Preferencias push incompletas: $token" } }
Write-Host "    Preferencias existentes respeitadas: OK." -ForegroundColor Green

Write-Host "[2083/2092] Validando envio Web Push real..." -ForegroundColor Cyan
foreach ($token in @('new VapidDetails','new WebPushClient','new PushSubscription','SendNotificationAsync')) { if (-not $pushService01939.Contains($token)) { throw "Web Push incompleto: $token" } }
Write-Host "    WebPush + VAPID: OK." -ForegroundColor Green

Write-Host "[2084/2092] Validando que falha de push nao quebra operacao clinica..." -ForegroundColor Cyan
foreach ($token in @('catch (Exception ex)','LogWarning','A requisicao principal nao sera interrompida')) { if (-not $pushService01939.Contains($token)) { throw "Tolerancia push incompleta: $token" } }
Write-Host "    Push best-effort isolado da operacao principal: OK." -ForegroundColor Green

Write-Host "[2085/2092] Validando eventos transacionais iniciais..." -ForegroundColor Cyan
foreach ($token in @('push.EnviarAsync(destinatarioUsuarioId.Value, "mensagem"','NotificarProfissionalCheckIn','push.EnviarAsync(usuarioId, "checkin"','NotificarNovoArquivo')) { if (-not $chat01939.Contains($token) -and -not $checkin01939.Contains($token) -and -not $files01939.Contains($token)) { throw "Evento push incompleto: $token" } }
$treinos01939 = Get-Content .\src\HealthPlatform.Api\Controllers\TreinosController.cs -Encoding UTF8 -Raw
$nutricao01939 = Get-Content .\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs -Encoding UTF8 -Raw
$execTreino01939 = Get-Content .\src\HealthPlatform.Api\Controllers\ExecucoesTreinoController.cs -Encoding UTF8 -Raw
foreach ($token in @('Seu treino foi atualizado','Seu plano alimentar foi atualizado','Treino com divergência','Treino concluído')) { if (-not $treinos01939.Contains($token) -and -not $nutricao01939.Contains($token) -and -not $execTreino01939.Contains($token)) { throw "Evento transacional ausente: $token" } }
Write-Host "    Chat + check-in + arquivo: OK." -ForegroundColor Green

Write-Host "[2086/2092] Validando opt-in manual no frontend..." -ForegroundColor Cyan
foreach ($token in @('Notification.requestPermission','pushManager.subscribe','data-push-enable','hpDisablePushV01939','/api/push/subscriptions')) { if (-not $appJs01939.Contains($token)) { throw "Opt-in push incompleto: $token" } }
if ($appJs01939.Contains('Notification.requestPermission();setTimeout')) { throw 'Permissao push nao deve ser solicitada automaticamente.' }
Write-Host "    Consentimento somente por gesto do usuario: OK." -ForegroundColor Green

Write-Host "[2087/2092] Validando Service Worker com app fechado..." -ForegroundColor Cyan
foreach ($token in @("self.addEventListener('push'","showNotification","self.addEventListener('notificationclick'","clients.openWindow","AESYN_PUSH_OPEN")) { if (-not $sw01939.Contains($token)) { throw "Service Worker push incompleto: $token" } }
Write-Host "    Push em background + clique: OK." -ForegroundColor Green

Write-Host "[2088/2092] Validando cache clinico continua proibido..." -ForegroundColor Cyan
if (-not $sw01939.Contains("url.pathname.startsWith('/api/')")) { throw 'Service Worker deixou de excluir /api/.' }
Write-Host "    Push nao altera politica de cache da API: OK." -ForegroundColor Green

Write-Host "[2089/2092] Validando UI mobile/dark de push..." -ForegroundColor Cyan
foreach ($token in @('.push-settings-v01939','.push-status-v01939','html[data-theme="dark"] .push-status-v01939','@media(max-width:720px)')) { if (-not $appCss01939.Contains($token)) { throw "CSS push incompleto: $token" } }
Write-Host "    Configuracao push responsiva: OK." -ForegroundColor Green

Write-Host "[2090/2092] Validando roadmap e isolamento de producao..." -ForegroundColor Cyan
if (-not $roadmap01939.Contains('**Status:** implementado na v0.19.40')) { throw 'Roadmap nao registra conclusao v0.19.40.' }
if ($pushService01939.Contains('187.6.166.168') -or $appJs01939.Contains('aesyn.com.br')) { throw 'Push de desenvolvimento nao deve depender de endpoint de producao hardcoded.' }
Write-Host "    Roadmap preservado + sem producao hardcoded: OK." -ForegroundColor Green

Write-Host "[2091/2092] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01939 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01939 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01939.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01939.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01939.Contains('/app.css?v=0.19.43') -or -not $index01939.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2092/2092] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / Push Notifications End-to-End: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.40 - In-App Notification Center =====
$notifications01940 = Get-Content .\src\HealthPlatform.Api\Controllers\NotificacoesController.cs -Encoding UTF8 -Raw
$appJs01940 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01940 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01940 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$roadmap01940 = Get-Content .\ROADMAP.md -Encoding UTF8 -Raw
$health01940 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01940 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$sw01940 = Get-Content .\src\HealthPlatform.Api\wwwroot\sw.js -Encoding UTF8 -Raw
$version01940 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2093/2105] Validando filtros da central interna..." -ForegroundColor Cyan
foreach ($token in @('incluirInativas = false','bool? lida = null','string? prioridade = null','string? tipo = null','NormalizarFiltro')) { if (-not $notifications01940.Contains($token)) { throw "API central incompleta: $token" } }
Write-Host "    Status/prioridade/categoria: OK." -ForegroundColor Green

Write-Host "[2094/2105] Validando historico preservado..." -ForegroundColor Cyan
foreach ($token in @('if (!incluirInativas)','x.Ativa','encerradas = g.Count(x => !x.Ativa)')) { if (-not $notifications01940.Contains($token)) { throw "Historico notificacoes incompleto: $token" } }
Write-Host "    Inativas consultaveis sem exclusao: OK." -ForegroundColor Green

Write-Host "[2095/2105] Validando resumo e categorias disponiveis..." -ForegroundColor Cyan
foreach ($token in @('tiposDisponiveis','naoLidas = g.Count','altas = g.Count','medias = g.Count')) { if (-not $notifications01940.Contains($token)) { throw "Resumo da central incompleto: $token" } }
Write-Host "    Resumo severidade/categorias: OK." -ForegroundColor Green

Write-Host "[2096/2105] Validando marcar como nao lida..." -ForegroundColor Cyan
foreach ($token in @('[HttpPut("{id:guid}/nao-lida")]','item.LidaEmUtc = null')) { if (-not $notifications01940.Contains($token)) { throw "Fluxo nao lida incompleto: $token" } }
Write-Host "    Reabertura visual de notificacao: OK." -ForegroundColor Green

Write-Host "[2097/2105] Validando UI de filtros..." -ForegroundColor Cyan
foreach ($token in @('data-notification-status="ativas"','data-notification-status="nao-lidas"','data-notification-status="historico"','notificationPriorityFilter','notificationTypeFilter')) { if (-not $index01940.Contains($token)) { throw "UI central incompleta: $token" } }
Write-Host "    Filtros visiveis: OK." -ForegroundColor Green

Write-Host "[2098/2105] Validando motor de filtros e badge..." -ForegroundColor Cyan
foreach ($token in @('hpNotificationCenterV01940','hpNotificationQueryV01940','incluirInativas','setNotificationBadge(d.naoLidas||0)','notification-mark-unread-v01940')) { if (-not $appJs01940.Contains($token)) { throw "Motor central incompleto: $token" } }
Write-Host "    Query + badge + nao lida: OK." -ForegroundColor Green

Write-Host "[2099/2105] Validando preferencias por categoria na central..." -ForegroundColor Cyan
foreach ($token in @('notificationPrefMessages','notificationPrefPlans','notificationPrefReminders','notificationPrefCheckins','notificationPreferencesSave')) { if (-not $index01940.Contains($token)) { throw "Preferencias UI incompletas: $token" } }
foreach ($token in @('hpLoadNotificationPreferencesV01940','hpSaveNotificationPreferencesV01940','/api/configuracoes/minha-conta/preferencias')) { if (-not $appJs01940.Contains($token)) { throw "Preferencias central incompletas: $token" } }
Write-Host "    Preferencias persistidas pela conta: OK." -ForegroundColor Green

Write-Host "[2100/2105] Validando deep links expandidos do paciente..." -ForegroundColor Cyan
foreach ($token in @("'chat'","'solicitacoes'","'arquivos'",'hpOpenPatientNotificationLink')) { if (-not $appJs01940.Contains($token)) { throw "Deep link paciente incompleto: $token" } }
Write-Host "    Chat/solicitacoes/arquivos: OK." -ForegroundColor Green

Write-Host "[2101/2105] Validando mobile e dark mode..." -ForegroundColor Cyan
foreach ($token in @('.notification-center-toolbar-v01940','html[data-theme="dark"] .notification-panel','@media(max-width:600px)','notification-preferences-v01940')) { if (-not $appCss01940.Contains($token)) { throw "CSS central incompleto: $token" } }
Write-Host "    Central responsiva/dark: OK." -ForegroundColor Green

Write-Host "[2102/2105] Validando ausencia de migration v0.19.40..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01940' }) { throw 'v0.19.40 nao deveria criar migration.' }
Write-Host "    Schema preservado: OK." -ForegroundColor Green

Write-Host "[2103/2105] Validando roadmap e isolamento de producao..." -ForegroundColor Cyan
if (-not $roadmap01940.Contains('**Status:** implementado na v0.19.40')) { throw 'Roadmap nao registra conclusao v0.19.40.' }
if ($appJs01940.Contains('187.6.166.168') -or $notifications01940.Contains('aesyn.com.br')) { throw 'Central de desenvolvimento nao deve depender de producao.' }
Write-Host "    Roadmap preservado + producao isolada: OK." -ForegroundColor Green

Write-Host "[2104/2105] Validando versao publica v0.19.40..." -ForegroundColor Cyan
if ($version01940 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.40.' }
if (($health01940 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.40.' }
if (-not $program01940.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.40.' }
if (-not $appJs01940.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.40.' }
if (-not $index01940.Contains('/app.css?v=0.19.43') -or -not $index01940.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.40 ausente.' }
if (-not $sw01940.Contains('aesyn-static-v0.19.43')) { throw 'Service Worker nao anuncia cache v0.19.40.' }
Write-Host "    API/UI/VERSION: 0.19.40 OK." -ForegroundColor Green

Write-Host "[2105/2105] Validando versao funcional v0.19.40..." -ForegroundColor Cyan
Write-Host "    v0.19.40 / In-App Notification Center: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green


# ===== v0.19.41 - UX State System =====
$appJs01941 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01941 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01941 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$roadmap01941 = Get-Content .\ROADMAP.md -Encoding UTF8 -Raw
$health01941 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01941 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$sw01941 = Get-Content .\src\HealthPlatform.Api\wwwroot\sw.js -Encoding UTF8 -Raw
$version01941 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2106/2118] Validando contrato de estados UX..." -ForegroundColor Cyan
if (-not $appJs01941.Contains('function hpUiState(')) { throw 'UX State System incompleto: function hpUiState(' }
foreach ($token in @('.hp-state-loading','.hp-state-empty','.hp-state-error','.hp-state-success')) { if (-not $appCss01941.Contains($token)) { throw "UX State System incompleto no CSS: $token" } }
Write-Host "    Loading/empty/error/success: OK." -ForegroundColor Green

Write-Host "[2107/2118] Validando skeleton padronizado..." -ForegroundColor Cyan
foreach ($token in @('hp-state-skeleton','aria-busy="true"','Carregando conteúdo')) { if (-not $appJs01941.Contains($token)) { throw "Skeleton padronizado incompleto: $token" } }
Write-Host "    Skeleton reutilizavel: OK." -ForegroundColor Green

Write-Host "[2108/2118] Validando retry reutilizavel..." -ForegroundColor Cyan
foreach ($token in @('function hpBindStateAction','function hpSetRegionState','function hpErrorState','data-hp-state-action')) { if (-not $appJs01941.Contains($token)) { throw "Retry UX incompleto: $token" } }
Write-Host "    Retry por regiao: OK." -ForegroundColor Green

Write-Host "[2109/2118] Validando feedback de sucesso e toast..." -ForegroundColor Cyan
foreach ($token in @('function hpSuccessState','function toast(','aria-live')) { if (-not $appJs01941.Contains($token)) { throw "Feedback de sucesso incompleto: $token" } }
Write-Host "    Success + toast: OK." -ForegroundColor Green

Write-Host "[2110/2118] Validando acoes assincronas pendentes..." -ForegroundColor Cyan
foreach ($token in @('function hpRunAction','hpSetActionPending(button,true','finally{hpSetActionPending(button,false)}')) { if (-not $appJs01941.Contains($token)) { throw "Acao pendente incompleta: $token" } }
Write-Host "    Pending state: OK." -ForegroundColor Green

Write-Host "[2111/2118] Validando modal de confirmacao..." -ForegroundColor Cyan
foreach ($token in @('hpConfirmModalV01941','data-confirm-title','data-confirm-message','data-confirm-yes','data-confirm-no')) { if (-not $index01941.Contains($token)) { throw "Modal confirmacao incompleto: $token" } }
foreach ($token in @('function hpConfirm(','Escape','aria-hidden')) { if (-not $appJs01941.Contains($token)) { throw "Motor confirmacao incompleto: $token" } }
Write-Host "    Confirmacao acessivel: OK." -ForegroundColor Green

Write-Host "[2112/2118] Validando CSS transversal..." -ForegroundColor Cyan
foreach ($token in @('v0.19.41 — UX State System','.hp-state-error','.hp-state-success','.hp-confirm-backdrop-v01941','.hp-confirm-actions-v01941')) { if (-not $appCss01941.Contains($token)) { throw "CSS UX State incompleto: $token" } }
Write-Host "    Estados e confirmacao estilizados: OK." -ForegroundColor Green

Write-Host "[2113/2118] Validando mobile e dark mode..." -ForegroundColor Cyan
foreach ($token in @('html[data-theme="dark"] .hp-state','@media(max-width:720px)','prefers-reduced-motion')) { if (-not $appCss01941.Contains($token)) { throw "UX mobile/dark incompleta: $token" } }
Write-Host "    Mobile/dark/reduced-motion: OK." -ForegroundColor Green

Write-Host "[2114/2118] Validando ausencia de migration v0.19.41..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01941' }) { throw 'v0.19.41 nao deveria criar migration.' }
Write-Host "    Schema preservado: OK." -ForegroundColor Green

Write-Host "[2115/2118] Validando roadmap v0.19.41..." -ForegroundColor Cyan
if (-not $roadmap01941.Contains('**Status:** implementado na v0.19.41')) { throw 'Roadmap nao registra conclusao v0.19.41.' }
if (-not $roadmap01941.Contains('## v0.19.42 — Offline & Poor Connection Resilience')) { throw 'Proxima etapa v0.19.42 ausente do roadmap.' }
Write-Host "    Roadmap preservado: OK." -ForegroundColor Green

Write-Host "[2116/2118] Validando isolamento de producao..." -ForegroundColor Cyan
if ($appJs01941.Contains('187.6.166.168') -or $appJs01941.Contains('https://aesyn.com.br/api')) { throw 'UX State de desenvolvimento nao deve depender de producao.' }
Write-Host "    Sem dependencia de producao: OK." -ForegroundColor Green

Write-Host "[2117/2118] Validando versao publica corrente apos v0.19.41..." -ForegroundColor Cyan
if ($version01941 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.42.' }
if (($health01941 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.42.' }
if (-not $program01941.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.42.' }
if (-not $appJs01941.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.42.' }
if (-not $index01941.Contains('/app.css?v=0.19.43') -or -not $index01941.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.42 ausente.' }
if (-not $sw01941.Contains('aesyn-static-v0.19.43')) { throw 'Service Worker nao anuncia cache v0.19.42.' }
Write-Host "    API/UI/VERSION: 0.19.42 OK." -ForegroundColor Green

Write-Host "[2118/2118] Validando compatibilidade funcional v0.19.41..." -ForegroundColor Cyan
Write-Host "    v0.19.41 / UX State System: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green

# ===== v0.19.42 - Offline & Poor Connection Resilience =====
$appJs01942 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01942 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01942 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$roadmap01942 = Get-Content .\ROADMAP.md -Encoding UTF8 -Raw
$health01942 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01942 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$sw01942 = Get-Content .\src\HealthPlatform.Api\wwwroot\sw.js -Encoding UTF8 -Raw
$version01942 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()

Write-Host "[2119/2132] Validando detector de conectividade..." -ForegroundColor Cyan
foreach ($token in @('hpConnectivityV01942','hpSetConnectivityV01942',"window.addEventListener('offline'", "window.addEventListener('online'")) { if (-not $appJs01942.Contains($token)) { throw "Conectividade v0.19.42 incompleta: $token" } }
Write-Host "    Online/offline monitorados: OK." -ForegroundColor Green

Write-Host "[2120/2132] Validando banner de conexao..." -ForegroundColor Cyan
foreach ($token in @('hpConnectivityBannerV01942','data-connectivity-title','data-connectivity-message','data-connectivity-retry')) { if (-not $index01942.Contains($token)) { throw "Banner conectividade incompleto: $token" } }
foreach ($token in @('.hp-connectivity-banner-v01942','is-offline','is-slow')) { if (-not $appCss01942.Contains($token)) { throw "CSS conectividade incompleto: $token" } }
Write-Host "    Banner offline/lento: OK." -ForegroundColor Green

Write-Host "[2121/2132] Validando erro de rede amigavel..." -ForegroundColor Cyan
foreach ($token in @('hpIsNetworkErrorV01942','Conexão indisponível ou instável','O que você digitou foi preservado')) { if (-not $appJs01942.Contains($token)) { throw "Tratamento de rede incompleto: $token" } }
Write-Host "    Falha de internet separada de erro interno: OK." -ForegroundColor Green

Write-Host "[2122/2132] Validando rascunhos de sessao..." -ForegroundColor Cyan
foreach ($token in @('HP_DRAFT_PREFIX_V01942','sessionStorage','hpBindDraftsV01942','hpClearDraftV01942','data-hp-draft="chat-message"')) { if (-not $appJs01942.Contains($token)) { throw "Preservacao de rascunho incompleta: $token" } }
if ($appJs01942.Contains('localStorage.setItem(HP_DRAFT_PREFIX_V01942')) { throw 'Rascunhos clinicos nao devem ser persistidos permanentemente em localStorage.' }
Write-Host "    Rascunho do chat preservado somente na sessao: OK." -ForegroundColor Green

Write-Host "[2123/2132] Validando fila offline opt-in..." -ForegroundColor Cyan
foreach ($token in @('HP_OFFLINE_QUEUE_KEY_V01942','queueIfOffline','queueKey','hpQueueSafeRequestV01942','hpFlushOfflineQueueV01942')) { if (-not $appJs01942.Contains($token)) { throw "Fila offline incompleta: $token" } }
Write-Host "    Fila explicita/deduplicada: OK." -ForegroundColor Green

Write-Host "[2124/2132] Validando escopo seguro da fila..." -ForegroundColor Cyan
foreach ($token in @("['PUT','POST'].includes(method)",'Esta ação não pode ser enfileirada offline com segurança.','filter(x=>x.key!==key)')) { if (-not $appJs01942.Contains($token)) { throw "Seguranca da fila offline incompleta: $token" } }
Write-Host "    Somente acoes declaradas + dedupe: OK." -ForegroundColor Green

Write-Host "[2125/2132] Validando notificacao lida resiliente..." -ForegroundColor Cyan
foreach ($token in @('queueIfOffline:true','notification-read:${id}')) { if (-not $appJs01942.Contains($token)) { throw "Fila segura de notificacao incompleta: $token" } }
Write-Host "    Leitura pode sincronizar apos reconexao: OK." -ForegroundColor Green

Write-Host "[2126/2132] Validando protecao contra submissao duplicada..." -ForegroundColor Cyan
foreach ($token in @('hpInflightActionsV01942','hpRunOnceV01942','Esta ação já está em andamento','dedupeKey:`chat-send:${url}`')) { if (-not $appJs01942.Contains($token)) { throw "Protecao duplicidade incompleta: $token" } }
Write-Host "    Lock por chave: OK." -ForegroundColor Green

Write-Host "[2127/2132] Validando reconexao e retry..." -ForegroundColor Cyan
foreach ($token in @('hpFlushOfflineQueueV01942','Conexão restabelecida. Ações pendentes foram sincronizadas.','data-connectivity-retry')) { if (-not $appJs01942.Contains($token)) { throw "Reconexao incompleta: $token" } }
Write-Host "    Flush + retry: OK." -ForegroundColor Green

Write-Host "[2128/2132] Validando Service Worker sem dados clinicos..." -ForegroundColor Cyan
if (-not $sw01942.Contains("url.pathname.startsWith('/api/')")) { throw 'Service Worker deve manter /api/ fora do cache.' }
if (-not $sw01942.Contains('aesyn-static-v0.19.43')) { throw 'Cache estatico nao foi versionado para v0.19.42.' }
Write-Host "    Shell offline sem cache de API: OK." -ForegroundColor Green

Write-Host "[2129/2132] Validando ausencia de migration v0.19.42..." -ForegroundColor Cyan
if (Get-ChildItem .\src\HealthPlatform.Infrastructure\Migrations -File | Where-Object { $_.Name -match '01942' }) { throw 'v0.19.42 nao deveria criar migration.' }
Write-Host "    Schema preservado: OK." -ForegroundColor Green

Write-Host "[2130/2132] Validando roadmap e proxima etapa..." -ForegroundColor Cyan
if (-not $roadmap01942.Contains('**Status:** implementado na v0.19.42')) { throw 'Roadmap nao registra conclusao v0.19.42.' }
if (-not $roadmap01942.Contains('## v0.19.43 — Audit, Privacy & Consent')) { throw 'Proxima etapa v0.19.43 ausente do roadmap.' }
Write-Host "    Roadmap preservado: OK." -ForegroundColor Green

Write-Host "[2131/2132] Validando versao publica v0.19.42..." -ForegroundColor Cyan
if ($version01942 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.42.' }
if (($health01942 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.42.' }
if (-not $program01942.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.42.' }
if (-not $appJs01942.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.42.' }
if (-not $index01942.Contains('/app.css?v=0.19.43') -or -not $index01942.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.42 ausente.' }
Write-Host "    API/UI/VERSION: 0.19.42 OK." -ForegroundColor Green

Write-Host "[2132/2132] Validando versao funcional v0.19.42..." -ForegroundColor Cyan
Write-Host "    v0.19.42 / Offline & Poor Connection Resilience: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green

# ===== v0.19.43 - Audit, Privacy & Consent =====
$appJs01943 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
$appCss01943 = Get-Content .\src\HealthPlatform.Api\wwwroot\app.css -Encoding UTF8 -Raw
$index01943 = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
$roadmap01943 = Get-Content .\ROADMAP.md -Encoding UTF8 -Raw
$health01943 = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
$program01943 = Get-Content .\src\HealthPlatform.Api\Program.cs -Encoding UTF8 -Raw
$sw01943 = Get-Content .\src\HealthPlatform.Api\wwwroot\sw.js -Encoding UTF8 -Raw
$version01943 = (Get-Content .\VERSION.txt -Encoding UTF8 -Raw).Trim()
$user01943 = Get-Content .\src\HealthPlatform.Infrastructure\Identity\Usuario.cs -Encoding UTF8 -Raw
$db01943 = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
$config01943 = Get-Content .\src\HealthPlatform.Api\Controllers\ConfiguracoesController.cs -Encoding UTF8 -Raw
$legal01943 = Get-Content .\src\HealthPlatform.Api\Services\LegalDocuments.cs -Encoding UTF8 -Raw
$migration01943 = Get-Content .\src\HealthPlatform.Infrastructure\Migrations\20260924221000_V01943AuditPrivacyConsent.cs -Encoding UTF8 -Raw
$lgpd01943 = Get-Content .\docs\LGPD-PRIVACY-CHECKLIST.md -Encoding UTF8 -Raw

Write-Host "[2133/2148] Validando contrato de aceite versionado..." -ForegroundColor Cyan
foreach ($token in @('TermosVersaoAceita','TermosAceitosEmUtc','PoliticaPrivacidadeVersaoAceita','PoliticaPrivacidadeAceitaEmUtc')) { if (-not $user01943.Contains($token)) { throw "Consentimento versionado incompleto: $token" } }
Write-Host "    Versao + timestamp persistidos: OK." -ForegroundColor Green

Write-Host "[2134/2148] Validando documentos legais versionados..." -ForegroundColor Cyan
foreach ($token in @('TermosVersaoAtual = "2026-09-24-v1"','PoliticaPrivacidadeVersaoAtual = "2026-09-24-v1"','RevisaoJuridica')) { if (-not $legal01943.Contains($token)) { throw "Documento legal incompleto: $token" } }
if (-not (Test-Path .\docs\TERMOS-DE-USO.md) -or -not (Test-Path .\docs\POLITICA-DE-PRIVACIDADE.md)) { throw 'Documentos operacionais de Termos/Privacidade ausentes.' }
Write-Host "    Termos + Privacidade versionados: OK." -ForegroundColor Green

Write-Host "[2135/2148] Validando API de privacidade da conta..." -ForegroundColor Cyan
foreach ($token in @('[HttpGet("minha-conta/privacidade")]','[HttpPost("minha-conta/consentimentos/aceitar")]','LEGAL_ACCEPTANCE')) { if (-not $config01943.Contains($token)) { throw "API privacidade incompleta: $token" } }
Write-Host "    Status + aceite + auditoria: OK." -ForegroundColor Green

Write-Host "[2136/2148] Validando solicitacao de exclusao controlada..." -ForegroundColor Cyan
foreach ($token in @('SolicitacaoExclusaoDadosEmUtc','[HttpPost("minha-conta/solicitar-exclusao-dados")]','DATA_DELETION_REQUEST','nao sao apagados automaticamente')) { if (-not $config01943.Contains($token)) { throw "Solicitacao de exclusao incompleta: $token" } }
Write-Host "    Pedido registrado sem delecao silenciosa: OK." -ForegroundColor Green

Write-Host "[2137/2148] Validando desativacao segura da conta..." -ForegroundColor Cyan
foreach ($token in @('[HttpPost("minha-conta/desativar")]','CheckPasswordAsync','"DESATIVAR"','ContaDesativadaEmUtc','ACCOUNT_DEACTIVATE','UpdateSecurityStampAsync','AESYN.WebPush')) { if (-not $config01943.Contains($token)) { throw "Desativacao de conta incompleta: $token" } }
Write-Host "    Senha + confirmacao + revogacao: OK." -ForegroundColor Green

Write-Host "[2138/2148] Validando auditoria consultavel pelo titular..." -ForegroundColor Cyan
foreach ($token in @('[HttpGet("minha-conta/auditoria")]','Take(limite)','x.Acao','x.Entidade','x.CreatedAtUtc')) { if (-not $config01943.Contains($token)) { throw "Auditoria da conta incompleta: $token" } }
Write-Host "    Metadados auditaveis sem payload sensivel: OK." -ForegroundColor Green

Write-Host "[2139/2148] Validando cobertura de auditoria critica..." -ForegroundColor Cyan
$auditFiles = @(
  '.\src\HealthPlatform.Api\Controllers\TreinosController.cs',
  '.\src\HealthPlatform.Api\Controllers\PlanosAlimentaresController.cs',
  '.\src\HealthPlatform.Api\Controllers\ArquivosPacienteController.cs',
  '.\src\HealthPlatform.Api\Controllers\PacientesController.cs',
  '.\src\HealthPlatform.Api\Controllers\ImpersonacaoController.cs'
)
foreach ($path in $auditFiles) { $source = Get-Content $path -Encoding UTF8 -Raw; if (-not $source.Contains('AuditLog')) { throw "Cobertura de auditoria ausente: $path" } }
Write-Host "    Treino/dieta/arquivos/paciente/impersonacao: OK." -ForegroundColor Green

Write-Host "[2140/2148] Validando UI de privacidade..." -ForegroundColor Cyan
foreach ($token in @('HP_AUDIT_PRIVACY_CONSENT','data-privacy-v01943','Termos, privacidade e seus dados','data-legal-accept-v01943','data-delete-request-v01943','data-deactivate-v01943')) { if (-not $appJs01943.Contains($token)) { throw "UI privacidade incompleta: $token" } }
Write-Host "    Minha Conta integrada: OK." -ForegroundColor Green

Write-Host "[2141/2148] Validando lembrete de aceite sem bloqueio cego..." -ForegroundColor Cyan
foreach ($token in @('hpEnsureLegalConsentV01943','Termos e privacidade precisam de revisão','data-legal-later-v01943','data-legal-open-v01943')) { if (-not $appJs01943.Contains($token)) { throw "Lembrete legal incompleto: $token" } }
Write-Host "    Revisao sinalizada sem esconder o produto: OK." -ForegroundColor Green

Write-Host "[2142/2148] Validando CSS mobile/dark de privacidade..." -ForegroundColor Cyan
foreach ($token in @('v0.19.43 — Audit, Privacy & Consent','.privacy-consent-v01943','.legal-reminder-v01943','html[data-theme="dark"]','@media(max-width:720px)')) { if (-not $appCss01943.Contains($token)) { throw "CSS privacidade incompleto: $token" } }
Write-Host "    Mobile + dark: OK." -ForegroundColor Green

Write-Host "[2143/2148] Validando migration v0.19.43..." -ForegroundColor Cyan
foreach ($token in @('V01943AuditPrivacyConsent','TermosVersaoAceita','PoliticaPrivacidadeVersaoAceita','ContaDesativadaEmUtc','SolicitacaoExclusaoDadosEmUtc')) { if (-not $migration01943.Contains($token)) { throw "Migration privacidade incompleta: $token" } }
foreach ($token in @('TermosVersaoAceita','PoliticaPrivacidadeVersaoAceita')) { if (-not $db01943.Contains($token)) { throw "Mapeamento de usuario incompleto: $token" } }
Write-Host "    Schema de governanca de conta: OK." -ForegroundColor Green

Write-Host "[2144/2148] Validando checklist LGPD sem alegacao de conformidade..." -ForegroundColor Cyan
foreach ($token in @('não certifica conformidade jurídica','política formal de retenção','procedimento operacional para pedidos do titular','operadores/suboperadores')) { if (-not $lgpd01943.Contains($token)) { throw "Checklist LGPD incompleto: $token" } }
Write-Host "    Pendencias juridicas explicitadas: OK." -ForegroundColor Green

Write-Host "[2145/2148] Validando higiene de dados sensiveis..." -ForegroundColor Cyan
if ($config01943.Contains('DadosNovosJson = JsonSerializer.Serialize(new { request.SenhaAtual')) { throw 'Senha nao pode entrar no AuditLog.' }
if ($config01943.Contains('DadosNovosJson = JsonSerializer.Serialize(new { FotoDataUrl')) { throw 'Conteudo bruto de foto nao pode entrar no AuditLog.' }
if ($appJs01943.Contains('localStorage.setItem(HP_DRAFT_PREFIX_V01942')) { throw 'Rascunhos clinicos nao devem ir para localStorage.' }
Write-Host "    Senha/foto/rascunho protegidos: OK." -ForegroundColor Green

Write-Host "[2146/2148] Validando roadmap e fechamento da Lista 03..." -ForegroundColor Cyan
if (-not $roadmap01943.Contains('**Status:** implementado na v0.19.43')) { throw 'Roadmap nao registra conclusao v0.19.43.' }
if (-not $roadmap01943.Contains('## v0.19.44 — Product Flow Gate / Lista 03 Closure')) { throw 'Proxima etapa v0.19.44 ausente do roadmap.' }
Write-Host "    Roadmap preservado: OK." -ForegroundColor Green

Write-Host "[2147/2148] Validando versao publica v0.19.43..." -ForegroundColor Cyan
if ($version01943 -ne '0.19.43') { throw 'VERSION.txt inesperado na v0.19.43.' }
if (($health01943 | Select-String -Pattern 'version = "0.19.43"' -AllMatches).Matches.Count -lt 2) { throw 'Health nao anuncia v0.19.43.' }
if (-not $program01943.Contains('Version = "v0.19.43"')) { throw 'Swagger nao anuncia v0.19.43.' }
if (-not $appJs01943.Contains("const HP_MVP_VERSION='0.19.43';")) { throw 'Interface nao anuncia v0.19.43.' }
if (-not $index01943.Contains('/app.css?v=0.19.43') -or -not $index01943.Contains('/app.js?v=0.19.43')) { throw 'Cache busting v0.19.43 ausente.' }
if (-not $sw01943.Contains('aesyn-static-v0.19.43')) { throw 'Service Worker nao anuncia cache v0.19.43.' }
Write-Host "    API/UI/VERSION: 0.19.43 OK." -ForegroundColor Green

Write-Host "[2148/2148] Validando versao funcional v0.19.43..." -ForegroundColor Cyan
Write-Host "    v0.19.43 / Audit, Privacy & Consent: OK." -ForegroundColor Green
Write-Host "TESTE DE FUMACA CONCLUIDO." -ForegroundColor Green
Write-Host "Nenhum registro foi criado ou alterado." -ForegroundColor Green
