...
if ($health.version -ne "0.6.8") { throw "Versao inesperada da API: $($health.version)" }
...
if ($version.Trim() -ne "0.6.8") { throw "VERSION.txt inesperado." }
...
$indexSource = Get-Content .\src\HealthPlatform.Api\wwwroot\index.html -Encoding UTF8 -Raw
if (-not $indexSource.Contains("MVP Preview • v0.6.8") -or
...
Write-Host "[600/664] Validando compatibilidade funcional v0.6.0..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8") { throw "VERSION.txt inesperado." }
...
Write-Host "[601/664] Validando entidade de prontidao diaria..."
...
Write-Host "[602/664] Validando mapeamento EF da prontidao..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[603/664] Validando endpoint PatientOnly de prontidao..."
...
Write-Host "[604/664] Validando travas de seguranca do score..."
...
Write-Host "[605/664] Validando Home Daily Athlete..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[606/664] Validando leitura profissional da prontidao..."
...
Write-Host "[607/664] Validando SQL e migration v0.6.1..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[608/664] Validando versao funcional v0.6.1..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Versao v0.6.1 inconsistente." }
Write-Host "    v0.6.1 / Daily Athlete + Prontidao Diaria: OK."



Write-Host "[609/664] Validando ledger de XP..."
...
Write-Host "[610/664] Validando mapeamento EF de XP..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[611/664] Validando XP seguro de treino..."
...
Write-Host "[612/664] Validando nivel, streak e consistencia..."
...
Write-Host "[613/664] Validando eventos de autocuidado..."
$portalMeSource = Get-Content .\src\HealthPlatform.Api\Controllers\MeuPortalPacienteController.cs -Encoding UTF8 -Raw
...
Write-Host "[614/664] Validando Home esportiva gamificada..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[615/664] Validando SQL e migration v0.6.2..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[616/664] Validando versao funcional v0.6.2..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Versao v0.6.2 inconsistente." }
Write-Host "    v0.6.2 / XP + Nivel + Streak + Consistencia: OK."

Write-Host "[617/664] Validando entidades de desafios e conquistas..."
...
Write-Host "[618/664] Validando mapeamento EF de missoes..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[619/664] Validando mapeamento EF de conquistas..."
...
Write-Host "[620/664] Validando missoes automaticas..."
$gameSource = Get-Content .\src\HealthPlatform.Api\Services\GamificacaoService.cs -Encoding UTF8 -Raw
...
Write-Host "[621/664] Validando conquistas esportivas..."
...
Write-Host "[622/664] Validando Home com missoes e conquistas..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[623/664] Validando SQL e migration v0.6.3..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[624/664] Validando compatibilidade funcional v0.6.3..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Versao atual inconsistente ao validar v0.6.3." }
Write-Host "    v0.6.3 / Missoes + Desafios + Conquistas preservada: OK."


Write-Host "[625/664] Validando entidade de ciclo esportivo..."
...
Write-Host "[626/664] Validando mapeamento EF de ciclos..."
$dbContextSource = Get-Content .\src\HealthPlatform.Infrastructure\Data\AppDbContext.cs -Encoding UTF8 -Raw
...
Write-Host "[627/664] Validando gestao profissional de ciclos..."
...
Write-Host "[628/664] Validando resumo longitudinal do ciclo..."
...
Write-Host "[629/664] Validando desafio semanal adaptado ao ciclo..."
$gameSource = Get-Content .\src\HealthPlatform.Api\Services\GamificacaoService.cs -Encoding UTF8 -Raw
...
Write-Host "[630/664] Validando Home e leitura profissional do ciclo..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[631/664] Validando SQL e migration v0.6.4..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[632/664] Validando compatibilidade funcional v0.6.4..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Compatibilidade v0.6.4 inconsistente na versao atual." }
Write-Host "    v0.6.4 / Ciclos Esportivos + Metas de Ciclo preservado: OK."

Write-Host "[633/664] Validando contrato da Estrategia do Dia..."
...
Write-Host "[634/664] Validando motor de adaptacao esportiva..."
...
Write-Host "[635/664] Validando trava contra sobrecarga automatica..."
...
Write-Host "[636/664] Validando integracao com ciclo e planos prescritos..."
...
Write-Host "[637/664] Validando Home Daily Athlete com Estrategia do Dia..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[638/664] Validando leitura profissional da estrategia diaria..."
...
Write-Host "[639/664] Validando ausencia de alteracao indevida de schema..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[640/664] Validando compatibilidade funcional v0.6.5..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Versao v0.6.6 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.6.8"')) { throw "Health endpoint nao anuncia v0.6.6." }
Write-Host "    v0.6.5 / Estrategia do Dia preservada: OK."



Write-Host "[641/664] Validando contrato de execucao guiada..."
$portalContracts = Get-Content .\src\HealthPlatform.Api\Contracts\Portal\PortalPacienteContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[642/664] Validando motor do Roteiro de Hoje..."
...
Write-Host "[643/664] Validando progresso baseado em registros reais..."
...
Write-Host "[644/664] Validando fechamento diario idempotente..."
$portalMeSource = Get-Content .\src\HealthPlatform.Api\Controllers\MeuPortalPacienteController.cs -Encoding UTF8 -Raw
...
Write-Host "[645/664] Validando Home com Roteiro de Hoje..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[646/664] Validando leitura profissional da execucao..."
...
Write-Host "[647/664] Validando ausencia de alteracao indevida de schema v0.6.6..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
...
Write-Host "[648/664] Validando compatibilidade funcional v0.6.6..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Versao v0.6.6 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.6.8"')) { throw "Health endpoint nao anuncia v0.6.6." }
...
Write-Host "[649/664] Validando contrato de tendencias de recuperacao..."
$portalContracts = Get-Content .\src\HealthPlatform.Api\Contracts\Portal\PortalPacienteContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[650/664] Validando janela longitudinal 7 vs 7 dias..."
...
Write-Host "[651/664] Validando sinais esportivos observaveis..."
...
Write-Host "[652/664] Validando alertas conservadores de recuperacao..."
...
Write-Host "[653/664] Validando ausencia de diagnostico automatico..."
...
Write-Host "[654/664] Validando Home e prontuario com tendencia de recuperacao..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[655/664] Validando ausencia de alteracao indevida de schema v0.6.7..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[37/37] Aplicando upgrade v0.6.4')) { throw "PREPARAR 37/37 foi alterado sem necessidade." }
...
Write-Host "[656/664] Validando compatibilidade funcional v0.6.7..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Versao v0.6.8 inconsistente." }
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if (-not $healthSource.Contains('version = "0.6.8"')) { throw "Health endpoint nao anuncia v0.6.8." }
Write-Host "    v0.6.7 / Tendencias de Recuperacao + Alertas Inteligentes preservada: OK."


Write-Host "[657/664] Validando contrato de performance esportiva..."
$portalContracts = Get-Content .\src\HealthPlatform.Api\Contracts\Portal\PortalPacienteContracts.cs -Encoding UTF8 -Raw
...
Write-Host "[658/664] Validando motor de performance a partir das execucoes reais..."
...
Write-Host "[659/664] Validando deteccao conservadora de PR..."
...
Write-Host "[660/664] Validando contexto de prontidao no PR..."
...
Write-Host "[661/664] Validando linguagem segura de performance..."
$appJsSource = Get-Content .\src\HealthPlatform.Api\wwwroot\app.js -Encoding UTF8 -Raw
...
Write-Host "[662/664] Validando Home e prontuario com Performance & PRs..."
...
Write-Host "[663/664] Validando ausencia de alteracao indevida de schema v0.6.8..."
$setupSource = Get-Content .\scripts\setup.ps1 -Encoding UTF8 -Raw
if (-not $setupSource.Contains('[37/37] Aplicando upgrade v0.6.4')) { throw "PREPARAR 37/37 foi alterado sem necessidade." }
if (Test-Path .\scripts\sql\v0.6.8_performance_prs.sql) { throw "v0.6.8 nao deveria exigir migration." }
Write-Host "    Performance e calculada sobre o historico existente: OK."

Write-Host "[664/664] Validando versao funcional v0.6.8..."
$version = Get-Content .\VERSION.txt -Encoding UTF8 -Raw
$healthSource = Get-Content .\src\HealthPlatform.Api\Controllers\HealthController.cs -Encoding UTF8 -Raw
if ($version.Trim() -ne "0.6.8" -or -not $appJsSource.Contains("const HP_MVP_VERSION='0.6.8';")) { throw "Versao v0.6.8 inconsistente." }
if (($healthSource | Select-String -Pattern 'version = "0.6.8"' -AllMatches).Matches.Count -lt 2) { throw "Health endpoint/fallback nao anunciam v0.6.8." }
Write-Host "    v0.6.8 / Performance & PRs: OK."
