...
[ApiController]
[Authorize]
public sealed class FasesNutricionaisController(
    AppDbContext db,
    CurrentUser currentUser,
    IHttpContextAccessor httpContextAccessor) : ControllerBase
{
    public sealed record CriarFaseNutricionalRequest(
        string Nome,
        string Tipo,
        string? Objetivo,
        DateOnly DataInicio,
        DateOnly? DataFim,
        Guid? PlanoAlimentarId,
        decimal? MetaPesoKg,
    ...
    public sealed record AtualizarFaseNutricionalRequest(
        string Nome,
        string Tipo,
        string? Objetivo,
        DateOnly DataInicio,
    ...
    public sealed record ReordenarFaseRequest(Guid FaseId, int Ordem);
    public sealed record ReordenarFasesRequest(IReadOnlyCollection<ReordenarFaseRequest> Fases);

    [HttpGet("api/pacientes/{pacienteId:guid}/fases-nutricionais")]
    public async Task<IActionResult> Listar(Guid pacienteId, CancellationToken ct = default)
    {
        ...
        var itens = await db.FasesNutricionais.AsNoTracking()
            .Where(x =>
    ...
    [HttpPost("api/pacientes/{pacienteId:guid}/fases-nutricionais")]
    public async Task<IActionResult> Criar(
        Guid pacienteId,
        CriarFaseNutricionalRequest request,
        CancellationToken ct = default)
    {
        ...
        if (!await PlanoValido(request.PlanoAlimentarId, pacienteId, ct))
            return BadRequest(new { message = "Plano alimentar vinculado nao pertence ao paciente/organizacao." });

        var profissional = await GetProfissionalAtual(ct);
        if (profissional is null)
            return Conflict(new { message = "Perfil profissional ativo nao encontrado." });

        var maiorOrdem = await db.FasesNutricionais
            .Where(x => x.PacienteId == pacienteId && x.OrganizacaoId == currentUser.OrganizationId)
            .Select(x => (int?)x.Ordem)
            .MaxAsync(ct) ?? 0;

        var fase = new FaseNutricional
        {
            OrganizacaoId = currentUser.OrganizationId,
            PacienteId = pacienteId,
            ProfissionalId = profissional.Id,
            PlanoAlimentarId = request.PlanoAlimentarId,
        ...
        db.FasesNutricionais.Add(fase);
        Auditar("CREATE", fase, null, Snapshot(fase));
        await db.SaveChangesAsync(ct);

        await db.Entry(fase).Reference(x => x.Profissional).LoadAsync(ct);
        if (fase.PlanoAlimentarId.HasValue)
            await db.Entry(fase).Reference(x => x.PlanoAlimentar).LoadAsync(ct);

        return Ok(ToResponse(fase));
    }

    [HttpPut("api/fases-nutricionais/{id:guid}")]
    public async Task<IActionResult> Atualizar(
        Guid id,
        AtualizarFaseNutricionalRequest request,
        CancellationToken ct = default)
    {
        var fase = await db.FasesNutricionais
            .Include(x => x.Profissional)
            .Include(x => x.PlanoAlimentar)
        ...
        if (!await PlanoValido(request.PlanoAlimentarId, fase.PacienteId, ct))
            return BadRequest(new { message = "Plano alimentar vinculado nao pertence ao paciente/organizacao." });
        ...
        fase.PlanoAlimentarId = request.PlanoAlimentarId;
        ...
        if (fase.PlanoAlimentarId.HasValue && fase.PlanoAlimentar is null)
            await db.Entry(fase).Reference(x => x.PlanoAlimentar).LoadAsync(ct);

        return Ok(ToResponse(fase));
    }

    [HttpPost("api/pacientes/{pacienteId:guid}/fases-nutricionais/reordenar")]
    public async Task<IActionResult> Reordenar(
        Guid pacienteId,
        ReordenarFasesRequest request,
        CancellationToken ct = default)
    {
        if (!await PacienteExiste(pacienteId, ct))
            return NotFound(new { message = "Paciente nao encontrado." });
        ...
        var fases = await db.FasesNutricionais
            .Where(x =>
                x.PacienteId == pacienteId &&
                x.OrganizacaoId == currentUser.OrganizationId)
            .ToListAsync(ct);
        ...
        var ordemDuplicada = request.Fases
            .GroupBy(x => x.Ordem)
            .Any(g => g.Count() > 1);
        ...
        return Ok(new { message = "Fases reordenadas." });
    }

    [HttpDelete("api/fases-nutricionais/{id:guid}")]
    public async Task<IActionResult> Excluir(Guid id, CancellationToken ct = default)
    {
        ...
        var antes = Snapshot(fase);
        db.FasesNutricionais.Remove(fase);
    ...
    private async Task<bool> PlanoValido(Guid? planoId, Guid pacienteId, CancellationToken ct)
    {
        ...
        return await db.PlanosAlimentares.AnyAsync(x =>
            x.Id == planoId.Value &&
            x.PacienteId == pacienteId &&
        ...
        string? criterioTransicao)
    {
        ...
        if (statusNormalizado is not ("Planejada" or "EmAndamento" or "Concluida" or "Cancelada"))
    ...
    private object ToResponse(FaseNutricional x) => new
    {
        x.Id,
        x.PacienteId,
        x.ProfissionalId,
        ...
        x.PlanoAlimentarId,
        planoNome = x.PlanoAlimentar?.Nome,
        planoVersao = x.PlanoAlimentar?.Versao,
        x.Nome,
        x.Tipo,
        x.Objetivo,
        x.DataInicio,
    ...
    private static object Snapshot(FaseNutricional x) => new
    {
        x.Id,
        x.PacienteId,
        x.ProfissionalId,
    ...
    private void Auditar(string acao, FaseNutricional fase, object? antes, object? depois)
    {
        db.AuditLogs.Add(new AuditLog
        {
            ...
            Acao = acao,
            Entidade = nameof(FaseNutricional),
