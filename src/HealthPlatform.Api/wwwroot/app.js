...
async function loadPrescriptionWorkspace(){
  ...
  content.innerHTML=`<section class="prescription-workspace-hero"><div><span class="eyebrow">PROFESSIONAL PRESCRIPTION WORKSPACE • v0.17.7</span><h3>Da avaliação à prescrição, sem perder o contexto do paciente.</h3><p>Organize avaliação corporal, objetivo, preferências, treino e alimentação em um único fluxo profissional. Sugestões futuras serão sempre propostas para revisão — nunca publicação automática.</p></div><button class="primary" id="workspaceNewPatient">+ Iniciar paciente</button></section>
