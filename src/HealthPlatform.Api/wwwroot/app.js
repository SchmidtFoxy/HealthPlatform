...
async function loadPrescriptionWorkspace(){
  ...
  content.innerHTML=`<section class="prescription-workspace-hero"><div><span class="eyebrow">PROFESSIONAL PRESCRIPTION WORKSPACE • v0.17.0</span><h3>Da avaliação à prescrição, sem perder o contexto do paciente.</h3><p>Organize avaliação corporal, objetivo, preferências, treino e alimentação em um único fluxo profissional. Sugestões futuras serão sempre propostas para revisão — nunca publicação automática.</p></div><button class="primary" id="workspaceNewPatient">+ Iniciar paciente</button></section>
  <div class="prescription-workspace-grid">
    <section class="card prescription-workspace-step"><span class="workspace-step-index">1</span><div><span class="eyebrow">ENTRADA</span><h3>Avaliação e contexto</h3><p>Cadastro essencial primeiro; depois avaliação corporal/bioimpedância, rotina, disponibilidade e preferências.</p><button class="secondary" id="workspacePatients">Abrir pacientes</button></div></section>
