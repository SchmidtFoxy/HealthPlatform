...
function hpMonitoringCard(d){
  ...
  return `<section class="card full-card patient-monitoring-card" data-patient-monitoring><div class="card-head"><div><span class="eyebrow">MONITORAMENTO REMOTO</span><h3>Sinais e sintomas • últimos ${d.dias} dias</h3><small>${d.totalRegistros||0} registro(s) enviados pelo paciente</small></div></div><div class="monitoring-metrics">
    <div><strong>${pressure}</strong><span>Pressão arterial</span></div>
    <div><strong>${value('Glicemia',' mg/dL')}</strong><span>Glicemia</span></div>
    <div><strong>${value('FrequenciaCardiaca',' bpm')}</strong><span>Frequência cardíaca</span></div>
    <div><strong>${value('Saturacao','%')}</strong><span>Saturação</span></div>
    <div><strong>${value('Temperatura',' °C')}</strong><span>Temperatura</span></div>
    <div><strong>${value('Dor')}</strong><span>Dor</span></div>
  </div><p class="monitoring-note">Resumo de acompanhamento informado pelo paciente; não substitui avaliação clínica.</p></section>`;
}
