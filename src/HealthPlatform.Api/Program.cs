...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "HealthPlatform API", Version = "v0.9.6" });
...
if (app.Environment.IsDevelopment())
{
        ...
        !string.IsNullOrWhiteSpace(adminPassword))
    {
        ...
        if (!await users.CheckPasswordAsync(admin, adminPassword))
        {
            ...
            Console.WriteLine("[v0.9.6] Admin local desbloqueado e senha sincronizada com Seed:AdminPassword.");
        }
        else
        {
            Console.WriteLine("[v0.9.6] Admin local desbloqueado; credencial ja esta sincronizada.");
        }
    }
}
